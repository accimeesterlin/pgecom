...
class Lexer
{
    ...
    public function tokenize($input)
    {
        ...
        while (false !== ($current = current($chars))) {
            ...
            if (!array_key_exists(self::$transitionTable[$current])) {
                $tokens[] = [
                    'type'  => self::T_UNKNOWN,
                    'pos'   => key($chars),
                    'value' => $current
            ...
            if ($state === self::STATE_SINGLE_CHAR) {
                ...
                do {
                    $buffer .= $current;
                    $current = next($chars);
                } while ($current !== false && array_key_exists($this->validIdentifier[$current]));
                $tokens[] = [
                ...
                do {
                    $buffer .= $current;
                    $current = next($chars);
                } while ($current !== false && array_key_exists($this->numbers[$current]));
                $tokens[] = [
                    'type'  => self::T_NUMBER,
                    'value' => (int)$buffer,
                    'pos'   => $start
