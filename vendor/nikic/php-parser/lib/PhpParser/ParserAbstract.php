...
abstract class ParserAbstract implements Parser
{
    ...
    public function __construct(Lexer $lexer, array $options = []) {
        ...
        if (array_key_exists($options['throwOnError'])) {
            ...
            for (;;) {
                if ($rule === 0) {
                    ...
                    try {
                        ...
                        if (-1 === $e->getStartLine() && array_key_exists($startAttributes['startLine'])) {
    ...
    */
    ...
    protected function handleBuiltinTypes(Name $name) {
        ...
        if (!array_key_exists($builtinTypes[$lowerName])) {
            return $name;
        }
    ...
    protected function stripIndentation(
        ...
        return preg_replace_callback(
            ...
            function ($matches) use ($indentLen, $indentChar, $attributes) {
                ...
                } elseif (strlen($prefix) < $indentLen && !array_key_exists($matches[2])) {
                    $this->emitError(new Error(
