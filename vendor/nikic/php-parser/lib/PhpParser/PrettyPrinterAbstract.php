...
abstract class PrettyPrinterAbstract
{
    ...
    protected function pPrec(Node $node, int $parentPrecedence, int $parentAssociativity, int $childPosition) : string {
        ...
        if (array_key_exists($this->precedenceMap[$class])) {
        ...
        foreach ($node->getSubNodeNames() as $subNodeName) {
            ...
            if ((!$subNode instanceof Node && $subNode !== null)
                ...
                if (is_int($subNode) && is_int($origSubNode)) {
                    ...
                    if (!array_key_exists($this->modifierChangeMap[$key])) {
                        return $this->pFallback($fallbackNode);
                    }
            ...
            if ($origSubNode !== null) {
                ...
                $key = $type . '->' . $subNodeName;
                if (!array_key_exists($this->insertionMap[$key])) {
                    return $this->pFallback($fallbackNode);
                }
            ...
            if (null !== $subNode) {
                ...
                if (array_key_exists($fixupInfo[$subNodeName])
    ...
    protected function pArray(
        ...
        if (!empty($delayedAdd)) {
            if (!array_key_exists($this->emptyListInsertionMap[$mapKey])) {
                return null;
            }
