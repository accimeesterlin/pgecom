...
class Lexer
{
    ...
    public function __construct(array $options = []) {
        ...
        // map of tokens to drop while lexing (the map is only used for array_key_exists lookup,
        ...
        $this->attributeStartLineUsed = array_key_exists($usedAttributes['startLine']);
        $this->attributeEndLineUsed = array_key_exists($usedAttributes['endLine']);
        $this->attributeStartTokenPosUsed = array_key_exists($usedAttributes['startTokenPos']);
        $this->attributeEndTokenPosUsed = array_key_exists($usedAttributes['endTokenPos']);
        $this->attributeStartFilePosUsed = array_key_exists($usedAttributes['startFilePos']);
        $this->attributeEndFilePosUsed = array_key_exists($usedAttributes['endFilePos']);
        $this->attributeCommentsUsed = array_key_exists($usedAttributes['comments']);
    }
    ...
    protected function postprocessTokens(ErrorHandler $errorHandler) {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if ($token[0] === \T_COMMENT && substr($token[1], 0, 2) !== '/*'
                ...
                $this->tokens[$i] = $token;
                if (array_key_exists($this->tokens[$i + 1]) && $this->tokens[$i + 1][0] === \T_WHITESPACE) {
            ...
            // into a single token.
            if (\is_array($token)
                    && ($token[0] === \T_NS_SEPARATOR || array_key_exists($this->identifierTokens[$token[0]]))) {
                $lastWasSeparator = $token[0] === \T_NS_SEPARATOR;
                $text = $token[1];
                for ($j = $i + 1; array_key_exists($this->tokens[$j]); $j++) {
                    if ($lastWasSeparator) {
                        if (!array_key_exists($this->identifierTokens[$this->tokens[$j][0]])) {
                            break;
                        }
            ...
            if ($token === '&') {
                $next = $i + 1;
                while (array_key_exists($this->tokens[$next]) && $this->tokens[$next][0] === \T_WHITESPACE) {
                    $next++;
                }
                $followedByVarOrVarArg = array_key_exists($this->tokens[$next]) &&
        ...
        if ($filePos !== \strlen($this->code)) {
            if (substr($this->code, $filePos, 2) === '/*') {
                ...
                $isDocComment = array_key_exists($comment[3]) && $comment[3] === '*';
    ...
    public function getNextToken(&$value = null, &$startAttributes = null, &$endAttributes = null) : int {
        ...
        while (1) {
            if (array_key_exists($this->tokens[++$this->pos])) {
            ...
            } elseif (!array_key_exists($this->dropTokens[$token[0]])) {
                $value = $token[1];
    ...
    private function defineCompatibilityTokens() {
        ...
        foreach ($compatTokens as $token) {
            if (!\defined($token)) {
                while (array_key_exists($usedTokenIds[$newTokenId])) {
                    $newTokenId--;
                }
    ...
    private function createIdentifierTokenMap(): array {
        ...
        return array_fill_keys([
            ...
            \T_FINALLY, \T_THROW, \T_USE, \T_INSTEADOF, \T_GLOBAL, \T_VAR, \T_UNSET, \T_array_key_exists, \T_EMPTY, \T_CONTINUE, \T_GOTO,
