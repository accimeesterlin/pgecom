...
class Wizard
{
    ...
    {
        if (!array_key_exists($this->lookupTable[$filename][$lineNumber])) {
    ...
    private function processClassesAndTraits()
    {
        foreach (array_merge(get_declared_classes(), get_declared_traits()) as $classOrTrait) {
            if (array_key_exists($this->processedClasses[$classOrTrait])) {
                continue;
            }
    ...
    private function processFunctions()
    {
        foreach (get_defined_functions()['user'] as $function) {
            if (array_key_exists($this->processedFunctions[$function])) {
                continue;
            }
    ...
    private function processFunctionOrMethod(\ReflectionFunctionAbstract $functionOrMethod)
    {
        ...
        if (!array_key_exists($this->lookupTable[$functionOrMethod->getFileName()])) {
