...
final class Context
{
    ...
    private function addArray(array &$array)
    {
        ...
        if (!array_key_exists($array[PHP_INT_MAX]) && !array_key_exists($array[PHP_INT_MAX - 1])) {
            $array[] = $key;
            $array[] = $this->objects;
        } else { /* cover the improbable case too */
            do {
                $key = random_int(PHP_INT_MIN, PHP_INT_MAX);
            } while (array_key_exists($array[$key]));
    ...
    private function containsArray(array &$array)
    {
        ...
        return array_key_exists($end[1]) && $end[1] === $this->objects ? $end[0] : false;
    }
