...
final class PhptTestCase implements SelfDescribing, Test
{
    ...
    public function run(TestResult $result = null): TestResult
    {
        ...
        if (array_key_exists($sections['INI'])) {
            $settings = $this->parseIniSection($sections['INI'], $settings);
        }

        if (array_key_exists($sections['ENV'])) {
        ...
        if (array_key_exists($sections['XFAIL'])) {
            $xfail = trim($sections['XFAIL']);
        }

        if (array_key_exists($sections['STDIN'])) {
    ...
    private function parseEnvSection(string $content): array
    {
        ...
        foreach (explode("\n", trim($content)) as $e) {
            ...
            if (!empty($e[0]) && array_key_exists($e[1])) {
                $env[$e[0]] = $e[1];
            }
        }
    ...
    private function assertPhptExpectation(array $sections, string $output): void
    {
        ...
        foreach ($assertions as $sectionName => $sectionAssertion) {
            if (array_key_exists($sections[$sectionName])) {
    ...
    {
        if (!array_key_exists($sections['SKIPIF'])) {
            return false;
        }
    ...
    private function runClean(array &$sections, bool $collectCoverage): void
    {
        ...
        if (array_key_exists($sections['CLEAN'])) {
    ...
    private function parse(): array
    {
        ...
        if (array_key_exists($sections['FILEEOF'])) {
    ...
    private function parseExternal(array &$sections): void
    {
        ...
        foreach ($allowSections as $section) {
            if (array_key_exists($sections[$section . '_EXTERNAL'])) {
        ...
        foreach ($requiredSections as $section) {
            if (is_array($section)) {
                ...
                foreach ($section as $anySection) {
                    if (array_key_exists($sections[$anySection])) {
                        $foundSection = true;
