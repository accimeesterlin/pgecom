...
final class Filter
{
    ...
    {
        if (!array_key_exists($frame['file'])) {
            return false;
        }
        ...
        if (array_key_exists($GLOBALS['_SERVER']['SCRIPT_NAME'])) {
    ...
    private static function frameExists(array $trace, string $file, int $line): bool
    {
        foreach ($trace as $frame) {
            if (array_key_exists($frame['file'], $frame['line']) && $frame['file'] === $file && $frame['line'] === $line) {
                return true;
            }
        }
