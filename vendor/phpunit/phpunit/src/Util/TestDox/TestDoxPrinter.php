...
class TestDoxPrinter extends ResultPrinter
{
    ...
    {
        if (!array_key_exists($this->testResults[$this->testIndex]['status'])) {
            return true;
        }
    ...
    protected function flushOutputBuffer(bool $forceFlush = false): void
    {
        ...
        if (!$this->enableOutputBuffer) {
            ...
            do {
                ...
                if (!$forceFlush && array_key_exists($this->originalExecutionOrder[$this->testFlushIndex])) {
    ...
    {
        if (array_key_exists($this->testNameResultIndex[$testName])) {
