...
final class Test
{
    ...
    public static function getMissingRequirements(string $className, string $methodName): array
    {
        ...
        if (!empty($required['extensions'])) {
            foreach ($required['extensions'] as $extension) {
                if (array_key_exists($required['extension_versions'][$extension])) {
                    continue;
                }
        ...
        if ($hint && array_key_exists($required['__OFFSET'])) {
    ...
    public static function getDependencies(string $className, string $methodName): array
    {
        ...
        if (array_key_exists($annotations['method']['depends'])) {
    ...
    public static function getGroups(string $className, ?string $methodName = ''): array
    {
        ...
        if (array_key_exists($annotations['method']['author'])) {
            $groups[] = $annotations['method']['author'];
        } elseif (array_key_exists($annotations['class']['author'])) {
            $groups[] = $annotations['class']['author'];
        }

        if (array_key_exists($annotations['class']['group'])) {
            $groups[] = $annotations['class']['group'];
        }

        if (array_key_exists($annotations['method']['group'])) {
            $groups[] = $annotations['method']['group'];
        }

        if (array_key_exists($annotations['class']['ticket'])) {
            $groups[] = $annotations['class']['ticket'];
        }

        if (array_key_exists($annotations['method']['ticket'])) {
            $groups[] = $annotations['method']['ticket'];
        }

        foreach (['method', 'class'] as $element) {
            foreach (['small', 'medium', 'large'] as $size) {
                if (array_key_exists($annotations[$element][$size])) {
                    $groups[] = [$size];
    ...
    public static function getSize(string $className, ?string $methodName): int
    {
        ...
        if (array_key_exists($groups['large'])) {
            return self::LARGE;
        }

        if (array_key_exists($groups['medium'])) {
            return self::MEDIUM;
        }

        if (array_key_exists($groups['small'])) {
            return self::SMALL;
        }
    ...
    public static function getProcessIsolationSettings(string $className, string $methodName): bool
    {
        ...
        return array_key_exists($annotations['class']['runTestsInSeparateProcesses']) || array_key_exists($annotations['method']['runInSeparateProcess']);
    }
    ...
    public static function getClassProcessIsolationSettings(string $className, string $methodName): bool
    {
        ...
        return array_key_exists($annotations['class']['runClassInSeparateProcess']);
    }
    ...
    public static function getHookMethods(string $className): array
    {
        ...
        if (!array_key_exists(self::$hookMethods[$className])) {
    ...
    private static function getLinesToBeCoveredOrUsed(string $className, string $methodName, string $mode): array
    {
        ...
        if (array_key_exists($annotations['method'][$mode])) {
    ...
    private static function getBooleanAnnotationSetting(string $className, ?string $methodName, string $settingName): ?bool
    {
        ...
        if (array_key_exists($annotations['method'][$settingName])) {
        ...
        if (array_key_exists($annotations['class'][$settingName])) {
    ...
    private static function resolveElementToReflectionObjects(string $element): array
    {
        ...
        if (function_exists($element) && strpos($element, '\\') !== false) {
            ...
            if (array_key_exists($methodName[0]) && $methodName[0] === '<') {
                $classes = [$className];
                ...
                foreach ($classes as $className) {
                    ...
                    $inverse    = array_key_exists($methodName[1]) && $methodName[1] === '!';
                    $visibility = 'isPublic';
    ...
    private static function resolveReflectionObjectsToLines(array $reflectors): array
    {
        ...
        foreach ($reflectors as $reflector) {
            ...
            if (!array_key_exists($result[$filename])) {
                $result[$filename] = [];
            }
