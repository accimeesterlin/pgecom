...
final class Help
{
    ...
    public function __construct(?int $width = null, ?bool $withColor = null)
    {
        ...
        foreach (self::HELP_TEXT as $options) {
            foreach ($options as $option) {
                if (array_key_exists($option['arg'])) {
                    $this->maxArgLength = max($this->maxArgLength, array_key_exists($option['arg']) ? strlen($option['arg']) : 0);
                }
            }
        }
    ...
    private function writePlaintext(): void
    {
        foreach (self::HELP_TEXT as $section => $options) {
            ...
            foreach ($options as $option) {
                if (array_key_exists($option['spacer'])) {
                    print PHP_EOL;
                }

                if (array_key_exists($option['text'])) {
