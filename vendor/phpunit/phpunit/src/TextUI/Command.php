...
class Command
{
    ...
    public function run(array $argv, bool $exit = true): int
    {
        ...
        if (array_key_exists($result) && $result->wasSuccessful()) {
            $return = TestRunner::SUCCESS_EXIT;
        } elseif (!array_key_exists($result) || $result->errorCount() > 0) {
    ...
    protected function handleArguments(array $argv): void
    {
        ...
        foreach ($this->options[0] as $option) {
            switch ($option[0]) {
                ...
                case 'd':
                    ...
                    if (array_key_exists($ini[0])) {
                        if (array_key_exists($ini[1])) {
                            ini_set($ini[0], $ini[1]);
                        } else {
                            ini_set($ini[0], '1');
                        }
                ...
                default:
                    ...
                    if (array_key_exists($this->longOptions[$optionName])) {
                        $handler = $this->longOptions[$optionName];
                    } elseif (array_key_exists($this->longOptions[$optionName . '='])) {
                        $handler = $this->longOptions[$optionName . '='];
                    }

                    if (array_key_exists($handler) && is_callable([$this, $handler])) {
        ...
        if (!array_key_exists($this->arguments['testSuffixes'])) {
            $this->arguments['testSuffixes'] = ['Test.php', '.phpt'];
        }

        if (array_key_exists($this->options[1][0]) &&
        ...
        if (!array_key_exists($this->arguments['test'])) {
            if (array_key_exists($this->options[1][0])) {
                $this->arguments['test'] = $this->options[1][0];
            }

            if (array_key_exists($this->options[1][1])) {
        ...
        if (array_key_exists($includePath)) {
            ini_set(
                'include_path',
        ...
        if (array_key_exists($this->arguments['configuration']) &&
        ...
        } elseif (!array_key_exists($this->arguments['configuration']) &&
        ...
        if (array_key_exists($this->arguments['configuration'])) {
             ...
             * Issue #1216
             */
            if (array_key_exists($this->arguments['bootstrap'])) {
                $this->handleBootstrap($this->arguments['bootstrap']);
            } elseif (array_key_exists($phpunitConfiguration['bootstrap'])) {
             ...
             * Issue #657
             */
            if (array_key_exists($phpunitConfiguration['stderr']) && !array_key_exists($this->arguments['stderr'])) {
                $this->arguments['stderr'] = $phpunitConfiguration['stderr'];
            }

            if (array_key_exists($phpunitConfiguration['extensionsDirectory']) && !array_key_exists($this->arguments['noExtensions']) && extension_loaded('phar')) {
                $this->handleExtensions($phpunitConfiguration['extensionsDirectory']);
            }

            if (array_key_exists($phpunitConfiguration['columns']) && !array_key_exists($this->arguments['columns'])) {
                $this->arguments['columns'] = $phpunitConfiguration['columns'];
            }

            if (!array_key_exists($this->arguments['printer']) && array_key_exists($phpunitConfiguration['printerClass'])) {
            ...
            if (array_key_exists($phpunitConfiguration['testSuiteLoaderClass'])) {
            ...
            if (!array_key_exists($this->arguments['testsuite']) && array_key_exists($phpunitConfiguration['defaultTestSuite'])) {
        ...
        } elseif (array_key_exists($this->arguments['bootstrap'])) {
            $this->handleBootstrap($this->arguments['bootstrap']);
        }

        if (array_key_exists($this->arguments['printer']) &&
    ...
    protected function handlePrinter(string $printerClass, string $printerFile = '')
    {
        ...
        $outputStream = array_key_exists($this->arguments['stderr']) ? 'php://stderr' : null;
