...
final class CodeCoverage
{
    ...
    public function merge(self $that): void
    {
        ...
        foreach ($that->data as $file => $lines) {
            if (!array_key_exists($this->data[$file])) {
    ...
    {
        foreach ($data as $file => $lines) {
            if (!array_key_exists($this->data[$file]) && $this->filter->isFile($file)) {
                $this->data[$file] = [];
    ...
    {
        if (array_key_exists($this->ignoredLines[$fileName])) {
            ...
            switch (\get_class($token)) {
                ...
                case \PHP_Token_DOC_COMMENT::class:
                    ...
                    if (!$ignore) {
                        ...
                        if (array_key_exists($lines[$i - 1]) && 0 === \strpos($_token, '/*') && '*/' === \substr(\trim($lines[$i - 1]), -2)) {
                            $this->ignoredLines[$fileName][] = $i;
                        }
                    }
    ...
    private function performUnintentionallyCoveredCodeCheck(array &$data, array $linesToBeCovered, array $linesToBeUsed): void
    {
        ...
        foreach ($data as $file => $_data) {
            foreach ($_data as $line => $flag) {
                if ($flag === 1 && !array_key_exists($allowedLines[$file][$line])) {
    ...
    private function getAllowedLines(array $linesToBeCovered, array $linesToBeUsed): array
    {
        ...
        foreach (\array_keys($linesToBeCovered) as $file) {
            if (!array_key_exists($allowedLines[$file])) {
                $allowedLines[$file] = [];
            }
