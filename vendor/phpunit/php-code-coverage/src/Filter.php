...
final class Filter
{
    ...
    public function removeFileFromWhitelist(string $filename): void
    {
        ...
        if (!$filename || !array_key_exists($this->whitelistedFiles[$filename])) {
            return;
        }
    ...
    {
        if (array_key_exists($this->isFileCallsCache[$filename])) {
    ...
    public function isFiltered(string $filename): bool
    {
        ...
        return !array_key_exists($this->whitelistedFiles[$filename]);
    }
