...
class PHP_Token_Stream implements ArrayAccess, Countable, SeekableIterator
{
    ...
    public function getIncludes($categorize = false, $category = null)
    {
        ...
        if (array_key_exists($this->includes[$category])) {
    ...
    public function getFunctionForLine($line)
    {
        ...
        if (array_key_exists($this->lineToFunctionMap[$line])) {
    ...
    {
        return array_key_exists($this->tokens[$this->position]);
    }
    ...
    {
        return array_key_exists($this->tokens[$offset]);
    }
    ...
    protected function scan($sourceCode)/*: void*/
    {
        ...
        for ($i = 0; $i < $numTokens; ++$i) {
            ...
            if (\is_array($token)) {
                ...
                } elseif ($name == 'USE' && array_key_exists($tokens[$i + 2][0]) && $tokens[$i + 2][0] == \T_FUNCTION) {
                    $name = 'USE_FUNCTION';
            ...
            if (PHP_MAJOR_VERSION >= 8 &&
                ...
                array_key_exists($this->tokens[$id - 1]) &&                           // Current token is not the first token
            ...
            if (!array_key_exists($this->tokensByLine[$line])) {
