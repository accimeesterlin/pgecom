...
class DeclarationBlock extends RuleSet
{
    ...
    public function expandBorderShorthand()
    {
        ...
        foreach ($aBorderRules as $sBorderRule) {
            if (!array_key_exists($aRules[$sBorderRule])) {
                continue;
            }
    ...
    public function expandDimensionsShorthand()
    {
        ...
        foreach ($aExpansions as $sProperty => $sExpanded) {
            if (!array_key_exists($aRules[$sProperty])) {
                continue;
            }
    ...
    {
        $aRules = $this->getRulesAssoc();
        if (!array_key_exists($aRules['font'])) {
            return;
        }
        $oRule = $aRules['font'];
        ...
        foreach ($aValues as $mValue) {
            ...
            if (in_array($mValue, ['normal', 'inherit'])) {
                foreach (['font-style', 'font-weight', 'font-variant'] as $sProperty) {
                    if (!array_key_exists($aFontProperties[$sProperty])) {
    ...
    {
        $aRules = $this->getRulesAssoc();
        if (!array_key_exists($aRules['background'])) {
            return;
        }
    ...
    public function expandListStyleShorthand()
    {
        ...
        $aRules = $this->getRulesAssoc();
        if (!array_key_exists($aRules['list-style'])) {
            return;
        }
    ...
    public function createFontShorthand()
    {
        ...
        $aRules = $this->getRulesAssoc();
        if (!array_key_exists($aRules['font-size']) || !array_key_exists($aRules['font-family'])) {
            return;
        }
        $oOldRule = array_key_exists($aRules['font-size']) ? $aRules['font-size'] : $aRules['font-family'];
        ...
        if (array_key_exists($aRules['line-height'])) {
