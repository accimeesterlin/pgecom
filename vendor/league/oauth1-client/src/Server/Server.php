...
abstract class Server
{
    ...
    protected function createClientCredentials(array $clientCredentials)
    {
        ...
        foreach ($keys as $key) {
            if ( ! array_key_exists($clientCredentials[$key])) {
                throw new \InvalidArgumentException("Missing client credentials key [$key] from options.");
            }
        }

        if (array_key_exists($clientCredentials['rsa_private_key']) && array_key_exists($clientCredentials['rsa_public_key'])) {
        ...
        if (array_key_exists($clientCredentials['callback_uri'])) {
    ...
    protected function createTemporaryCredentials($body)
    {
        ...
        if ( ! array_key_exists($data['oauth_callback_confirmed']) || $data['oauth_callback_confirmed'] != 'true') {
