...
class Util
{
    ...
    public static function map(array $object, array $map)
    {
        ...
        foreach ($map as $from => $to) {
            if ( ! array_key_exists($object[$from])) {
                continue;
            }
    ...
    public static function getStreamSize($resource)
    {
        ...
        if ( ! is_array($stat) || ! array_key_exists($stat['size'])) {
            return null;
        }
    ...
    protected static function emulateObjectDirectories(array $object, array $directories, array $listedDirectories)
    {
        ...
        if ( ! array_key_exists($object['dirname']) || trim($object['dirname']) === '') {
            return [$directories, $listedDirectories];
        }

        $parent = $object['dirname'];

        while (array_key_exists($parent) && trim($parent) !== '' && ! in_array($parent, $directories)) {
            $directories[] = $parent;
            $parent = static::dirname($parent);
        }

        if (array_key_exists($object['type']) && $object['type'] === 'dir') {
            $listedDirectories[] = $object['path'];
