...
final class Color
{
    ...
    public static function parse($color)
    {
        ...
        if (array_key_exists($lookupResult)) {
            return $lookupResult;
        }
        ...
        if (!array_key_exists($r) || !array_key_exists($g) || !array_key_exists($b) || !array_key_exists($a)) {
    ...
    private static function parseRGBAComponent($str, $base = 255, $scalar = 1)
    {
        ...
        if (array_key_exists($matches[2]) && $matches[2] === '%') {
    ...
    private static function parseHSLAComponents($str)
    {
        ...
        // convert HSL to RGB
        $r = $g = $b = null;
        if (array_key_exists($h) && array_key_exists($s) && array_key_exists($l)) {
