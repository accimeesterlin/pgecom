...
abstract class SVGNode
{
    ...
    {
        return array_key_exists($this->value) ? $this->value : '';
    }
    ...
    {
        return array_key_exists($this->styles[$name]) ? $this->styles[$name] : null;
    }
    ...
    public function getComputedStyle($name)
    {
        ...
        if ($style === null && array_key_exists($this->parent)) {
            $containerStyles = $this->parent->getContainerStyleForNode($this);
            $style = array_key_exists($containerStyles[$name]) ? $containerStyles[$name] : null;
        }

        // If still no style then get parent's style
        if (($style === null || $style === 'inherit') && array_key_exists($this->parent)) {
    ...
    {
        return array_key_exists($this->attributes[$name]) ? $this->attributes[$name] : null;
    }
