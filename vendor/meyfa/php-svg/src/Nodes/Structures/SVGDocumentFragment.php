...
class SVGDocumentFragment extends SVGNodeContainer
{
    ...
    public function getComputedStyle($name)
    {
        ...
        if (array_key_exists($style) || !array_key_exists(self::$initialStyles[$name])) {
            return $style;
        }
    ...
    public function getSerializableAttributes()
    {
        ...
        if (array_key_exists($attrs['width']) && $attrs['width'] === '100%') {
            unset($attrs['width']);
        }
        if (array_key_exists($attrs['height']) && $attrs['height'] === '100%') {
            unset($attrs['height']);
        }
