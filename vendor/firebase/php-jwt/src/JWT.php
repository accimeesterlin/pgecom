...
class JWT
{
    ...
    public static function decode($jwt, $keyOrKeyArray, array $allowed_algs = array())
    {
        ...
        if (array_key_exists($payload->nbf) && $payload->nbf > ($timestamp + static::$leeway)) {
        ...
        if (array_key_exists($payload->iat) && $payload->iat > ($timestamp + static::$leeway)) {
            throw new BeforeValidException(
        ...
        if (array_key_exists($payload->exp) && ($timestamp - static::$leeway) >= $payload->exp) {
    ...
    public static function encode($payload, $key, $alg = 'HS256', $keyId = null, $head = null)
    {
        ...
        if (array_key_exists($head) && \is_array($head)) {
    ...
    private static function getKeyMaterialAndAlgorithm($keyOrKeyArray, $kid = null)
    {
        ...
        if (is_array($keyOrKeyArray) || $keyOrKeyArray instanceof ArrayAccess) {
            if (!array_key_exists($kid)) {
                throw new UnexpectedValueException('"kid" empty, unable to lookup correct key');
            }
            if (!array_key_exists($keyOrKeyArray[$kid])) {
    ...
    private static function handleJsonError($errno)
    {
        ...
        throw new DomainException(
            array_key_exists($messages[$errno])
            ? $messages[$errno]
