...
if (array_key_exists($args['file']) || array_key_exists($args['suite']) || array_key_exists($args['case'])) {
    if (!array_key_exists($args['file']) || !array_key_exists($args['suite']) || !array_key_exists($args['case'])) {
        die($description);
    }
    ...
    $set = $json[$args['suite']];
    $data = $set['given'];
    if (!array_key_exists($expression)) {
        $expression = $set['cases'][$args['case']]['expression'];
        echo "Expects\n=======\n";
        if (array_key_exists($set['cases'][$args['case']]['result'])) {
            echo json_encode($set['cases'][$args['case']]['result'], JSON_PRETTY_PRINT) . "\n\n";
        } elseif (array_key_exists($set['cases'][$args['case']]['error'])) {
            echo "{$set['cases'][$argv['case']]['error']} error\n\n";
        } else {
            echo "NULL\n\n";
        }
    }
} elseif (array_key_exists($expression)) {
