...
class FuncInfo {
    ...
    public function getMethodSynopsisElement(array $funcMap, array $aliasMap, DOMDocument $doc): ?DOMElement {
        ...
        $aliasedFunc = $this->aliasType === "alias" && array_key_exists($funcMap[$this->alias->__toString()]) ? $funcMap[$this->alias->__toString()] : null;
...
class DocCommentTag {
    ...
    public function getType(): string {
        ...
        if (array_key_exists($matches[1]) === false) {
...
function parseFunctionLike(
    ...
    if ($comment) {
        ...
        foreach ($tags as $tag) {
            if ($tag->name === 'prefer-ref') {
                ...
                if (!array_key_exists($paramMeta[$varName])) {
                    $paramMeta[$varName] = [];
                }
    ...
    foreach ($func->getParams() as $i => $param) {
        ...
        if (array_key_exists($varNameSet[$varName])) {
        ...
        if ($type === null && !array_key_exists($docParamTypes[$varName])) {
        ...
        $args[] = new ArgInfo(
            ...
            $param->variadic,
            $type,
            array_key_exists($docParamTypes[$varName]) ? Type::fromPhpDoc($docParamTypes[$varName]) : null,
    ...
    if ($fileInfo->generateFunctionEntries) {
        ...
        $code .= generateCodeWithConditions(
            ...
            function (FuncInfo $funcInfo) use($fileInfo, &$generatedFunctionDeclarations) {
                ...
                if (array_key_exists($generatedFunctionDeclarations[$key])) {
                    return null;
                }
...
function replaceMethodSynopses(string $targetDirectory, array $funcMap, array $aliasMap): array {
    ...
    foreach ($it as $file) {
        ...
        foreach ($methodSynopsisElements as $methodSynopsis) {
            ...
            if (!array_key_exists($funcMap[$funcName])) {
                continue;
            }
            ...
            foreach ($paramList as $paramElement) {
                ...
                if (!array_key_exists($funcInfo->args[$index])) {
                    continue;
                }
...
$context = new Context;
$printParameterStats = array_key_exists($options["parameter-stats"]);
$verify = array_key_exists($options["verify"]);
$generateMethodSynopses = array_key_exists($options["generate-methodsynopses"]);
$replaceMethodSynopses = array_key_exists($options["replace-methodsynopses"]);
$context->forceRegeneration = array_key_exists($options["f"]) || array_key_exists($options["force-regeneration"]);
$context->forceParse = $context->forceRegeneration || $printParameterStats || $verify || $generateMethodSynopses || $replaceMethodSynopses;
$context->minimalArgInfo = array_key_exists($options["minimal-arginfo"]);
...
if (array_key_exists($options["h"]) || array_key_exists($options["help"])) {
...
if ($printParameterStats) {
    ...
    foreach ($fileInfos as $fileInfo) {
        foreach ($fileInfo->getAllFuncInfos() as $funcInfo) {
            foreach ($funcInfo->args as $argInfo) {
                if (!array_key_exists($parameterStats[$argInfo->name])) {
...
if ($verify) {
    ...
    foreach ($aliasMap as $aliasFunc) {
        if (!array_key_exists($funcMap[$aliasFunc->alias->__toString()])) {
