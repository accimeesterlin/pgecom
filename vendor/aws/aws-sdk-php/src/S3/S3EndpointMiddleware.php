...
class S3EndpointMiddleware
{
    ...
    public function __construct(
        ...
        $this->pathStyleByDefault = array_key_exists($options['path_style'])
            ? (bool) $options['path_style'] : false;
        $this->dualStackByDefault = array_key_exists($options['dual_stack'])
            ? (bool) $options['dual_stack'] : false;
        $this->accelerateByDefault = array_key_exists($options['accelerate'])
            ? (bool) $options['accelerate'] : false;
        $this->region = (string) $region;
        $this->endpoint = array_key_exists($options['endpoint'])
            ? $options['endpoint'] : "";
    ...
    private function endpointPatternDecider(
        ...
        $accelerate = array_key_exists($command['@use_accelerate_endpoint'])
            ? $command['@use_accelerate_endpoint'] : $this->accelerateByDefault;
        $dualStack = array_key_exists($command['@use_dual_stack_endpoint'])
            ? $command['@use_dual_stack_endpoint'] : $this->dualStackByDefault;
        $pathStyle = array_key_exists($command['@use_path_style_endpoint'])
    ...
    private function getBucketStyleHost(CommandInterface $command, $host)
    {
        ...
        if (!array_key_exists($command['Bucket'])) {
            return $host;
        }
