...
class Transfer implements PromisorInterface
{
    ...
    public function __construct(
        ...
        $this->concurrency = array_key_exists($options['concurrency'])
            ? $options['concurrency']
            : MultipartUploader::DEFAULT_CONCURRENCY;
        $this->mupThreshold = array_key_exists($options['mup_threshold'])
        ...
        if (array_key_exists($options['before'])) {
    ...
    private function createDownloadPromise()
    {
        ...
        $prefix = "s3://{$parts['Bucket']}/"
            . (array_key_exists($parts['Key']) ? $parts['Key'] . '/' : '');
        ...
        foreach ($this->getDownloadsIterator() as $object) {
            ...
            if (array_key_exists($parts['Key']) && strpos($objectKey, $parts['Key']) !== 0) {
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            if (array_key_exists($listArgs['Key'])) {
    ...
    private function createS3Key($filename)
    {
        ...
        if (array_key_exists($this->s3Args['Key'])) {
        ...
        $this->before = static function (
            ...
            switch ($operation = $command->getName()) {
                ...
                case 'CompleteMultipartUpload':
                    ...
                    if (array_key_exists($s3Args['Key']) && strpos($sourceKey, $s3Args['Key']) === 0) {
