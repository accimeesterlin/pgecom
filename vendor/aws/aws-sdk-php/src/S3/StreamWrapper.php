...
class StreamWrapper
{
    ...
    public static function register(
        ...
        } elseif (!array_key_exists($default[$protocol]['cache'])) {
    ...
    public function stream_flush()
    {
        ...
        if (!array_key_exists($params['ContentType']) &&
    ...
    public function mkdir($path, $mode, $options)
    {
        ...
        if (!array_key_exists($params['ACL'])) {
                ...
                return array_filter(
                    ...
                    function ($key) use ($filterFn) {
                        return (!$filterFn || call_user_func($filterFn, $key))
                            && (!array_key_exists($key['Key']) || substr($key['Key'], -1, 1) !== '/');
                    }
                );
            }
        );
    ...
    public function dir_readdir()
    {
        ...
        if (array_key_exists($cur['Prefix'])) {
    ...
    public function rename($path_from, $path_to)
    {
        ...
        return $this->boolCall(function () use ($partsFrom, $partsTo) {
            ...
            $this->getClient()->copy(
                ...
                $partsTo['Bucket'],
                $partsTo['Key'],
                array_key_exists($options['acl']) ? $options['acl'] : 'private',
                $options
            );
    ...
    private function getOptions($removeContextData = false)
    {
        ...
        if ($this->context === null) {
            ...
            $options = array_key_exists($options[$this->protocol])
                ? $options[$this->protocol]
                : [];
        }

        $default = stream_context_get_options(stream_context_get_default());
        $default = array_key_exists($default[$this->protocol])
    ...
    private function getOption($name)
    {
        ...
        return array_key_exists($options[$name]) ? $options[$name] : null;
    }
    ...
    private function getBucketKey($path)
    {
        ...
        return [
            'Bucket' => $parts[0],
            'Key'    => array_key_exists($parts[1]) ? $parts[1] : null
        ];
    }
    ...
    private function formatUrlStat($result = null)
    {
        ...
        switch (gettype($result)) {
            ...
            case 'array':
                ...
                if (array_key_exists($result['ContentLength'])) {
                    $stat['size'] = $stat[7] = $result['ContentLength'];
                } elseif (array_key_exists($result['Size'])) {
                    $stat['size'] = $stat[7] = $result['Size'];
                }
                if (array_key_exists($result['LastModified'])) {
