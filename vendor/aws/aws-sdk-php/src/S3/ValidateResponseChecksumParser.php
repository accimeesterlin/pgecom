...
class ValidateResponseChecksumParser extends AbstractParser
{
    ...
    public function __invoke(
        ...
        $checksumInfo = array_key_exists($op['httpChecksum'])
            ? $op['httpChecksum']
            : [];
        ...
        $checksumModeEnabledMember = array_key_exists($checksumInfo['requestValidationModeMember'])
            ? $checksumInfo['requestValidationModeMember']
            : "";
        $checksumModeEnabled = array_key_exists($command[$checksumModeEnabledMember])
            ? $command[$checksumModeEnabledMember]
            : "";
        $responseAlgorithms = array_key_exists($checksumInfo['responseAlgorithms'])
    ...
    public function validateChecksum($checksumPriority, ResponseInterface $response)
    {
        ...
        if (!empty($checksumToValidate)) {
            ...
            if (array_key_exists($checksumHeaderValue)) {
