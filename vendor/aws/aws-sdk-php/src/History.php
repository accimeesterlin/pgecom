...
class History implements \Countable, \IteratorAggregate
{
    ...
    public function getLastReturn()
    {
        ...
        if (array_key_exists($last['result'])) {
            return $last['result'];
        }

        if (array_key_exists($last['exception'])) {
    ...
    {
        if (!array_key_exists($this->entries[$ticket])) {
            throw new \InvalidArgumentException('Invalid history ticket');
        }

        if (array_key_exists($this->entries[$ticket]['result'])
            || array_key_exists($this->entries[$ticket]['exception'])
        ) {
