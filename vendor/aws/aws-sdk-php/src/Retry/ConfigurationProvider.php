    ...
    implements ConfigurationProviderInterface
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (array_key_exists($config['retries'])
    ...
    public static function ini(
        ...
        return function () use ($profile, $filename) {
            ...
            if (!array_key_exists($data[$profile][self::INI_MODE])) {
                return self::reject("Required retry config values
                    not present in INI profile '{$profile}' ({$filename})");
            }

            $maxAttempts = array_key_exists($data[$profile][self::INI_MAX_ATTEMPTS])
    ...
    public static function unwrap($config)
    {
        ...
        if (is_array($config) && array_key_exists($config['mode'])) {
            $maxAttempts = array_key_exists($config['max_attempts'])
                ? $config['max_attempts']
                : self::DEFAULT_MAX_ATTEMPTS;
