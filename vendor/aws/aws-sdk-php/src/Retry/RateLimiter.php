...
class RateLimiter
{
    ...
    {
        $this->beta = array_key_exists($options['beta'])
            ? $options['beta']
            : 0.7;
        $this->minCapacity = array_key_exists($options['min_capacity'])
            ? $options['min_capacity']
            : 1;
        $this->minFillRate = array_key_exists($options['min_fill_rate'])
            ? $options['min_fill_rate']
            : 0.5;
        $this->scaleConstant = array_key_exists($options['scale_constant'])
            ? $options['scale_constant']
            : 0.4;
        $this->smooth = array_key_exists($options['smooth'])
            ? $options['smooth']
            : 0.8;
        $this->timeProvider = array_key_exists($options['time_provider'])
    ...
    {
        $timestamp = $this->time();
        if (!array_key_exists($this->lastTimestamp)) {
