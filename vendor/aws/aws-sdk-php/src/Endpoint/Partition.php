...
final class Partition implements ArrayAccess, PartitionInterface
{
    ...
    public function __construct(array $definition)
    {
        foreach (['partition', 'regions', 'services', 'dnsSuffix'] as $key) {
            if (!array_key_exists($definition[$key])) {
    ...
    {
        if (array_key_exists($this->data['regions'][$region])
            || array_key_exists($this->data['services'][$service]['endpoints'][$region])
        ) {
            return true;
        }

        if (array_key_exists($this->data['regionRegex'])) {
    ...
    public function getAvailableEndpoints(
        ...
        if (array_key_exists($this->data['services'][$service]['endpoints'])) {
    ...
    public function __invoke(array $args = [])
    {
        ...
        $scheme = array_key_exists($args['scheme']) ? $args['scheme'] : 'https';
        $options = array_key_exists($args['options']) ? $args['options'] : [];
        $data = $this->getEndpointData($service, $region, $options);
        $variant = $this->getVariant($options, $data);
        if (array_key_exists($variant['hostname'])) {
            $template = $variant['hostname'];
        } else {
            $template = array_key_exists($data['hostname']) ? $data['hostname'] : '';
        }
        $dnsSuffix = array_key_exists($variant['dnsSuffix'])
            ? $variant['dnsSuffix']
            : $this->data['dnsSuffix'];
        return [
            ...
            'signingRegion' => array_key_exists($data['credentialScope']['region'])
                ? $data['credentialScope']['region']
                : $region,
            'signingName' => array_key_exists($data['credentialScope']['service'])
    ...
    private function getEndpointData($service, $region, $options)
    {
        ...
        $data = array_key_exists($this->data['services'][$service]['endpoints'][$defaultRegion])
            ? $this->data['services'][$service]['endpoints'][$defaultRegion]
            : [];
        $data += array_key_exists($this->data['services'][$service]['defaults'])
            ? $this->data['services'][$service]['defaults']
            : [];
        $data += array_key_exists($this->data['defaults'])
            ? $this->data['defaults']
            : [];
    ...
    private function getSignatureVersion(array $data)
    {
        ...
        $possibilities = array_intersect(
            $supportedBySdk,
            array_key_exists($data['signatureVersions'])
    ...
    {
        if (array_key_exists($this->data['services'][$service]['endpoints'][$region])
    ...
    {
        return array_key_exists($this->data['services'][$service]['isRegionalized'])
            && false === $this->data['services'][$service]['isRegionalized']
            && array_key_exists($this->data['services'][$service]['partitionEndpoint']);
    }
    ...
    private function getVariant(array $options, array $data)
    {
        ...
        if (!empty($variantTags)) {
            ...
            if (array_key_exists($this->data['defaults']['variants'])) {
