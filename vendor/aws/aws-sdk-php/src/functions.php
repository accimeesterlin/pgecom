...
function load_compiled_json($path)
{
    ...
    if (!array_key_exists($compiledList[$compiledFilepath])) {
...
function recursive_dir_iterator($path, $context = null)
{
    ...
    do {
        while ($iterator->valid()) {
            ...
            $iterator->next();
            if (array_key_exists($invalid[basename($file)])) {
                continue;
            }
            $fullPath = "{$path}/{$file}";
            yield $fullPath;
...
function manifest($service = null)
{
    ...
    if (array_key_exists($manifest[$service])) {
        return $manifest[$service] + ['endpoint' => $service];
    }

    if (array_key_exists($aliases[$service])) {
