...
class DocModel
{
    ...
    {
        return array_key_exists($this->docs['service']) ? $this->docs['service'] : null;
    }
    ...
    {
        return array_key_exists($this->docs['operations'][$operation])
    ...
    {
        return array_key_exists($this->docs['shapes'][$error]['base'])
    ...
    {
        if (!array_key_exists($this->docs['shapes'][$shapeName])) {
            return '';
        }

        $result = '';
        $d = $this->docs['shapes'][$shapeName];
        if (array_key_exists($d['refs']["{$parentName}\$${ref}"])) {
            $result = $d['refs']["{$parentName}\$${ref}"];
        } elseif (array_key_exists($d['base'])) {
            $result = $d['base'];
        }

        if (array_key_exists($d['append'])) {
            $result .= $d['append'];
        }
