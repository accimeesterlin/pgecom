...
class ApiProvider
{
    ...
    public static function resolve(callable $provider, $type, $service, $version)
    {
        ...
        if (is_array($result)) {
            if (!array_key_exists($result['metadata']['serviceIdentifier'])) {
        ...
        if (!array_key_exists(self::$typeMap[$type])) {
    ...
    {
        if (!array_key_exists($this->manifest)) {
            $this->buildVersionsList($service);
        }

        if (!array_key_exists($this->manifest[$service]['versions'])) {
            return [];
        }
    ...
    public function __invoke($type, $service, $version)
    {
        ...
        if (!array_key_exists($this->manifest[$service]['versions'][$version])) {
            return null;
        }
