...
abstract class RestSerializer
{
    ...
    public function __invoke(CommandInterface $command)
    {
        ...
        return new Psr7\Request(
            ...
            $uri,
            array_key_exists($opts['headers']) ? $opts['headers'] : [],
            array_key_exists($opts['body']) ? $opts['body'] : null
        );
    }
    ...
    private function serialize(Operation $operation, array $args)
    {
        ...
        if (array_key_exists($bodyMembers)) {
            $this->payload($operation->getInput(), $bodyMembers, $opts);
        } else if (!array_key_exists($opts['body']) && $this->hasPayloadParam($input, $payload)) {
    ...
    private function applyQuery($name, Shape $member, $value, array &$opts)
    {
        if ($member instanceof MapShape) {
            $opts['query'] = array_key_exists($opts['query']) && is_array($opts['query'])
                ? $opts['query'] + $value
                : $value;
        } elseif ($value !== null) {
            $type = $member->getType();
    ...
    private function buildEndpoint(Operation $operation, array $args, array $opts)
    {
        ...
        $relative = preg_replace_callback(
            ...
            function (array $matches) use ($varspecs) {
                ...
                if (!array_key_exists($varspecs[$k])) {
                    return '';
                }
    ...
    private function hasPayloadParam(StructureShape $input, $payload)
    {
        ...
        foreach ($input->getMembers() as $member) {
            if (!array_key_exists($member['location'])) {
                return true;
            }
        }
        return false;
