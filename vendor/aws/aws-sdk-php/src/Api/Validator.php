...
class Validator
{
    ...
    private function dispatch(Shape $shape, $value)
    {
        ...
        $type = $shape->getType();
        if (array_key_exists($methods[$type])) {
    ...
    {
        $isDocument = (array_key_exists($shape['document']) && $shape['document']);
        $isUnion = (array_key_exists($shape['union']) && $shape['union']);
        if ($isDocument) {
            ...
            foreach ($value as $name => $v) {
                if ($shape->hasMember($name)) {
                    ...
                    $this->dispatch(
                        ...
                        array_key_exists($value[$name]) ? $value[$name] : null
                    );
                    array_pop($this->path);
                }
            }
    ...
    private function check_blob(Shape $shape, $value)
    {
        ...
        $type = gettype($value);
        if (!array_key_exists($valid[$type])) {
    ...
    private function checkCanString($value)
    {
        ...
        return array_key_exists($valid[$type]) ||
