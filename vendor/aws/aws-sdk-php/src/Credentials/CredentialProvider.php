...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (
            !array_key_exists($config['use_aws_shared_config_files'])
        ...
        if (array_key_exists($config['credentials'])
            && $config['credentials'] instanceof CacheInterface
        ) {
            foreach ($cacheable as $provider) {
                if (array_key_exists($defaultChain[$provider])) {
    ...
    public static function sso($ssoProfileName, $filename = null, $config = [])
    {
        ...
        return function () use ($ssoProfileName, $filename, $config) {
            ...
            if (!array_key_exists($profiles[$ssoProfileName])) {
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use ($config) {
            ...
            $stsClient = array_key_exists($config['stsClient'])
                ? $config['stsClient']
                : null;
            $region = array_key_exists($config['region'])
                ? $config['region']
                : null;
            ...
            $profileName = getenv(self::ENV_PROFILE) ?: 'default';
            if (array_key_exists($config['filename'])) {
                $profiles = self::loadProfiles($config['filename']);
            } else {
                $profiles = self::loadDefaultProfiles();
            }

            if (array_key_exists($profiles[$profileName])) {
                $profile = $profiles[$profileName];
                if (array_key_exists($profile['region'])) {
                    $region = $profile['region'];
                }
                if (array_key_exists($profile['web_identity_token_file'])
                    && array_key_exists($profile['role_arn'])
                ) {
                    $sessionName = array_key_exists($profile['role_session_name'])
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use ($profile, $filename, $config) {
            $preferStaticCredentials = array_key_exists($config['preferStaticCredentials'])
                ? $config['preferStaticCredentials']
                : false;
            $disableAssumeRole = array_key_exists($config['disableAssumeRole'])
                ? $config['disableAssumeRole']
                : false;
            $stsClient = array_key_exists($config['stsClient']) ? $config['stsClient'] : null;
            ...
            if (!array_key_exists($data[$profile])) {
            ...
            */
            $preferStaticCredentialsToRoleArn = ($preferStaticCredentials
                && array_key_exists($data[$profile]['aws_access_key_id'])
                && array_key_exists($data[$profile]['aws_secret_access_key']));

            if (array_key_exists($data[$profile]['role_arn'])
            ...
            if (!array_key_exists($data[$profile]['aws_access_key_id'])
                || !array_key_exists($data[$profile]['aws_secret_access_key'])
            ) {
            ...
            if (empty($data[$profile]['aws_session_token'])) {
                $data[$profile]['aws_session_token']
                    = array_key_exists($data[$profile]['aws_security_token'])
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            if (!array_key_exists($data[$profile]['credential_process'])) {
            ...
            if (array_key_exists($processData['Version'])) {
                if ($processData['Version'] !== 1) {
                    return self::reject("credential_process does not return Version == 1");
                }
            }

            if (!array_key_exists($processData['AccessKeyId'])
                || !array_key_exists($processData['SecretAccessKey']))
            {
                return self::reject("credential_process does not return valid credentials");
            }

            if (array_key_exists($processData['Expiration'])) {
                try {
    ...
    private static function loadRoleProfile(
        ...
        $roleArn = array_key_exists($roleProfile['role_arn']) ? $roleProfile['role_arn'] : '';
        $roleSessionName = array_key_exists($roleProfile['role_session_name'])
        ...
        if (empty($stsClient)) {
            $sourceRegion = array_key_exists($profiles[$sourceProfileName]['region'])
    ...
    private static function loadProfiles($filename)
    {
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials'
            ...
            foreach ($configProfileData as $name => $profile) {
                ...
                if (!array_key_exists($profileData[$name])) {
    ...
    private static function loadDefaultProfiles() {
        ...
        if (file_exists($configFile)) {
            ...
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
                if (!array_key_exists($profiles[$name])) {
