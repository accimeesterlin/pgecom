...
class AssumeRoleWithWebIdentityCredentialProvider
{
    ...
    {
        if (!array_key_exists($config['RoleArn'])) {
            throw new \InvalidArgumentException(self::ERROR_MSG . "'RoleArn'.");
        }
        $this->arn = $config['RoleArn'];

        if (!array_key_exists($config['WebIdentityTokenFile'])) {
        ...
        $this->session = array_key_exists($config['SessionName'])
            ? $config['SessionName']
        ...
        if (array_key_exists($config['client'])) {
