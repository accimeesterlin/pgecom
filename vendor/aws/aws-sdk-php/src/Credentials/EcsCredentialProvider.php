...
class EcsCredentialProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!$timeout) {
            $timeout = array_key_exists($_SERVER[self::ENV_TIMEOUT])
                ? $_SERVER[self::ENV_TIMEOUT]
                : (array_key_exists($config['timeout']) ? $config['timeout'] : 1.0);
        }
    ...
    private function getEcsUri()
    {
        ...
        if ($credsUri === false) {
            $credsUri = array_key_exists($_SERVER[self::ENV_URI]) ? $_SERVER[self::ENV_URI] : '';
        }

        if(empty($credsUri)){
            $credFullUri = getenv(self::ENV_FULL_URI);
            if($credFullUri === false){
                $credFullUri = array_key_exists($_SERVER[self::ENV_FULL_URI]) ? $_SERVER[self::ENV_FULL_URI] : '';
            }
    ...
    private function decodeResult($response)
    {
        ...
        if (!array_key_exists($result['AccessKeyId'])) {
