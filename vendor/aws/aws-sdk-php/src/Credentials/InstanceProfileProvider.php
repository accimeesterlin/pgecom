...
class InstanceProfileProvider
{
    ...
    {
        $this->timeout = (float) getenv(self::ENV_TIMEOUT) ?: (array_key_exists($config['timeout']) ? $config['timeout'] : 1.0);
        $this->profile = array_key_exists($config['profile']) ? $config['profile'] : null;
        $this->retries = (int) getenv(self::ENV_RETRIES) ?: (array_key_exists($config['retries']) ? $config['retries'] : 3);
        $this->attempts = 0;
        $this->client = array_key_exists($config['client'])
    ...
    private function handleRetryableException(
        ...
        if (!empty($status = $this->getExceptionStatusCode($e))
            && array_key_exists($retryOptions['blacklist'])
