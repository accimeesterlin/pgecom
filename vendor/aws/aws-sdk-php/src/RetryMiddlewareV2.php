...
class RetryMiddlewareV2
{
    ...
    public function __construct(
        ...
        $this->maxBackoff = array_key_exists($options['max_backoff'])
            ? $options['max_backoff']
            : 20000;

        $this->collectStats = array_key_exists($options['collect_stats'])
            ? (bool) $options['collect_stats']
            : false;

        $this->decider = array_key_exists($options['decider'])
            ? $options['decider']
        ...
        $this->delayer = array_key_exists($options['delayer'])
            ? $options['delayer']
            : function ($attempts) {
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = array_key_exists($options['rate_limiter'])
    ...
    private static function isRetryable(
        ...
        if (!$isError) {
            if (!array_key_exists($result['@metadata']['statusCode'])) {
                return false;
            }
            return array_key_exists($statusCodes[$result['@metadata']['statusCode']]);
        }
        ...
        if (count($retryCurlErrors)
            ...
            if (method_exists($previous, 'getHandlerContext')) {
                ...
                return !empty($context['errno'])
                    && array_key_exists($retryCurlErrors[$context['errno']]);
            }
