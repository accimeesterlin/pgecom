...
class ClientResolver
{
    ...
    public function resolve(array $args, HandlerList $list)
    {
        ...
        foreach ($this->argDefinitions as $key => $a) {
            ...
            foreach ($a['valid'] as $check) {
                if (array_key_exists(self::$typeMap[$check])) {
    ...
    private function getArgMessage($name, $args = [], $useRequired = false)
    {
        ...
        $msg = '';
        $modifiers = [];
        if (array_key_exists($arg['valid'])) {
            $modifiers[] = implode('|', $arg['valid']);
        }
        if (array_key_exists($arg['choice'])) {
        ...
        } elseif (array_key_exists($arg['doc'])) {
            $msg .= wordwrap("\n\n  {$arg['doc']}", 75, "\n  ");
        }
    ...
    private function throwRequired(array $args)
    {
        ...
        foreach ($this->argDefinitions as $k => $a) {
            if (empty($a['required'])
                || array_key_exists($a['default'])
                || array_key_exists($args[$k])
            ) {
                continue;
            }
    ...
    public static function _apply_defaults($value, array &$args, HandlerList $list)
    {
        ...
        if ($config->getMode() !== 'legacy') {
            if (!array_key_exists($args['retries']) && !is_null($config->getRetryMode())) {
                $args['retries'] = ['mode' => $config->getRetryMode()];
            }
            if (
                !array_key_exists($args['sts_regional_endpoints'])
                && !is_null($config->getStsRegionalEndpoints())
            ) {
                $args['sts_regional_endpoints'] = ['mode' => $config->getStsRegionalEndpoints()];
            }
            if (
                !array_key_exists($args['s3_us_east_1_regional_endpoint'])
            ...
            if (!array_key_exists($args['http'])) {
                $args['http'] = [];
            }
            if (
                !array_key_exists($args['http']['connect_timeout'])
                && !is_null($config->getConnectTimeoutInMillis())
            ) {
                $args['http']['connect_timeout'] = $config->getConnectTimeoutInMillis() / 1000;
            }
            if (
                !array_key_exists($args['http']['timeout'])
    ...
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            ...
            && array_key_exists($value['key'])
            && array_key_exists($value['secret'])
        ) {
            $args['credentials'] = CredentialProvider::fromCredentials(
                new Credentials(
                    $value['key'],
                    $value['secret'],
                    array_key_exists($value['token']) ? $value['token'] : null,
                    array_key_exists($value['expires']) ? $value['expires'] : null
                )
            );
        } elseif ($value === false) {
            $args['credentials'] = CredentialProvider::fromCredentials(
    ...
    public static function _apply_api_provider(callable $value, array &$args)
    {
        ...
        if (
            ...
            && array_key_exists($api['metadata']['signingName'])
        ) {
    ...
    {
        if (!array_key_exists($args['endpoint'])) {
            $endpointPrefix = array_key_exists($args['api']['metadata']['endpointPrefix'])
            ...
            $serviceEndpoints =
                is_array($value) && array_key_exists($value['services'][$args['service']]['endpoints'])
                    ? $value['services'][$args['service']]['endpoints']
                    : null;
            if (array_key_exists($serviceEndpoints[$args['region']]['deprecated'])) {
            ...
            if (
                ...
                && array_key_exists($result['signatureVersion'])
            ) {
            ...
            if (
                ...
                && array_key_exists($result['signingRegion'])
            ) {
            ...
            if (
                empty($args['config']['signing_name'])
                && array_key_exists($result['signingName'])
            ) {
    ...
    {
        if (array_key_exists($args['config']['signature_version'])) {
            return $args['config']['signature_version'];
        }

        $args['__partition_result'] = array_key_exists($args['__partition_result'])
            ? array_key_exists($args['__partition_result'])
            : call_user_func(PartitionEndpointProvider::defaultProvider(), [
                'service' => $args['service'],
                'region' => $args['region'],
            ]);

        return array_key_exists($args['__partition_result']['signatureVersion'])
    ...
    {
        if (array_key_exists($args['config']['signing_name'])) {
        ...
        if (array_key_exists($args['__partition_result']['signingName'])) {
    ...
    {
        if (array_key_exists($args['config']['signing_region'])) {
        ...
        return array_key_exists($args['__partition_result']['signingRegion'])
...
EOT;
    ...
    private static function getEndpointProviderOptions(array $args)
    {
        ...
        foreach ($configKeys as $key) {
            if (array_key_exists($args['config'][$key])) {
