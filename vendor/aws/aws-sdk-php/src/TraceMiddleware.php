...
class TraceMiddleware
{
    ...
    private function compareStep(array $a, array $b, $title, array $keys)
    {
        $changes = [];
        foreach ($keys as $key) {
            $av = array_key_exists($a[$key]) ? $a[$key] : null;
            $bv = array_key_exists($b[$key]) ? $b[$key] : null;
    ...
    {
        if ($this->config['http'] && !array_key_exists($command['@http']['debug'])) {
    ...
    {
        if (!array_key_exists($this->service["shapes"])) {
            return $cmd->toArray();
        }
        ...
        foreach ($iterator as $parameter => $value) {
           if (array_key_exists($shapes[$parameter]['sensitive']) &&
