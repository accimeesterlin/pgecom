    ...
    OutpostsArnInterface
{
    ...
    private static function parseOutpostData(array $data)
    {
        ...
        $data['outpost_id'] = array_key_exists($resourceData[0])
            ? $resourceData[0]
            : null;
        $data['accesspoint_type'] = array_key_exists($resourceData[1])
            ? $resourceData[1]
            : null;
        $data['accesspoint_name'] = array_key_exists($resourceData[2])
            ? $resourceData[2]
            : null;
        if (array_key_exists($resourceData[3])) {
