    ...
    implements ConfigurationProviderInterface
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (array_key_exists($config['csm']) && $config['csm'] instanceof CacheInterface) {
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            if (!array_key_exists($data[$profile]['csm_enabled'])) {
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof ConfigurationInterface) {
            return $config;
        } elseif (is_array($config) && array_key_exists($config['enabled'])) {
            $client_id = array_key_exists($config['client_id']) ? $config['client_id']
                : self::DEFAULT_CLIENT_ID;
            $host = array_key_exists($config['host']) ? $config['host']
                : self::DEFAULT_HOST;
            $port = array_key_exists($config['port']) ? $config['port']
                : self::DEFAULT_PORT;
