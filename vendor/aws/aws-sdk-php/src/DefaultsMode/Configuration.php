...
class Configuration implements ConfigurationInterface
{
    ...
    public function __construct($mode = 'legacy')
    {
        ...
        if (array_key_exists($data['modes'][$mode])) {
            $modeData = $data['modes'][$mode];
            foreach ($modeData as $settingName => $settingValue) {
                if (array_key_exists($this->$settingName)) {
                    if (array_key_exists($settingValue['override'])) {
                        $this->$settingName = $settingValue['override'];
                    } else if (array_key_exists($settingValue['multiply'])) {
                        $this->$settingName *= $settingValue['multiply'];
                    } else if (array_key_exists($settingValue['add'])) {
