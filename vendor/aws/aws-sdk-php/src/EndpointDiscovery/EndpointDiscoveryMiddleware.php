...
class EndpointDiscoveryMiddleware
{
    ...
    public function __invoke(CommandInterface $cmd, RequestInterface $request)
    {
        ...
        if (array_key_exists($op['endpointdiscovery'])) {
            ...
            if ($config->isEnabled()) {
                if (array_key_exists($op['endpointoperation'])) {
                ...
                // Check/create cache
                if (!array_key_exists(self::$cache)) {
    ...
    private function discoverEndpoint(
        ...
        if (array_key_exists($result['Endpoints'])) {
            $endpointData = [];
    ...
    private function getDiscoveryCommand(
        ...
        if (!array_key_exists($endpointOperation)) {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        if (array_key_exists($parsed['host'])) {
            return $parsed;
        }

        // parse_url() will put host & path in 'path' if scheme is not provided
        if (array_key_exists($parsed['path'])) {
            $split = explode('/', $parsed['path'], 2);
            $parsed['host'] = $split[0];
            if (array_key_exists($split[1])) {
