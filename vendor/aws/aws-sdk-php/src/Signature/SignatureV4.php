...
class SignatureV4 implements SignatureInterface
{
    ...
    public function __construct($service, $region, array $options = [])
    {
        ...
        $this->region = $region;
        $this->unsigned = array_key_exists($options['unsigned-body']) ? $options['unsigned-body'] : false;
        $this->useV4a = array_key_exists($options['use_v4a']) && $options['use_v4a'] === true;
    }
    ...
    public function signRequest(
        ...
        $service = array_key_exists($signingService) ? $signingService : $this->service;
    ...
    private function getPresignHeaders(array $headers)
    {
        ...
        foreach ($headers as $name => $value) {
            ...
            if (!array_key_exists($blacklist[$lName])
    ...
    public function presign(
        ...
        $startTimestamp = array_key_exists($options['start_time'])
    ...
    private function createContext(array $parsedRequest, $payload)
    {
        ...
        foreach ($parsedRequest['headers'] as $key => $values) {
            $key = strtolower($key);
            if (!array_key_exists($blacklist[$key])) {
                foreach ($values as $v) {
                    $aggregate[$key][] = $v;
                }
            }
