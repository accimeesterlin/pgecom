...
class AwsClient implements AwsClientInterface
{
    ...
    public function __construct(array $args)
    {
        ...
        if (!array_key_exists($args['service'])) {
            $args['service'] = manifest($service)['endpoint'];
        }
        if (!array_key_exists($args['exception_class'])) {
        ...
        $this->region = array_key_exists($config['region']) ? $config['region'] : null;
        ...
        if (array_key_exists($args['with_resolved'])) {
    ...
    public function getConfig($option = null)
    {
        return $option === null
            ? $this->config
            : (array_key_exists($this->config[$option])
                ? $this->config[$option]
                : null);
    }
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        if (!array_key_exists($this->getApi()['operations'][$name])) {
            $name = ucfirst($name);
            if (!array_key_exists($this->getApi()['operations'][$name])) {
    ...
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (
            ...
            if (array_key_exists($c['@context']['signature_version'])) {
    ...
    {
        if (!array_key_exists($this->aliases)) {
            if (is_null($file)) {
