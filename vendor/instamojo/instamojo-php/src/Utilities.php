<?php
    ...
    function api_request($method, $request_url, array $data=[], array $headers=[]) 
    {
        ...
        if(!in_array($http_code, $http_success_codes) || (array_key_exists($response_obj['success']) && $response_obj['success'] == false)) {
            
            $message = array_key_exists($response_obj['message']) ? $response_obj['message'] : 'Invalid request';
            ...
            switch($http_code) {
                ...
                default:
                    $message = array_key_exists($response_obj['reason']) ? $response_obj['reason'] :$message;
