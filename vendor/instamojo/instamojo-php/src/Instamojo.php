...
final class Instamojo {
    ...
    static function init($type='app', $params, $test=false)
    {
        ...
        self::$username     = array_key_exists($params['username']) ? $params['username'] : '';
        self::$password     = array_key_exists($params['password']) ? $params['password'] : '';
        self::$baseUrl      = Instamojo::PRODUCTION_BASE_URL;
        self::$scope        = array_key_exists($params['scope']) ? $params['scope'] : null;
        ...
        self::$refreshToken = array_key_exists($auth_response['refresh_token']) ? $auth_response['refresh_token'] : '';
        self::$scope = array_key_exists($auth_response['scope']) ? $auth_response['scope'] : '';
    ...
    public function auth($refresh=false) {
        ...
        if (!array_key_exists($response['access_token'])) {
            throw new AuthenticationException();
        }

        // check refresh token, incase of auth refresh
        if ($refresh) {
            if (!array_key_exists($response['refresh_token'])) {
                throw new AuthenticationException();
            } else {
