...
class PayPalHttpConfig
{
    ...
    public function __construct($url = null, $method = self::HTTP_POST, $configs = array())
    {
        ...
        $curl = curl_version();
        $sslVersion = array_key_exists($curl['ssl_version']) ? $curl['ssl_version'] : '';
    ...
    public function setSSLCert($certPath, $passPhrase = null)
    {
        ...
        if (array_key_exists($passPhrase) && trim($passPhrase) != "") {
    ...
    public function setHttpProxy($proxy)
    {
        ...
        if (array_key_exists($urlParts["port"])) {
            $this->curlOptions[CURLOPT_PROXY] .= ":" . $urlParts["port"];
        }
        if (array_key_exists($urlParts["user"])) {
