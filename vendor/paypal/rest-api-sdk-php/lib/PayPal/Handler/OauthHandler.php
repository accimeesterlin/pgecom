...
class OauthHandler implements IPayPalHandler
{
    ...
    public function handle($httpConfig, $request, $options)
    {
        ...
        $httpConfig->setUrl(
            ...
            (array_key_exists($options['path']) ? $options['path'] : '')
        );
    ...
    {
        if (array_key_exists($config['oauth.EndPoint'])) {
            $baseEndpoint = $config['oauth.EndPoint'];
        } elseif (array_key_exists($config['service.EndPoint'])) {
            $baseEndpoint = $config['service.EndPoint'];
        } elseif (array_key_exists($config['mode'])) {
