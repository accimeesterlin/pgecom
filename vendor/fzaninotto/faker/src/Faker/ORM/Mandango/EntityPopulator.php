    ...
    public function execute(Mandango $mandango, $insertedEntities)
    {
        ...
        foreach ($this->columnFormatters as $column => $format) {
            if (null !== $format) {
                ...
                if (array_key_exists($metadata['fields'][$column]) ||
                    array_key_exists($metadata['referencesOne'][$column])) {
                    $obj->set($column, $value);
                }

                if (array_key_exists($metadata['referencesMany'][$column])) {
