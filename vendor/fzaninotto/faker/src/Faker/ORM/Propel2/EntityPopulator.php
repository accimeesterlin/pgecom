        ...
        foreach ($tableMap->getColumns() as $columnMap) {
            ...
            if ($columnMap->isForeignKey()) {
                ...
                $formatters[$columnMap->getPhpName()] = function ($inserted) use ($relatedClass) {
                    ...
                    return array_key_exists($inserted[$relatedClass]) ? $inserted[$relatedClass][mt_rand(0, count($inserted[$relatedClass]) - 1)] : null;
                };
                continue;
            }
            ...
            switch ($name) {
                case 'nested_set':
                    $modifiers['nested_set'] = function ($obj, $inserted) use ($class, $generator) {
                        if (array_key_exists($inserted[$class])) {
                ...
                case 'sortable':
                    $modifiers['sortable'] = function ($obj, $inserted) use ($class) {
                        $maxRank = array_key_exists($inserted[$class]) ? count($inserted[$class]) : 0;
