...
class EntityPopulator
{
    ...
    public function guessColumnFormatters(\Faker\Generator $generator)
    {
        ...
        foreach ($this->class->getFieldNames() as $fieldName) {
            ...
            $size = array_key_exists($this->class->fieldMappings[$fieldName]['length']) ? $this->class->fieldMappings[$fieldName]['length'] : null;
        ...
        foreach ($this->class->getAssociationNames() as $assocName) {
                ...
                foreach ($mappings as $mapping) {
                    if ($mapping['targetEntity'] == $relatedClass) {
                        if ($mapping['type'] == \Doctrine\ORM\Mapping\ClassMetadata::ONE_TO_ONE) {
                            $unique = true;
                            $optional = array_key_exists($mapping['joinColumns'][0]['nullable']) ? $mapping['joinColumns'][0]['nullable'] : false;
                            break;
                        }
                    }
                }
                ...
                foreach ($mappings as $mapping) {
                    if ($mapping['targetDocument'] == $relatedClass) {
                        if ($mapping['type'] == \Doctrine\ODM\MongoDB\Mapping\ClassMetadata::ONE && $mapping['association'] == \Doctrine\ODM\MongoDB\Mapping\ClassMetadata::REFERENCE_ONE) {
                            $unique = true;
                            $optional = array_key_exists($mapping['nullable']) ? $mapping['nullable'] : false;
                            break;
                        }
                    }
                }
            ...
            $formatters[$assocName] = function ($inserted) use ($relatedClass, &$index, $unique, $optional) {
                ...
                if (array_key_exists($inserted[$relatedClass])) {
                    if ($unique) {
                        $related = null;
                        if (array_key_exists($inserted[$relatedClass][$index]) || !$optional) {
