...
final class TypeResolver
{
    ...
    private function parseTypes(ArrayIterator $tokens, Context $context, int $parserContext): Type
    {
        ...
        while ($tokens->valid()) {
            ...
            if ($token === '|' || $token === '&') {
                ...
                $types[] = $resolvedType;
            } elseif ($parserContext === self::PARSER_IN_ARRAY_EXPRESSION && array_key_exists($token[0]) && $token[0] === ')') {
                break;
            } elseif ($token === '<') {
                if (count($types) === 0) {
                    throw new RuntimeException(
    ...
    {
        return (array_key_exists($type[0]) && $type[0] !== self::OPERATOR_NAMESPACE) && !$this->isKeyword($type);
    }
