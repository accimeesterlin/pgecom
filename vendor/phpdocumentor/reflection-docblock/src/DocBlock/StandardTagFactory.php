...
final class StandardTagFactory implements TagFactory
{
    ...
    private function findHandlerClassName(string $tagName, TypeContext $context): string
    {
        ...
        if (array_key_exists($this->tagHandlerMappings[$tagName])) {
            $handlerClassName = $this->tagHandlerMappings[$tagName];
        } elseif ($this->isAnnotation($tagName)) {
            // TODO: Annotation support is planned for a later stage and as such is disabled for now
            $tagName = (string) $this->fqsenResolver->resolve($tagName, $context);
            if (array_key_exists($this->annotationMappings[$tagName])) {
    ...
    private function getArgumentsForParametersFromWiring(array $parameters, array $locator): array
    {
        ...
        foreach ($parameters as $parameter) {
            ...
            if (array_key_exists($locator[$typeHint])) {
                $arguments[] = $locator[$typeHint];
                continue;
            }

            $parameterName = $parameter->getName();
            if (array_key_exists($locator[$parameterName])) {
    ...
    {
        if (!array_key_exists($this->tagHandlerParameterCache[$handlerClassName])) {
