...
class Style
{
    ...
    public function fromStyleSheets(AbstractTag $tag, $attributes) {
        $class = array_key_exists($attributes["class"]) ? preg_split('/\s+/', trim($attributes["class"])) : null;
    ...
    protected function fillStyles($styles)
    {
        foreach ($this->getStyleMap() as $from => $spec) {
            if (array_key_exists($styles[$from])) {
                list($to, $type) = $spec;
                $value = null;
                switch ($type) {
    ...
    static function parseColor($color)
    {
        ...
        // SVG color name
        if (array_key_exists(self::$colorNames[$color])) {
