...
abstract class AbstractTag
{
    ...
    protected function applyTransform($attributes)
    {
        ...
        if (array_key_exists($attributes["transform"])) {
            ...
            foreach ($transformations as $t) {
                switch ($t[0]) {
                    ...
                    case "translate":
                        $surface->translate($t[1], array_key_exists($t[2]) ? $t[2] : 0);
                        break;

                    case "scale":
                        $surface->scale($t[1], array_key_exists($t[2]) ? $t[2] : $t[1]);
                        break;

                    case "rotate":
                        if (array_key_exists($t[2])) {
                            $t[3] = array_key_exists($t[3]) ? $t[3] : 0;
