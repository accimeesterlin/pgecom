...
class AdobeFontMetrics {
  ...
  function write($file, $encoding = null) {
    ...
    foreach ($records as $id => $record) {
      if (!array_key_exists(name::$nameIdCodes[$id]) || preg_match("/[\r\n]/", $record->string)) {
        continue;
      }
    ...
    if (array_key_exists($hhea["ascent"])) {
    ...
    if ($glyphIndexArray) {
      ...
      if ($encoding) {
        foreach ($map_data as $code => $value) {
          ...
          if (!array_key_exists($glyphIndexArray[$c])) {
            continue;
          }

          $g = $glyphIndexArray[$c];

          if (!array_key_exists($hmtx[$g])) {
            $hmtx[$g] = $hmtx[0];
          }
        ...
        foreach ($glyphIndexArray as $c => $g) {
          ...
          $this->addMetric(array(
            "U"  => $c,
            "WX" => $font->normalizeFUnit($hmtx[$g][0]),
            "N"  => (array_key_exists($names[$g]) ? $names[$g] : sprintf("uni%04x", $c)),
            "G"  => $g,
          ));
        }
      }
      ...
      if (!$encoding && is_array($tree)) {
        ...
        foreach ($tree as $left => $values) {
          ...
          if (!array_key_exists($glyphIndexArray[$left])) {
            continue;
          }

          $left_gid = $glyphIndexArray[$left];

          if (!array_key_exists($names[$left_gid])) {
            continue;
          }
          ...
          foreach ($values as $right => $value) {
            if (!array_key_exists($glyphIndexArray[$right])) {
              continue;
            }

            $right_gid = $glyphIndexArray[$right];

            if (!array_key_exists($names[$right_gid])) {
              continue;
            }
