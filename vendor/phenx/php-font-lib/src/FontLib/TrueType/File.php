...
class File extends BinaryStream {
  ...
  function setSubset($subset) {
    ...
    foreach ($subset as $code) {
      if (!array_key_exists($glyphIndexArray[$code])) {
        continue;
      }
  ...
  function encode($tags = array()) {
    ...
    $entries = array();
    foreach ($tags as $tag) {
      if (!array_key_exists($this->directory[$tag])) {
  ...
  protected function readTable($tag) {
    ...
    if (!self::$raw) {
      ...
      if (!array_key_exists($this->directory[$tag]) || !@class_exists($class)) {
        return;
      }
    }
    else {
  ...
  function getNameTableString($nameID) {
    ...
    if (!array_key_exists($records[$nameID])) {
      return null;
    }
