...
class Collection extends BinaryStream implements Iterator, Countable {
  ...
  function parse() {
    if (array_key_exists($this->numFonts)) {
      return;
    }
  ...
  function getFont($fontId) {
    ...
    if (!array_key_exists($this->collectionOffsets[$fontId])) {
      throw new OutOfBoundsException();
    }

    if (array_key_exists($this->collection[$fontId])) {
  ...
  function valid() {
    ...
    return array_key_exists($this->collectionOffsets[$this->position]);
  }
