...
class Swift_CharacterStream_ArrayCharacterStream implements Swift_CharacterStream
{
    ...
    {
        if (!array_key_exists($this->charReader)) {
    ...
    public function read($length)
    {
        ...
        for ($i = $this->offset; $i < $end; ++$i) {
            if (!array_key_exists($this->array[$i])) {
                break;
            }
            $arrays[] = $this->array[$i];
        }
    ...
    public function write($chars)
    {
        ...
        do {
            ...
            if ($buf_len - $buf_pos > 0) {
                $size = 0;
                for ($i = 0; $i < $startLength && array_key_exists($buffer[$buf_pos]); ++$i) {
                    ++$size;
                ...
                if ($need > 0) {
                    ...
                    for ($i = 0; $i < $need && array_key_exists($buffer[$buf_pos]); ++$i) {
                        $bytes[] = $buffer[$buf_pos++];
                    }
                }
                $this->array[] = $bytes;
    ...
    {
        if (!array_key_exists(self::$charMap)) {
            self::$charMap = [];
