...
class Sanitizer
{
    ...
    public static function jsonRequest(&$json)
    {
        ...
        foreach ($keys as $key) {
            if (!array_key_exists($json[$key])) continue;
    ...
    {
        if (array_key_exists($field['first_name'])) {
            $first_name = new self;
            $field['first_name'] = $first_name->maxLength(255)->apply($field['first_name']);
        }

        if (array_key_exists($field['last_name'])) {
            $last_name = new self;
        ...
        if (array_key_exists($field['phone'])) {
            $phone = new self;
        ...
        if (!empty($field['billing_address']) || !empty($field['shipping_address'])) {
            ...
            foreach ($keys as $key) {
                if (!array_key_exists($field[$key])) continue;
    ...
    private static function fieldBillingAddress(&$field)
    {
        ...
        if (array_key_exists($field['postal_code'])) {
