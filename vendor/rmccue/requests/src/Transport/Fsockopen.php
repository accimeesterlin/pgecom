...
final class Fsockopen implements Transport {
	...
	public function request($url, $headers = [], $data = [], $options = []) {
		...
		// HTTPS support
		if (array_key_exists($url_parts['scheme']) && strtolower($url_parts['scheme']) === 'https') {
			$remote_socket = 'ssl://' . $host;
			if (!array_key_exists($url_parts['port'])) {
			...
			if (defined('OPENSSL_TLSEXT_SERVER_NAME') && OPENSSL_TLSEXT_SERVER_NAME) {
				...
				if (array_key_exists($options['verifyname']) && $options['verifyname'] === false) {
		...
		if ($options['type'] !== Requests::TRACE) {
			...
			if (!empty($data) || $options['type'] === Requests::POST) {
				if (!array_key_exists($case_insensitive_headers['Content-Length'])) {
					$headers['Content-Length'] = strlen($request_body);
				}

				if (!array_key_exists($case_insensitive_headers['Content-Type'])) {
					$headers['Content-Type'] = 'application/x-www-form-urlencoded; charset=UTF-8';
				}
			}
		}

		if (!array_key_exists($case_insensitive_headers['Host'])) {
		...
		if (!array_key_exists($case_insensitive_headers['User-Agent'])) {
			$out .= sprintf("User-Agent: %s\r\n", $options['useragent']);
		}

		$accept_encoding = $this->accept_encoding();
		if (!array_key_exists($case_insensitive_headers['Accept-Encoding']) && !empty($accept_encoding)) {
		...
		if (!array_key_exists($case_insensitive_headers['Connection'])) {
	...
	private static function format_get($url_parts, $data) {
		...
		if (array_key_exists($url_parts['path'])) {
			if (array_key_exists($url_parts['query'])) {
	...
	public static function test($capabilities = []) {
		...
		if (array_key_exists($capabilities[Capability::SSL]) && $capabilities[Capability::SSL]) {
