...
class Requests {
	...
	protected static function get_transport_class(array $capabilities = []) {
		...
		if (array_key_exists(self::$transport[$cap_string])) {
	...
	public static function request_multiple($requests, $options = []) {
		...
		foreach ($requests as $id => &$request) {
			...
			if (!array_key_exists($request['type'])) {
	...
	protected static function set_defaults(&$url, &$headers, &$data, &$type, &$options) {
		...
		if (!array_key_exists($options['data_format'])) {
	...
	protected static function parse_response($headers, $url, $req_headers, $req_data, $options) {
		...
		if (array_key_exists($return->headers['transfer-encoding'])) {
			$return->body = self::decode_chunked($return->body);
			unset($return->headers['transfer-encoding']);
		}

		if (array_key_exists($return->headers['content-encoding'])) {
			$return->body = self::decompress($return->body);
		}

		//fsockopen and cURL compatibility
		if (array_key_exists($return->headers['connection'])) {
		...
		if ($return->is_redirect() && $options['follow_redirects'] === true) {
			if (array_key_exists($return->headers['location']) && $options['redirected'] < $options['redirects']) {
	...
	public static function decompress($data) {
		...
		if (!array_key_exists(self::$magic_compression_headers[$marker])) {
