...
class getid3_write_id3v1
{
	...
	public function WriteID3v1() {
		...
		if (!empty($this->filename) && is_readable($this->filename) && getID3::is_writable($this->filename) && is_file($this->filename)) {
			...
			if ($fp_source = fopen($this->filename, 'r+b')) {
				...
				$this->tag_data['track_number'] = (array_key_exists($this->tag_data['track_number']) ? $this->tag_data['track_number'] : '');

				$new_id3v1_tag_data = getid3_id3v1::GenerateID3v1Tag(
														(array_key_exists($this->tag_data['title']       ) ? $this->tag_data['title']        : ''),
														(array_key_exists($this->tag_data['artist']      ) ? $this->tag_data['artist']       : ''),
														(array_key_exists($this->tag_data['album']       ) ? $this->tag_data['album']        : ''),
														(array_key_exists($this->tag_data['year']        ) ? $this->tag_data['year']         : ''),
														(array_key_exists($this->tag_data['genreid']     ) ? $this->tag_data['genreid']      : ''),
														(array_key_exists($this->tag_data['comment']     ) ? $this->tag_data['comment']      : ''),
														(array_key_exists($this->tag_data['track_number']) ? $this->tag_data['track_number'] : ''));
	...
	public function FixID3v1Padding() {
		...
		if (array_key_exists($ThisFileInfo['tags']['id3v1'])) {
			$id3v1data = array();
