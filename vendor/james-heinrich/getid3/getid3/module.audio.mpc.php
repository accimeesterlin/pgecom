...
class getid3_mpc extends getid3_handler
{
	...
	public function MPCprofileNameLookup($profileid) {
		...
		return (array_key_exists($MPCprofileNameLookup[$profileid]) ? $MPCprofileNameLookup[$profileid] : 'invalid');
	}
	...
	public function MPCfrequencyLookup($frequencyid) {
		...
		return (array_key_exists($MPCfrequencyLookup[$frequencyid]) ? $MPCfrequencyLookup[$frequencyid] : 'invalid');
	}
	...
	public function MPCsv8PacketName($packetKey) {
		...
		return (array_key_exists($MPCsv8PacketName[$packetKey]) ? $MPCsv8PacketName[$packetKey] : $packetKey);
	}
}
