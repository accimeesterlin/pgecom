...
class getID3
{
	...
	public function setOption($optArray) {
		...
		foreach ($optArray as $opt => $val) {
			if (array_key_exists($this->$opt) === false) {
				continue;
			}
			$this->$opt = $val;
		}
	...
	public function analyze($filename, $filesize=null, $original_filename='', $fp=null) {
		try {
			...
			if (array_key_exists($this->info['id3v2']['tag_offset_start'])) {
				$this->info['avdataoffset'] = max($this->info['avdataoffset'], $this->info['id3v2']['tag_offset_end']);
			}
			foreach (array('id3v1'=>'id3v1', 'apetag'=>'ape', 'lyrics3'=>'lyrics3') as $tag_name => $tag_key) {
				if (array_key_exists($this->info[$tag_key]['tag_offset_start'])) {
			...
			if (array_key_exists($determined_format['fail_id3']) && (in_array('id3v1', $this->info['tags']) || in_array('id3v2', $this->info['tags']))) {
			...
			if (array_key_exists($determined_format['fail_ape']) && in_array('ape', $this->info['tags'])) {
	...
	public function error($message) {
		$this->CleanUp();
		if (!array_key_exists($this->info['error'])) {
	...
	private function CleanUp() {
		...
		foreach ($AVpossibleEmptyKeys as $dummy => $key) {
			if (empty($this->info['audio'][$key]) && array_key_exists($this->info['audio'][$key])) {
				unset($this->info['audio'][$key]);
			}
			if (empty($this->info['video'][$key]) && array_key_exists($this->info['video'][$key])) {
		...
		if (empty($this->info['fileformat'])) {
			if (array_key_exists($this->info['avdataoffset'])) {
				unset($this->info['avdataoffset']);
			}
			if (array_key_exists($this->info['avdataend'])) {
...
//						),
	...
	public function HandleAllTags() {
		...
		foreach ($tags as $comment_name => $tagname_encoding_array) {
			...
			if (array_key_exists($this->info[$comment_name]) && !array_key_exists($this->info[$comment_name]['encoding'])) {
			...
			if (!empty($this->info[$comment_name]['comments'])) {
				...
				if (!array_key_exists($this->info['tags'][$tag_name])) {
		...
		if (!empty($this->info['tags'])) {
			...
			foreach ($this->info['tags'] as $tagtype => $tagarray) {
				foreach ($tagarray as $tagname => $tagdata) {
					if ($tagname == 'picture') {
						foreach ($tagdata as $key => $tagarray) {
							...
							if (array_key_exists($tagarray['data']) && array_key_exists($tagarray['image_mime'])) {
								if (array_key_exists($this->info['tags'][$tagtype][$tagname][$key])) {
									unset($this->info['tags'][$tagtype][$tagname][$key]);
								}
								if (array_key_exists($this->info['tags_html'][$tagtype][$tagname][$key])) {
				...
				if (array_key_exists($this->info[$tagtype]['comments']['picture'])) {
	...
	public function getHashdata($algorithm) {
		...
		if (!empty($this->info['fileformat']) && !empty($this->info['dataformat']) && ($this->info['fileformat'] == 'ogg') && ($this->info['audio']['dataformat'] == 'vorbis')) {
			...
			if (!empty($this->info['avdataoffset']) || (array_key_exists($this->info['avdataend']) && ($this->info['avdataend'] < $this->info['filesize']))) {
	...
	public function ChannelsBitratePlaytimeCalculations() {
		...
		if (!empty($this->info['audio']['channelmode']) || !array_key_exists($this->info['audio']['channels'])) {
			// ignore
		...
		$CombinedBitrate  = 0;
		$CombinedBitrate += (array_key_exists($this->info['audio']['bitrate']) ? $this->info['audio']['bitrate'] : 0);
		$CombinedBitrate += (array_key_exists($this->info['video']['bitrate']) ? $this->info['video']['bitrate'] : 0);
		if (($CombinedBitrate > 0) && empty($this->info['bitrate'])) {
			$this->info['bitrate'] = $CombinedBitrate;
		}
		//if ((array_key_exists($this->info['video']) && !array_key_exists($this->info['video']['bitrate'])) || (array_key_exists($this->info['audio']) && !array_key_exists($this->info['audio']['bitrate']))) {
		...
		if (array_key_exists($this->info['video']['dataformat']) && $this->info['video']['dataformat'] && (!array_key_exists($this->info['video']['bitrate']) || ($this->info['video']['bitrate'] == 0))) {
			// if video bitrate not set
			if (array_key_exists($this->info['audio']['bitrate']) && ($this->info['audio']['bitrate'] > 0) && ($this->info['audio']['bitrate'] == $this->info['bitrate'])) {
				// AND if audio bitrate is set to same as overall bitrate
				if (array_key_exists($this->info['playtime_seconds']) && ($this->info['playtime_seconds'] > 0)) {
					// AND if playtime is set
					if (array_key_exists($this->info['avdataend']) && array_key_exists($this->info['avdataoffset'])) {
		...
		if ((!array_key_exists($this->info['playtime_seconds']) || ($this->info['playtime_seconds'] <= 0)) && !empty($this->info['bitrate'])) {
			$this->info['playtime_seconds'] = (($this->info['avdataend'] - $this->info['avdataoffset']) * 8) / $this->info['bitrate'];
		}

		if (!array_key_exists($this->info['bitrate']) && !empty($this->info['playtime_seconds'])) {
			$this->info['bitrate'] = (($this->info['avdataend'] - $this->info['avdataoffset']) * 8) / $this->info['playtime_seconds'];
		}
		if (array_key_exists($this->info['bitrate']) && empty($this->info['audio']['bitrate']) && empty($this->info['video']['bitrate'])) {
			if (array_key_exists($this->info['audio']['dataformat']) && empty($this->info['video']['resolution_x'])) {
				// audio only
				$this->info['audio']['bitrate'] = $this->info['bitrate'];
			} elseif (array_key_exists($this->info['video']['resolution_x']) && empty($this->info['audio']['dataformat'])) {
				// video only
	...
	public function CalculateReplayGain() {
		if (array_key_exists($this->info['replay_gain'])) {
			if (!array_key_exists($this->info['replay_gain']['reference_volume'])) {
				$this->info['replay_gain']['reference_volume'] = 89.0;
			}
			if (array_key_exists($this->info['replay_gain']['track']['adjustment'])) {
				$this->info['replay_gain']['track']['volume'] = $this->info['replay_gain']['reference_volume'] - $this->info['replay_gain']['track']['adjustment'];
			}
			if (array_key_exists($this->info['replay_gain']['album']['adjustment'])) {
				$this->info['replay_gain']['album']['volume'] = $this->info['replay_gain']['reference_volume'] - $this->info['replay_gain']['album']['adjustment'];
			}

			if (array_key_exists($this->info['replay_gain']['track']['peak'])) {
				$this->info['replay_gain']['track']['max_noclip_gain'] = 0 - getid3_lib::RGADamplitude2dB($this->info['replay_gain']['track']['peak']);
			}
			if (array_key_exists($this->info['replay_gain']['album']['peak'])) {
	...
	public function ProcessAudioStreams() {
		if (!empty($this->info['audio']['bitrate']) || !empty($this->info['audio']['channels']) || !empty($this->info['audio']['sample_rate'])) {
			if (!array_key_exists($this->info['audio']['streams'])) {
...
abstract class getid3_handler
{
	...
	*/
	...
	public function AnalyzeString($string) {
		...
		$saved_filesize     = (array_key_exists($this->getid3->info['filesize']) ? $this->getid3->info['filesize'] : null); // may be not set if called as dependency without openfile() call
	...
	public function saveAttachment($name, $offset, $length, $image_mime=null) {
		...
		try {
			...
			if (array_key_exists($fp_dest) && is_resource($fp_dest)) {
				fclose($fp_dest);
			}

			if (array_key_exists($dest) && file_exists($dest)) {
				unlink($dest);
			}
