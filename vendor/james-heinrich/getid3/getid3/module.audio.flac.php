...
class getid3_flac extends getid3_handler
{
	...
	public function parseMETAdata() {
		...
		if (array_key_exists($info['flac']['PICTURE']) && ($this->getid3->option_save_attachments !== getID3::ATTACHMENTS_NONE)) {
			foreach ($info['flac']['PICTURE'] as $entry) {
				if (!empty($entry['data'])) {
					if (!array_key_exists($info['flac']['comments']['picture'])) {
						$info['flac']['comments']['picture'] = array();
					}
					$comments_picture_data = array();
					foreach (array('data', 'image_mime', 'image_width', 'image_height', 'imagetype', 'picturetype', 'description', 'datalength') as $picture_key) {
						if (array_key_exists($entry[$picture_key])) {
		...
		if (array_key_exists($info['flac']['STREAMINFO'])) {
		...
		if (array_key_exists($info['flac']['STREAMINFO']['audio_signature'])) {
		...
		if (array_key_exists($info['flac']['STREAMINFO']['bits_per_sample'])) {
	...
	public static function metaBlockTypeLookup($blocktype) {
		...
		return (array_key_exists($lookup[$blocktype]) ? $lookup[$blocktype] : 'reserved');
	}
	...
	public static function applicationIDLookup($applicationid) {
		...
		return (array_key_exists($lookup[$applicationid]) ? $lookup[$applicationid] : 'reserved');
	}
	...
	public static function pictureTypeLookup($type_id) {
		...
		return (array_key_exists($lookup[$type_id]) ? $lookup[$type_id] : 'reserved');
	}
