...
class getid3_svg extends getid3_handler
{
	...
	public function Analyze() {
		...
		if (array_key_exists($info['svg']['svg']['raw'])) {

			$sections_to_fix = array('xml', 'doctype', 'svg');
			foreach ($sections_to_fix as $section_to_fix) {
				if (!array_key_exists($info['svg'][$section_to_fix])) {
					continue;
				}
				$section_data = array();
				...
				foreach ($section_data as $keyvaluepair) {
					...
					if ($keyvaluepair) {
						...
						$key   = (array_key_exists($keyvalueexploded[0]) ? $keyvalueexploded[0] : '');
						$value = (array_key_exists($keyvalueexploded[1]) ? $keyvalueexploded[1] : '');
			...
			if (!array_key_exists($info['svg']['version']) && array_key_exists($info['svg']['doctype']['sections'])) {
