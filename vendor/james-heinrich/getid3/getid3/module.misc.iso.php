...
class getid3_iso extends getid3_handler
{
	...
	public function ParsePathTable() {
		$info = &$this->getid3->info;
		if (!array_key_exists($info['iso']['supplementary_volume_descriptor']['raw']['path_table_l_location']) && !array_key_exists($info['iso']['primary_volume_descriptor']['raw']['path_table_l_location'])) {
			return false;
		}
		if (array_key_exists($info['iso']['supplementary_volume_descriptor']['raw']['path_table_l_location'])) {
	...
	public function ParseDirectoryRecord($directorydata) {
		$info = &$this->getid3->info;
		if (array_key_exists($info['iso']['supplementary_volume_descriptor'])) {
			$TextEncoding = 'UTF-16BE';   // Big-Endian Unicode
		} else {
			$TextEncoding = 'ISO-8859-1'; // Latin-1
		}
