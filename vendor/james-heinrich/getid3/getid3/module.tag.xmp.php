				...
				case 'rdf:li':
					...
					if ($xml_elem['type'] == 'complete')
					{
						if (array_key_exists('attributes', $xml_elem))
						{
							...
							if (array_key_exists($xml_elem['attributes']['xml:lang']) && ($xml_elem['attributes']['xml:lang'] != 'x-default'))
							{
								break;
							}
						}
						if ($current_property != '')
						{
							$xmp_array[$current_property][$container_index] = (array_key_exists($xml_elem['value']) ? $xml_elem['value'] : '');
							$container_index += 1;
						}
...
//					{
						switch ($xml_elem['type'])
						{
							...
							case 'complete':
								...
								$xmp_array[$xml_elem['tag']] = (array_key_exists($xml_elem['attributes']) ? $xml_elem['attributes'] : (array_key_exists($xml_elem['value']) ? $xml_elem['value'] : ''));
								break;
