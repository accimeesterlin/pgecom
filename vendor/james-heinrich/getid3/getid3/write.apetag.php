...
class getid3_write_apetag
{
	...
	public function WriteAPEtag() {
		...
		if (array_key_exists($ThisFileInfo['ape']['tag_offset_start']) && array_key_exists($ThisFileInfo['lyrics3']['tag_offset_end'])) {
		...
		if ($this->always_preserve_replaygain) {
			...
			foreach ($ReplayGainTagsToPreserve as $rg_key) {
				if (array_key_exists($ThisFileInfo['ape']['items'][strtolower($rg_key)]['data'][0]) && !array_key_exists($this->tag_data[strtoupper($rg_key)][0])) {
		...
		if ($APEtag = $this->GenerateAPEtag()) {
			if (getID3::is_writable($this->filename) && is_file($this->filename) && ($fp = fopen($this->filename, 'a+b'))) {
				...
				if (array_key_exists($ThisFileInfo['ape']['tag_offset_end'])) {
					$PostAPEdataOffset = max($PostAPEdataOffset, $ThisFileInfo['ape']['tag_offset_end']);
				}
				if (array_key_exists($ThisFileInfo['lyrics3']['tag_offset_start'])) {
				...
				fseek($fp, $PostAPEdataOffset);
				if (array_key_exists($ThisFileInfo['ape']['tag_offset_start'])) {
	...
	public function DeleteAPEtag() {
		$getID3 = new getID3;
		$ThisFileInfo = $getID3->analyze($this->filename);
		if (array_key_exists($ThisFileInfo['ape']['tag_offset_start']) && array_key_exists($ThisFileInfo['ape']['tag_offset_end'])) {
			if (getID3::is_writable($this->filename) && is_file($this->filename) && ($fp = fopen($this->filename, 'a+b'))) {
