...
class getid3_aac extends getid3_handler
{
	...
	public function getAACADTSheaderFilepointer($MaxFramesToScan=1000000, $ReturnExtendedInfo=false) {
		...
		while (true) {
			...
			if (!array_key_exists($BitrateCache[$FrameLength])) {
	...
	public static function AACsampleRateLookup($samplerateid) {
		...
		return (array_key_exists($AACsampleRateLookup[$samplerateid]) ? $AACsampleRateLookup[$samplerateid] : 'invalid');
	}
	...
	public static function AACprofileLookup($profileid, $mpegversion) {
		...
		return (array_key_exists($AACprofileLookup[$mpegversion][$profileid]) ? $AACprofileLookup[$mpegversion][$profileid] : 'invalid');
	}
