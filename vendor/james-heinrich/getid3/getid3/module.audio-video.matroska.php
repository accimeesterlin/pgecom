...
class getid3_matroska extends getid3_handler
{
	...
	public function Analyze()
	{
		...
		// calculate playtime
		if (array_key_exists($info['matroska']['info']) && is_array($info['matroska']['info'])) {
			foreach ($info['matroska']['info'] as $key => $infoarray) {
				if (array_key_exists($infoarray['Duration'])) {
					// TimecodeScale is how many nanoseconds each Duration unit is
					$info['playtime_seconds'] = $infoarray['Duration'] * ((array_key_exists($infoarray['TimecodeScale']) ? $infoarray['TimecodeScale'] : 1000000) / 1000000000);
					break;
				}
			}
		}
		...
		// extract tags
		if (array_key_exists($info['matroska']['tags']) && is_array($info['matroska']['tags'])) {
		...
		// process tracks
		if (array_key_exists($info['matroska']['tracks']['tracks']) && is_array($info['matroska']['tracks']['tracks'])) {
			foreach ($info['matroska']['tracks']['tracks'] as $key => $trackarray) {

				$track_info = array();
				$track_info['dataformat'] = self::CodecIDtoCommonName($trackarray['CodecID']);
				$track_info['default'] = (array_key_exists($trackarray['FlagDefault']) ? $trackarray['FlagDefault'] : true);
				if (array_key_exists($trackarray['Name'])) { $track_info['name'] = $trackarray['Name']; }
				...
				switch ($trackarray['TrackType']) {
					...
					case 1: // Video
						...
						$track_info['display_unit'] = self::displayUnit(array_key_exists($trackarray['DisplayUnit']) ? $trackarray['DisplayUnit'] : 0);
						$track_info['display_x']    = (array_key_exists($trackarray['DisplayWidth']) ? $trackarray['DisplayWidth'] : $trackarray['PixelWidth']);
						$track_info['display_y']    = (array_key_exists($trackarray['DisplayHeight']) ? $trackarray['DisplayHeight'] : $trackarray['PixelHeight']);

						if (array_key_exists($trackarray['PixelCropBottom'])) { $track_info['crop_bottom'] = $trackarray['PixelCropBottom']; }
						if (array_key_exists($trackarray['PixelCropTop']))    { $track_info['crop_top']    = $trackarray['PixelCropTop']; }
						if (array_key_exists($trackarray['PixelCropLeft']))   { $track_info['crop_left']   = $trackarray['PixelCropLeft']; }
						if (array_key_exists($trackarray['PixelCropRight']))  { $track_info['crop_right']  = $trackarray['PixelCropRight']; }
						if (array_key_exists($trackarray['DefaultDuration'])) { $track_info['frame_rate']  = round(1000000000 / $trackarray['DefaultDuration'], 3); }
						if (array_key_exists($trackarray['CodecName']))       { $track_info['codec']       = $trackarray['CodecName']; }
					...
					case 2: // Audio
						$track_info['sample_rate'] = (array_key_exists($trackarray['SamplingFrequency']) ? $trackarray['SamplingFrequency'] : 8000.0);
						$track_info['channels']    = (array_key_exists($trackarray['Channels']) ? $trackarray['Channels'] : 1);
						$track_info['language']    = (array_key_exists($trackarray['Language']) ? $trackarray['Language'] : 'eng');
						if (array_key_exists($trackarray['BitDepth']))  { $track_info['bits_per_sample'] = $trackarray['BitDepth']; }
						if (array_key_exists($trackarray['CodecName'])) { $track_info['codec']           = $trackarray['CodecName']; }
						...
						switch ($trackarray['CodecID']) {
							...
							case 'A_FLAC':
								...
								if (!array_key_exists($info['matroska']['track_data_offsets'][$trackarray['TrackNumber']])) {
								...
								if (!empty($getid3_temp->info[$header_data_key])) {
									...
									if (array_key_exists($getid3_temp->info['audio']) && is_array($getid3_temp->info['audio'])) {
							...
							case 'A_VORBIS':
								if (!array_key_exists($trackarray['CodecPrivate'])) {
							...
							default:
								$this->warning('Unhandled audio type "'.(array_key_exists($trackarray['CodecID']) ? $trackarray['CodecID'] : '').'"');
								break;
						}
		...
		// process attachments
		if (array_key_exists($info['matroska']['attachments']) && $this->getid3->option_save_attachments !== getID3::ATTACHMENTS_NONE) {
		...
		} elseif (array_key_exists($info['mime_type'])) {
			...
			foreach ($info['matroska']['tags'] as $key1 => $value1) {
				if (!empty($value1['Targets']['TagTrackUID'][0]) && !empty($value1['SimpleTag'])) {
					foreach ($value1['SimpleTag'] as $key2 => $value2) {
						if (!empty($value2['TagName']) && array_key_exists($value2['TagString'])) {
			...
			foreach (array('audio','video') as $avtype) {
				if (!empty($info[$avtype]['streams'])) {
					foreach ($info[$avtype]['streams'] as $trackUID => $trackdata) {
						if (!array_key_exists($trackdata['bitrate']) && !empty($_STATISTICS_byTrackUID[$trackUID]['BPS'])) {
								...
								while ($this->getEBMLelement($seek_entry, $element_data['end'])) {
									switch ($seek_entry['id']) {
										...
										case EBML_ID_SEEK: // Contains a single seek entry to an EBML element
											...
											if (!array_key_exists($seek_entry['target_id'])) {
	...
	private function HandleEMBLClusterBlock($element, $block_type, &$info) {
		...
		if (!array_key_exists($info['matroska']['track_data_offsets'][$block_data['tracknumber']])) {
		...
		//$info['matroska']['track_data_offsets'][$block_data['tracknumber']]['duration']      = $block_data['timecode'] * ((array_key_exists($info['matroska']['info'][0]['TimecodeScale']) ? $info['matroska']['info'][0]['TimecodeScale'] : 1000000) / 1000000000);
	...
	public static function TargetTypeValue($target_type) {
		...
		return (array_key_exists($TargetTypeValue[$target_type]) ? $TargetTypeValue[$target_type] : $target_type);
	}
	...
	public static function BlockLacingType($lacingtype) {
		...
		return (array_key_exists($BlockLacingType[$lacingtype]) ? $BlockLacingType[$lacingtype] : $lacingtype);
	}
	...
	public static function CodecIDtoCommonName($codecid) {
		...
		return (array_key_exists($CodecIDlist[$codecid]) ? $CodecIDlist[$codecid] : $codecid);
	}
	...
	private static function EBMLidName($value) {
		...
		return (array_key_exists($EBMLidList[$value]) ? $EBMLidList[$value] : dechex($value));
	}
	...
	public static function displayUnit($value) {
		...
		return (array_key_exists($units[$value]) ? $units[$value] : 'unknown');
	}
