	...
	public function Analyze() {
		...
		if (!empty($info['quicktime']['comments']['chapters']) && is_array($info['quicktime']['comments']['chapters']) && (count($info['quicktime']['comments']['chapters']) > 0)) {
			...
			for ($i = 0; $i < count($info['quicktime']['comments']['chapters']); $i++) {
				...
				if (array_key_exists($durations[$i])) {
		...
		if (array_key_exists($info['quicktime']['temp_meta_key_names'])) {
				...
				if (preg_match('#^([\\+\\-])([0-9]{2}|[0-9]{4}|[0-9]{6})(\\.[0-9]+)?([\\+\\-])([0-9]{3}|[0-9]{5}|[0-9]{7})(\\.[0-9]+)?(([\\+\\-])([0-9]{3}|[0-9]{5}|[0-9]{7})(\\.[0-9]+)?)?/$#', $ISO6709string, $matches)) {
					...
					foreach (array('latitude', 'longitude', 'altitude') as $key) {
						if ($ISO6709parsed[$key] !== false) {
							...
							if (!array_key_exists($info['quicktime']['comments']['gps_'.$key]) || !in_array($value, $info['quicktime']['comments']['gps_'.$key])) {
		...
		if (!array_key_exists($info['bitrate']) && !empty($info['playtime_seconds'])) {
			$info['bitrate'] = (($info['avdataend'] - $info['avdataoffset']) * 8) / $info['playtime_seconds'];
		}
		if (array_key_exists($info['bitrate']) && !array_key_exists($info['audio']['bitrate']) && !array_key_exists($info['quicktime']['video'])) {
		...
		if (!empty($info['playtime_seconds']) && !array_key_exists($info['video']['frame_rate']) && !empty($info['quicktime']['stts_framecount'])) {
		...
		if (array_key_exists($info['video']) && ($info['mime_type'] == 'audio/mp4') && empty($info['video']['resolution_x']) && empty($info['video']['resolution_y']))  {
			unset($info['video']);
		}
			...
			switch ($atomname) {
				...
				case 'stbl': // Sample TaBLe container atom
					...
					foreach ($atom_structure['subatoms'] as $key => $value_array) {
						if (array_key_exists($value_array['sample_description_table'])) {
							foreach ($value_array['sample_description_table'] as $key2 => $value_array2) {
								if (array_key_exists($value_array2['data_format'])) {
						...
						} elseif (array_key_exists($value_array['time_to_sample_table'])) {
							foreach ($value_array['time_to_sample_table'] as $key2 => $value_array2) {
								if (array_key_exists($value_array2['sample_count']) && array_key_exists($value_array2['sample_duration']) && ($value_array2['sample_duration'] > 0)) {
				...
				case 'ptv ': // Print To Video - defines a movie's full screen mode
					...
					if (array_key_exists($ptv_lookup[$atom_structure['display_size_raw']])) {
...
//					if (true) {
				...
				case 'mdhd': // MeDia HeaDer atom
					...
					$info['quicktime']['time_scale'] = ((array_key_exists($info['quicktime']['time_scale']) && ($info['quicktime']['time_scale'] < 1000)) ? max($info['quicktime']['time_scale'], $atom_structure['time_scale']) : $atom_structure['time_scale']);
				...
				case 'tkhd': // TracK HeaDer atom
					...
					if (!array_key_exists($info['video']['rotate']) || (($info['video']['rotate'] == 0) && ($matrixRotation > 0))) {
						$info['quicktime']['video']['rotate'] = $info['video']['rotate'] = $matrixRotation;
					}

					if ($atom_structure['flags']['enabled'] == 1) {
						if (!array_key_exists($info['video']['resolution_x']) || !array_key_exists($info['video']['resolution_y'])) {
						...
						//if (array_key_exists($info['video']['resolution_x'])) { unset($info['video']['resolution_x']); }
						//if (array_key_exists($info['video']['resolution_y'])) { unset($info['video']['resolution_y']); }
						//if (array_key_exists($info['quicktime']['video']))    { unset($info['quicktime']['video']);    }
					}
					break;
	...
	/* due to lack of known documentation, this is a kludge implementation. If you know of documentation on how mdat is properly structed, please send it to info@getid3.org */
					...
					if (($atomsize > 8) && (!array_key_exists($info['avdataend_tmp']) || ($info['quicktime'][$atomname]['size'] > ($info['avdataend_tmp'] - $info['avdataoffset'])))) {
				...
				case 'NCM2': // Nikon Camera preview iMage 2
					...
					if (preg_match('/^\xFF\xD8\xFF/', $atom_data)) {
						...
						$atom_structure['data'] = $atom_data;
						$atom_structure['image_mime'] = 'image/jpeg';
						$atom_structure['description'] = array_key_exists($descriptions[$atomname]) ? $descriptions[$atomname] : 'Nikon preview image';
				...
				case 'uuid': // user-defined atom often seen containing XML data, also used for potentially many other purposes, only a few specifically handled by getID3 (e.g. 360fly spatial data)
					...
					switch ($atom_structure['uuid_field_id']) {   // http://fileformats.archiveteam.org/wiki/Boxes/atoms_format#UUID_boxes
						...
						case 'efe1589a-bb77-49ef-8095-27759eb1dc6f': // 360fly data
							...
							//if (array_key_exists($debug_structure['debug_items']) && count($debug_structure['debug_items']) > 0) {
				...
				case 'rcif':
					...
					if (substr($atom_data, 0, 7) == "\x00\x00\x00\x00\x55\xC4".'{') {
						if ($json_decoded = @json_decode(rtrim(substr($atom_data, 6), "\x00"), true)) {
							...
							if (($atomname == 'rcif') && array_key_exists($info['quicktime']['camera']['rcif']['wxcamera']['rotate'])) {
	...
	public function QuicktimeLanguageLookup($languageid) {
		...
		return (array_key_exists($QuicktimeLanguageLookup[$languageid]) ? $QuicktimeLanguageLookup[$languageid] : 'invalid');
	}
	...
	public function QuicktimeVideoCodecLookup($codecid) {
		...
		return (array_key_exists($QuicktimeVideoCodecLookup[$codecid]) ? $QuicktimeVideoCodecLookup[$codecid] : '');
	}
	...
	public function QuicktimeAudioCodecLookup($codecid) {
		...
		return (array_key_exists($QuicktimeAudioCodecLookup[$codecid]) ? $QuicktimeAudioCodecLookup[$codecid] : '');
	}
	...
	public function QuicktimeDCOMLookup($compressionid) {
		...
		return (array_key_exists($QuicktimeDCOMLookup[$compressionid]) ? $QuicktimeDCOMLookup[$compressionid] : '');
	}
	...
	public function QuicktimeColorNameLookup($colordepthid) {
		...
		return (array_key_exists($QuicktimeColorNameLookup[$colordepthid]) ? $QuicktimeColorNameLookup[$colordepthid] : 'invalid');
	}
	...
	public function QuicktimeSTIKLookup($stik) {
		...
		return (array_key_exists($QuicktimeSTIKLookup[$stik]) ? $QuicktimeSTIKLookup[$stik] : 'invalid');
	}
	...
	public function QuicktimeIODSaudioProfileName($audio_profile_id) {
		...
		return (array_key_exists($QuicktimeIODSaudioProfileNameLookup[$audio_profile_id]) ? $QuicktimeIODSaudioProfileNameLookup[$audio_profile_id] : 'ISO Reserved / User Private');
	}
	...
	public function QuicktimeIODSvideoProfileName($video_profile_id) {
		...
		return (array_key_exists($QuicktimeIODSvideoProfileNameLookup[$video_profile_id]) ? $QuicktimeIODSvideoProfileNameLookup[$video_profile_id] : 'ISO Reserved Profile');
	}
	...
	public function QuicktimeContentRatingLookup($rtng) {
		...
		return (array_key_exists($QuicktimeContentRatingLookup[$rtng]) ? $QuicktimeContentRatingLookup[$rtng] : 'invalid');
	}
	...
	public function QuicktimeStoreAccountTypeLookup($akid) {
		...
		return (array_key_exists($QuicktimeStoreAccountTypeLookup[$akid]) ? $QuicktimeStoreAccountTypeLookup[$akid] : 'invalid');
	}
	...
	public function QuicktimeStoreFrontCodeLookup($sfid) {
		...
		return (array_key_exists($QuicktimeStoreFrontCodeLookup[$sfid]) ? $QuicktimeStoreFrontCodeLookup[$sfid] : 'invalid');
	}
	...
	public function CopyToAppropriateCommentsSection($keyname, $data, $boxname='') {
		...
		if ($boxname && ($boxname != $keyname)) {
			$comment_key = (array_key_exists($handyatomtranslatorarray[$boxname]) ? $handyatomtranslatorarray[$boxname] : $boxname);
		} elseif (array_key_exists($handyatomtranslatorarray[$keyname])) {
