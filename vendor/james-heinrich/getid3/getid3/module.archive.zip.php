...
class getid3_zip extends getid3_handler
{
		...
		if (!getid3_lib::intValueSupported($info['filesize'])) {
			...
			while ($EOCDsearchCounter++ < 512) {
				...
				if (strstr($EOCDsearchData, 'PK'."\x05\x06")) {
					...
					if (array_key_exists($info['zip']['central_directory'][0]['compression_method'])) {
						$info['zip']['compression_method'] = $info['zip']['central_directory'][0]['compression_method'];
					}
					if (array_key_exists($info['zip']['central_directory'][0]['flags']['compression_speed'])) {
						$info['zip']['compression_speed']  = $info['zip']['central_directory'][0]['flags']['compression_speed'];
					}
					if (array_key_exists($info['zip']['compression_method']) && ($info['zip']['compression_method'] == 'store') && !array_key_exists($info['zip']['compression_speed'])) {
					...
					if (!empty($info['zip']['entries'][0]['filename']) &&
						...
						array_key_exists($info['zip']['entries'][0]['data_offset'])) {
	...
	public static function ZIPversionOSLookup($index) {
		...
		return (array_key_exists($ZIPversionOSLookup[$index]) ? $ZIPversionOSLookup[$index] : '[unknown]');
	}
	...
	public static function ZIPcompressionMethodLookup($index) {
		...
		return (array_key_exists($ZIPcompressionMethodLookup[$index]) ? $ZIPcompressionMethodLookup[$index] : '[unknown]');
	}
