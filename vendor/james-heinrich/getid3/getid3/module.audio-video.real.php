...
class getid3_real extends getid3_handler
{
					...
					if ($thisfile_real_chunks_currentchunk['object_version'] == 0) {
						...
						if ($thisfile_real_chunks_currentchunk['duration'] > 0) {
							switch ($thisfile_real_chunks_currentchunk['mime_type']) {
								case 'audio/x-pn-realaudio':
								case 'audio/x-pn-multirate-realaudio':
									$info['audio']['bitrate']    = (array_key_exists($info['audio']['bitrate']) ? $info['audio']['bitrate'] : 0) + $thisfile_real_chunks_currentchunk['avg_bit_rate'];
								...
								case 'video/x-pn-realvideo':
								case 'video/x-pn-multirate-realvideo':
									$info['video']['bitrate']            = (array_key_exists($info['video']['bitrate']) ? $info['video']['bitrate'] : 0) + $thisfile_real_chunks_currentchunk['avg_bit_rate'];
							...
							$info['bitrate'] = (array_key_exists($info['video']['bitrate']) ? $info['video']['bitrate'] : 0) + (array_key_exists($info['audio']['bitrate']) ? $info['audio']['bitrate'] : 0);
						}
					}
					break;
	...
	public function RealAudioCodecFourCClookup($fourcc, $bitrate) {
		...
		if (array_key_exists($RealAudioCodecFourCClookup[$fourcc][$roundbitrate])) {
			return $RealAudioCodecFourCClookup[$fourcc][$roundbitrate];
		} elseif (array_key_exists($RealAudioCodecFourCClookup[$fourcc][0])) {
