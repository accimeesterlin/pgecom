			...
			switch ($NextObjectGUID) {
				...
				case GETID3_ASF_File_Properties_Object:
					...
					if ($thisfile_asf_filepropertiesobject['flags']['broadcast']) {
						...
						$info['bitrate'] = ((array_key_exists($thisfile_asf_filepropertiesobject['filesize']) ? $thisfile_asf_filepropertiesobject['filesize'] : $info['filesize']) * 8) / $info['playtime_seconds'];
					}
					break;
					...
					for ($CodecEntryCounter = 0; $CodecEntryCounter < $thisfile_asf_codeclistobject['codec_entries_count']; $CodecEntryCounter++) {
						...
						if ($thisfile_asf_codeclistobject_codecentries_current['type_raw'] == 2) { // audio codec
							...
							if (strpos($thisfile_asf_codeclistobject_codecentries_current['description'], ',') === false) {
								...
								if (!array_key_exists($thisfile_audio['bitrate']) && strstr($AudioCodecBitrate, 'kbps')) {
									$thisfile_audio['bitrate'] = (int) trim(str_replace('kbps', '', $AudioCodecBitrate)) * 1000;
								}
								//if (!array_key_exists($thisfile_video['bitrate']) && array_key_exists($thisfile_audio['bitrate']) && array_key_exists($thisfile_asf['file_properties_object']['max_bitrate']) && ($thisfile_asf_codeclistobject['codec_entries_count'] > 1)) {
								...
								if (!array_key_exists($thisfile_audio['channels'])) {
...
/*
								...
								if (!array_key_exists($thisfile_asf_comments['picture'])) {
...
*/
		...
		if (array_key_exists($thisfile_asf_streambitratepropertiesobject['bitrate_records_count'])) {
			$ASFbitrateAudio = 0;
			$ASFbitrateVideo = 0;
			for ($BitrateRecordsCounter = 0; $BitrateRecordsCounter < $thisfile_asf_streambitratepropertiesobject['bitrate_records_count']; $BitrateRecordsCounter++) {
				if (array_key_exists($thisfile_asf_codeclistobject['codec_entries'][$BitrateRecordsCounter])) {
		...
		if (array_key_exists($thisfile_asf['stream_properties_object']) && is_array($thisfile_asf['stream_properties_object'])) {
					...
					case GETID3_ASF_Audio_Media:
						...
						if (!empty($thisfile_asf['stream_bitrate_properties_object']['bitrate_records'])) {
							foreach ($thisfile_asf['stream_bitrate_properties_object']['bitrate_records'] as $dummy => $dataarray) {
								if (array_key_exists($dataarray['flags']['stream_number']) && ($dataarray['flags']['stream_number'] == $streamnumber)) {
		...
		if (array_key_exists($thisfile_asf_codeclistobject['codec_entries']) && is_array($thisfile_asf_codeclistobject['codec_entries'])) {
		...
		switch (array_key_exists($thisfile_audio['codec']) ? $thisfile_audio['codec'] : '') {
			case 'MPEG Layer-3':
		...
		if (array_key_exists($thisfile_asf_codeclistobject['codec_entries'])) {
			foreach ($thisfile_asf_codeclistobject['codec_entries'] as $streamnumber => $streamdata) {
				switch ($streamdata['type_raw']) {
		...
		if (array_key_exists($info['audio'])) {
			$thisfile_audio['lossless']           = (array_key_exists($thisfile_audio['lossless'])           ? $thisfile_audio['lossless']           : false);
			$thisfile_audio['dataformat']         = (!empty($thisfile_audio['dataformat'])        ? $thisfile_audio['dataformat']         : 'asf');
		}
		if (!empty($thisfile_video['dataformat'])) {
			$thisfile_video['lossless']           = (array_key_exists($thisfile_audio['lossless'])           ? $thisfile_audio['lossless']           : false);
			$thisfile_video['pixel_aspect_ratio'] = (array_key_exists($thisfile_audio['pixel_aspect_ratio']) ? $thisfile_audio['pixel_aspect_ratio'] : (float) 1);
			$thisfile_video['dataformat']         = (!empty($thisfile_video['dataformat'])        ? $thisfile_video['dataformat']         : 'asf');
		}
		...
		$info['bitrate'] = (array_key_exists($thisfile_audio['bitrate']) ? $thisfile_audio['bitrate'] : 0) + (array_key_exists($thisfile_video['bitrate']) ? $thisfile_video['bitrate'] : 0);

		if ((!array_key_exists($info['playtime_seconds']) || ($info['playtime_seconds'] <= 0)) && ($info['bitrate'] > 0)) {
	...
	public static function codecListObjectTypeLookup($CodecListType) {
		...
		return (array_key_exists($lookup[$CodecListType]) ? $lookup[$CodecListType] : 'Invalid Codec Type');
	}
	...
	public static function ASFIndexObjectIndexTypeLookup($id) {
		...
		return (array_key_exists($ASFIndexObjectIndexTypeLookup[$id]) ? $ASFIndexObjectIndexTypeLookup[$id] : 'invalid');
	}
	...
	public static function WMpictureTypeLookup($WMpictureType) {
		...
		return (array_key_exists($lookup[$WMpictureType]) ? $lookup[$WMpictureType] : '');
	}
	...
	public static function metadataLibraryObjectDataTypeLookup($id) {
		...
		return (array_key_exists($lookup[$id]) ? $lookup[$id] : 'invalid');
	}
	...
	public function ASF_WMpicture(&$data) {
		...
		if (!array_key_exists($this->getid3->info['asf']['comments']['picture'])) {
