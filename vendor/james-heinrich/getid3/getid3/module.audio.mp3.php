...
class getid3_mp3 extends getid3_handler
{
	...
	public function Analyze() {
		...
		if (array_key_exists($info['mpeg']['audio']['bitrate_mode'])) {
			$info['audio']['bitrate_mode'] = strtolower($info['mpeg']['audio']['bitrate_mode']);
		}

		$CurrentDataLAMEversionString = null;
		if (((array_key_exists($info['id3v2']['headerlength']) && ($info['avdataoffset'] > $info['id3v2']['headerlength'])) || (!array_key_exists($info['id3v2']) && ($info['avdataoffset'] > 0) && ($info['avdataoffset'] != $initialOffset)))) {

			$synchoffsetwarning = 'Unknown data before synch ';
			if (array_key_exists($info['id3v2']['headerlength'])) {
			...
			if (array_key_exists($info['audio']['bitrate_mode']) && ($info['audio']['bitrate_mode'] == 'cbr')) {
		...
		if (array_key_exists($info['mpeg']['audio']['LAME'])) {
			$info['audio']['codec'] = 'LAME';
		...
		$CurrentDataLAMEversionString = (!empty($CurrentDataLAMEversionString) ? $CurrentDataLAMEversionString : (array_key_exists($info['audio']['encoder']) ? $info['audio']['encoder'] : ''));
		...
		switch (array_key_exists($info['mpeg']['audio']['layer']) ? $info['mpeg']['audio']['layer'] : '') {
			case 1:
			case 2:
				$info['audio']['dataformat'] = 'mp'.$info['mpeg']['audio']['layer'];
				break;
		}
		if (array_key_exists($info['fileformat']) && ($info['fileformat'] == 'mp3')) {
		...
		// Calculate playtime
		if (!array_key_exists($info['playtime_seconds']) && array_key_exists($info['audio']['bitrate']) && ($info['audio']['bitrate'] > 0)) {
			...
			$xingVBRheaderFrameLength = ((array_key_exists($info['mpeg']['audio']['VBR_frames']) && array_key_exists($info['mpeg']['audio']['framelength'])) ? $info['mpeg']['audio']['framelength'] : 0);
	...
	public function GuessEncoderOptions() {
		...
		if (array_key_exists($thisfile_mpeg_audio['VBR_method']) && ($thisfile_mpeg_audio['VBR_method'] == 'Fraunhofer') && !empty($thisfile_mpeg_audio['VBR_quality'])) {

			$encoder_options = 'VBR q'.$thisfile_mpeg_audio['VBR_quality'];

		} elseif (!empty($thisfile_mpeg_audio_lame['preset_used']) && array_key_exists($thisfile_mpeg_audio_lame['preset_used_id']) && (!in_array($thisfile_mpeg_audio_lame['preset_used_id'], $NamedPresetBitrates))) {
			...
			} elseif (array_key_exists($KnownEncoderValues['**'][$thisfile_mpeg_audio_lame['vbr_quality']][$thisfile_mpeg_audio_lame['raw']['vbr_method']][$thisfile_mpeg_audio_lame['raw']['noise_shaping']][$thisfile_mpeg_audio_lame['raw']['stereo_mode']][$thisfile_mpeg_audio_lame['ath_type']][$thisfile_mpeg_audio_lame['lowpass_frequency']])) {
				...
				case '--r3mix':
					...
					switch ($ExplodedOptions[1]) {
						...
						case 'r3mix':
							...
							if (!array_key_exists($ExpectedLowpass[$ExplodedOptions[1].'|'.$thisfile_mpeg_audio_lame['lowpass_frequency']]) && ($thisfile_mpeg_audio_lame['lowpass_frequency'] < 22050) && (round($thisfile_mpeg_audio_lame['lowpass_frequency'] / 1000) < round($thisfile_mpeg_audio['sample_rate'] / 2000))) {
		...
		if (array_key_exists($thisfile_mpeg_audio_lame['raw']['source_sample_freq'])) {
								...
								switch ($ExplodedOptions[1]) {
									...
									default:
										static $ExpectedResampledRate = array(
												...
												'fm|32000'                => 32000, // 3.90
												'voice|32000'             => 32000);
										if (!array_key_exists($ExpectedResampledRate[$ExplodedOptions[1].'|'.$thisfile_mpeg_audio['sample_rate']])) {
											$encoder_options .= ' --resample '.$thisfile_mpeg_audio['sample_rate'];
										}
										break;
								}
	...
	public function decodeMPEGaudioHeader($offset, &$info, $recursivesearch=true, $ScanAsCBR=false, $FastMPEGheaderScan=false) {
		...
		if (array_key_exists($MPEGaudioHeaderDecodeCache[$head4_key])) {
		...
		if (!array_key_exists($MPEGaudioHeaderValidCache[$head4_key])) { // Not in cache
		...
		// shortcut
		if (!array_key_exists($info['mpeg']['audio'])) {
		...
		if ($thisfile_mpeg_audio['bitrate'] != 'free') {
			...
			if (array_key_exists($thisfile_mpeg_audio['framelength'])) {
...
//				}
				...
				if (!empty($thisfile_mpeg_audio['VBR_frames'])) {
					...
					if (!empty($thisfile_mpeg_audio['VBR_bytes'])) {
						...
						$used_filesize -= (array_key_exists($info['id3v2']['headerlength']) ? intval($info['id3v2']['headerlength']) : 0);
						$used_filesize -= (array_key_exists($info['id3v1']) ? 128 : 0);
						$used_filesize -= (array_key_exists($info['tag_offset_end']) ? $info['tag_offset_end'] - $info['tag_offset_start'] : 0);
		...
		if (array_key_exists($thisfile_mpeg_audio['VBR_frames']) ? $thisfile_mpeg_audio['VBR_frames'] : '') {
			switch ($thisfile_mpeg_audio['bitrate_mode']) {
				...
				case 'abr':
					...
					$thisfile_mpeg_audio['VBR_bitrate'] = (array_key_exists($thisfile_mpeg_audio['VBR_bytes']) ? (($thisfile_mpeg_audio['VBR_bytes'] / $thisfile_mpeg_audio['VBR_frames']) * 8) * ($info['audio']['sample_rate'] / $bytes_per_frame) : 0);
		...
		if ($recursivesearch) {
			...
			if (array_key_exists($this->getid3->info['mp3_validity_check_bitrates'])) {
		...
		for ($i = 0; $i < $this->mp3_valid_check_frames; $i++) {
			...
			if ($this->decodeMPEGaudioHeader($nextframetestoffset, $nextframetestarray, false)) {
				...
				if ($ScanAsCBR) {
					...
					if (!array_key_exists($nextframetestarray['mpeg']['audio']['bitrate']) || !array_key_exists($firstframetestarray['mpeg']['audio']['bitrate']) || ($nextframetestarray['mpeg']['audio']['bitrate'] != $firstframetestarray['mpeg']['audio']['bitrate'])) {
						return false;
					}
				}
				...
				if (array_key_exists($nextframetestarray['mpeg']['audio']['framelength']) && ($nextframetestarray['mpeg']['audio']['framelength'] > 0)) {
		...
		while ($this->ftell() < $info['avdataend']) {
			...
			if ($MPEGaudioHeaderValidCache[$head4]) {
				...
				if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
					...
					$next4 = $this->fread(4);
					if ($next4[0] == "\xFF") {
						if (!array_key_exists($MPEGaudioHeaderDecodeCache[$next4])) {
							$MPEGaudioHeaderDecodeCache[$next4] = self::MPEGaudioHeaderDecode($next4);
						}
						if (!array_key_exists($MPEGaudioHeaderValidCache[$next4])) {
						...
						if ($MPEGaudioHeaderValidCache[$next4]) {
							...
							$Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]] = array_key_exists($Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]]) ? ++$Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]] : 1;
							$Distribution['layer'][$LongMPEGlayerLookup[$head4]] = array_key_exists($Distribution['layer'][$LongMPEGlayerLookup[$head4]]) ? ++$Distribution['layer'][$LongMPEGlayerLookup[$head4]] : 1;
							$Distribution['version'][$LongMPEGversionLookup[$head4]] = array_key_exists($Distribution['version'][$LongMPEGversionLookup[$head4]]) ? ++$Distribution['version'][$LongMPEGversionLookup[$head4]] : 1;
							$Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])] = array_key_exists($Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])]) ? ++$Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])] : 1;
							$Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]] = array_key_exists($Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]]) ? ++$Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]] : 1;
		...
		while ($SynchSeekOffset < $sync_seek_buffer_size) {
			if ((($avdataoffset + $SynchSeekOffset)  < $info['avdataend']) && !feof($this->getid3->fp)) {
				...
				if ($SynchSeekOffset > $sync_seek_buffer_size) {
					...
					if (array_key_exists($info['audio']['bitrate'])) {
					...
					if (array_key_exists($info['mpeg']) && (!is_array($info['mpeg']) || (count($info['mpeg']) == 0))) {
						unset($info['mpeg']);
					}
					return false;
				}
			...
			if (($header[$SynchSeekOffset] == "\xFF") && ($header[($SynchSeekOffset + 1)] > "\xE0")) { // possible synch detected
				...
				if (!array_key_exists($FirstFrameThisfileInfo) && !array_key_exists($info['mpeg']['audio'])) {
				...
				if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset, $dummy, true)) {
					...
					switch (array_key_exists($info['fileformat']) ? $info['fileformat'] : '') {
						case '':
						case 'id3':
						case 'ape':
						case 'mp3':
					...
					if (array_key_exists($FirstFrameThisfileInfo) && array_key_exists($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode']) && ($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode'] == 'vbr')) {
					...
					if (array_key_exists($info['mpeg']['audio']['bitrate_mode']) && ($info['mpeg']['audio']['bitrate_mode'] == 'vbr') && !array_key_exists($info['mpeg']['audio']['VBR_method'])) {
			...
			if (($avdataoffset + $SynchSeekOffset) >= $info['avdataend']) {
				...
				if (empty($info['mpeg']['audio'])) {
					...
					if (array_key_exists($info['mpeg']) && (!is_array($info['mpeg']) || empty($info['mpeg']))) {
						unset($info['mpeg']);
					}
					return false;
	...
	public static function MPEGaudioHeaderValid($rawarray, $echoerrors=false, $allowBitrate15=false) {
		if (!array_key_exists($rawarray['synch']) || ($rawarray['synch'] & 0x0FFE) != 0x0FFE) {
			return false;
		}
		...
		if (array_key_exists($MPEGaudioVersionLookup[$rawarray['version']])) {
			$decodedVersion = $MPEGaudioVersionLookup[$rawarray['version']];
		} else {
			echo ($echoerrors ? "\n".'invalid Version ('.$rawarray['version'].')' : '');
			return false;
		}
		if (array_key_exists($MPEGaudioLayerLookup[$rawarray['layer']])) {
			$decodedLayer = $MPEGaudioLayerLookup[$rawarray['layer']];
		} else {
			echo ($echoerrors ? "\n".'invalid Layer ('.$rawarray['layer'].')' : '');
			return false;
		}
		if (!array_key_exists($MPEGaudioBitrateLookup[$decodedVersion][$decodedLayer][$rawarray['bitrate']])) {
		...
		if (!array_key_exists($MPEGaudioFrequencyLookup[$decodedVersion][$rawarray['sample_rate']])) {
			echo ($echoerrors ? "\n".'invalid Frequency ('.$rawarray['sample_rate'].')' : '');
			return false;
		}
		if (!array_key_exists($MPEGaudioChannelModeLookup[$rawarray['channelmode']])) {
			echo ($echoerrors ? "\n".'invalid ChannelMode ('.$rawarray['channelmode'].')' : '');
			return false;
		}
		if (!array_key_exists($MPEGaudioModeExtensionLookup[$decodedLayer][$rawarray['modeextension']])) {
			echo ($echoerrors ? "\n".'invalid Mode Extension ('.$rawarray['modeextension'].')' : '');
			return false;
		}
		if (!array_key_exists($MPEGaudioEmphasisLookup[$rawarray['emphasis']])) {
	...
	public static function MPEGaudioFrameLength(&$bitrate, &$version, &$layer, $padding, &$samplerate) {
		...
		if (!array_key_exists($AudioFrameLengthCache[$bitrate][$version][$layer][$padding][$samplerate])) {
	...
	public static function ClosestStandardMP3Bitrate($bit_rate) {
		...
		if (!array_key_exists($bit_rate_table[$round_bit_rate])) {
	...
	public static function LAMEvbrMethodLookup($VBRmethodID) {
		...
		return (array_key_exists($LAMEvbrMethodLookup[$VBRmethodID]) ? $LAMEvbrMethodLookup[$VBRmethodID] : '');
	}
	...
	public static function LAMEmiscStereoModeLookup($StereoModeID) {
		...
		return (array_key_exists($LAMEmiscStereoModeLookup[$StereoModeID]) ? $LAMEmiscStereoModeLookup[$StereoModeID] : '');
	}
	...
	public static function LAMEmiscSourceSampleFrequencyLookup($SourceSampleFrequencyID) {
		...
		return (array_key_exists($LAMEmiscSourceSampleFrequencyLookup[$SourceSampleFrequencyID]) ? $LAMEmiscSourceSampleFrequencyLookup[$SourceSampleFrequencyID] : '');
	}
	...
	public static function LAMEsurroundInfoLookup($SurroundInfoID) {
		...
		return (array_key_exists($LAMEsurroundInfoLookup[$SurroundInfoID]) ? $LAMEsurroundInfoLookup[$SurroundInfoID] : 'reserved');
	}
	...
	public static function LAMEpresetUsedLookup($LAMEtag) {
		...
		return (array_key_exists($LAMEpresetUsedLookup[$LAMEtag['preset_used_id']]) ? $LAMEpresetUsedLookup[$LAMEtag['preset_used_id']] : 'new/unknown preset: '.$LAMEtag['preset_used_id'].' - report to info@getid3.org');
	}
