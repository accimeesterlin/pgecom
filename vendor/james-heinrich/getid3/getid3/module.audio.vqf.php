...
class getid3_vqf extends getid3_handler
{
	...
	public function Analyze() {
		...
		if (array_key_exists($thisfile_vqf['DSIZ']) && (($thisfile_vqf['DSIZ'] != ($info['avdataend'] - $info['avdataoffset'] - strlen('DATA'))))) {
	...
	public function VQFchannelFrequencyLookup($frequencyid) {
		...
		return (array_key_exists($VQFchannelFrequencyLookup[$frequencyid]) ? $VQFchannelFrequencyLookup[$frequencyid] : $frequencyid * 1000);
	}
	...
	public function VQFcommentNiceNameLookup($shortname) {
		...
		return (array_key_exists($VQFcommentNiceNameLookup[$shortname]) ? $VQFcommentNiceNameLookup[$shortname] : $shortname);
	}
