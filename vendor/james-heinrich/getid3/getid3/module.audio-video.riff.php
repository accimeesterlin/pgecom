				...
				while ($nextRIFFoffset < min($info['filesize'], $info['avdataend'])) {
					...
					switch ($nextRIFFheaderID) {
						case 'RIFF':
							...
							if (!array_key_exists($thisfile_riff[$nextRIFFtype])) {
		...
		switch ($RIFFsubtype) {
			...
			case 'WAVE':
				...
				if (array_key_exists($thisfile_riff_WAVE['fmt '][0]['data'])) {
					...
					if (array_key_exists($thisfile_riff_WAVE['data'][0]['offset']) && array_key_exists($thisfile_riff_raw['fmt ']['wFormatTag'])) {
				...
				if (array_key_exists($thisfile_riff_WAVE['rgad'][0]['data'])) {
				...
				if (array_key_exists($thisfile_riff_WAVE['fact'][0]['data'])) {
				...
				if (array_key_exists($thisfile_riff_WAVE['bext'][0]['data'])) {
					// shortcut
				...
				if (array_key_exists($thisfile_riff_WAVE['MEXT'][0]['data'])) {
					// shortcut
				...
				if (array_key_exists($thisfile_riff_WAVE['cart'][0]['data'])) {
					// shortcut
				...
				if (array_key_exists($thisfile_riff_WAVE['SNDM'][0]['data'])) {
					// SoundMiner metadata
					...
					foreach ($tagmapping as $fromkey => $tokey) {
						if (array_key_exists($thisfile_riff_WAVE_SNDM_0['parsed'][$fromkey])) {
							$thisfile_riff['comments'][$tokey][] = $thisfile_riff_WAVE_SNDM_0['parsed'][$fromkey];
						}
					}
				}

				if (array_key_exists($thisfile_riff_WAVE['iXML'][0]['data'])) {
					// requires functions simplexml_load_string and get_object_vars
					if ($parsedXML = getid3_lib::XML2array($thisfile_riff_WAVE['iXML'][0]['data'])) {
						$thisfile_riff_WAVE['iXML'][0]['parsed'] = $parsedXML;
						if (array_key_exists($parsedXML['SPEED']['MASTER_SPEED'])) {
						...
						if (array_key_exists($parsedXML['SPEED']['TIMECODE_RATE'])) {
						...
						if (array_key_exists($parsedXML['SPEED']['TIMESTAMP_SAMPLES_SINCE_MIDNIGHT_LO']) && !empty($parsedXML['SPEED']['TIMESTAMP_SAMPLE_RATE']) && !empty($thisfile_riff_WAVE['iXML'][0]['timecode_rate'])) {
				...
				if (array_key_exists($thisfile_riff_WAVE['guan'][0]['data'])) {
					// shortcut
				...
				if (!array_key_exists($thisfile_audio['bitrate']) && array_key_exists($thisfile_riff_audio[$streamindex]['bitrate'])) {
					$thisfile_audio['bitrate'] = $thisfile_riff_audio[$streamindex]['bitrate'];
					$info['playtime_seconds'] = (float) ((($info['avdataend'] - $info['avdataoffset']) * 8) / $thisfile_audio['bitrate']);
				}
				...
				if (array_key_exists($thisfile_riff_raw['fmt ']['wFormatTag'])) {
					switch ($thisfile_riff_raw['fmt ']['wFormatTag']) {
						case 0x0001: // PCM
							if (!empty($info['ac3'])) {
				...
				if (array_key_exists($thisfile_audio_dataformat) && ($thisfile_audio_dataformat == 'ac3')) {
			...
			case 'AVI ':
				...
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['movi']['offset'])) {
					$info['avdataoffset'] = $thisfile_riff[$RIFFsubtype]['movi']['offset'] + 8;
					if (array_key_exists($thisfile_riff['AVIX'])) {
				...
				if (array_key_exists($thisfile_riff['AVI ']['hdrl']['strl']['indx'])) {
					//$bIndexType = array(
				...
				if (array_key_exists($thisfile_riff['AVI ']['hdrl']['avih'][$streamindex]['data'])) {
				...
				if (array_key_exists($thisfile_riff['AVI ']['hdrl']['strl']['strh'][0]['data'])) {
					if (is_array($thisfile_riff['AVI ']['hdrl']['strl']['strh'])) {
						...
						for ($i = 0; $i < count($thisfile_riff['AVI ']['hdrl']['strl']['strh']); $i++) {
							if (array_key_exists($thisfile_riff['AVI ']['hdrl']['strl']['strh'][$i]['data'])) {
								$strhData = $thisfile_riff['AVI ']['hdrl']['strl']['strh'][$i]['data'];
								$strhfccType = substr($strhData,  0, 4);

								if (array_key_exists($thisfile_riff['AVI ']['hdrl']['strl']['strf'][$i]['data'])) {
									...
									switch ($strhfccType) {
										case 'auds':
											$thisfile_audio['bitrate_mode'] = 'cbr';
											$thisfile_audio_dataformat      = 'wav';
											if (array_key_exists($thisfile_riff_audio) && is_array($thisfile_riff_audio)) {
										...
										case 'vids':
											...
											if (!$thisfile_riff_video_current['codec'] && array_key_exists($thisfile_riff_raw_strf_strhfccType_streamindex['fourcc']) && self::fourccLookup($thisfile_riff_raw_strf_strhfccType_streamindex['fourcc'])) {
							...
							if (array_key_exists($thisfile_riff_raw_strf_strhfccType_streamindex) && array_key_exists($thisfile_riff_raw_strf_strhfccType_streamindex['fourcc'])) {
			...
			case 'CDDA':
				...
				if (array_key_exists($thisfile_riff['CDDA']['fmt '][0]['data'])) {
					// shortcut
			...
			case 'AIFC':
				...
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['SSND'][0]['offset'])) {
				...
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['COMM'][0]['data'])) {
				...
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['COMT'])) {
					$offset = 0;
				...
				foreach ($CommentsChunkNames as $key => $value) {
					if (array_key_exists($thisfile_riff[$RIFFsubtype][$key][0]['data'])) {
						$thisfile_riff['comments'][$value][] = $thisfile_riff[$RIFFsubtype][$key][0]['data'];
					}
				}
/*
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['ID3 '])) {
...
*/
			...
			case '8SVX':
				...
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['BODY'][0]['offset'])) {
				...
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['VHDR'][0]['offset'])) {
					// shortcut
				...
				if (array_key_exists($thisfile_riff[$RIFFsubtype]['CHAN'][0]['data'])) {
		...
		switch ($RIFFsubtype) {
			...
			case 'AIFC':
				...
				foreach ($ID3v2_keys_bad as $ID3v2_key_bad) {
					if (array_key_exists($thisfile_riff[$RIFFsubtype][$ID3v2_key_bad]) && !array_key_exists($ID3v2_key_good, $thisfile_riff[$RIFFsubtype])) {
		...
		if (array_key_exists($thisfile_riff_WAVE['DISP']) && is_array($thisfile_riff_WAVE['DISP'])) {
			$thisfile_riff['comments']['title'][] = trim(substr($thisfile_riff_WAVE['DISP'][count($thisfile_riff_WAVE['DISP']) - 1]['data'], 4));
		}
		if (array_key_exists($thisfile_riff_WAVE['INFO']) && is_array($thisfile_riff_WAVE['INFO'])) {
			self::parseComments($thisfile_riff_WAVE['INFO'], $thisfile_riff['comments']);
		}
		if (array_key_exists($thisfile_riff['AVI ']['INFO']) && is_array($thisfile_riff['AVI ']['INFO'])) {
		...
		if (!array_key_exists($info['playtime_seconds'])) {
			$info['playtime_seconds'] = 0;
		}
		if (array_key_exists($thisfile_riff_raw['strh'][0]['dwLength']) && array_key_exists($thisfile_riff_raw['avih']['dwMicroSecPerFrame'])) {
		...
		} elseif (array_key_exists($thisfile_riff_raw['avih']['dwTotalFrames']) && array_key_exists($thisfile_riff_raw['avih']['dwMicroSecPerFrame'])) {
			$info['playtime_seconds'] = $thisfile_riff_raw['avih']['dwTotalFrames'] * ($thisfile_riff_raw['avih']['dwMicroSecPerFrame'] / 1000000);
		}

		if ($info['playtime_seconds'] > 0) {
			if (array_key_exists($thisfile_riff_audio) && array_key_exists($thisfile_riff_video)) {

				if (!array_key_exists($info['bitrate'])) {
					$info['bitrate'] = ((($info['avdataend'] - $info['avdataoffset']) / $info['playtime_seconds']) * 8);
				}

			} elseif (array_key_exists($thisfile_riff_audio) && !array_key_exists($thisfile_riff_video)) {

				if (!array_key_exists($thisfile_audio['bitrate'])) {
					$thisfile_audio['bitrate'] = ((($info['avdataend'] - $info['avdataoffset']) / $info['playtime_seconds']) * 8);
				}

			} elseif (!array_key_exists($thisfile_riff_audio) && array_key_exists($thisfile_riff_video)) {

				if (!array_key_exists($thisfile_video['bitrate'])) {
		...
		if (array_key_exists($thisfile_riff_video) && array_key_exists($thisfile_audio['bitrate']) && ($thisfile_audio['bitrate'] > 0) && ($info['playtime_seconds'] > 0)) {
				...
				switch ($chunkname) {
					case 'LIST':
						...
						if (preg_match('#^(movi|rec )$#i', $listname)) {
								...
								if ($AudioChunkStreamType == 'wb') {
									...
									if (preg_match('/^\xFF[\xE2-\xE7\xF2-\xF7\xFA-\xFF][\x00-\xEB]/s', $FirstFourBytes)) {
										...
										if (getid3_mp3::MPEGaudioHeaderBytesValid($FirstFourBytes)) {
											...
											if (array_key_exists($getid3_temp->info['mpeg']['audio'])) {
							...
							if (!array_key_exists($RIFFchunk[$listname])) {
					...
					default:
						...
						$thisindex = 0;
						if (array_key_exists($RIFFchunk[$chunkname]) && is_array($RIFFchunk[$chunkname])) {
						...
						switch ($chunkname) {
							...
							default:
								if (!empty($LISTchunkParent) && array_key_exists($LISTchunkMaxOffset) && (($RIFFchunk[$chunkname][$thisindex]['offset'] + $RIFFchunk[$chunkname][$thisindex]['size']) <= $LISTchunkMaxOffset)) {
									$RIFFchunk[$LISTchunkParent][$chunkname][$thisindex]['offset'] = $RIFFchunk[$chunkname][$thisindex]['offset'];
									$RIFFchunk[$LISTchunkParent][$chunkname][$thisindex]['size']   = $RIFFchunk[$chunkname][$thisindex]['size'];
									unset($RIFFchunk[$chunkname][$thisindex]['offset']);
									unset($RIFFchunk[$chunkname][$thisindex]['size']);
									if (array_key_exists($RIFFchunk[$chunkname][$thisindex]) && empty($RIFFchunk[$chunkname][$thisindex])) {
										unset($RIFFchunk[$chunkname][$thisindex]);
									}
									if (array_key_exists($RIFFchunk[$chunkname]) && empty($RIFFchunk[$chunkname])) {
	...
	public function ParseRIFFdata(&$RIFFdata) {
		...
		if ($RIFFdata) {
			...
			$getid3_temp->info['audio']        = (array_key_exists($info['audio']) ? $info['audio'] : array());
			$getid3_temp->info['video']        = (array_key_exists($info['video']) ? $info['video'] : array());
	...
	public static function parseComments(&$RIFFinfoArray, &$CommentsTargetArray) {
		...
		foreach ($RIFFinfoKeyLookup as $key => $value) {
			if (array_key_exists($RIFFinfoArray[$key])) {
				foreach ($RIFFinfoArray[$key] as $commentid => $commentdata) {
					if (trim($commentdata['data']) != '') {
						if (array_key_exists($CommentsTargetArray[$value])) {
	...
	public static function ParseDIVXTAG($DIVXTAG, $raw=false) {
		...
		$parsed['genre']  = (array_key_exists($DIVXTAGgenre[$parsed['genre_id']])   ? $DIVXTAGgenre[$parsed['genre_id']]   : $parsed['genre_id']);
		$parsed['rating'] = (array_key_exists($DIVXTAGrating[$parsed['rating_id']]) ? $DIVXTAGrating[$parsed['rating_id']] : $parsed['rating_id']);
