...
class getid3_lib
{
	...
	public static function flipped_array_merge_noclobber($array1, $array2) {
		...
		foreach (array_flip($array2) as $key => $val) {
			if (!array_key_exists($newarray[$key])) {
	...
	*/
	...
	public static function iconv_fallback($in_charset, $out_charset, $string) {
		...
		if (array_key_exists($ConversionFunctionList[strtoupper($in_charset)][strtoupper($out_charset)])) {
	...
	public static function RGADnameLookup($namecode) {
		...
		return (array_key_exists($RGADname[$namecode]) ? $RGADname[$namecode] : '');
	}
	...
	public static function RGADoriginatorLookup($originatorcode) {
		...
		return (array_key_exists($RGADoriginator[$originatorcode]) ? $RGADoriginator[$originatorcode] : '');
	}
	...
	public static function GetDataImageSize($imgData, &$imageinfo=array()) {
		if (PHP_VERSION_ID >= 50400) {
			...
			if ($GetDataImageSize === false || !array_key_exists($GetDataImageSize[0], $GetDataImageSize[1])) {
				return false;
			}
		...
		if ($tempfilename = tempnam($tempdir, 'gI3')) {
			if (is_writable($tempfilename) && is_file($tempfilename) && ($tmp = fopen($tempfilename, 'wb'))) {
				...
				if (($GetDataImageSize === false) || !array_key_exists($GetDataImageSize[0]) || !array_key_exists($GetDataImageSize[1])) {
					return false;
				}
				$GetDataImageSize['height'] = $GetDataImageSize[0];
				$GetDataImageSize['width']  = $GetDataImageSize[1];
	...
	public static function CopyTagsToComments(&$ThisFileInfo, $option_tags_html=true) {
		...
		if (!empty($ThisFileInfo['tags'])) {
			...
			foreach ($processLastTagTypes as $processLastTagType) {
				if (array_key_exists($ThisFileInfo['tags'][$processLastTagType])) {
						...
						if (!empty($value)) {
							...
							if (is_array($value) || empty($ThisFileInfo['comments'][$tagname]) || !in_array(trim($value), $ThisFileInfo['comments'][$tagname])) {
								...
								if (!is_int($key) && !ctype_digit($key)) {
									...
									if (!array_key_exists($ThisFileInfo['comments'][$tagname])) {
	...
	public static function EmbeddedLookup($key, $begin, $end, $file, $name) {
		...
		// Cached
		static $cache;
		if (array_key_exists($cache[$file][$name])) {
			return (array_key_exists($cache[$file][$name][$key]) ? $cache[$file][$name][$key] : '');
		}
		...
		while (0 < $line_count--) {
			...
			$ThisKey   = (array_key_exists($explodedLine[0]) ? $explodedLine[0] : '');
			$ThisValue = (array_key_exists($explodedLine[1]) ? $explodedLine[1] : '');
	...
	public static function getFileSizeSyscall($path) {
		...
		if (array_key_exists($commandline)) {
