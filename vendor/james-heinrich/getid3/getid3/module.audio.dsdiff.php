...
class getid3_dsdiff extends getid3_handler
{
	...
	public function Analyze() {
		...
		while (!$this->feof() && ($ChunkHeader = $this->fread(12))) {
			if (strlen($ChunkHeader) < 12) {
				$this->error('Expecting chunk header at offset '.(array_key_exists($thisChunk['offset']) ? $thisChunk['offset'] : 'N/A').', found insufficient data in file, aborting parsing');
				break;
			}
			$thisChunk = array();
	...
	public static function DSDIFFcmtType($cmtType) {
		...
		return (array_key_exists($DSDIFFcmtType[$cmtType]) ? $DSDIFFcmtType[$cmtType] : 'reserved');
	}
	...
	public static function DSDIFFcmtRef($cmtType, $cmtRef) {
		...
		switch ($cmtType) {
			...
			case 2:
			case 3:
				return (array_key_exists($DSDIFFcmtRef[$cmtType][$cmtRef]) ? $DSDIFFcmtRef[$cmtType][$cmtRef] : 'reserved');
		}
	...
	public static function DSDIFFmarkType($markType) {
		...
		return (array_key_exists($DSDIFFmarkType[$markType]) ? $DSDIFFmarkType[$markType] : 'reserved');
	}
