...
class getid3_cue extends getid3_handler
{
		...
		foreach ($file as $line) {
			...
			switch ($key)
			{
				...
				case 'track':
					...
					if (array_key_exists($currentFile)) // if there's a file
					{
	...
	public function parseComment($line, $track_on)
	{
		...
		$comment_REM  = (array_key_exists($explodedline[0]) ? $explodedline[0] : '');
		$comment_type = (array_key_exists($explodedline[1]) ? $explodedline[1] : '');
		$comment_data = (array_key_exists($explodedline[2]) ? $explodedline[2] : '');
	...
	public function parseIndex($line, $track_on)
	{
		...
		$minutes = (array_key_exists($explodedline[0]) ? $explodedline[0] : '');
		$seconds = (array_key_exists($explodedline[1]) ? $explodedline[1] : '');
		$frames  = (array_key_exists($explodedline[2]) ? $explodedline[2] : '');
