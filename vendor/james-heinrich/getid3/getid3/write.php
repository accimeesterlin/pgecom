...
class getid3_writetags
{
	...
	public function WriteTags() {
		...
		if (filesize($this->filename) == 0) {
			...
			switch (array_key_exists($this->ThisFileInfo['fileformat']) ? $this->ThisFileInfo['fileformat'] : '') {
				case 'mp3':
				case 'mp2':
				case 'mp1':
				...
				case 'ogg':
					switch (array_key_exists($this->ThisFileInfo['audio']['dataformat']) ? $this->ThisFileInfo['audio']['dataformat'] : '') {
						case 'flac':
							//$AllowedTagFormats = array('metaflac');
			...
			foreach ($this->tagformats as $requested_tag_format) {
				if (!in_array($requested_tag_format, $AllowedTagFormats)) {
					$errormessage = 'Tag format "'.$requested_tag_format.'" is not allowed on "'.(array_key_exists($this->ThisFileInfo['fileformat']) ? $this->ThisFileInfo['fileformat'] : '');
					$errormessage .= (array_key_exists($this->ThisFileInfo['audio']['dataformat']) ? '.'.$this->ThisFileInfo['audio']['dataformat'] : '');
					$errormessage .= '" files';
		...
		if (array_key_exists($this->tag_data['TRACK']) && !array_key_exists($this->tag_data['TRACK_NUMBER'])) {
...
//			if (!array_key_exists($this->ThisFileInfo['tags'][$TagFormat])) {
//				return false;
//			}
//			$tag_data = array_merge_recursive($tag_data, $this->ThisFileInfo['tags'][$TagFormat]);
	...
	public function FormatDataForID3v1() {
		...
		$tag_data_id3v1['title']        =        getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['TITLE']       ) ? $this->tag_data['TITLE']        : array())));
		$tag_data_id3v1['artist']       =        getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['ARTIST']      ) ? $this->tag_data['ARTIST']       : array())));
		$tag_data_id3v1['album']        =        getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['ALBUM']       ) ? $this->tag_data['ALBUM']        : array())));
		$tag_data_id3v1['year']         =        getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['YEAR']        ) ? $this->tag_data['YEAR']         : array())));
		$tag_data_id3v1['comment']      =        getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['COMMENT']     ) ? $this->tag_data['COMMENT']      : array())));
		$tag_data_id3v1['track_number'] = intval(getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['TRACK_NUMBER']) ? $this->tag_data['TRACK_NUMBER'] : array()))));
			...
			switch ($ID3v2_framename) {
				case 'APIC':
					foreach ($valuearray as $key => $apic_data_array) {
						if (array_key_exists($apic_data_array['data']) &&
							array_key_exists($apic_data_array['picturetypeid']) &&
							array_key_exists($apic_data_array['description']) &&
							array_key_exists($apic_data_array['mime'])) {
				...
				case 'POPM':
					if (array_key_exists($valuearray['email']) &&
						array_key_exists($valuearray['rating']) &&
						array_key_exists($valuearray['data'])) {
				...
				case 'GRID':
					if (
						array_key_exists($valuearray['groupsymbol']) &&
						array_key_exists($valuearray['ownerid']) &&
						array_key_exists($valuearray['data'])
					) {
				...
				case 'UFID':
					if (array_key_exists($valuearray['ownerid']) &&
						array_key_exists($valuearray['data'])) {
				...
				case 'TXXX':
					foreach ($valuearray as $key => $txxx_data_array) {
						if (array_key_exists($txxx_data_array['description']) && array_key_exists($txxx_data_array['data'])) {
				...
				default:
					...
					foreach ($valuearray as $key => $value) {
						if (array_key_exists($ID3v2_text_encoding_lookup[$id3v2_majorversion][$this->tag_encoding])) {
	...
	public function FormatDataForReal() {
		$tag_data_real              = array();
		$tag_data_real['title']     = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['TITLE']    ) ? $this->tag_data['TITLE']     : array())));
		$tag_data_real['artist']    = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['ARTIST']   ) ? $this->tag_data['ARTIST']    : array())));
		$tag_data_real['copyright'] = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['COPYRIGHT']) ? $this->tag_data['COPYRIGHT'] : array())));
		$tag_data_real['comment']   = getid3_lib::iconv_fallback($this->tag_encoding, 'ISO-8859-1', implode(' ', (array_key_exists($this->tag_data['COMMENT']  ) ? $this->tag_data['COMMENT']   : array())));
