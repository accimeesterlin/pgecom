...
class getid3_lyrics3 extends getid3_handler
{
	...
	public function Analyze() {
		...
		if ($lyrics3end == 'LYRICSEND') {
			...
			if (array_key_exists($info['ape']['tag_offset_start']) && ($info['ape']['tag_offset_start'] > 15)) {
		...
		if (array_key_exists($lyrics3offset) && array_key_exists($lyrics3version) && array_key_exists($lyrics3size)) {
			$info['avdataend'] = $lyrics3offset;
			$this->getLyrics3Data($lyrics3offset, $lyrics3version, $lyrics3size);

			if (!array_key_exists($info['ape'])) {
				if (array_key_exists($info['lyrics3']['tag_offset_start'])) {
	...
	public function getLyrics3Data($endoffset, $version, $length) {
		...
		switch ($version) {
			...
			case 2:
				if (substr($rawdata, strlen($rawdata) - 9, 9) == 'LYRICS200') {
					...
					if (array_key_exists($ParsedLyrics3['raw']['IND'])) {
						$i = 0;
					...
					foreach ($fieldnametranslation as $key => $value) {
						if (array_key_exists($ParsedLyrics3['raw'][$key])) {
							$ParsedLyrics3['comments'][$value][] = trim($ParsedLyrics3['raw'][$key]);
						}
					}

					if (array_key_exists($ParsedLyrics3['raw']['IMG'])) {
						$imagestrings = explode("\r\n", $ParsedLyrics3['raw']['IMG']);
						foreach ($imagestrings as $key => $imagestring) {
							if (strpos($imagestring, '||') !== false) {
								$imagearray = explode('||', $imagestring);
								$ParsedLyrics3['images'][$key]['filename']     =                                (array_key_exists($imagearray[0]) ? $imagearray[0] : '');
								$ParsedLyrics3['images'][$key]['description']  =                                (array_key_exists($imagearray[1]) ? $imagearray[1] : '');
								$ParsedLyrics3['images'][$key]['timestamp']    = $this->Lyrics3Timestamp2Seconds(array_key_exists($imagearray[2]) ? $imagearray[2] : '');
							}
						}
					}
					if (array_key_exists($ParsedLyrics3['raw']['LYR'])) {
						$this->Lyrics3LyricsTimestampParse($ParsedLyrics3);
					}
				} else {
		...
		if (array_key_exists($info['id3v1']['tag_offset_start']) && ($info['id3v1']['tag_offset_start'] <= $ParsedLyrics3['tag_offset_end'])) {
	...
	public function Lyrics3LyricsTimestampParse(&$Lyrics3data) {
		...
		foreach ($lyricsarray as $key => $lyricline) {
			...
			if (array_key_exists($thislinetimestamps) && is_array($thislinetimestamps)) {
				sort($thislinetimestamps);
				foreach ($thislinetimestamps as $timestampkey => $timestamp) {
					if (array_key_exists($Lyrics3data['synchedlyrics'][$timestamp])) {
		...
		if (array_key_exists($Lyrics3data['synchedlyrics']) && is_array($Lyrics3data['synchedlyrics'])) {
