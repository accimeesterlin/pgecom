...
class getid3_id3v2 extends getid3_handler
{
	...
	//        Flags         $xx xx
		...
		if ($sizeofframes > 0) {
			...
			while (array_key_exists($framedata) && (strlen($framedata) > 0)) { // cycle through until no more frame data is left to parse
				...
				if (($frame_size <= strlen($framedata)) && ($this->IsValidID3v2FrameName($frame_name, $id3v2_majorversion))) {
					...
					} elseif (!array_key_exists($framedata) || ($frame_size > strlen($framedata))) {

						$this->error('error parsing "'.$frame_name.'" ('.$framedataoffset.' bytes into the ID3v2.'.$id3v2_majorversion.' tag). (ERROR: $frame_size ('.$frame_size.') > strlen($framedata) ('.(array_key_exists($framedata) ? strlen($framedata) : 'null').')).');
	...
	//        ID3v2 size             4 * %0xxxxxxx
		...
		if (array_key_exists($thisfile_id3v2_flags['isfooter']) && $thisfile_id3v2_flags['isfooter']) {
		...
		if (array_key_exists($thisfile_id3v2['comments']['genre'])) {
			$genres = array();
		...
		if (array_key_exists($thisfile_id3v2['comments']['track_number'])) {
		...
		if (!array_key_exists($thisfile_id3v2['comments']['year']) && !empty($thisfile_id3v2['comments']['recording_time'][0]) && preg_match('#^([0-9]{4})#', trim($thisfile_id3v2['comments']['recording_time'][0]), $matches)) {
		...
		if (array_key_exists($thisfile_id3v2['footer'])) {
	...
	public function ParseID3v2Frame(&$parsedFrame) {
		...
		if (array_key_exists($parsedFrame['datalength']) && ($parsedFrame['datalength'] == 0)) {
			...
			$parsedFrame['ownerid'] = (array_key_exists($exploded[0]) ? $exploded[0] : '');
			$parsedFrame['data']    = (array_key_exists($exploded[1]) ? $exploded[1] : '');
			...
			if (!empty($parsedFrame['framenameshort']) && !empty($parsedFrame['data'])) {
				$commentkey = ($parsedFrame['description'] ? $parsedFrame['description'] : (array_key_exists($info['id3v2']['comments'][$parsedFrame['framenameshort']]) ? count($info['id3v2']['comments'][$parsedFrame['framenameshort']]) : 0));
				if (!array_key_exists($info['id3v2']['comments'][$parsedFrame['framenameshort']]) || !array_key_exists($commentkey, $info['id3v2']['comments'][$parsedFrame['framenameshort']])) {
			...
			if ($frame_offset >= $parsedFrame['datalength']) {
				...
				if ($id3v2_majorversion == 2) {
					$parsedFrame['imagetype'] = array_key_exists($frame_imagetype) ? $frame_imagetype : null;
				} else {
					$parsedFrame['mime']      = array_key_exists($frame_mimetype) ? $frame_mimetype : null;
				}
...
*/
					...
					if (is_string($this->getid3->option_save_attachments)) {
						...
						if (!empty($parsedFrame['framenameshort']) && !empty($parsedFrame['data'])) {
							if (!array_key_exists($info['id3v2']['comments']['picture'])) {
								$info['id3v2']['comments']['picture'] = array();
							}
							$comments_picture_data = array();
							foreach (array('data', 'image_mime', 'image_width', 'image_height', 'imagetype', 'picturetype', 'description', 'datalength') as $picture_key) {
								if (array_key_exists($parsedFrame[$picture_key])) {
			...
			foreach (array('id', 'time_begin', 'time_end', 'offset_begin', 'offset_end', 'chapter_name', 'chapter_description', 'chapter_url', 'picture_present') as $id3v2_chapter_key) {
				if (array_key_exists($parsedFrame[$id3v2_chapter_key])) {
					$id3v2_chapter_entry[$id3v2_chapter_key] = $parsedFrame[$id3v2_chapter_key];
				}
			}
			if (!array_key_exists($info['id3v2']['chapters'])) {
	...
	public function LookupExtendedHeaderRestrictionsTagSizeLimits($index) {
		...
		return (array_key_exists($LookupExtendedHeaderRestrictionsTagSizeLimits[$index]) ? $LookupExtendedHeaderRestrictionsTagSizeLimits[$index] : '');
	}
	...
	public function LookupExtendedHeaderRestrictionsTextEncodings($index) {
		...
		return (array_key_exists($LookupExtendedHeaderRestrictionsTextEncodings[$index]) ? $LookupExtendedHeaderRestrictionsTextEncodings[$index] : '');
	}
	...
	public function LookupExtendedHeaderRestrictionsTextFieldSize($index) {
		...
		return (array_key_exists($LookupExtendedHeaderRestrictionsTextFieldSize[$index]) ? $LookupExtendedHeaderRestrictionsTextFieldSize[$index] : '');
	}
	...
	public function LookupExtendedHeaderRestrictionsImageEncoding($index) {
		...
		return (array_key_exists($LookupExtendedHeaderRestrictionsImageEncoding[$index]) ? $LookupExtendedHeaderRestrictionsImageEncoding[$index] : '');
	}
	...
	public function LookupExtendedHeaderRestrictionsImageSizeSize($index) {
		...
		return (array_key_exists($LookupExtendedHeaderRestrictionsImageSizeSize[$index]) ? $LookupExtendedHeaderRestrictionsImageSizeSize[$index] : '');
	}
	...
	public static function ETCOEventLookup($index) {
		...
		return (array_key_exists($EventLookup[$index]) ? $EventLookup[$index] : '');
	}
	...
	public static function SYTLContentTypeLookup($index) {
		...
		return (array_key_exists($SYTLContentTypeLookup[$index]) ? $SYTLContentTypeLookup[$index] : '');
	}
	...
	public static function APICPictureTypeLookup($index, $returnarray=false) {
		...
		return (array_key_exists($APICPictureTypeLookup[$index]) ? $APICPictureTypeLookup[$index] : '');
	}
	...
	public static function COMRReceivedAsLookup($index) {
		...
		return (array_key_exists($COMRReceivedAsLookup[$index]) ? $COMRReceivedAsLookup[$index] : '');
	}
	...
	public static function RVA2ChannelTypeLookup($index) {
		...
		return (array_key_exists($RVA2ChannelTypeLookup[$index]) ? $RVA2ChannelTypeLookup[$index] : '');
	}
	...
	public static function TextEncodingTerminatorLookup($encoding) {
		...
		return (array_key_exists($TextEncodingTerminatorLookup[$encoding]) ? $TextEncodingTerminatorLookup[$encoding] : "\x00");
	}
	...
	public static function TextEncodingNameLookup($encoding) {
		...
		return (array_key_exists($TextEncodingNameLookup[$encoding]) ? $TextEncodingNameLookup[$encoding] : 'ISO-8859-1');
	}
	...
	public static function ID3v22iTunesBrokenFrameName($frame_name) {
		...
		if (strlen($frame_name) == 4) {
			if ((substr($frame_name, 3, 1) == ' ') || (substr($frame_name, 3, 1) == "\x00")) {
				if (array_key_exists($ID3v22_iTunes_BrokenFrames[substr($frame_name, 0, 3)])) {
