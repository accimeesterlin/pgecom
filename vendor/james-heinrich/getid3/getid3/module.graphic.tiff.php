...
class getid3_tiff extends getid3_handler
{
	...
	public function Analyze() {
		...
		foreach ($info['tiff']['ifd'] as $IFDid => $IFDarray) {
			if(!array_key_exists($IFDarray['fields'])) {
				continue;
			}
			...
			foreach ($IFDarray['fields'] as $key => $fieldarray) {
				switch ($fieldarray['raw']['tag']) {
					...
					case 259: // Compression
						if (!array_key_exists($fieldarray['value'])) {
					...
					case 700:
						$XMPmagic = '<?xpacket';
						$this->fseek($fieldarray['offset']);
						$xmpkey = (array_key_exists($info['tiff']['XMP']) ? count($info['tiff']['XMP']) : 0);
				...
				switch ($fieldarray['raw']['tag']) {
					...
					case 316: // HostComputer
						...
						if (array_key_exists($info['tiff']['comments'][$TIFFcommentName])) {
	...
	public function TIFFcompressionMethod($id) {
		...
		return (array_key_exists($TIFFcompressionMethod[$id]) ? $TIFFcompressionMethod[$id] : 'unknown/invalid ('.$id.')');
	}
	...
	public function TIFFcommentName($id) {
		...
		return (array_key_exists($TIFFcommentName[$id]) ? $TIFFcommentName[$id] : 'unknown/invalid ('.$id.')');
	}
