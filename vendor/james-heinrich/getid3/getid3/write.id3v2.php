...
class getid3_write_id3v2
{
	...
	public function WriteID3v2() {
		...
		if (!empty($this->filename) && (getID3::is_writable($this->filename) || (!file_exists($this->filename) && getID3::is_writable(dirname($this->filename))))) {
			...
			$this->paddedlength = (array_key_exists($OldThisFileInfo['id3v2']['headerlength']) ? max($OldThisFileInfo['id3v2']['headerlength'], $this->paddedlength) : $this->paddedlength);

			if ($NewID3v2Tag = $this->GenerateID3v2Tag()) {

				if (file_exists($this->filename) && getID3::is_writable($this->filename) && array_key_exists($OldThisFileInfo['id3v2']['headerlength']) && ($OldThisFileInfo['id3v2']['headerlength'] == strlen($NewID3v2Tag))) {
		...
		if (($this->majorversion < 3) || ($this->majorversion > 4)) {
			...
			switch ($frame_name) {
				...
				case 'TXXX':
					...
					$source_data_array['encodingid'] = (array_key_exists($source_data_array['encodingid']) ? $source_data_array['encodingid'] : $this->id3v2_default_encodingid);
				...
				case 'WXXX':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
						$this->errors[] = 'Invalid Text Encoding in '.$frame_name.' ('.$source_data_array['encodingid'].') for ID3v2.'.$this->majorversion;
					} elseif (!array_key_exists($source_data_array['data']) || !$this->IsValidURL($source_data_array['data'], false)) {
					...
					if (($source_data_array['timestampformat'] > 2) || ($source_data_array['timestampformat'] < 1)) {
						...
						foreach ($source_data_array as $key => $val) {
							if (!$this->ID3v2IsValidETCOevent($val['typeid'])) {
								...
								if (($val['timestamp'] > 0) && array_key_exists($previousETCOtimestamp) && ($previousETCOtimestamp >= $val['timestamp'])) {
									//   The 'Time stamp' is set to zero if directly at the beginning of the sound
									//   or after the previous event. All events MUST be sorted in chronological order.
	...
	public function ID3v2FrameIsAllowed($frame_name, $source_data_array) {
		...
		if ($this->majorversion == 4) {
			switch ($frame_name) {
				...
				case 'ENCR':
				case 'GRID':
					if (!array_key_exists($source_data_array['ownerid'])) {
				...
				case 'APIC':
				case 'GEOB':
					if (!array_key_exists($source_data_array['description'])) {
				...
				case 'USER':
					if (!array_key_exists($source_data_array['language'])) {
				...
				case 'COMM':
					if (!array_key_exists($source_data_array['language'])) {
						$this->errors[] = '[language] not specified for '.$frame_name;
					} elseif (!array_key_exists($source_data_array['description'])) {
						$this->errors[] = '[description] not specified for '.$frame_name;
				...
				case 'POPM':
					if (!array_key_exists($source_data_array['email'])) {
				...
				case 'LINK':
					...
					if (!array_key_exists($source_data_array['frameid'])) {
				...
				case 'SIGN':
					if (!array_key_exists($source_data_array['ownerid'])) {
						$this->errors[] = '[ownerid] not specified for '.$frame_name;
					} elseif (!array_key_exists($source_data_array['data'])) {
		...
		if (is_array($this->tag_data)) {
			foreach ($this->tag_data as $frame_name => $frame_rawinputdata) {
				foreach ($frame_rawinputdata as $irrelevantindex => $source_data_array) {
							...
							if ($frame_data = $this->GenerateID3v2FrameData($frame_name, $source_data_array)) {
								...
								if ($this->majorversion >= 4) {
									...
									if (strlen($unsynchdata) != strlen($frame_data)) {
										...
										if (array_key_exists($TagUnsynchronisation) && $TagUnsynchronisation === false) {
											// only set to true if ALL frames are unsynchronised
										} else {
											$TagUnsynchronisation = true;
										}
									} else {
										if (array_key_exists($TagUnsynchronisation)) {
					...
					if (null !== $frame_name && array_key_exists($frame_length) && array_key_exists($frame_flags) && array_key_exists($frame_data)) {
	...
	public function ID3v2IsValidTextEncoding($textencodingbyte) {
		...
		return array_key_exists($ID3v2IsValidTextEncoding_cache[$this->majorversion][$textencodingbyte]);
	}
	...
	public function array_join_merge($arr1, $arr2) {
		...
		if (is_array($arr1) && is_array($arr2)) {
			...
			if ($this->is_hash($arr1) && $this->is_hash($arr2)) {
				...
				foreach ($keys as $key) {
					$new_array[$key] = $this->array_join_merge((array_key_exists($arr1[$key]) ? $arr1[$key] : ''), (array_key_exists($arr2[$key]) ? $arr2[$key] : ''));
				}
			} else {
	...
	public static function ID3v2ShortFrameNameLookup($majorversion, $long_description) {
		...
		return (array_key_exists($ID3v2ShortFrameNameLookup[$majorversion][strtolower($long_description)]) ? $ID3v2ShortFrameNameLookup[$majorversion][strtolower($long_description)] : '');
