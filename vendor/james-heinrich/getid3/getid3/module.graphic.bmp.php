...
class getid3_bmp extends getid3_handler
{
	...
	public function Analyze() {
		...
		if ($this->ExtractPalette || $this->ExtractData) {
			...
			} elseif (array_key_exists($thisfile_bmp_header_raw['colors_used']) && ($thisfile_bmp_header_raw['colors_used'] > 0) && ($thisfile_bmp_header_raw['colors_used'] <= 256)) {
		...
		if ($this->ExtractData) {
			...
			$pixeldataoffset = 0;
			$thisfile_bmp_header_raw['compression'] = (array_key_exists($thisfile_bmp_header_raw['compression']) ? $thisfile_bmp_header_raw['compression'] : '');
	...
	public function PlotBMP(&$BMPinfo) {
		$starttime = time();
		if (!array_key_exists($BMPinfo['bmp']['data']) || !is_array($BMPinfo['bmp']['data'])) {
		...
		if ($im = imagecreatetruecolor($BMPinfo['resolution_x'], $BMPinfo['resolution_y'])) {
			for ($row = 0; $row < $BMPinfo['resolution_y']; $row++) {
				for ($col = 0; $col < $BMPinfo['resolution_x']; $col++) {
					if (array_key_exists($BMPinfo['bmp']['data'][$row][$col])) {
	...
	public function BMPcompressionWindowsLookup($compressionid) {
		...
		return (array_key_exists($BMPcompressionWindowsLookup[$compressionid]) ? $BMPcompressionWindowsLookup[$compressionid] : 'invalid');
	}
	...
	public function BMPcompressionOS2Lookup($compressionid) {
		...
		return (array_key_exists($BMPcompressionOS2Lookup[$compressionid]) ? $BMPcompressionOS2Lookup[$compressionid] : 'invalid');
	}
