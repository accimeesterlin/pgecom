...
class getid3_flv extends getid3_handler
{
	...
	public function Analyze() {
				...
				case GETID3_FLV_TAG_VIDEO:
					...
					if (!$found_video) {
						...
						if ($info['flv']['video']['videoCodec'] == GETID3_FLV_VIDEO_H264) {
							...
							if (!array_key_exists($info['video']['resolution_x'])) { // only when meta data isn't set
				...
				case GETID3_FLV_TAG_META:
					if (!$found_meta) {
						...
						foreach ($copykeys as $sourcekey => $destkey) {
							if (array_key_exists($info['flv']['meta']['onMetaData'][$sourcekey])) {
								switch ($sourcekey) {
									case 'width':
									case 'height':
		...
		if (array_key_exists($info['flv']['meta']['onMetaData']['audiocodecid'])) {
			$info['audio']['codec'] = self::audioFormatLookup($info['flv']['meta']['onMetaData']['audiocodecid']);
		}
		if (array_key_exists($info['flv']['meta']['onMetaData']['videocodecid'])) {
	...
	public static function audioFormatLookup($id) {
		...
		return (array_key_exists($lookup[$id]) ? $lookup[$id] : false);
	}
