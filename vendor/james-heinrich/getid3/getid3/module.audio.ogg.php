...
class getid3_ogg extends getid3_handler
{
	...
	public function Analyze() {
		...
		if (array_key_exists($info['id3v2'])) {
			$this->warning('Illegal ID3v2 tag present.');
		}
		if (array_key_exists($info['id3v1'])) {
		...
		if (array_key_exists($info['audio']['bitrate']) && !array_key_exists($info['playtime_seconds'])) {
		...
		if (array_key_exists($info['ogg']['vendor'])) {
	...
	public function ParseVorbisComments() {
		...
		for ($i = 0; $i < $CommentsCount; $i++) {
			...
			while ((strlen($commentdata) - $commentdataoffset) < $ThisFileInfo_ogg_comments_raw[$i]['size']) {
				...
				if ($oggpageinfo = $this->ParseOggPageHeader()) {
					...
					//$commentdata .= $this->fread($info['ogg']['pageheader'][$oggpageinfo['page_seqno']]['page_length']);
					if (!array_key_exists($info['ogg']['pageheader'][$VorbisCommentPage])) {
			...
			if (!$commentstring) {
				...
				$ThisFileInfo_ogg_comments_raw[$i]['value'] = (array_key_exists($commentexploded[1]) ? $commentexploded[1] : '');
				...
				if ($ThisFileInfo_ogg_comments_raw[$i]['key'] == 'METADATA_BLOCK_PICTURE') {
					...
					if ($imageinfo === false || !array_key_exists($imageinfo['mime'])) {
		...
		if (array_key_exists($info['ogg']['comments']) && is_array($info['ogg']['comments'])) {
	...
	public static function SpeexBandModeLookup($mode) {
		...
		return (array_key_exists($SpeexBandModeLookup[$mode]) ? $SpeexBandModeLookup[$mode] : null);
	}
	...
	public static function TheoraColorSpace($colorspace_id) {
		...
		return (array_key_exists($TheoraColorSpaceLookup[$colorspace_id]) ? $TheoraColorSpaceLookup[$colorspace_id] : null);
	}
	...
	public static function TheoraPixelFormat($pixelformat_id) {
		...
		return (array_key_exists($TheoraPixelFormatLookup[$pixelformat_id]) ? $TheoraPixelFormatLookup[$pixelformat_id] : null);
	}
