...
class getid3_apetag extends getid3_handler
{
	...
	public function Analyze() {
		...
		if (!array_key_exists($info['ape']['tag_offset_end'])) {
		...
		if (array_key_exists($thisfile_ape['footer']['flags']['header']) && $thisfile_ape['footer']['flags']['header']) {
		...
		if (array_key_exists($info['id3v1']['tag_offset_start']) && ($info['id3v1']['tag_offset_start'] < $thisfile_ape['tag_offset_end'])) {
			...
			switch (strtolower($item_key)) {
				...
				case 'cover art (studio)':
					...
					do {
						...
						if (($imagechunkcheck === false) || !array_key_exists($imagechunkcheck[2])) {
						...
						if (is_string($this->inline_attachments)) {
							...
							if (!array_key_exists($info['ape']['comments']['picture'])) {
								$info['ape']['comments']['picture'] = array();
							}
							$comments_picture_data = array();
							foreach (array('data', 'image_mime', 'image_width', 'image_height', 'imagetype', 'picturetype', 'description', 'datalength') as $picture_key) {
								if (array_key_exists($thisfile_ape_items_current[$picture_key])) {
	...
	public function APEcontentTypeFlagLookup($contenttypeid) {
		...
		return (array_key_exists($APEcontentTypeFlagLookup[$contenttypeid]) ? $APEcontentTypeFlagLookup[$contenttypeid] : 'invalid');
	}
