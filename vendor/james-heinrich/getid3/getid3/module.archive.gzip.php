...
class getid3_gzip extends getid3_handler
{
		...
		foreach ($arr_members as $member) {
			...
			if ($this->parse_contents) {
				...
				if (function_exists('gzinflate')) {
					...
					$determined_format['module'] = (array_key_exists($determined_format['module']) ? $determined_format['module'] : '');
	...
	public function get_os_type($key) {
		...
		return (array_key_exists($os_type[$key]) ? $os_type[$key] : '');
	}
	...
	public function get_xflag_type($key) {
		...
		return (array_key_exists($xflag_type[$key]) ? $xflag_type[$key] : '');
	}
}
