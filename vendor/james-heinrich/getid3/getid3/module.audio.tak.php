...
class getid3_tak extends getid3_handler
{
	...
	public function TAKFramesizeLookup($framesize) {
		...
		return (array_key_exists($TAKFramesizeLookup[$framesize]) ? $TAKFramesizeLookup[$framesize] : 'invalid');
	}
	public function TAKCodecTypeLookup($code) {
		...
		return (array_key_exists($TAKCodecTypeLookup[$code]) ? $TAKCodecTypeLookup[$code] : 'invalid');
	}
	public function TAKProfileLookup($code) {
		...
		return (array_key_exists($TAKEvaluationLookup[$evaluation]) ? $out .= $compresion . $TAKEvaluationLookup[$evaluation] : 'invalid');
	}
