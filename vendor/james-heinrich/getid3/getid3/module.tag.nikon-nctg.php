...
class getid3_tag_nikon_nctg
{
			...
			while ($offset < $datalength) {
				...
				switch ($record_type) {
					...
					case 0x01108822: // ExposureProgram
						$data = array_key_exists(static::$exposurePrograms[$data]) ? static::$exposurePrograms[$data] : $data;
						break;
					case 0x01109207: // MeteringMode
						$data = array_key_exists(static::$meteringModes[$data]) ? static::$meteringModes[$data] : $data;
						break;
					...
					case 0x01200002: // GPSLatitude
						if (is_array($data)) {
							$direction_multiplier = ((array_key_exists($parsed['GPSLatitudeRef']) && ($parsed['GPSLatitudeRef'] === 'S')) ? -1 : 1);
							$parsed['GPS']['computed']['latitude'] = $direction_multiplier * ($data[0] + ($data[1] / 60) + ($data[2] / 3600));
						}
						break;
					case 0x01200004: // GPSLongitude
						if (is_array($data)) {
							$direction_multiplier = ((array_key_exists($parsed['GPSLongitudeRef']) && ($parsed['GPSLongitudeRef'] === 'W')) ? -1 : 1);
							$parsed['GPS']['computed']['longitude'] = $direction_multiplier * ($data[0] + ($data[1] / 60) + ($data[2] / 3600));
						}
						break;
					case 0x01200006:  // GPSAltitude
						if (array_key_exists($parsed['GPSAltitudeRef'])) {
					...
					case 0x0120001d: // GPSDateStamp
						if (array_key_exists($parsed['GPSTimeStamp']) && is_array($parsed['GPSTimeStamp']) && $data !== '') {
					...
					case 0x0200001b: // CropHiSpeed
						if (is_array($data) && count($data) === 7) {
							$name = array_key_exists(static::$cropHiSpeeds[$data[0]]) ? static::$cropHiSpeeds[$data[0]] : sprintf('Unknown (%d)', $data[0]);
							$data = array(
								'Name' => $name,
					...
					case 0x0200001e: // ColorSpace
						$data = array_key_exists(static::$colorSpaces[$data]) ? static::$colorSpaces[$data] : $data;
						break;
					case 0x0200001f: // VRInfo
						$data = array(
							'VRInfoVersion' => substr($data, 0, 4),
							'VibrationReduction' => array_key_exists(static::$vibrationReductions[ord(substr($data, 4, 1))])
					...
					case 0x02000022: // ActiveDLighting
						$data = array_key_exists(static::$activeDLightnings[$data]) ? static::$activeDLightnings[$data] : $data;
						break;
					...
					case 0x0200002a: // VignetteControl
						$data = array_key_exists(static::$vignetteControls[$data]) ? static::$vignetteControls[$data] : $data;
						break;
					...
					case 0x02000039: // LocationInfo
						$encoding = array_key_exists(static::$nikonTextEncodings[ord(substr($data, 4, 1))])
					...
					case 0x02000087: // FlashMode
						$data = array_key_exists(static::$flashModes[$data]) ? static::$flashModes[$data] : $data;
						break;
					...
					case 0x020000b1: // HighISONoiseReduction
						$data = array_key_exists(static::$highISONoiseReductions[$data]) ? static::$highISONoiseReductions[$data] : $data;
						break;
				...
				$tag_name = (array_key_exists($NCTGtagName[$record_type]) ? $NCTGtagName[$record_type] : '0x' . str_pad(dechex($record_type), 8, '0', STR_PAD_LEFT));
	...
	protected function flashFirmwareLookup($firstByte, $secondByte)
	{
		...
		if (array_key_exists(static::$flashInfoExternalFlashFirmwares[$indexKey])) {
