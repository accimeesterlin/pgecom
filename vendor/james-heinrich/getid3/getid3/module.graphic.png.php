...
class getid3_png extends getid3_handler
{
		...
		while ((($this->ftell() - (strlen($PNGfiledata) - $offset)) < $info['filesize'])) {
			...
			switch ($chunk['type_text']) {
				...
				case 'zTXt': // Compressed Textual Data
					...
					if (array_key_exists($thisfile_png_chunk_type_text['text'])) {
						$thisfile_png['comments'][$thisfile_png_chunk_type_text['keyword']][] = $thisfile_png_chunk_type_text['text'];
					}
					break;
				...
				case 'gIFg': // GIF Graphic Control Extension
					$gIFgCounter = 0;
					if (array_key_exists($thisfile_png_chunk_type_text) && is_array($thisfile_png_chunk_type_text)) {
				...
				case 'IDAT': // Image Data
					...
					if (array_key_exists($thisfile_png['IDAT']) && is_array($thisfile_png['IDAT'])) {
	...
	public function PNGsRGBintentLookup($sRGB) {
		...
		return (array_key_exists($PNGsRGBintentLookup[$sRGB]) ? $PNGsRGBintentLookup[$sRGB] : 'invalid');
	}
	...
	public function PNGcompressionMethodLookup($compressionmethod) {
		...
		return (array_key_exists($PNGcompressionMethodLookup[$compressionmethod]) ? $PNGcompressionMethodLookup[$compressionmethod] : 'invalid');
	}
	...
	public function PNGpHYsUnitLookup($unitid) {
		...
		return (array_key_exists($PNGpHYsUnitLookup[$unitid]) ? $PNGpHYsUnitLookup[$unitid] : 'invalid');
	}
	...
	public function PNGoFFsUnitLookup($unitid) {
		...
		return (array_key_exists($PNGoFFsUnitLookup[$unitid]) ? $PNGoFFsUnitLookup[$unitid] : 'invalid');
	}
	...
	public function PNGpCALequationTypeLookup($equationtype) {
		...
		return (array_key_exists($PNGpCALequationTypeLookup[$equationtype]) ? $PNGpCALequationTypeLookup[$equationtype] : 'invalid');
	}
	...
	public function PNGsCALUnitLookup($unitid) {
		...
		return (array_key_exists($PNGsCALUnitLookup[$unitid]) ? $PNGsCALUnitLookup[$unitid] : 'invalid');
	}
