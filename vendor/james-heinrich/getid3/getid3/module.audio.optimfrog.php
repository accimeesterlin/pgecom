...
class getid3_optimfrog extends getid3_handler
{
	...
	public function ParseOptimFROGheader45() {
		...
		while (!feof($this->getid3->fp) && ($this->ftell() < $info['avdataend'])) {
			...
			if (!array_key_exists($info['ofr'][$BlockName])) {
		...
		if (array_key_exists($info['ofr']['TAIL']['offset'])) {
	...
	public static function OptimFROGsampleTypeLookup($SampleType) {
		...
		return (array_key_exists($OptimFROGsampleTypeLookup[$SampleType]) ? $OptimFROGsampleTypeLookup[$SampleType] : false);
	}
	...
	public static function OptimFROGbitsPerSampleTypeLookup($SampleType) {
		...
		return (array_key_exists($OptimFROGbitsPerSampleTypeLookup[$SampleType]) ? $OptimFROGbitsPerSampleTypeLookup[$SampleType] : false);
	}
	...
	public static function OptimFROGchannelConfigurationLookup($ChannelConfiguration) {
		...
		return (array_key_exists($OptimFROGchannelConfigurationLookup[$ChannelConfiguration]) ? $OptimFROGchannelConfigurationLookup[$ChannelConfiguration] : false);
	}
	...
	public static function OptimFROGchannelConfigNumChannelsLookup($ChannelConfiguration) {
		...
		return (array_key_exists($OptimFROGchannelConfigNumChannelsLookup[$ChannelConfiguration]) ? $OptimFROGchannelConfigNumChannelsLookup[$ChannelConfiguration] : false);
	}


	// static function OptimFROGalgorithmNameLookup($AlgorithID) {
	//     static $OptimFROGalgorithmNameLookup = array();
	//     return (array_key_exists($OptimFROGalgorithmNameLookup[$AlgorithID]) ? $OptimFROGalgorithmNameLookup[$AlgorithID] : false);
	// }
	...
	public static function OptimFROGencoderNameLookup($EncoderID) {
		...
		return $EncoderVersion.' ('.(array_key_exists($OptimFROGencoderSystemLookup[$EncoderSystemID]) ? $OptimFROGencoderSystemLookup[$EncoderSystemID] : 'undefined encoder type (0x'.dechex($EncoderSystemID).')').')';
	}
	...
	public static function OptimFROGcompressionLookup($CompressionID) {
		...
		return (array_key_exists($OptimFROGencoderModeLookup[$CompressionModeID]) ? $OptimFROGencoderModeLookup[$CompressionModeID] : 'undefined mode (0x'.str_pad(dechex($CompressionModeID), 2, '0', STR_PAD_LEFT).')');
	}
	...
	public static function OptimFROGspeedupLookup($CompressionID) {
		...
		return (array_key_exists($OptimFROGencoderSpeedupLookup[$CompressionSpeedupID]) ? $OptimFROGencoderSpeedupLookup[$CompressionSpeedupID] : 'undefined mode (0x'.dechex($CompressionSpeedupID));
	}
