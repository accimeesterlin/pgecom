...
class getid3_voc extends getid3_handler
{
	...
	public function VOCcompressionTypeLookup($index) {
		...
		return (array_key_exists($VOCcompressionTypeLookup[$index]) ? $VOCcompressionTypeLookup[$index] : 'Multi DAC ('.($index - 3).') channels');
	}
	...
	public function VOCwFormatLookup($index) {
		...
		return (array_key_exists($VOCwFormatLookup[$index]) ? $VOCwFormatLookup[$index] : false);
	}
