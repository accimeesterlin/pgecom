...
class getid3_wavpack extends getid3_handler
{
	...
	public function Analyze() {
		...
		while (true) {
			...
			if ($this->ftell() >= $info['avdataend']) {
				...
				array_key_exists($info['wavpack']['blockheader']['total_samples']) &&
				array_key_exists($info['wavpack']['blockheader']['block_samples']) &&
				($info['wavpack']['blockheader']['total_samples'] > 0) &&
				($info['wavpack']['blockheader']['block_samples'] > 0) &&
				(!array_key_exists($info['wavpack']['riff_trailer_size']) || ($info['wavpack']['riff_trailer_size'] <= 0)) &&
				((array_key_exists($info['wavpack']['config_flags']['md5_checksum']) && ($info['wavpack']['config_flags']['md5_checksum'] === false)) || !empty($info['md5_data_source']))) {
					break;
			}
			...
			if ($blockheader_magic != $magic) {
				...
				switch (array_key_exists($info['audio']['dataformat']) ? $info['audio']['dataformat'] : '') {
					case 'wavpack':
					case 'wvc':
						break;
					default:
			...
			while (!feof($this->getid3->fp) && ($this->ftell() < ($blockheader_offset + $blockheader_size + 8))) {
				...
				$found_blocks[$metablock['function_name']] = (array_key_exists($found_blocks[$metablock['function_name']]) ? $found_blocks[$metablock['function_name']] : 0) + 1; // cannot use getid3_lib::safe_inc without warnings(?)
				...
				if ($metablock['size'] > 0) {
					...
					switch ($metablock['function_id']) {
						...
						case 0x22: // ID_RIFF_TRAILER
							$metablockRIFFfooter = array_key_exists($metablockRIFFheader) ? $metablockRIFFheader : ''.$metablock['data'];
							getid3_lib::IncludeDependency(GETID3_INCLUDEPATH.'module.audio-video.riff.php', __FILE__, true);
						...
						case 0x25: // ID_CONFIG_BLOCK
							...
							} elseif (array_key_exists($info['audio']['encoder_options'])) {
	...
	public function WavPackMetablockNameLookup(&$id) {
		...
		return (array_key_exists($WavPackMetablockNameLookup[$id]) ? $WavPackMetablockNameLookup[$id] : '');
	}
