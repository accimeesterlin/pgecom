...
if (array_key_exists($_REQUEST['deletefile']) && GETID3_DEMO_BROWSE_ALLOW_DELETE_LINK) {
	...
	if (array_key_exists($_REQUEST['noalert'])) {
...
if (array_key_exists($_REQUEST['filename'])) {
	...
	$AutoGetHashes = (bool) (array_key_exists($ThisFileInfo['filesize']) && ($ThisFileInfo['filesize'] > 0) && ($ThisFileInfo['filesize'] < (50 * 1048576))); // auto-get md5_data, md5_file, sha1_data, sha1_file if filesize < 50MB, and NOT zero (which may indicate a file>2GB)
	...
	$listdirectory = (array_key_exists($_REQUEST['listdirectory']) ? $_REQUEST['listdirectory'] : '.');
	...
	if ($handle = opendir($listdirectory)) {
		...
		while ($file = readdir($handle)) {
			...
			if ($TargetObjectType == 'dir') {
				...
				$getID3->setOption(array('option_md5_data' => (array_key_exists($_REQUEST['ShowMD5']) && GETID3_DEMO_BROWSE_ALLOW_MD5_LINK)));
				$fileinformation = $getID3->analyze($currentfilename);

				$getID3->CopyTagsToComments($fileinformation);

				$TotalScannedFilesize += (array_key_exists($fileinformation['filesize']) ? $fileinformation['filesize'] : 0);

				if (array_key_exists($_REQUEST['ShowMD5']) && GETID3_DEMO_BROWSE_ALLOW_MD5_LINK) {
				...
				if (!empty($fileinformation['fileformat'])) {
					...
					$TotalScannedPlaytime += (array_key_exists($fileinformation['playtime_seconds']) ? $fileinformation['playtime_seconds'] : 0);
					$TotalScannedBitrate  += (array_key_exists($fileinformation['bitrate'])          ? $fileinformation['bitrate']          : 0);
				...
				if (array_key_exists($fileinformation['playtime_seconds']) && ($fileinformation['playtime_seconds'] > 0)) {
					$TotalScannedPlaytimeFiles++;
				}
				if (array_key_exists($fileinformation['bitrate']) && ($fileinformation['bitrate'] > 0)) {
		...
		foreach ($DirectoryContents as $dirname => $val) {
			if (array_key_exists($DirectoryContents[$dirname]['dir']) && is_array($DirectoryContents[$dirname]['dir'])) {
			...
			echo '<th>Title</th>';
			if (array_key_exists($_REQUEST['ShowMD5']) && GETID3_DEMO_BROWSE_ALLOW_MD5_LINK) {
				echo '<th>MD5&nbsp;File (File) (<a href="'.htmlentities($_SERVER['PHP_SELF'].'?listdirectory='.rawurlencode(array_key_exists($_REQUEST['listdirectory']) ? $_REQUEST['listdirectory'] : '.'), ENT_QUOTES | ENT_SUBSTITUTE, $PageEncoding).'">disable</a>)</th>';
				echo '<th>MD5&nbsp;Data (File) (<a href="'.htmlentities($_SERVER['PHP_SELF'].'?listdirectory='.rawurlencode(array_key_exists($_REQUEST['listdirectory']) ? $_REQUEST['listdirectory'] : '.'), ENT_QUOTES | ENT_SUBSTITUTE, $PageEncoding).'">disable</a>)</th>';
				echo '<th>MD5&nbsp;Data (Source) (<a href="'.htmlentities($_SERVER['PHP_SELF'].'?listdirectory='.rawurlencode(array_key_exists($_REQUEST['listdirectory']) ? $_REQUEST['listdirectory'] : '.'), ENT_QUOTES | ENT_SUBSTITUTE, $PageEncoding).'">disable</a>)</th>';
			} else {
				echo '<th colspan="3">MD5&nbsp;Data'.(GETID3_DEMO_BROWSE_ALLOW_MD5_LINK ?' (<a href="'.htmlentities($_SERVER['PHP_SELF'].'?listdirectory='.rawurlencode(array_key_exists($_REQUEST['listdirectory']) ? $_REQUEST['listdirectory'] : '.').'&ShowMD5=1', ENT_QUOTES | ENT_SUBSTITUTE, $PageEncoding).'">enable</a>)' : '').'</th>';
			}
			echo '<th>Tags</th>';
			...
			if (array_key_exists($DirectoryContents[$dirname]['known']) && is_array($DirectoryContents[$dirname]['known'])) {
				...
				foreach ($DirectoryContents[$dirname]['known'] as $filename => $fileinfo) {
					...
					echo '<td align="right">&nbsp;'.(array_key_exists($fileinfo['playtime_string']) ? $fileinfo['playtime_string'] : '-').'</td>';
					echo '<td align="right">&nbsp;'.(array_key_exists($fileinfo['bitrate']) ? BitrateText($fileinfo['bitrate'] / 1000, 0, ((array_key_exists($fileinfo['audio']['bitrate_mode']) && ($fileinfo['audio']['bitrate_mode'] == 'vbr')) ? true : false)) : '-').'</td>';
					echo '<td align="left">&nbsp;'.(array_key_exists($fileinfo['comments_html']['artist']) ? implode('<br>', $fileinfo['comments_html']['artist']) : ((array_key_exists($fileinfo['video']['resolution_x']) && array_key_exists($fileinfo['video']['resolution_y'])) ? $fileinfo['video']['resolution_x'].'x'.$fileinfo['video']['resolution_y'] : '')).'</td>';
					echo '<td align="left">&nbsp;'.(array_key_exists($fileinfo['comments_html']['title'])  ? implode('<br>', $fileinfo['comments_html']['title'])  :  (array_key_exists($fileinfo['video']['frame_rate'])                                                 ? number_format($fileinfo['video']['frame_rate'], 3).'fps'                  : '')).'</td>';
					if (array_key_exists($_REQUEST['ShowMD5']) && GETID3_DEMO_BROWSE_ALLOW_MD5_LINK) {
						echo '<td align="left"><tt>'.(array_key_exists($fileinfo['md5_file'])        ? $fileinfo['md5_file']        : '&nbsp;').'</tt></td>';
						echo '<td align="left"><tt>'.(array_key_exists($fileinfo['md5_data'])        ? $fileinfo['md5_data']        : '&nbsp;').'</tt></td>';
						echo '<td align="left"><tt>'.(array_key_exists($fileinfo['md5_data_source']) ? $fileinfo['md5_data_source'] : '&nbsp;').'</tt></td>';
					} else {
					...
					if (GETID3_DEMO_BROWSE_ALLOW_EDIT_LINK) {
						echo '<td align="left">&nbsp;';
						$fileinfo['fileformat'] = (array_key_exists($fileinfo['fileformat']) ? $fileinfo['fileformat'] : '');
						switch ($fileinfo['fileformat']) {
							...
							case 'real':
								...
								break;
							case 'ogg':
								if (array_key_exists($fileinfo['audio']['dataformat']) && ($fileinfo['audio']['dataformat'] == 'vorbis')) {
									echo '<a href="'.htmlentities($writescriptfilename.'?Filename='.urlencode($dirname.$filename), ENT_QUOTES | ENT_SUBSTITUTE, $PageEncoding).'" title="Edit tags">edit&nbsp;tags</a>';
								}
								break;
							default:
			...
			if (array_key_exists($DirectoryContents[$dirname]['other']) && is_array($DirectoryContents[$dirname]['other'])) {
				...
				foreach ($DirectoryContents[$dirname]['other'] as $filename => $fileinfo) {
					...
					echo '<td align="right">&nbsp;'.(array_key_exists($fileinfo['filesize']) ? number_format($fileinfo['filesize']) : '-').'</td>';
					echo '<td align="right">&nbsp;'.NiceDisplayFiletypeFormat($fileinfo).'</td>';
					echo '<td align="right">&nbsp;'.(array_key_exists($fileinfo['playtime_string']) ? $fileinfo['playtime_string'] : '-').'</td>';
					echo '<td align="right">&nbsp;'.(array_key_exists($fileinfo['bitrate']) ? BitrateText($fileinfo['bitrate'] / 1000) : '-').'</td>';
	...
	switch (gettype($variable)) {
		case 'array':
			...
			foreach ($variable as $key => $value) {
				...
				if (($key == 'data') && array_key_exists($variable['image_mime'])) {
					$imageinfo = array();
