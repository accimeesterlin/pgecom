		...
		switch (gettype($variable)) {
			case 'array':
				...
				foreach ($variable as $key => $value) {
					...
					if (($key == 'data') && array_key_exists($variable['image_mime']) && array_key_exists($variable['dataoffset'])) {
	...
	function array_join_merge($arr1, $arr2) {
		...
		if (is_array($arr1) && is_array($arr2)) {
			...
			if (is_hash($arr1) && is_hash($arr2)) {
				...
				foreach ($keys as $key) {
					$arr1[$key] = (array_key_exists($arr1[$key]) ? $arr1[$key] : '');
					$arr2[$key] = (array_key_exists($arr2[$key]) ? $arr2[$key] : '');
...
if (!function_exists('array_merge_clobber')) {
	function array_merge_clobber($array1, $array2) {
		...
		foreach ($array2 as $key => $val) {
			if (is_array($val) && array_key_exists($newarray[$key]) && is_array($newarray[$key])) {
...
if (!function_exists('array_merge_noclobber')) {
	function array_merge_noclobber($array1, $array2) {
		...
		foreach ($array2 as $key => $val) {
			...
			} elseif (!array_key_exists($newarray[$key])) {
				$newarray[$key] = $val;
			}
		}
		return $newarray;
...
if (!function_exists('ID3v1matchesID3v2')) {
	function ID3v1matchesID3v2($id3v1, $id3v2) {
		...
		foreach ($requiredindices as $requiredindex) {
			if (!array_key_exists($id3v1["$requiredindex"])) {
				$id3v1["$requiredindex"] = '';
			}
			if (!array_key_exists($id3v2["$requiredindex"])) {
				$id3v2["$requiredindex"] = '';
			}
		}
		...
		if (array_key_exists($id3v1['track_number'])) {
			if (!array_key_exists($id3v1['track_number']) || (trim($id3v1['track_number']) != trim($id3v2['track_number']))) {
				return false;
			}
...
if (!function_exists('image_type_to_mime_type')) {
	function image_type_to_mime_type($imagetypeid) {
		...
		return (array_key_exists($image_type_to_mime_type[$imagetypeid]) ? $image_type_to_mime_type[$imagetypeid] : 'application/octet-stream');
	}
}
...
if (!function_exists('safe_parse_url')) {
	function safe_parse_url($url) {
		...
		$errormessage = ob_get_contents();
		ob_end_clean();
		$parts['scheme'] = (array_key_exists($parts['scheme']) ? $parts['scheme'] : '');
		$parts['host']   = (array_key_exists($parts['host'])   ? $parts['host']   : '');
		$parts['user']   = (array_key_exists($parts['user'])   ? $parts['user']   : '');
		$parts['pass']   = (array_key_exists($parts['pass'])   ? $parts['pass']   : '');
		$parts['path']   = (array_key_exists($parts['path'])   ? $parts['path']   : '');
		$parts['query']  = (array_key_exists($parts['query'])  ? $parts['query']  : '');
		return $parts;
	}
}
...
echo '<input type="text" name="HeaderHexBytes" value="'.htmlentities(array_key_exists($_POST['HeaderHexBytes']) ? strtoupper($_POST['HeaderHexBytes']) : '', ENT_QUOTES).'" size="11" maxlength="11">';
...
foreach ($MPEGgenerateValues as $name => $dataarray) {
	...
	foreach ($dataarray as $key => $value) {
		echo '<option'.((array_key_exists($_POST["$name"]) && ($_POST["$name"] == $value)) ? ' SELECTED' : '').'>'.$value.'</option>';
	}
	echo '</select></td></tr>';
}

if (array_key_exists($_POST['bitrate'])) {
...
if (array_key_exists($_POST['Analyze']) && $_POST['HeaderHexBytes']) {
...
} elseif (array_key_exists($_POST['Generate'])) {
	...
	if (array_key_exists($MPEGaudioBitrateLookup[$_POST['version']][$_POST['layer']][$_POST['bitrate']])) {
	...
	if (array_key_exists($MPEGaudioFrequencyLookup[$_POST['version']][$_POST['frequency']])) {
	...
	if ($_POST['channelmode'] != 'joint stereo') {
		$headerbitstream .= '00';
	} elseif (array_key_exists($MPEGaudioModeExtensionLookup[$_POST['layer']][$_POST['modeextension']])) {
	...
	if (array_key_exists($MPEGaudioEmphasisLookup[$_POST['emphasis']])) {
...
function MPEGaudioVersionLookup($rawversion) {
	...
	return (array_key_exists($MPEGaudioVersionLookup["$rawversion"]) ? $MPEGaudioVersionLookup["$rawversion"] : FALSE);
}

function MPEGaudioLayerLookup($rawlayer) {
	$MPEGaudioLayerLookup = array(FALSE, 'III', 'II', 'I');
	return (array_key_exists($MPEGaudioLayerLookup["$rawlayer"]) ? $MPEGaudioLayerLookup["$rawlayer"] : FALSE);
}
...
function MPEGaudioBitrateLookup($version, $layer, $rawbitrate) {
	...
	return (array_key_exists($MPEGaudioBitrateLookup["$version"]["$layer"]["$rawbitrate"]) ? $MPEGaudioBitrateLookup["$version"]["$layer"]["$rawbitrate"] : FALSE);
}
...
function MPEGaudioFrequencyLookup($version, $rawfrequency) {
	...
	return (array_key_exists($MPEGaudioFrequencyLookup["$version"]["$rawfrequency"]) ? $MPEGaudioFrequencyLookup["$version"]["$rawfrequency"] : FALSE);
}

function MPEGaudioChannelModeLookup($rawchannelmode) {
	$MPEGaudioChannelModeLookup = array('stereo', 'joint stereo', 'dual channel', 'mono');
	return (array_key_exists($MPEGaudioChannelModeLookup["$rawchannelmode"]) ? $MPEGaudioChannelModeLookup["$rawchannelmode"] : FALSE);
}
...
function MPEGaudioModeExtensionLookup($layer, $rawmodeextension) {
	...
	return (array_key_exists($MPEGaudioModeExtensionLookup["$layer"]["$rawmodeextension"]) ? $MPEGaudioModeExtensionLookup["$layer"]["$rawmodeextension"] : FALSE);
}

function MPEGaudioEmphasisLookup($rawemphasis) {
	$MPEGaudioEmphasisLookup = array('none', '50/15ms', FALSE, 'CCIT J.17');
	return (array_key_exists($MPEGaudioEmphasisLookup["$rawemphasis"]) ? $MPEGaudioEmphasisLookup["$rawemphasis"] : FALSE);
}
...
function getMP3headerFilepointer(&$fd, &$ThisFileInfo) {
	...
	if (array_key_exists($ThisFileInfo['mpeg']['audio']['bitrate_mode'])) {
		$ThisFileInfo['audio']['bitrate_mode'] = strtolower($ThisFileInfo['mpeg']['audio']['bitrate_mode']);
	}

	if (((array_key_exists($ThisFileInfo['id3v2']) && ($ThisFileInfo['avdataoffset'] > $ThisFileInfo['id3v2']['headerlength'])) || (!array_key_exists($ThisFileInfo['id3v2']) && ($ThisFileInfo['avdataoffset'] > 0)))) {

		$ThisFileInfo['warning'] .= "\n".'Unknown data before synch ';
		if (array_key_exists($ThisFileInfo['id3v2']['headerlength'])) {
	...
	if (array_key_exists($ThisFileInfo['mpeg']['audio']['layer']) && ($ThisFileInfo['mpeg']['audio']['layer'] == 'II')) {
		$ThisFileInfo['audio']['dataformat'] = 'mp2';
	} elseif (array_key_exists($ThisFileInfo['mpeg']['audio']['layer']) && ($ThisFileInfo['mpeg']['audio']['layer'] == 'I')) {
	...
	// Calculate playtime
	if (!array_key_exists($ThisFileInfo['playtime_seconds']) && array_key_exists($ThisFileInfo['audio']['bitrate']) && ($ThisFileInfo['audio']['bitrate'] > 0)) {
		$ThisFileInfo['playtime_seconds'] = ($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']) * 8 / $ThisFileInfo['audio']['bitrate'];
	}

	if (array_key_exists($ThisFileInfo['mpeg']['audio']['LAME'])) {
		$ThisFileInfo['audio']['codec'] = 'LAME';
...
function decodeMPEGaudioHeader($fd, $offset, &$ThisFileInfo, $recursivesearch=true, $ScanAsCBR=false, $FastMPEGheaderScan=false) {
	...
	if ($ThisFileInfo['mpeg']['audio']['bitrate'] != 'free') {
		...
		if (array_key_exists($ThisFileInfo['mpeg']['audio']['framelength'])) {
	...
	if (($ThisFileInfo['mpeg']['audio']['bitrate_mode'] == 'vbr') && array_key_exists($ThisFileInfo['mpeg']['audio']['VBR_frames']) && ($ThisFileInfo['mpeg']['audio']['VBR_frames'] > 1)) {
	...
	for ($i = 0; $i < MPEG_VALID_CHECK_FRAMES; $i++) {
		...
		if (decodeMPEGaudioHeader($fd, $nextframetestoffset, $nextframetestarray, false)) {
			if ($ScanAsCBR) {
				...
				if (!array_key_exists($nextframetestarray['mpeg']['audio']['bitrate']) || !array_key_exists($ThisFileInfo['mpeg']['audio']['bitrate']) || ($nextframetestarray['mpeg']['audio']['bitrate'] != $ThisFileInfo['mpeg']['audio']['bitrate'])) {
					return false;
				}
			}
	...
	while (true) {
		...
		if (($SynchSeekOffset > $header_len) && (($avdataoffset + $SynchSeekOffset)  < $ThisFileInfo['avdataend']) && !feof($fd)) {
			...
			if ($SynchSeekOffset > 131072) {
				...
				if (array_key_exists($ThisFileInfo['audio']['bitrate'])) {
					unset($ThisFileInfo['audio']['bitrate']);
				}
				if (array_key_exists($ThisFileInfo['mpeg']['audio'])) {
					unset($ThisFileInfo['mpeg']['audio']);
				}
				if (array_key_exists($ThisFileInfo['mpeg']) && (!is_array($ThisFileInfo['mpeg']) || (count($ThisFileInfo['mpeg']) == 0))) {
		...
		if (($header[$SynchSeekOffset] == CONST_FF) && ($header[($SynchSeekOffset + 1)] > CONST_E0)) { // synch detected
			...
			if (!array_key_exists($FirstFrameThisfileInfo) && !array_key_exists($ThisFileInfo['mpeg']['audio'])) {
			...
			if (decodeMPEGaudioHeader($fd, $avdataoffset + $SynchSeekOffset, $dummy, true)) {
				...
				if (array_key_exists($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode']) && ($FirstFrameThisfileInfo['mpeg']['audio']['bitrate_mode'] == 'vbr')) {
				...
				if (array_key_exists($ThisFileInfo['mpeg']['audio']['bitrate_mode']) && ($ThisFileInfo['mpeg']['audio']['bitrate_mode'] == 'vbr') && !array_key_exists($ThisFileInfo['mpeg']['audio']['VBR_method'])) {
...
//							return false;
		...
		if (($avdataoffset + $SynchSeekOffset) >= $ThisFileInfo['avdataend']) {
			...
			if (empty($ThisFileInfo['mpeg']['audio'])) {
				...
				if (array_key_exists($ThisFileInfo['mpeg']) && (!is_array($ThisFileInfo['mpeg']) || empty($ThisFileInfo['mpeg']))) {
					unset($ThisFileInfo['mpeg']);
				}
				return false;
