...
} elseif (!array_key_exists($_REQUEST['m3u']) && !array_key_exists($_REQUEST['m3uartist']) && !array_key_exists($_REQUEST['m3utitle'])) {
...
function AcceptableExtensions($fileformat, $audio_dataformat='', $video_dataformat='') {
	...
	if (!empty($video_dataformat)) {
		return (array_key_exists($AcceptableExtensionsVideo[$fileformat][$video_dataformat]) ? $AcceptableExtensionsVideo[$fileformat][$video_dataformat] : array());
	} else {
		return (array_key_exists($AcceptableExtensionsAudio[$fileformat][$audio_dataformat]) ? $AcceptableExtensionsAudio[$fileformat][$audio_dataformat] : array());
	}
}
...
if (!array_key_exists($ExistingTableFields['encoder_options'])) { // Added in 1.7.0b2
...
if (array_key_exists($ExistingTableFields['track']) && ($ExistingTableFields['track']['Type'] != 'varchar(7)')) { // Changed in 1.7.0b2
...
if (!array_key_exists($ExistingTableFields['track_volume'])) { // Added in 1.7.0b5
...
if (!array_key_exists($ExistingTableFields['remix'])) { // Added in 1.7.3b1
...
if (array_key_exists($ExistingTableFields['comments_all']) && ($ExistingTableFields['comments_all']['Type'] != 'longblob')) { // Changed to "longtext" in 1.9.0, changed to "longblob" in 1.9.20-202010140821
...
if (!empty($_REQUEST['scan']) || !empty($_REQUEST['newscan']) || !empty($_REQUEST['rescanerrors'])) {
	...
	foreach ($FilesInDir as $filename) {
		...
		if (empty($ThisFileInfo['fileformat'])) {
			...
			if (!empty($_REQUEST['rescanerrors'])) {
				...
				$SQLquery .= ' WHERE (`filename` = "'. mysqli_real_escape_string($con, array_key_exists($ThisFileInfo['filenamepath']) ? $ThisFileInfo['filenamepath'] : '').'")';
	...
	if (!empty($_REQUEST['m3u'])) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if (array_key_exists($CommentArray['encoded_by'][0])) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$CommentArray = unserialize($row['comments_id3v2']);
			if (array_key_exists($CommentArray['encoded_by'][0])) {
				if (array_key_exists($EncodedBy[$row['encoder_options']][$CommentArray['encoded_by'][0]])) {
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		if (array_key_exists($BitrateDistribution[$this_bitrate])) {
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		if (array_key_exists($Comments['ape']['tracknumber'])) {
			$Comments['ape']['track_number'] = $Comments['ape']['tracknumber'];
			unset($Comments['ape']['tracknumber']);
		}
		if (array_key_exists($Comments['ape']['track'])) {
				...
				foreach ($FieldsToCompare as $fieldname) {
					...
					if ($tagtype == 'id3v1') {
						...
						if (($fieldname == 'genre') && !empty($Comments['all'][$fieldname][0]) && !getid3_id3v1::LookupGenreID($Comments['all'][$fieldname][0])) {
							...
							$tagvalues .= $fieldname.' = '.(array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : '')."\n";

						} elseif ($fieldname == 'comment') {

							if (array_key_exists($Comments[$tagtype][$fieldname][0]) && array_key_exists($Comments['all'][$fieldname][0]) && (rtrim(substr($Comments[$tagtype][$fieldname][0], 0, 28)) != rtrim(substr($Comments['all'][$fieldname][0], 0, 28)))) {
							...
							$trackA = (array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : '');
							$trackB = (array_key_exists($Comments['all'][$fieldname][0])    ? $Comments['all'][$fieldname][0]    : '');
						...
						} elseif ((array_key_exists($Comments[$tagtype][$fieldname][0]) ? rtrim(substr($Comments[$tagtype][$fieldname][0], 0, 30)) : '') != (array_key_exists($Comments['all'][$fieldname][0]) ? rtrim(substr($Comments['all'][$fieldname][0], 0, 30)) : '')) {

							$tagvalues .= $fieldname.' = [['.(array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : '').']]'."\n";
							if (strtolower(RemoveAccents(trim(substr((array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : ''), 0, 30)))) == strtolower(RemoveAccents(trim(substr((array_key_exists($Comments['all'][$fieldname][0]) ? $Comments['all'][$fieldname][0] : ''), 0, 30))))) {
								$SemiMatched[$tagtype] = true;
							} else {
								$Mismatched[$tagtype]  = true;
							}
							...
							$tagvalues .= $fieldname.' = '.(array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : '')."\n";
							if (array_key_exists($Comments[$tagtype][$fieldname][0]) && (strlen(trim($Comments[$tagtype][$fieldname][0])) > 0)) {
								$EmptyTags[$tagtype] = false;
							}
						...
						if (((array_key_exists($Comments['ape']['date'][0]) ? $Comments['ape']['date'][0] : '') != (array_key_exists($Comments['all']['year'][0]) ? $Comments['all']['year'][0] : '')) && ((array_key_exists($Comments['ape']['year'][0]) ? $Comments['ape']['year'][0] : '') != (array_key_exists($Comments['all']['year'][0]) ? $Comments['all']['year'][0] : ''))) {

							$tagvalues .= $fieldname.' = [['.(array_key_exists($Comments['ape']['date'][0]) ? $Comments['ape']['date'][0] : '').']]'."\n";
							$Mismatched[$tagtype]  = true;
							$FileOK = false;
							if (array_key_exists($Comments['ape']['date'][0]) && (strlen(trim($Comments['ape']['date'][0])) > 0)) {
								$EmptyTags[$tagtype] = false;
							}
					...
					} elseif ((array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : '') != (array_key_exists($Comments['all'][$fieldname][0]) ? $Comments['all'][$fieldname][0] : '')) {
						...
						if (!$skiptracknumberfield) {
							$tagvalues .= $fieldname.' = [['.(array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : '').']]'."\n";
							$tagA = (array_key_exists($Comments[$tagtype][$fieldname][0]) ? $Comments[$tagtype][$fieldname][0] : '');
							$tagB = (array_key_exists($Comments['all'][$fieldname][0])    ? $Comments['all'][$fieldname][0]    : '');
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		for ($i = 0; $i < $PatternLength; $i++) {
			if (array_key_exists($patterns[$Pattern[$i]])) {
	...
	if (array_key_exists($_REQUEST['showtagfiles'])) {
	...
	} elseif (!array_key_exists($_REQUEST['m3u'])) {
	...
	if (!array_key_exists($_REQUEST['m3u'])) {
	...
	if (array_key_exists($_REQUEST['m3uartist']) && array_key_exists($_REQUEST['m3utitle'])) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if (!empty($ThisFileInfo['tags'])) {
				...
				if ($Album && $Track) {
					...
					$WriteID3v1_title   = (array_key_exists($ThisFileInfo['comments']['title'][0])   ? $ThisFileInfo['comments']['title'][0]   : '');
					$WriteID3v1_artist  = (array_key_exists($ThisFileInfo['comments']['artist'][0])  ? $ThisFileInfo['comments']['artist'][0]  : '');
					$WriteID3v1_year    = (array_key_exists($ThisFileInfo['comments']['year'][0])    ? $ThisFileInfo['comments']['year'][0]    : '');
					$WriteID3v1_comment = (array_key_exists($ThisFileInfo['comments']['comment'][0]) ? $ThisFileInfo['comments']['comment'][0] : '');
					$WriteID3v1_genreid = (array_key_exists($ThisFileInfo['comments']['genreid'][0]) ? $ThisFileInfo['comments']['genreid'][0] : '');
...
} elseif (array_key_exists($_REQUEST['genredistribution'])) {
