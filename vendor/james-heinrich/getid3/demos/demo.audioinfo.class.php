...
class AudioInfo {
	...
	*/
	...
	public function Info($file) {
		...
		// Init wrapper object
		$this->result = array();
		$this->result['format_name']     = (array_key_exists($this->info['fileformat']) ? $this->info['fileformat'] : '').'/'.(array_key_exists($this->info['audio']['dataformat']) ? $this->info['audio']['dataformat'] : '').(array_key_exists($this->info['video']['dataformat']) ? '/'.$this->info['video']['dataformat'] : '');
		$this->result['encoder_version'] = (array_key_exists($this->info['audio']['encoder'])         ? $this->info['audio']['encoder']         : '');
		$this->result['encoder_options'] = (array_key_exists($this->info['audio']['encoder_options']) ? $this->info['audio']['encoder_options'] : '');
		$this->result['bitrate_mode']    = (array_key_exists($this->info['audio']['bitrate_mode'])    ? $this->info['audio']['bitrate_mode']    : '');
		$this->result['channels']        = (array_key_exists($this->info['audio']['channels'])        ? $this->info['audio']['channels']        : '');
		$this->result['sample_rate']     = (array_key_exists($this->info['audio']['sample_rate'])     ? $this->info['audio']['sample_rate']     : '');
		$this->result['bits_per_sample'] = (array_key_exists($this->info['audio']['bits_per_sample']) ? $this->info['audio']['bits_per_sample'] : '');
		$this->result['playing_time']    = (array_key_exists($this->info['playtime_seconds'])         ? $this->info['playtime_seconds']         : '');
		$this->result['avg_bit_rate']    = (array_key_exists($this->info['audio']['bitrate'])         ? $this->info['audio']['bitrate']         : '');
		$this->result['tags']            = (array_key_exists($this->info['tags'])                     ? $this->info['tags']                     : '');
		$this->result['comments']        = (array_key_exists($this->info['comments'])                 ? $this->info['comments']                 : '');
		$this->result['warning']         = (array_key_exists($this->info['warning'])                  ? $this->info['warning']                  : '');
		$this->result['md5']             = (array_key_exists($this->info['md5_data'])                 ? $this->info['md5_data']                 : '');

		// Post getID3() data handling based on file format
		$method = (array_key_exists($this->info['fileformat']) ? $this->info['fileformat'] : '').'Info';
