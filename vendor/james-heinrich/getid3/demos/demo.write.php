...
$Filename = (array_key_exists($_REQUEST['Filename']) ? $_REQUEST['Filename'] : '');



if (array_key_exists($_POST['WriteTags'])) {

	$TagFormatsToWrite = (array_key_exists($_POST['TagFormatsToWrite']) ? $_POST['TagFormatsToWrite'] : array());
...
if (!empty($Filename)) {
	...
	if (file_exists($Filename)) {
		...
		foreach ($ValidTagTypes as $ValidTagType) {
			...
			if (count($ValidTagTypes) == 1) {
				...
				switch ($ValidTagType) {
					...
					case 'id3v2.3':
					case 'id3v2.4':
						if (array_key_exists($OldThisFileInfo['tags']['id3v2'])) {
							echo ' checked="checked"';
						}
						break;

					default:
						if (array_key_exists($OldThisFileInfo['tags'][$ValidTagType])) {
							echo ' checked="checked"';
						}
						break;
				}
		...
		echo '<tr><td align="right"><b>Comment</b></td><td><textarea cols="30" rows="3" name="Comment" wrap="virtual">'.((array_key_exists($OldThisFileInfo['comments']['comment']) && is_array($OldThisFileInfo['comments']['comment'])) ? implode("\n", $OldThisFileInfo['comments']['comment']) : '').'</textarea></td></tr>';
