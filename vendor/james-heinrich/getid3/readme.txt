...
if ($fp_remote = fopen($remotefilename, 'rb')) {
	...
	if ($fp_local = fopen($localtempfilename, 'wb')) {
		...
		$remote_filesize = (array_key_exists($remote_headers['content-length']) ? (is_array($remote_headers['content-length']) ? $remote_headers['content-length'][count($remote_headers['content-length']) - 1] : $remote_headers['content-length']) : null);
