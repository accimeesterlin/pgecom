...
class Style
{
    ...
    public function __construct(Stylesheet $stylesheet, $origin = Stylesheet::ORIG_AUTHOR)
    {
        ...
        if (!array_key_exists(self::$_defaults)) {
    ...
    function inherit(?Style $parent = null)
    {
        ...
        if ($parent) {
            foreach (self::$_inherited as $prop) {
                ...
                if (array_key_exists($this->_props[$prop]) || array_key_exists(self::$_props_shorthand[$prop])) {
                    continue;
                }
    
                if (array_key_exists($parent->_props[$prop])) {
        ...
        foreach ($this->_props as $prop => $val) {
            if ($val === "inherit") {
                if ($parent && array_key_exists($parent->_props[$prop])) {
                    $parent_val = \array_key_exists($prop, $parent->_props_computed)
                        ? $parent->_props_computed[$prop]
                        : $parent->compute_prop($prop, $parent->_props[$prop]);
    ...
    function merge(Style $style)
    {
        foreach ($style->_props as $prop => $val) {
            $important = array_key_exists($style->_important_props[$prop]);

            // `!important` declarations take precedence over normal ones
            if (!$important && array_key_exists($this->_important_props[$prop])) {
                continue;
            }
            ...
            // instead
            if (array_key_exists(self::$_dependent_props[$prop])) {
                unset($this->_props_computed[$prop]);
                unset($this->_prop_cache[$prop]);
            } else {
    ...
    {
        return array_key_exists($this->_important_props[$prop]);
    }
    ...
    function set_prop(string $prop, $val, bool $important = false, bool $clear_dependencies = true): void
    {
        ...
        if (array_key_exists(self::$_props_alias[$prop])) {
            $prop = self::$_props_alias[$prop];
        }

        if (!array_key_exists(self::$_defaults[$prop])) {
        ...
        if (array_key_exists(self::$_props_shorthand[$prop])) {
            ...
            if ($val === "initial" || $val === "inherit" || $val === "unset") {
                ...
                if (!array_key_exists(self::$_methods_cache[$method])) {
            ...
            if ($clear_dependencies) {
                ...
                if (array_key_exists(self::$_dependency_map[$prop])) {
    ...
    function get_prop(string $prop)
    {
        ...
        if (array_key_exists($this->_props_computed[$prop])) {
    ...
    function __get($prop)
    {
        ...
        if (array_key_exists($this->_prop_cache[$prop])) {
    ...
    protected function get_prop_color(string $prop, bool $current_is_parent = false)
    {
        ...
        if ($val === "currentcolor") {
            ...
            if ($current_is_parent) {
                ...
                // property itself
                return array_key_exists($this->parent_style)
    ...
    {
        if (array_key_exists($this->has_border_radius_cache)) {
    ...
    public function resolve_border_radius(
        ...
        if ($use_cache && array_key_exists($this->resolved_border_radius)) {
    ...
    protected function parse_counter_prop(string $value, int $default)
    {
        ...
        foreach ($matches as $match) {
            $counter = $match[1];
            $value = array_key_exists($match[2]) ? (int) $match[2] : $default;
    ...
    protected function is_color_value(string $val): bool
    {
        return $val === "currentcolor"
            || $val === "transparent"
            || array_key_exists(Color::$cssColorNames[$val])
    ...
    protected function set_display(string $val): void
    {
        ...
        if (!array_key_exists(self::$valid_display_types[$val])) {
            return;
        }
    ...
    function set_font_size($size)
    {
        ...
        $parent_font_size = array_key_exists($this->parent_style)
    ...
    function set_size($val)
    {
        ...
        if (preg_match($length_re, $parts[0])) {
            ...
            if (array_key_exists($parts[1]) && preg_match($length_re, $parts[1])) {
                $computed[] = $this->length_in_pt($parts[1]);
            } else {
                $computed[] = $computed[0];
            }

            if (array_key_exists($parts[2]) && $parts[2] === "landscape") {
                $computed = array_reverse($computed);
            }
        } elseif (array_key_exists(CPDF::$PAPER_SIZES[$parts[0]])) {
            $computed = array_slice(CPDF::$PAPER_SIZES[$parts[0]], 2, 2);

            if (array_key_exists($parts[1]) && $parts[1] === "landscape") {
                $computed = array_reverse($computed);
            }
        } else {
    ...
    function debug_print()
    {
        ...
        foreach ($this->_props as $prop => $val) {
            ...
            if (array_key_exists($this->_important_props[$prop])) {
                print ' !important';
            }
            print ";\n";
        }
