...
class Stylesheet
{
    ...
    function __construct(Dompdf $dompdf)
    {
        ...
        $this->_loaded_files = [];
        $script = __FILE__;
        if (array_key_exists($_SERVER["SCRIPT_FILENAME"])) {
    ...
    {
        if (!array_key_exists($this->_styles[$key])) {
            $this->_styles[$key] = [];
        }
    ...
    function load_css_file($file, $origin = self::ORIG_AUTHOR)
    {
        ...
        if (array_key_exists($this->_loaded_files[$file])) {
            return;
        }
        ...
        if (strpos($file, "data:") === 0) {
            ...
            if (array_key_exists($http_response_header) && !$this->_dompdf->getQuirksmode()) {
        ...
        while ($i < $len) {
            ...
            switch ($s) {
                ...
                case ":":
                    ...
                    if (($i2 < 0 || !array_key_exists($selector[$i2]) || (in_array($selector[$i2], $delimiters) && $selector[$i2] != ":")) && substr($query, -1) != "*") {
                        $query .= "*";
                    }
...
//    }
    ...
    protected function _selector_an_plus_b($expr, $last = false)
    {
        ...
        $a = ((array_key_exists($matches["a"]) && $matches["a"] !== "") ? intval($matches["a"]) : 1);
        $b = ((array_key_exists($matches["b"]) && $matches["b"] !== "") ? intval($matches["b"]) : 0);
