...
class Cellmap
{
    ...
    {
        if (!array_key_exists($this->_columns[$i])) {
            $this->_columns[$i] = [
                "x"          => 0,
                "min-width"  => 0,
                "max-width"  => 0,
    ...
    {
        if (!array_key_exists($this->_rows[$j])) {
            $this->_rows[$j] = [
                "y"            => 0,
                "first-column" => 0,
                "height"       => null,
    ...
    {
        if (!array_key_exists($this->_borders[$i][$j][$h_v])) {
    ...
    public function get_spanned_cells(Frame $frame)
    {
        ...
        if (array_key_exists($this->_frames[$key])) {
    ...
    public function frame_exists_in_cellmap(Frame $frame)
    {
        ...
        return array_key_exists($this->_frames[$key]);
    }
    ...
    public function get_frame_position(Frame $frame)
    {
        ...
        if (!array_key_exists($this->_columns[$col])) {
        ...
        if (!array_key_exists($this->_rows[$row])) {
    ...
    public function get_frame_height(Frame $frame)
    {
        ...
        $h = 0;
        foreach ($rows as $i) {
            if (!array_key_exists($this->_rows[$i])) {
    ...
    protected function _resolve_border($i, $j, $h_v, $border_spec)
    {
        ...
        if ($n_style === "hidden" || $n_width > $o_width
            || ($o_width == $n_width
                && array_key_exists(self::$_BORDER_STYLE_SCORE[$n_style])
                && array_key_exists(self::$_BORDER_STYLE_SCORE[$o_style])
    ...
    public function add_frame(Frame $frame): void
    {
        ...
        $ac = $this->__col;
        while (array_key_exists($this->_cells[$this->__row][$ac])) {
            $ac++;
        }
    ...
    protected function calculate_column_widths(): void
    {
        ...
        foreach ($this->_frames as $frame_info) {
            ...
            for ($cs = 0; $cs < $colspan; $cs++) {
                ...
                if (array_key_exists($var) && $val > $col[$var]) {
                    $col[$var] = $val;
                    $col["auto"] = false;
                }
    ...
    public function add_row()
    {
        ...
        $i = 0;
        while (array_key_exists($this->_cells[$this->__row][$i])) {
            $i++;
        }
    ...
    public function remove_row(Frame $row)
    {
        ...
        foreach ($rows as $r) {
            foreach ($columns as $c) {
                if (array_key_exists($this->_cells[$r][$c])) {
                    ...
                    // has multiple rows?
                    if (array_key_exists($this->_frames[$id]) && count($this->_frames[$id]["rows"]) > 1) {
