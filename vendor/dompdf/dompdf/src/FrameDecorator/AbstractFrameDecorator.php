...
abstract class AbstractFrameDecorator extends Frame
{
    ...
    function find_block_parent()
    {
        ...
        if (array_key_exists($this->_block_parent)) {
    ...
    function find_positionned_parent()
    {
        ...
        if (array_key_exists($this->_positionned_parent)) {
    ...
    public function increment_counter(string $id = self::DEFAULT_COUNTER, int $increment = 1): void
    {
        ...
        if ($counter_frame) {
            if (!array_key_exists($counter_frame->_counters[$id])) {
    ...
    function lookup_counter_frame($id = self::DEFAULT_COUNTER)
    {
        ...
        while ($f) {
            if (array_key_exists($f->_counters[$id])) {
                return $f;
            }
            $fp = $f->get_parent();
    ...
    function counter_value(string $id = self::DEFAULT_COUNTER, string $type = "decimal")
    {
        ...
        if (!array_key_exists($this->_counters[$id])) {
            $this->_counters[$id] = 0;
        }
