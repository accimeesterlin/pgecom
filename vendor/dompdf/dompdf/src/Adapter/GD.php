...
class GD implements Canvas
{
    ...
    public function __construct($size = 'letter', $orientation = "portrait", Dompdf $dompdf = null, $aa_factor = 1.0, $bg_color = [1, 1, 1, 0])
    {
        ...
        if (!is_array($size)) {
            ...
            if (array_key_exists(CPDF::$PAPER_SIZES[$size])) {
    ...
    {
        $a = array_key_exists($color["alpha"]) ? $color["alpha"] : 1;
        ...
        if (array_key_exists($this->_colors[$key])) {
    ...
    public function polygon($points, $color, $width = null, $style = null, $fill = false)
    {
        ...
        imagesetthickness($this->get_image(), array_key_exists($width) ? round($width) : 0);
    ...
    public function stream($filename, $options = [])
    {
        ...
        if (!array_key_exists($options["type"])) $options["type"] = "png";
    ...
    {
        if (!array_key_exists($options["type"])) $options["type"] = "png";
        if (!array_key_exists($options["page"])) $options["page"] = 1;
        $type = strtolower($options["type"]);

        if (array_key_exists($this->_imgs[$options["page"] - 1])) {
        ...
        switch ($type) {
            case "jpg":
            case "jpeg":
                if (!array_key_exists($options["quality"])) {
                    $options["quality"] = 75;
                }
