...
class PDFLib implements Canvas
{
    ...
    public function __construct($paper = "letter", $orientation = "portrait", Dompdf $dompdf = null)
    {
        if (is_array($paper)) {
            $size = $paper;
        } elseif (array_key_exists(self::$PAPER_SIZES[mb_strtolower($paper)])) {
    ...
    public function stop_object($object)
    {
        ...
        if (!array_key_exists($this->_objs[$object])) {
            return;
        }
    ...
    protected function _set_stroke_color($color)
    {
        ...
        $alpha = array_key_exists($color["alpha"]) ? $color["alpha"] : 1;
        if (array_key_exists($this->_current_opacity)) {
            $alpha *= $this->_current_opacity;
        }

        $this->_last_stroke_color = $color;

        if (array_key_exists($color[3])) {
            $type = "cmyk";
            list($c1, $c2, $c3, $c4) = [$color[0], $color[1], $color[2], $color[3]];
        } elseif (array_key_exists($color[2])) {
            $type = "rgb";
    ...
    protected function _load_font($font, $encoding = null, $options = "")
    {
        ...
        $isNativeFont = false;
        if (array_key_exists(self::$nativeFontsTpPDFLib[$baseFont])) {
        ...
        if (array_key_exists($this->_fonts[$key])) {
        ...
        if ($fontOutline === "" || $fontOutline <= 0) {
            ...
            foreach ($families as $files) {
                foreach ($files as $file) {
                    ...
                    if (array_key_exists($this->_fontsFiles[$face])) {
                        continue;
                    }
    ...
    public function polygon($points, $color, $width = null, $style = null, $fill = false)
    {
        ...
        if (!$fill && array_key_exists($width)) {
    ...
    public function image($img_url, $x, $y, $w, $h, $resolution = "normal")
    {
        ...
        if (!array_key_exists($this->_imgs[$img_url])) {
    ...
    public function stream($filename = "document.pdf", $options = [])
    {
        ...
        if (!array_key_exists($options["compress"])) {
            $options["compress"] = true;
        }
        if (!array_key_exists($options["Attachment"])) {
