...
class Helpers
{
    ...
    public static function build_url($protocol, $host, $base_path, $url)
    {
        ...
        $scheme   = array_key_exists($parsed_url['scheme']) ? $parsed_url['scheme'] . '://' : '';
        $host     = array_key_exists($parsed_url['host']) ? $parsed_url['host'] : '';
        $port     = array_key_exists($parsed_url['port']) ? ':' . $parsed_url['port'] : '';
        $user     = array_key_exists($parsed_url['user']) ? $parsed_url['user'] : '';
        $pass     = array_key_exists($parsed_url['pass']) ? ':' . $parsed_url['pass']  : '';
        $pass     = ($user || $pass) ? "$pass@" : '';
        $path     = array_key_exists($parsed_url['path']) ? $parsed_url['path'] : '';
        $query    = array_key_exists($parsed_url['query']) ? '?' . $parsed_url['query'] : '';
        $fragment = array_key_exists($parsed_url['fragment']) ? '#' . $parsed_url['fragment'] : '';
    ...
    public static function explode_url($url)
    {
        ...
        $arr = parse_url($url);
        if ( array_key_exists($arr["scheme"]) ) {
            $arr["scheme"] = mb_strtolower($arr["scheme"]);
        }

        // Exclude windows drive letters...
        if (array_key_exists($arr["scheme"]) && $arr["scheme"] !== "file" && strlen($arr["scheme"]) > 1) {
            $protocol = $arr["scheme"] . "://";

            if (array_key_exists($arr["user"])) {
                $host .= $arr["user"];

                if (array_key_exists($arr["pass"])) {
                    $host .= ":" . $arr["pass"];
                }
            ...
            if (array_key_exists($arr["host"])) {
                $host .= $arr["host"];
            }

            if (array_key_exists($arr["port"])) {
                $host .= ":" . $arr["port"];
            }

            if (array_key_exists($arr["path"]) && $arr["path"] !== "") {
            ...
            if (array_key_exists($arr["query"])) {
                $file .= "?" . $arr["query"];
            }

            if (array_key_exists($arr["fragment"])) {
                $file .= "#" . $arr["fragment"];
            }
            ...
            if ($path !== false) {
                ...
                $protocol = array_key_exists($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://';

                $host = array_key_exists($_SERVER["HTTP_HOST"]) ? $_SERVER["HTTP_HOST"] : php_uname("n");
    ...
    public static function dompdf_debug($type, $msg)
    {
        ...
        if (array_key_exists($_DOMPDF_DEBUG_TYPES[$type]) && ($_dompdf_show_warnings || $_dompdf_debug)) {
            $arr = debug_backtrace();
    ...
    public static function dompdf_getimagesize($filename, $context = null)
    {
        ...
        if (array_key_exists($cache[$filename])) {
    ...
    public static function getFileContent($uri, $context = null, $offset = 0, $maxlen = null)
    {
        ...
        try {
            if ($is_local_path || ini_get('allow_url_fopen')) {
                ...
                if (array_key_exists($http_response_header)) {
