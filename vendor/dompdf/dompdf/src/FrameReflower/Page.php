...
class Page extends AbstractFrameReflower
{
    ...
    function apply_page_style(Frame $frame, $page_number)
    {
        ...
        if (count($page_styles) > 1) {
            ...
            // FIXME RTL
            if ($odd && array_key_exists($page_styles[":right"])) {
                $style->merge($page_styles[":right"]);
            }

            if ($odd && array_key_exists($page_styles[":odd"])) {
                $style->merge($page_styles[":odd"]);
            }

            // FIXME RTL
            if (!$odd && array_key_exists($page_styles[":left"])) {
                $style->merge($page_styles[":left"]);
            }

            if (!$odd && array_key_exists($page_styles[":even"])) {
                $style->merge($page_styles[":even"]);
            }

            if ($first && array_key_exists($page_styles[":first"])) {
