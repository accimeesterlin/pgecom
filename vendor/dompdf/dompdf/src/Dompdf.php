...
class Dompdf
{
    ...
    */
    ...
    {
        if (array_key_exists($options) && $options instanceof Options) {
    ...
    public function loadHtml($str, $encoding = null)
    {
        ...
        foreach ($metatags as $metatag) {
            if (preg_match($metatag, $str, $matches)) {
                if (array_key_exists($matches[1]) && in_array($matches[1], mb_list_encodings())) {
                    $document_encoding = $matches[1];
                    break;
                }
            }
        }
        if (array_key_exists($document_encoding) && in_array(strtoupper($document_encoding), ['UTF-8','UTF8']) === false) {
            $str = preg_replace('/charset=([^\s"]+)/i', 'charset=UTF-8', $str);
        } elseif (array_key_exists($document_encoding) === false && strpos($str, '<head>') !== false) {
            $str = str_replace('<head>', '<head><meta http-equiv="Content-Type" content="text/html;charset=UTF-8">', $str);
        } elseif (array_key_exists($document_encoding) === false) {
    ...
    public function render()
    {
        ...
        foreach ($metas as $meta) {
            ...
            if (array_key_exists($labels[$name])) {
        ...
        if ($_dompdf_show_warnings && array_key_exists($_dompdf_warnings)) {
    ...
    public function getPaperSize($paperSize = null)
    {
        ...
        if (is_array($size)) {
            return $size;
        } else if (array_key_exists(Adapter\CPDF::$PAPER_SIZES[mb_strtolower($size)])) {
    ...
    public function setOptions(Options $options)
    {
        ...
        if (array_key_exists($fontMetrics)) {
    ...
    public function setCallbacks($callbacks)
    {
        if (is_array($callbacks)) {
            ...
            foreach ($callbacks as $c) {
                if (is_array($c) && array_key_exists($c["event"]) && array_key_exists($c["f"])) {
                    $event = $c["event"];
                    $f = $c["f"];
