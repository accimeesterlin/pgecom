...
class FontMetrics
{
    ...
    public function registerFont($style, $remoteFile, $context = null)
    {
        ...
        $entry = [];
        if (array_key_exists($families[$fontname])) {
        ...
        if (array_key_exists($entry[$styleString]) && $localFile == $entry[$styleString]) {
            return true;
        }
    ...
    public function getTextWidth($text, $font, $size, $wordSpacing = 0.0, $charSpacing = 0.0)
    {
        ...
        $useCache = !array_key_exists($text[50]); // Faster than strlen
        ...
        if ($useCache && array_key_exists($cache[$key][$text])) {
    ...
    public function getFont($familyRaw, $subtypeRaw = "normal")
    {
        ...
        if (array_key_exists($cache[$familyRaw][$subtypeRaw])) {
        ...
        if ($familyRaw) {
            ...
            if (array_key_exists($this->fontLookup[$family][$subtype])) {
        ...
        foreach ($fallback_families as $family) {
            ...
            if (!array_key_exists($this->fontLookup[$family])) {
                continue;
            }
            ...
            if (array_key_exists($family[$subtype])) {
