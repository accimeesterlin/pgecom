...
class Cpdf
{
    ...
    protected function o_catalog($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'viewerPreferences':
                if (!array_key_exists($o['info']['viewerPreferences'])) {
                    $this->numObj++;
        ...
        switch ($action) {
            ...
            case 'page':
                if (!is_array($options)) {
                    ...
                    if (array_key_exists($options['id']) && array_key_exists($options['rid']) && array_key_exists($options['pos'])) {
                        $i = array_search($options['rid'], $o['info']['pages']);
                        if (array_key_exists($o['info']['pages'][$i]) && $o['info']['pages'][$i] == $options['rid']) {
            ...
            case 'out':
                if (count($o['info']['pages'])) {
                    ...
                    if ((array_key_exists($o['info']['fonts']) && count($o['info']['fonts'])) ||
                        array_key_exists($o['info']['procset']) ||
                        (array_key_exists($o['info']['extGStates']) && count($o['info']['extGStates']))
                    ) {
                        $res .= "\n/Resources <<";

                        if (array_key_exists($o['info']['procset'])) {
                        ...
                        if (array_key_exists($o['info']['xObjects']) && count($o['info']['xObjects'])) {
                            $res .= "\n/XObject << ";
                        ...
                        if (array_key_exists($o['info']['extGStates']) && count($o['info']['extGStates'])) {
                            $res .= "\n/ExtGState << ";
                        ...
                        $res .= "\n>>";
                        if (array_key_exists($o['info']['mediaBox'])) {
                            $tmp = $o['info']['mediaBox'];
                            $res .= "\n/MediaBox [" . sprintf(
                                    '%.3F %.3F %.3F %.3F',
                                    $tmp[0],
    ...
    protected function o_font($id, $action, $options = '')
    {
        ...
        switch ($action) {
            case 'new':
                ...
                if (array_key_exists($options['differences'])) {
                    // then we'll need an encoding dictionary
                    $this->numObj++;
                    $this->o_fontEncoding($this->numObj, 'new', $options);
                    $this->objects[$id]['info']['encodingDictionary'] = $this->numObj;
                } else {
                    if (array_key_exists($options['encoding'])) {
            ...
            case 'add':
                ...
                if ($font_options !== false) {
                    ...
                    if (array_key_exists($o['info']['cidFont'])) {
            ...
            case 'out':
                if ($this->fonts[$this->objects[$id]['info']['fontFileName']]['isUnicode']) {
                    ...
                    if (array_key_exists($o['info']['encodingDictionary'])) {
                        // then place a reference to the dictionary
                        $res .= "/Encoding " . $o['info']['encodingDictionary'] . " 0 R\n";
                    } else {
                        if (array_key_exists($o['info']['encoding'])) {
                            // use the specified encoding
                            $res .= "/Encoding /" . $o['info']['encoding'] . "\n";
                        }
                    }

                    if (array_key_exists($o['info']['FirstChar'])) {
                        $res .= "/FirstChar " . $o['info']['FirstChar'] . "\n";
                    }

                    if (array_key_exists($o['info']['LastChar'])) {
                        $res .= "/LastChar " . $o['info']['LastChar'] . "\n";
                    }

                    if (array_key_exists($o['info']['Widths'])) {
                        $res .= "/Widths " . $o['info']['Widths'] . " 0 R\n";
                    }

                    if (array_key_exists($o['info']['FontDescriptor'])) {
    ...
    private function processFont(int $fontObjId, array $object_info)
    {
        ...
        if (!array_key_exists($this->fonts[$fontFileName])) {
            return false;
        }
        ...
        if (!$fileSuffix) {
            ...
            $adobeFontName = array_key_exists($font['PostScriptName']) ? $font['PostScriptName'] : $font['FontName'];
            //        $fontObj = $this->numObj;
            ...
            if (array_key_exists($object['differences'])) {
                foreach ($object['differences'] as $charNum => $charName) {
                    ...
                    if (array_key_exists($font['C'][$charName])) {
            ...
            if (array_key_exists($font['MissingWidth'])) {
                $missing_width = $font['MissingWidth'];
            }
            if (array_key_exists($font['StdVW'])) {
                $stemV = $font['StdVW'];
            } else {
                if (array_key_exists($font['Weight']) && preg_match('!(bold|black)!i', $font['Weight'])) {
                    $stemV = 120;
                }
            }
            ...
            if (!$font['isSubsetting']) {
                ...
                if ($subtable) {
                    ...
                    foreach ($glyphIndexArray as $cid => $gid) {
                        ...
                        $width = $font_obj->normalizeFUnit(array_key_exists($hmtx[$gid]) ? $hmtx[$gid][0] : $hmtx[0][0]);
...
EOT;
    ...
    protected function o_fontDescendentCID($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'out':
                ...
                if (array_key_exists($o['info']['MissingWidth'])) {
                    $res .= "/DW " . $o['info']['MissingWidth'] . "\n";
                }

                if (array_key_exists($o['info']['fontFileName']) && array_key_exists($this->fonts[$o['info']['fontFileName']]['CIDWidths'])) {
    ...
    protected function o_fontGIDtoCIDMap($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'out':
                ...
                $compressed = array_key_exists($this->fonts[$fontFileName]['CIDtoGID_Compressed']) &&
                    $this->fonts[$fontFileName]['CIDtoGID_Compressed'];

                if (!$compressed && array_key_exists($o['raw'])) {
                    $res .= $tmp;
                } else {
                    $res .= "<<";
    ...
    protected function o_action($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'out':
                ...
                switch ($o['type']) {
                    case 'ilink':
                        if (!array_key_exists($this->destinations[(string)$o['info']['label']])) {
                            break;
                        }
    ...
    protected function o_page($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'annot':
                ...
                if (!array_key_exists($o['info']['annot'])) {
                    $o['info']['annot'] = [];
                }
    ...
    protected function o_contents($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'out':
                ...
                if (array_key_exists($this->objects[$id]['raw'])) {
                    $res .= $tmp;
                } else {
                    $res .= "<<";
    ...
    protected function o_image($id, $action, $options = '')
    {
        switch ($action) {
            case 'new':
                ...
                if (array_key_exists($options['masked']) && $options['masked']) {
                    $info['SMask'] = ($this->numObj - 1) . ' 0 R';
                }

                if (!array_key_exists($options['type']) || $options['type'] === 'jpg') {
                    if (!array_key_exists($options['channels'])) {
                        $options['channels'] = 3;
                    }
                    ...
                    if ($options['type'] === 'png') {
                        ...
                        if ($options['isMask']) {
                            ...
                            if (mb_strlen($options['pdata'], '8bit')) {
                                ...
                                if (array_key_exists($options['transparency'])) {
    ...
    protected function o_xobject($id, $action, $options = '')
    {
        switch ($action) {
            ...
            case 'out':
                ...
                $res .= "/Resources <<";
                if (array_key_exists($o['procset'])) {
                    $res .= "\n/ProcSet " . $o['procset'] . " 0 R";
                } else {
                    $res .= "\n/ProcSet [/PDF /Text /ImageB /ImageC /ImageI]";
                }
                if (array_key_exists($o['fonts']) && count($o['fonts'])) {
                    $res .= "\n/Font << ";
                ...
                if (array_key_exists($o['xObjects']) && count($o['xObjects'])) {
                    $res .= "\n/XObject << ";
    ...
    function setEncryption($userPass = '', $ownerPass = '', $pc = [])
    {
        ...
        foreach ($pc as $k => $v) {
            if ($v && array_key_exists($options[$k])) {
                $p += $options[$k];
            } else {
    ...
    private function openFont($font)
    {
        ...
        if (file_exists($fontcache . '/' . $cache_name)) {
            ...
            if (!array_key_exists($this->fonts[$font]['_version_']) || $this->fonts[$font]['_version_'] != $this->fontcacheVersion) {
        ...
        if (!array_key_exists($this->fonts[$font]) && file_exists($dir . $metrics_name)) {
                    ...
                    switch ($key) {
                        ...
                        case 'C': // Found in AFM files
                            ...
                            if ($c >= 0) {
                                ...
                                $data['C'][$c] = $width;
                            } elseif (array_key_exists($n)) {
                                $data['C'][$n] = $width;
                            }

                            if (!array_key_exists($data['MissingWidth']) && $c === -1 && $n === '.notdef') {
        ...
        if (!array_key_exists($this->fonts[$font])) {
    ...
    function selectFont($fontName, $encoding = '', $set = true, $isSubsetting = true)
    {
        ...
        if (!array_key_exists($this->fonts[$fontName])) {
            ...
            if (array_key_exists($this->fonts[$fontName])) {
                ...
                if (is_array($encoding)) {
                    ...
                    if (array_key_exists($encoding['differences'])) {
        ...
        if ($set && array_key_exists($this->fonts[$fontName])) {
    ...
    private function setCurrentFont()
    {
        ...
        //     && array_key_exists($this->fontFamilies[$cf])
        //       && array_key_exists($this->fontFamilies[$cf][$this->currentTextState])){
    ...
    {
        $new_color = [$color[0], $color[1], $color[2], array_key_exists($color[3]) ? $color[3] : null];

        if (!$force && $this->currentColor == $new_color) {
            return;
        }

        if (array_key_exists($new_color[3])) {
            $this->currentColor = $new_color;
            $this->addContent(vsprintf("\n%.3F %.3F %.3F %.3F k", $this->currentColor));
        } else {
            if (array_key_exists($new_color[2])) {
                $this->currentColor = $new_color;
    ...
    function setLineStyle($width = 1, $cap = '', $join = '', $dash = '', $phase = 0)
    {
        ...
        if (array_key_exists($ca[$cap])) {
            $string .= " $ca[$cap] J";
        }

        $ja = ['miter' => 0, 'round' => 1, 'bevel' => 2];

        if (array_key_exists($ja[$join])) {
            $string .= " $ja[$join] j";
        }
    ...
    function newPage($insert = 0, $id = 0, $pos = 'after')
    {
        ...
        if (array_key_exists($this->currentColor)) {
            $this->setColor($this->currentColor, true);
        }

        if (array_key_exists($this->currentStrokeColor)) {
    ...
    function stream($filename = "document.pdf", $options = [])
    {
        ...
        if (!array_key_exists($options["compress"])) $options["compress"] = true;
        if (!array_key_exists($options["Attachment"])) $options["Attachment"] = true;
    ...
    function getFontHeight($size)
    {
        ...
        if (array_key_exists($font['Ascender']) && array_key_exists($font['Descender'])) {
        ...
        if (array_key_exists($font['FontHeightOffset'])) {
    ...
    function getFontXHeight($size)
    {
        ...
        // for the current font, and the given size, what is the height of the font in user units
        if (array_key_exists($font['XHeight'])) {
    ...
    function filterText($text, $bom = true, $convert_encoding = true)
    {
        ...
        if ($convert_encoding) {
            $cf = $this->currentFont;
            if (array_key_exists($this->fonts[$cf]) && $this->fonts[$cf]['isUnicode']) {
    ...
    function registerText($font, $text)
    {
        ...
        if (!array_key_exists($this->stringSubsets[$font])) {
    ...
    function getTextWidth($size, $text, $word_spacing = 0, $char_spacing = 0)
    {
        ...
        if ($current_font['isUnicode']) {
            ...
            foreach ($unicode as $char) {
                ...
                if (array_key_exists($current_font['differences'][$char])) {
                    $char = $current_font['differences'][$char];
                }

                if (array_key_exists($current_font['C'][$char])) {
                    ...
                    if (array_key_exists($current_font['codeToName'][$char]) && $current_font['codeToName'][$char] === 'space') {  // Space
            ...
            for ($i = 0; $i < $len; $i++) {
                $c = $text[$i];
                $char = array_key_exists($ord_cache[$c]) ? $ord_cache[$c] : ($ord_cache[$c] = ord($c));
    ...
    function reopenObject($id)
    {
        ...
        if (array_key_exists($this->objects[$id]['onPage'])) {
    ...
    function stopObject($id)
    {
        ...
        if (array_key_exists($this->addLooseObjects[$id])) {
    ...
    function addObject($id, $options = 'add')
    {
        ...
        if (array_key_exists($this->looseObjects[$id]) && $this->currentContents != $id) {
            ...
            switch ($options) {
                ...
                case 'add':
                    if (array_key_exists($this->objects[$this->currentContents]['onPage'])) {
    ...
    {
        return array_key_exists($this->imagelist[$imgname]);
    }
    ...
    function addImagePng(&$img, $file, $x, $y, $w = 0.0, $h = 0.0, $is_mask = false, $mask = null)
    {
        ...
        if (array_key_exists($this->imagelist[$file])) {
            $data = null;
        } else {
        ...
        }  //End array_key_exists($this->imagelist[$file]) (png Duplicate removal)
            ...
            for ($xpx = 0; $xpx < $wpx; ++$xpx) {
                for ($ypx = 0; $ypx < $hpx; ++$ypx) {
                    ...
                    if ($eight_bit) {
                        ...
                        if (!array_key_exists($allocated_colors[$key])) {
    ...
    function addPngFromFile($file, $x, $y, $w = 0, $h = 0)
    {
        ...
        if (array_key_exists($this->imageAlphaList[$file])) {
    ...
    function addPngFromBuf(&$data, $file, $x, $y, $w = 0.0, $h = 0.0, $is_mask = false, $mask = null)
    {
        if (array_key_exists($this->imagelist[$file])) {
            ...
            if (!$error) {
                ...
                if (array_key_exists($info['interlaceMethod']) && $info['interlaceMethod']) {
                    $error = 1;
            ...
            if (array_key_exists($transparency)) {
    ...
    function addJpegFromFile($img, $x, $y, $w = 0, $h = 0)
    {
        ...
        if ($this->image_iscached($img)) {
            ...
            if (array_key_exists($tmp['channels'])) {
    ...
    function transaction($action)
    {
        switch ($action) {
            ...
            case 'commit':
                if (is_array($this->checkpoint) && array_key_exists($this->checkpoint['checkpoint'])) {
