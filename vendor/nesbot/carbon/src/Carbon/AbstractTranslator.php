...
abstract class AbstractTranslator extends Translation\Translator
{
    ...
    public static function get($locale = null)
    {
        ...
        if (!array_key_exists(static::$singletons[$key])) {
    ...
    {
        return array_key_exists($this->messages[$locale]) || $this->resetMessages($locale);
    }
    ...
    public function setLocale($locale)
    {
        ...
        if ($previousLocale === $locale && array_key_exists($this->messages[$locale])) {
            return true;
        }
        ...
        if (array_key_exists($this->aliases[$locale])) {
    ...
    private static function compareChunkLists($referenceChunks, $chunks)
    {
        ...
        foreach ($referenceChunks as $index => $chunk) {
            if (!array_key_exists($chunks[$index])) {
                $score++;
