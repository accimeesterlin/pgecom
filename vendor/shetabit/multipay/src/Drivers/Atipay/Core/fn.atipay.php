...
function fn_atipay_get_token($params)
{
    ...
    $return = array();
    if ($r) {
        if (array_key_exists($r['status']) && !empty($r['status'])) {
            $status = $r['status'];
            if ($status == 1) {
                $return['success']=1;
            ...
            $return['success']=0;
            if (array_key_exists($r['faMessage']) && !empty($r['faMessage'])) {
...
{
    $result = array();
    if (array_key_exists($params['state']) && !empty($params['state'])) {
        $state = $params['state'];
        if ($state == 'OK') {
            $result['success']=1;
            $result['error']="";
...
function fn_atipay_verify_payment($params, $amount)
{
    ...
    $return = array();
    if ($r) {
        if (array_key_exists($r['amount']) && !empty($r['amount'])) {
            if ($r['amount'] == $amount) {
                $return['success']=1;
                $return['errorMessage']="";
            } else {
...
{
    if (!array_key_exists($params['state']) ||
        !array_key_exists($params['status']) ||
        !array_key_exists($params['reservationNumber']) ||
        !array_key_exists($params['referenceNumber']) ||
        !array_key_exists($params['terminalId']) ||
        !array_key_exists($params['traceNumber'])) {
        return false;
    } else {
        return true;
    }
