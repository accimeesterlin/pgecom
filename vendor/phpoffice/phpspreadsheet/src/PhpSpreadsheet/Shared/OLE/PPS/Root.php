...
class Root extends PPS
{
    ...
    public function save($fileHandle)
    {
        ...
        $this->bigBlockSize = (int) (2 ** (
            (array_key_exists($this->bigBlockSize)) ? self::adjust2($this->bigBlockSize) : 9
        ));
        $this->smallBlockSize = (int) (2 ** (
            (array_key_exists($this->smallBlockSize)) ? self::adjust2($this->smallBlockSize) : 6
        ));
    ...
    private function saveBigData($iStBlk, &$raList): void
    {
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            if ($raList[$i]->Type != OLE::OLE_PPS_TYPE_DIR) {
                $raList[$i]->Size = $raList[$i]->getDataLen();
                if (($raList[$i]->Size >= OLE::OLE_DATA_SIZE_SMALL) || (($raList[$i]->Type == OLE::OLE_PPS_TYPE_ROOT) && array_key_exists($raList[$i]->_data))) {
