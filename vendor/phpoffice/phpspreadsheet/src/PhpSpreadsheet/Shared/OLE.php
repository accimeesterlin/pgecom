...
class OLE
{
    ...
    public function readPpsWks($blockId)
    {
        ...
        for ($pos = 0; true; $pos += 128) {
            ...
            if (array_key_exists($this->root) && $this->ppsTreeComplete($this->root->No)) {
                break;
            }
        }
        fclose($fh);
    ...
    {
        return array_key_exists($this->_list[$index]) &&
    ...
    {
        if (array_key_exists($this->_list[$index])) {
    ...
    public function getData($index, $position, $length)
    {
        ...
        if (!array_key_exists($this->_list[$index]) || ($position >= $this->_list[$index]->Size) || ($position < 0)) {
            return '';
        }
