...
class Xlsx extends BaseWriter
{
    ...
    public function save($filename, int $flags = 0): void
    {
        ...
        for ($i = 0; $i < $this->spreadSheet->getSheetCount(); ++$i) {
            ...
            if (array_key_exists($unparsedLoadedData['sheets'][$sheetCodeName]['ctrlProps'])) {
                foreach ($unparsedLoadedData['sheets'][$sheetCodeName]['ctrlProps'] as $ctrlProp) {
                    $zipContent[$ctrlProp['filePath']] = $ctrlProp['content'];
                }
            }
            if (array_key_exists($unparsedLoadedData['sheets'][$sheetCodeName]['printerSettings'])) {
            ...
            } elseif (array_key_exists($unparsedLoadedData['sheets'][$sheetCodeName]['drawingAlternateContents'])) {
                // Drawings
                $zipContent['xl/drawings/drawing' . ($i + 1) . '.xml'] = $this->getWriterPartDrawing()->writeDrawings($this->spreadSheet->getSheet($i), $this->includeCharts);
            }

            // Add unparsed drawings
            if (array_key_exists($unparsedLoadedData['sheets'][$sheetCodeName]['Drawings'])) {
            ...
            if (array_key_exists($unparsedLoadedData['sheets'][$sheetCodeName]['vmlDrawings'])) {
