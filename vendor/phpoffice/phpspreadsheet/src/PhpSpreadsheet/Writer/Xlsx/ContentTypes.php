...
class ContentTypes extends WriterPart
{
    ...
    public function writeContentTypes(Spreadsheet $spreadsheet, $includeCharts = false)
    {
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            ...
            $hasUnparsedDrawing = array_key_exists($unparsedLoadedData['sheets'][$spreadsheet->getSheet($i)->getCodeName()]['drawingOriginalIds']);
        ...
        for ($i = 0; $i < $mediaCount; ++$i) {
            ...
            if (!array_key_exists($aMediaContentTypes[$extension])) {
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            if (count($spreadsheet->getSheet($i)->getHeaderFooter()->getImages()) > 0) {
                foreach ($spreadsheet->getSheet($i)->getHeaderFooter()->getImages() as $image) {
                    if (!array_key_exists($aMediaContentTypes[strtolower($image->getExtension())])) {
            ...
            if (count($spreadsheet->getSheet($i)->getComments()) > 0) {
                foreach ($spreadsheet->getSheet($i)->getComments() as $comment) {
                    ...
                    if (!array_key_exists($aMediaContentTypes[$bgImageExtentionKey])) {
        ...
        // unparsed defaults
        if (array_key_exists($unparsedLoadedData['default_content_types'])) {
        ...
        // unparsed overrides
        if (array_key_exists($unparsedLoadedData['override_content_types'])) {
