...
class Worksheet extends WriterPart
{
    ...
    private function writePageSetup(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet): void
    {
        ...
        if (array_key_exists($getUnparsedLoadedData['sheets'][$worksheet->getCodeName()]['pageSetupRelId'])) {
    ...
    private function writeSheetData(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet, array $stringTable): void
    {
        ...
        while ($currentRow++ < $highestRow) {
            ...
            // Write current row?
            $writeCurrentRow = array_key_exists($cellsByRow[$currentRow]) || $rowDimension->getRowHeight() >= 0 || $rowDimension->getVisible() == false || $rowDimension->getCollapsed() == true || $rowDimension->getOutlineLevel() > 0 || $rowDimension->getXfIndex() !== null;
            ...
            if ($writeCurrentRow) {
                ...
                // Write cells
                if (array_key_exists($cellsByRow[$currentRow])) {
    ...
    private function writeCellString(XMLWriter $objWriter, string $mappedType, $cellValue, array $flippedStringTable): void
    {
        $objWriter->writeAttribute('t', $mappedType);
        if (!$cellValue instanceof RichText) {
            self::writeElementIf($objWriter, array_key_exists($flippedStringTable[$cellValue]), 'v', $flippedStringTable[$cellValue] ?? '');
        } else {
    ...
    private function writeDrawings(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet, $includeCharts = false): void
    {
        ...
        $hasUnparsedDrawing = array_key_exists($unparsedLoadedData['sheets'][$worksheet->getCodeName()]['drawingOriginalIds']);
        ...
        $rId = 'rId1';
        if (array_key_exists($unparsedLoadedData['sheets'][$worksheet->getCodeName()]['drawingOriginalIds'])) {
