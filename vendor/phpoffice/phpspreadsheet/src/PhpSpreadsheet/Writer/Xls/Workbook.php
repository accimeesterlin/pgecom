...
class Workbook extends BIFFwriter
{
    ...
    public function addXfWriter(Style $style, $isStyleXf = false)
    {
        ...
        if ($style->getNumberFormat()->getBuiltInFormatCode() === false) {
            ...
            if (array_key_exists($this->addedNumberFormats[$numberFormatHashCode])) {
    ...
    public function addFont(\PhpOffice\PhpSpreadsheet\Style\Font $font)
    {
        ...
        if (array_key_exists($this->addedFonts[$fontHashCode])) {
    ...
    {
        if (!array_key_exists($this->colors[$rgb])) {
            $color =
                [
        ...
        if (count($definedNames) > 0) {
            ...
            foreach ($definedNames as $definedName) {
                ...
                try {
                    ...
                    if (array_key_exists($formulaData[0]) && ($formulaData[0] == "\x7A" || $formulaData[0] == "\x5A")) {
