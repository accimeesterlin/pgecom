...
class Alignment extends Supervisor
{
    ...
    public function applyFromArray(array $styleArray)
    {
        if ($this->isSupervisor) {
            ...
            if (array_key_exists($styleArray['horizontal'])) {
                $this->setHorizontal($styleArray['horizontal']);
            }
            if (array_key_exists($styleArray['vertical'])) {
                $this->setVertical($styleArray['vertical']);
            }
            if (array_key_exists($styleArray['textRotation'])) {
                $this->setTextRotation($styleArray['textRotation']);
            }
            if (array_key_exists($styleArray['wrapText'])) {
                $this->setWrapText($styleArray['wrapText']);
            }
            if (array_key_exists($styleArray['shrinkToFit'])) {
                $this->setShrinkToFit($styleArray['shrinkToFit']);
            }
            if (array_key_exists($styleArray['indent'])) {
                $this->setIndent($styleArray['indent']);
            }
            if (array_key_exists($styleArray['readOrder'])) {
