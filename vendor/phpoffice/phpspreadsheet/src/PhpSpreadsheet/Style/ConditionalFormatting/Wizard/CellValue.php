...
class CellValue extends WizardAbstract implements WizardInterface
{
    ...
    {
        if ((!array_key_exists(self::SINGLE_OPERATORS[$operator])) && (!array_key_exists(self::RANGE_OPERATORS[$operator]))) {
    ...
    {
        if (!array_key_exists(self::RANGE_OPERATORS[$this->operator])) {
    ...
    {
        if (!array_key_exists(self::MAGIC_OPERATIONS[$methodName]) && $methodName !== 'and') {
            throw new Exception('Invalid Operator for Cell Value CF Rule Wizard');
        }
