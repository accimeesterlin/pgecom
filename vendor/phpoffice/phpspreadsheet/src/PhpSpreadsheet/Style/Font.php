...
class Font extends Supervisor
{
    ...
    public function applyFromArray(array $styleArray)
    {
        if ($this->isSupervisor) {
            ...
            if (array_key_exists($styleArray['name'])) {
                $this->setName($styleArray['name']);
            }
            if (array_key_exists($styleArray['bold'])) {
                $this->setBold($styleArray['bold']);
            }
            if (array_key_exists($styleArray['italic'])) {
                $this->setItalic($styleArray['italic']);
            }
            if (array_key_exists($styleArray['superscript'])) {
                $this->setSuperscript($styleArray['superscript']);
            }
            if (array_key_exists($styleArray['subscript'])) {
                $this->setSubscript($styleArray['subscript']);
            }
            if (array_key_exists($styleArray['underline'])) {
                $this->setUnderline($styleArray['underline']);
            }
            if (array_key_exists($styleArray['strikethrough'])) {
                $this->setStrikethrough($styleArray['strikethrough']);
            }
            if (array_key_exists($styleArray['color'])) {
                $this->getColor()->applyFromArray($styleArray['color']);
            }
            if (array_key_exists($styleArray['size'])) {
