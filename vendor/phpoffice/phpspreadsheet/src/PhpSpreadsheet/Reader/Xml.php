...
class Xml extends BaseReader
{
    ...
    public function listWorksheetInfo($filename)
    {
        ...
        foreach ($xml_ss->Worksheet as $worksheet) {
            ...
            if (array_key_exists($worksheet_ss['Name'])) {
                $tmpInfo['worksheetName'] = (string) $worksheet_ss['Name'];
            }

            if (array_key_exists($worksheet->Table->Row)) {
                $rowIndex = 0;
    ...
    public function loadIntoExisting($filename, Spreadsheet $spreadsheet)
    {
        ...
        foreach ($xml_ss->Worksheet as $worksheetx) {
            ...
            if (
                array_key_exists($this->loadSheetsOnly, $worksheet_ss['Name']) &&
            ...
            if (array_key_exists($worksheet->Names[0])) {
            ...
            if (array_key_exists($worksheet->Table->Row)) {
                ...
                foreach ($worksheet->Table->Row as $rowData) {
                    ...
                    if (array_key_exists($row_ss['Index'])) {
                    ...
                    foreach ($rowData->Cell as $cell) {
                        ...
                        if (array_key_exists($cell_ss['Index'])) {
                        ...
                        $cellDataFormula = '';
                        if (array_key_exists($cell_ss['Formula'])) {
                        ...
                        if (array_key_exists($cell->Data)) {
                            ...
                            if (array_key_exists($cellData_ss['Type'])) {
                        ...
                        if (array_key_exists($cell->Comment)) {
                            $this->parseCellComment($cell->Comment, $namespaces, $spreadsheet, $columnID, $rowID);
                        }

                        if (array_key_exists($cell_ss['StyleID'])) {
                            $style = (string) $cell_ss['StyleID'];
                            if ((array_key_exists($this->styles[$style])) && (!empty($this->styles[$style]))) {
                    ...
                    if ($rowHasData) {
                        if (array_key_exists($row_ss['Height'])) {
                ...
                if (array_key_exists($namespaces['x'])) {
                    $xmlX = $worksheet->children($namespaces['x']);
                    if (array_key_exists($xmlX->WorksheetOptions)) {
        ...
        if (array_key_exists($xml->Names[0])) {
    ...
    protected function parseCellComment(
        ...
        $author = 'unknown';
        if (array_key_exists($commentAttributes->Author)) {
