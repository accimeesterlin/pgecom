...
class Chart
{
    ...
    private static function getAttribute(SimpleXMLElement $component, $name, $format)
    {
        ...
        if (array_key_exists($attributes[$name])) {
            if ($format == 'string') {
    ...
    private static function readColor($color, $background = false)
    {
        ...
        } elseif (array_key_exists($color['indexed'])) {
                                ...
                                foreach ($chartDetails as $chartDetailKey => $chartDetail) {
                                    switch ($chartDetailKey) {
                                        case 'layout':
                                            ...
                                            break;
                                        case 'catAx':
                                            if (array_key_exists($chartDetail->title)) {
                                        ...
                                        case 'dateAx':
                                            ...
                                            break;
                                        case 'valAx':
                                            if (array_key_exists($chartDetail->title, $chartDetail->axPos)) {
                                        ...
                                        case 'pieChart':
                                        case 'pie3DChart':
                                            $explosion = array_key_exists($chartDetail->ser->explosion);
                                            $plotSer = self::chartDataSeries($chartDetail, $namespacesChartMeta, $chartDetailKey);
    ...
    {
        if (!array_key_exists($chartDetail->manualLayout)) {
            return null;
        }
    ...
    private static function chartDataSeriesValueSet($seriesDetail, $namespacesChartMeta, $marker = null)
    {
        if (array_key_exists($seriesDetail->strRef)) {
            ...
            return $seriesValues;
        } elseif (array_key_exists($seriesDetail->multiLvlStrRef)) {
            $seriesSource = (string) $seriesDetail->multiLvlStrRef->f;
            $seriesValues = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, $seriesSource, null, null, null, $marker);

            if (array_key_exists($seriesDetail->multiLvlStrRef->multiLvlStrCache)) {
            ...
            return $seriesValues;
        } elseif (array_key_exists($seriesDetail->multiLvlNumRef)) {
            $seriesSource = (string) $seriesDetail->multiLvlNumRef->f;
            $seriesValues = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, $seriesSource, null, null, null, $marker);

            if (array_key_exists($seriesDetail->multiLvlNumRef->multiLvlNumCache)) {
    ...
    private static function parseRichText(SimpleXMLElement $titleDetailPart)
    {
        ...
        foreach ($titleDetailPart as $titleDetailElementKey => $titleDetailElement) {
            if (array_key_exists($titleDetailElement->t)) {
                $objText = $value->createTextRun((string) $titleDetailElement->t);
            }
            if (array_key_exists($titleDetailElement->rPr)) {
                if (array_key_exists($titleDetailElement->rPr->rFont['val'])) {
