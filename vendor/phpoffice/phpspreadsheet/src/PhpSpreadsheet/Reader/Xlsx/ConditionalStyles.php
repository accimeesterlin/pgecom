...
class ConditionalStyles
{
    ...
    private function readConditionalsFromExt(SimpleXMLElement $extLst, StyleReader $styleReader): array
    {
        ...
        if (array_key_exists($extLst->ext['uri']) && (string) $extLst->ext['uri'] === '{78C0D931-6437-407d-A8EE-F0AAD7539E65}') {
    ...
    private function readConditionalStyles($xmlSheet): array
    {
        ...
        foreach ($xmlSheet->conditionalFormatting as $conditional) {
            foreach ($conditional->cfRule as $cfRule) {
                if (Conditional::isValidConditionType((string) $cfRule['type']) && array_key_exists($this->dxfs[(int) ($cfRule['dxfId'])])) {
    ...
    private function readStyleRules($cfRules, $extLst)
    {
        ...
        foreach ($cfRules as $cfRule) {
            ...
            if (array_key_exists($cfRule['stopIfTrue']) && (int) $cfRule['stopIfTrue'] === 1) {
            ...
            if (array_key_exists($cfRule->dataBar)) {
    ...
    private function readDataBarOfConditionalRule($cfRule, $conditionalFormattingRuleExtensions): ConditionalDataBar
    {
        ...
        //dataBar attribute
        if (array_key_exists($cfRule->dataBar['showValue'])) {
        ...
        //color
        if (array_key_exists($cfRule->dataBar->color)) {
    ...
    {
        if (array_key_exists($cfRule->extLst)) {
            ...
            foreach ((count($cfRule->extLst) > 0 ? $cfRule->extLst->ext : [$cfRule->extLst->ext]) as $ext) {
                ...
                if (array_key_exists($conditionalFormattingRuleExtensions[$extId]) && (string) $ext['uri'] === '{B025F937-C7B1-47D3-B67F-A62EFF666E3E}') {
