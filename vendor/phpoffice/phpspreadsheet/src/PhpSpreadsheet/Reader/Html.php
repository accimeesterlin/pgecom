...
class Html extends BaseReader
{
    ...
    private function processDomElementSpanEtc(Worksheet $sheet, int &$row, string &$column, string &$cellContent, DOMElement $child, array &$attributeArray): void
    {
        if (in_array((string) $child->nodeName, self::SPAN_ETC, true)) {
            if (array_key_exists($attributeArray['class']) && $attributeArray['class'] === 'comment') {
            ...
            if (array_key_exists($this->formats[$child->nodeName])) {
    ...
    private function processDomElementTr(Worksheet $sheet, int &$row, string &$column, string &$cellContent, DOMElement $child, array &$attributeArray): void
    {
        if ($child->nodeName === 'tr') {
            ...
            if (array_key_exists($attributeArray['height'])) {
    ...
    {
        if (array_key_exists($attributeArray['bgcolor'])) {
    ...
    {
        if (array_key_exists($attributeArray['width'])) {
    ...
    {
        if (array_key_exists($attributeArray['align'])) {
    ...
    {
        if (array_key_exists($attributeArray['valign'])) {
    ...
    {
        if (array_key_exists($attributeArray['data-format'])) {
    ...
    {
        while (array_key_exists($this->rowspan[$column . $row])) {
            ++$column;
        }
        $this->processDomElement($child, $sheet, $row, $column, $cellContent);
        ...
        if (array_key_exists($attributeArray['rowspan'], $attributeArray['colspan'])) {
            ...
            $column = $columnTo;
        } elseif (array_key_exists($attributeArray['rowspan'])) {
            ...
            $sheet->mergeCells($range);
        } elseif (array_key_exists($attributeArray['colspan'])) {
    ...
    {
        if (!array_key_exists($attributeArray['style'])) {
            return;
        }
        ...
        foreach ($styles as $st) {
            $value = explode(':', $st);
            $styleName = array_key_exists($value[0]) ? trim($value[0]) : null;
            $styleValue = array_key_exists($value[1]) ? trim($value[1]) : null;
    ...
    {
        if (!array_key_exists($attributes['src'])) {
            return;
        }

        $src = urldecode($attributes['src']);
        $width = array_key_exists($attributes['width']) ? (float) $attributes['width'] : null;
        $height = array_key_exists($attributes['height']) ? (float) $attributes['height'] : null;
