...
class Gnumeric extends BaseReader
{
    ...
    public function canRead(string $filename): bool
    {
        ...
        return array_key_exists($data) && $data === chr(0x1F) . chr(0x8B);
    }
    ...
    {
        if ((!$this->readDataOnly) && (array_key_exists($sheet->Objects))) {
    ...
    {
        if (array_key_exists($gnmXML->UIData)) {
    ...
    {
        if ($sheet !== null && array_key_exists($sheet->Selections)) {
    ...
    private function processMergedCells(?SimpleXMLElement $sheet): void
    {
        ...
        if ($sheet !== null && array_key_exists($sheet->MergedRegions)) {
    ...
    {
        if ($sheet !== null && array_key_exists($sheet->Filters)) {
            foreach ($sheet->Filters->Filter as $autofilter) {
                if ($autofilter !== null) {
                    $attributes = $autofilter->attributes();
                    if (array_key_exists($attributes['Area'])) {
    ...
    private function processColumnLoop(int $whichColumn, int $maxCol, ?SimpleXMLElement $columnOverride, float $defaultWidth): int
    {
        ...
        $hidden = (array_key_exists($columnAttributes['Hidden'])) && ((string) $columnAttributes['Hidden'] == '1');
    ...
    {
        if ((!$this->readDataOnly) && $sheet !== null && (array_key_exists($sheet->Cols))) {
            //    Column Widths
            $defaultWidth = 0;
    ...
    private function processRowLoop(int $whichRow, int $maxRow, ?SimpleXMLElement $rowOverride, float $defaultHeight): int
    {
        ...
        $hidden = (array_key_exists($rowAttributes['Hidden'])) && ((string) $rowAttributes['Hidden'] == '1');
    ...
    {
        if ((!$this->readDataOnly) && $sheet !== null && (array_key_exists($sheet->Rows))) {
            //    Row Heights
            $defaultHeight = 0;
    ...
    private function processDefinedNames(?SimpleXMLElement $gnmXML): void
    {
        ...
        if ($gnmXML !== null && array_key_exists($gnmXML->Names)) {
    ...
    private function loadCell(
        ...
        if (array_key_exists($cellAttributes->ValueFormat)) {
