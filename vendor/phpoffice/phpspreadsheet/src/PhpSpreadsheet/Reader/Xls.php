...
class Xls extends BaseReader
{
    ...
    protected function loadSpreadsheetFromFile(string $filename): Spreadsheet
    {
        ...
        if (!$this->readDataOnly) {
            foreach ($this->objFonts as $objFont) {
                if (array_key_exists($objFont->colorIndex)) {
        ...
        foreach ($this->sheets as $sheet) {
            ...
            if (array_key_exists($this->loadSheetsOnly) && !in_array($sheet['name'], $this->loadSheetsOnly)) {
                continue;
            }
            ...
            foreach ($this->objs as $n => $obj) {
                ...
                if (array_key_exists($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    switch ($obj['otObjType']) {
                        case 0x19:
                            // Note
                            if (array_key_exists($this->cellNotes[$obj['idObjID']])) {
                                $cellNote = $this->cellNotes[$obj['idObjID']];

                                if (array_key_exists($this->textObjects[$obj['idObjID']])) {
            ...
            if (!empty($this->cellNotes)) {
                foreach ($this->cellNotes as $note => $noteDetails) {
                    if (!array_key_exists($noteDetails['objTextData'])) {
                        if (array_key_exists($this->textObjects[$note])) {
    ...
    {
        if (!array_key_exists($this->summaryInformation)) {
            return;
        }
    ...
    {
        if (!array_key_exists($this->documentSummaryInformation)) {
            return;
        }
...
//                                                    ->setAuthor($author)
    ...
    private function readXf(): void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (array_key_exists($this->formats[$numberFormatIndex])) {
                ...
                switch ($extType) {
                    case 4:        // fill start color
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (array_key_exists($this->mapCellXfIndex[$ixfe])) {
    ...
    private function readColInfo(): void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            for ($i = $firstColumnIndex + 1; $i <= $lastColumnIndex + 1; ++$i) {
                ...
                if (array_key_exists($this->mapCellXfIndex[$xfIndex])) {
    ...
    private function readRow(): void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($hasExplicitFormat && array_key_exists($this->mapCellXfIndex[$xfIndex])) {
    ...
    private function readRk(): void
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if (!$this->readDataOnly && array_key_exists($this->mapCellXfIndex[$xfIndex])) {
    ...
    private function readLabelSst(): void
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
                ...
                for ($i = 0; $i <= $sstCount; ++$i) {
                    ...
                    if (StringHelper::countCharacters($text) > 0) {
                        if ($i == 0) { // first text run, no style
                            ...
                            if (array_key_exists($fmtRuns[$i - 1])) {
            ...
            if (!$this->readDataOnly && !$emptyCell && array_key_exists($this->mapCellXfIndex[$xfIndex])) {
                // add style information
                $cell->setXfIndex($this->mapCellXfIndex[$xfIndex]);
            }
        }
    ...
    private function readBlank(): void
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            // add style information
            if (!$this->readDataOnly && $this->readEmptyCells && array_key_exists($this->mapCellXfIndex[$xfIndex])) {
                $this->phpSheet->getCell($columnString . ($row + 1))->setXfIndex($this->mapCellXfIndex[$xfIndex]);
            }
        }
    }
    ...
    private function readWindow2(): void
    {
        ...
        if ($this->version === self::XLS_BIFF8) {
            ...
            if (!array_key_exists($recordData[10])) {
                $zoomscaleInPageBreakPreview = 0;
            } else {
            ...
            if (!array_key_exists($recordData[12])) {
                $zoomscaleInNormalView = 0;
            } else {
    ...
    {
        if (array_key_exists($this->ref[$index])) {
