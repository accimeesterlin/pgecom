...
class Worksheet implements IComparable
{
    ...
    public function getChartByIndex($index)
    {
        ...
        if (!array_key_exists($this->chartCollection[$index])) {
            return false;
        }
    ...
    public function calculateColumnWidths()
    {
        ...
        if (!empty($autoSizes)) {
            ...
            foreach ($this->getCoordinates(false) as $coordinate) {
                ...
                if ($cell !== null && array_key_exists($autoSizes[$this->cellCollection->getCurrentColumn()])) {
                    //Determine if cell is in merge range
                    $isMerged = array_key_exists($isMergeCell[$this->cellCollection->getCurrentCoordinate()]);
    ...
    {
        // Get row dimension
        if (!array_key_exists($this->rowDimensions[$row])) {
    ...
    public function getColumnDimension(string $column): ColumnDimension
    {
        ...
        // Fetch dimensions
        if (!array_key_exists($this->columnDimensions[$column])) {
    ...
    public function conditionalStylesExists($coordinate): bool
    {
        $coordinate = strtoupper($coordinate);
        if (strpos($coordinate, ':') !== false) {
            return array_key_exists($this->conditionalStylesCollection[$coordinate]);
        }
    ...
    public function setBreak($coordinate, $break)
    {
        ...
        if ($break === self::BREAK_NONE) {
            if (array_key_exists($this->breaks[$cellAddress])) {
    ...
    public function unmergeCells($range)
    {
        ...
        if (strpos($range, ':') !== false) {
            if (array_key_exists($this->mergeCells[$range])) {
    ...
    public function unprotectCells($range)
    {
        ...
        if (array_key_exists($this->protectedCells[$range])) {
    ...
    public function getComment($cellCoordinate)
    {
        ...
        if (array_key_exists($this->comments[$cellAddress])) {
    ...
    public function getHyperlink($cellCoordinate)
    {
        ...
        if (array_key_exists($this->hyperlinkCollection[$cellCoordinate])) {
    ...
    {
        return array_key_exists($this->hyperlinkCollection[$coordinate]);
    }
    ...
    public function getDataValidation($cellCoordinate)
    {
        ...
        if (array_key_exists($this->dataValidationCollection[$cellCoordinate])) {
    ...
    {
        return array_key_exists($this->dataValidationCollection[$coordinate]);
    }
