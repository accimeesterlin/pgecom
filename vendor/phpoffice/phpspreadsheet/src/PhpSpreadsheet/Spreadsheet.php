...
class Spreadsheet
{
    ...
    public function getRibbonBinObjects($what = 'all')
    {
        ...
        switch ($what) {
            ...
            case 'names':
            case 'data':
                if (is_array($this->ribbonBinObjects) && array_key_exists($this->ribbonBinObjects[$what])) {
            ...
            case 'types':
                if (
                    ...
                    array_key_exists($this->ribbonBinObjects['data']) && is_array($this->ribbonBinObjects['data'])
                ) {
    ...
    {
        if (!array_key_exists($this->workSheetCollection[$sheetIndex])) {
    ...
    {
        if (array_key_exists($this->definedNames[$name]) && $this->definedNames[$name]->isFormula() === $type) {
    ...
    {
        if (
            ($worksheet !== null) && array_key_exists($this->definedNames[$worksheet->getTitle() . '!' . $name])
    ...
    public function getDefinedName(string $definedName, ?Worksheet $worksheet = null): ?DefinedName
    {
        ...
        if ($definedName !== '') {
            ...
            if (array_key_exists($this->definedNames[$definedName])) {
                $returnValue = $this->definedNames[$definedName];
            }

            // then look for local defined name (has priority over global defined name if both names exist)
            if (($worksheet !== null) && array_key_exists($this->definedNames[$worksheet->getTitle() . '!' . $definedName])) {
    ...
    public function removeDefinedName(string $definedName, ?Worksheet $worksheet = null): self
    {
        ...
        if ($worksheet === null) {
            ...
            if (array_key_exists($this->definedNames[$worksheet->getTitle() . '!' . $definedName])) {
                unset($this->definedNames[$worksheet->getTitle() . '!' . $definedName]);
            } elseif (array_key_exists($this->definedNames[$definedName])) {
                unset($this->definedNames[$definedName]);
            }
        }
    ...
    {
        if (array_key_exists($this->cellXfCollection[0])) {
