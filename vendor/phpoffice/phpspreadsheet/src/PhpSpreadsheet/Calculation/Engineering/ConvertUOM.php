...
class ConvertUOM
{
    ...
    {
        if (array_key_exists(self::$conversionUnits[$uom])) {
        ...
        $uom = substr($uom, 1);
        if (array_key_exists(self::$conversionUnits[$uom], self::$conversionMultipliers[$multiplierType])) {
        ...
        if (array_key_exists(self::$conversionUnits[$uom], self::$binaryConversionMultipliers[$multiplierType])) {
            if (self::$conversionUnits[$uom]['AllowPrefix'] === false) {
                throw new Exception('Prefix not allowed for UoM');
            }
            $unitCategory = self::$conversionUnits[$uom]['Group'];
