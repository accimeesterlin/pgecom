...
class Calculation
{
    ...
    public static function getInstance(?Spreadsheet $spreadsheet = null): self
    {
        if ($spreadsheet !== null) {
            ...
            if (array_key_exists($instance)) {
                return $instance;
            }
        }

        if (!array_key_exists(self::$instance) || (self::$instance === null)) {
    ...
    {
        if (array_key_exists($this->calculationCache[$worksheetName])) {
    ...
    {
        if (array_key_exists($this->calculationCache[$fromWorksheetName])) {
            ...
            if ($locale !== 'en_us') {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    ...
                    if (strpos($localeFunction, '=') !== false) {
                        ...
                        if ((array_key_exists(self::$phpSpreadsheetFunctions[$fName])) && ($lfName != '') && ($fName != $lfName)) {
                            self::$localeFunctions[$fName] = $lfName;
                        }
                    }
                }
                //    Default the TRUE and FALSE constants to the locale names of the TRUE() and FALSE() functions
                if (array_key_exists(self::$localeFunctions['TRUE'])) {
                    self::$localeBoolean['TRUE'] = self::$localeFunctions['TRUE'];
                }
                if (array_key_exists(self::$localeFunctions['FALSE'])) {
    ...
    public static function localeFunc($function)
    {
        if (self::$localeLanguage !== 'en_us') {
            ...
            if (array_key_exists(self::$localeFunctions[$functionName])) {
    ...
    {
        if (is_string($value)) {
            if ((array_key_exists($value[0])) && ($value[0] == self::FORMULA_STRING_QUOTE) && (substr($value, -1) == self::FORMULA_STRING_QUOTE)) {
    ...
    public function parseFormula($formula)
    {
        ...
        if ((!array_key_exists($formula[0])) || ($formula[0] != '=')) {
            return [];
        }
        $formula = ltrim(substr($formula, 1));
        if (!array_key_exists($formula[0])) {
            return [];
        }
    ...
    public function getValueFromCache(string $cellReference, &$cellValue): bool
    {
        ...
        if (($this->calculationCacheEnabled) && (array_key_exists($this->calculationCache[$cellReference]))) {
    ...
    private function internalParseFormula($formula, ?Cell $cell = null)
    {
        ...
        while (true) {
            ...
            if ((array_key_exists(self::$comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (array_key_exists(self::$comparisonOperators[$formula[$index + 1]]))) {
            ...
            if ($opCharacter == '-' && !$expectingOperator) {                //    Is it a negation instead of a minus?
            ...
            } elseif ((array_key_exists(self::$operators[$opCharacter]) || $isOperandOrFunction) && $expectingOperator) {    //    Are we putting an operator on the stack?
                while (
                    $stack->count() > 0 &&
                    ($o2 = $stack->last()) &&
                    array_key_exists(self::$operators[$o2['value']]) &&
                ...
                if (is_array($d) && preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $d['value'], $matches)) {
                    ...
                    if (array_key_exists(self::$controlFunctions[$functionName])) {
                        $expectedArgumentCount = self::$controlFunctions[$functionName]['argumentCount'];
                        $functionCall = self::$controlFunctions[$functionName]['functionCall'];
                    } elseif (array_key_exists(self::$phpSpreadsheetFunctions[$functionName])) {
                ...
                if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $val, $matches)) {
                    ...
                    if (array_key_exists(self::$phpSpreadsheetFunctions[strtoupper($matches[1])]) || array_key_exists(self::$controlFunctions[strtoupper($matches[1])])) {    // it's a function
                    ...
                    } elseif (array_key_exists(self::$excelConstants[trim(strtoupper($val))])) {
            ...
            } elseif (array_key_exists(self::$operators[$opCharacter]) && !$expectingOperator) {
            ...
            if ($index == strlen($formula)) {
                ...
                if ((array_key_exists(self::$operators[$opCharacter])) && ($opCharacter != '%')) {
    ...
    private function processTokenStack($tokens, $cellID = null, ?Cell $cell = null)
    {
        ...
        foreach ($tokens as $tokenData) {
            ...
            if ($this->branchPruningEnabled && array_key_exists($tokenData['onlyIf'])) {
                ...
                if (
                    (array_key_exists($storeValue) || $tokenData['reference'] === 'NULL')
                    && (!$storeValueAsBool || ErrorValue::isError($storeValue) || ($storeValue === 'Pruned branch'))
                ) {
                    // If branching value is not true, we don't need to compute
                    if (!array_key_exists($fakedForBranchPruning['onlyIf-' . $onlyIfStoreKey])) {
                        $stack->push('Value', 'Pruned branch (only if ' . $onlyIfStoreKey . ') ' . $token);
                        $fakedForBranchPruning['onlyIf-' . $onlyIfStoreKey] = true;
                    }

                    if (array_key_exists($storeKey)) {
            ...
            if ($this->branchPruningEnabled && array_key_exists($tokenData['onlyIfNot'])) {
                ...
                if (
                    ...
                    if (!array_key_exists($fakedForBranchPruning['onlyIfNot-' . $onlyIfNotStoreKey])) {
            ...
            if (!is_numeric($token) && array_key_exists(self::$binaryOperators[$token])) {
                ...
                if ((array_key_exists(self::$phpSpreadsheetFunctions[$functionName])) || (array_key_exists(self::$controlFunctions[$functionName]))) {    // function
                    $passByReference = false;
                    $passCellReference = false;
                    $functionCall = null;
                    if (array_key_exists(self::$phpSpreadsheetFunctions[$functionName])) {
                        $functionCall = self::$phpSpreadsheetFunctions[$functionName]['functionCall'];
                        $passByReference = array_key_exists(self::$phpSpreadsheetFunctions[$functionName]['passByReference']);
                        $passCellReference = array_key_exists(self::$phpSpreadsheetFunctions[$functionName]['passCellReference']);
                    } elseif (array_key_exists(self::$controlFunctions[$functionName])) {
                        $functionCall = self::$controlFunctions[$functionName]['functionCall'];
                        $passByReference = array_key_exists(self::$controlFunctions[$functionName]['passByReference']);
                        $passCellReference = array_key_exists(self::$controlFunctions[$functionName]['passCellReference']);
                    }
                    ...
                    for ($i = 0; $i < $argCount; ++$i) {
                        ...
                        if (
                            ($passByReference) &&
                            (array_key_exists(self::$phpSpreadsheetFunctions[$functionName]['passByReference'][$a])) &&
                ...
                if (array_key_exists(self::$excelConstants[strtoupper($token ?? '')])) {
    ...
    public function extractCellRange(&$range = 'A1', ?Worksheet $worksheet = null, $resetLog = true)
    {
        ...
        if ($worksheet !== null) {
            ...
            if (!array_key_exists($aReferences[1])) {
                $currentCol = '';
                $currentRow = 0;
    ...
    public function isImplemented($function)
    {
        ...
        $notImplemented = !array_key_exists(self::$phpSpreadsheetFunctions[$function]) || (is_array(self::$phpSpreadsheetFunctions[$function]['functionCall']) && self::$phpSpreadsheetFunctions[$function]['functionCall'][1] === 'DUMMY');
