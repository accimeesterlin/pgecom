...
class HTMLPurifier_EntityParser
{
    ...
    protected function entityCallback($matches)
    {
        ...
        if ($hex_part !== NULL && $hex_part !== "") {
            ...
            if (array_key_exists($this->_entity_lookup->table[$named_part])) {
    ...
    protected function nonSpecialEntityCallback($matches)
    {
        ...
        if ($is_num) {
            ...
            if (array_key_exists($this->_special_dec2str[$code])) {
                return $entity;
            }
            return HTMLPurifier_Encoder::unichr($code);
        } else {
            if (array_key_exists($this->_special_ent2dec[$matches[3]])) {
                return $entity;
            }
            if (!$this->_entity_lookup) {
                $this->_entity_lookup = HTMLPurifier_EntityLookup::instance();
            }
            if (array_key_exists($this->_entity_lookup->table[$matches[3]])) {
    ...
    protected function specialEntityCallback($matches)
    {
        ...
        if ($is_num) {
            ...
            return array_key_exists($this->_special_dec2str[$int]) ?
                $this->_special_dec2str[$int] :
                $entity;
        } else {
            return array_key_exists($this->_special_ent2dec[$matches[3]]) ?
