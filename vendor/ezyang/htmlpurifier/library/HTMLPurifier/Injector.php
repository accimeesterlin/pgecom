...
abstract class HTMLPurifier_Injector
{
    ...
    public function checkNeeded($config)
    {
        ...
        foreach ($this->needed as $element => $attributes) {
            ...
            if (!array_key_exists($def->info[$element])) {
                return $element;
            }
            ...
            foreach ($attributes as $name) {
                if (!array_key_exists($def->info[$element]->attr[$name])) {
                    return "$element.$name";
                }
            }
        }
    ...
    public function allowsElement($name)
    {
        ...
        if (!array_key_exists($parent->child->elements[$name]) || array_key_exists($parent->excludes[$name])) {
            return false;
        }
        // check for exclusion
        if (!empty($this->currentNesting)) {
            for ($i = count($this->currentNesting) - 2; $i >= 0; $i--) {
                ...
                if (array_key_exists($def->excludes[$name])) {
                    return false;
                }
            }
        }
