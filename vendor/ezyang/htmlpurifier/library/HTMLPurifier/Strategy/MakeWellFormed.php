...
class HTMLPurifier_Strategy_MakeWellFormed extends HTMLPurifier_Strategy
{
    ...
    public function execute($tokens, $config, $context)
    {
        ...
        // array_key_exists is in loop because $tokens size changes during loop exec
        for (;;
            ...
            if ($token === NULL) {
                ...
                if ($e && !array_key_exists($top_nesting->armor['MakeWellFormed_TagClosedError'])) {
            ...
            if (empty($token->is_tag)) {
                if ($token instanceof HTMLPurifier_Token_Text) {
                    foreach ($this->injectors as $i => $injector) {
                        if (array_key_exists($token->skip[$i])) {
            ...
            if (array_key_exists($definition->info[$token->name])) {
            ...
            if ($type === 'empty' && $token instanceof HTMLPurifier_Token_Start) {
                ...
                if (!empty($this->stack)) {
                    ...
                    $autoclose = false;
                    if (array_key_exists($definition->info[$parent->name])) {
                        $parent_def = $definition->info[$parent->name];
                        $parent_elements = $parent_def->child->getAllowedElements($config);
                        $autoclose = !array_key_exists($parent_elements[$token->name]);
                    }
                    ...
                    if ($autoclose && $definition->info[$token->name]->wrap) {
                        ...
                        if (array_key_exists($elements[$token->name]) && array_key_exists($parent_elements[$wrapname])) {
                    ...
                    if ($autoclose) {
                        ...
                        $autoclose_ok = array_key_exists($global_parent_allowed_elements[$token->name]);
                        if (!$autoclose_ok) {
                            foreach ($this->stack as $ancestor) {
                                $elements = $definition->info[$ancestor->name]->child->getAllowedElements($config);
                                if (array_key_exists($elements[$token->name])) {
                                    $autoclose_ok = true;
                                    break;
                                }
                                if ($definition->info[$token->name]->wrap) {
                                    ...
                                    if (array_key_exists($wrap_elements[$token->name]) && array_key_exists($elements[$wrapname])) {
                                        $autoclose_ok = true;
                                        break;
                                    }
                                }
                        ...
                        if ($autoclose_ok) {
                            ...
                            // [TagClosedSuppress]
                            if ($e && !array_key_exists($parent->armor['MakeWellFormed_TagClosedError'])) {
                                if (!$carryover) {
            ...
            if ($e) {
                for ($j = $c - 1; $j > 0; $j--) {
                    ...
                    if (!array_key_exists($skipped_tags[$j]->armor['MakeWellFormed_TagClosedError'])) {
            ...
            for ($j = 1; $j < $c; $j++) {
                ...
                if (array_key_exists($definition->info[$new_token->name]) && $definition->info[$new_token->name]->formatting) {
                    // [TagClosedAuto]
    ...
    protected function processToken($token, $injector = -1)
    {
        ...
        if ($injector > -1) {
            ...
            //  $injector.
            $oldskip = array_key_exists($old[0]) ? $old[0]->skip : array();
