...
class HTMLPurifier_Strategy_RemoveForeignElements extends HTMLPurifier_Strategy
{
    ...
    public function execute($tokens, $config, $context)
    {
        ...
        } elseif ($remove_script_contents === false && array_key_exists($hidden_elements['script'])) {
        ...
        foreach ($tokens as $token) {
            ...
            if (!empty($token->is_tag)) {
                ...
                if (array_key_exists($definition->info_tag_transform[$token->name])) {
                ...
                if (array_key_exists($definition->info[$token->name])) {
                    ...
                    if (($token instanceof HTMLPurifier_Token_Start || $token instanceof HTMLPurifier_Token_Empty) &&
                        ...
                        foreach ($definition->info[$token->name]->required_attr as $name) {
                            if (!array_key_exists($token->attr[$name])) {
                                $ok = false;
                                break;
                            }
                        }
                    ...
                    if (array_key_exists($hidden_elements[$token->name]) && $token instanceof HTMLPurifier_Token_Start) {
                    ...
                    // CAN BE GENERICIZED
                    if (array_key_exists($hidden_elements[$token->name])) {
