...
class HTMLPurifier_LanguageFactory
{
    ...
    public function loadLanguage($code)
    {
        ...
        if (array_key_exists($this->cache[$code])) {
            return;
        }
        ...
        if (!empty($fallback)) {
            ...
            if (array_key_exists($languages_seen[$code])) {
                trigger_error(
            ...
            foreach ($this->keys as $key) {
                if (array_key_exists($cache[$key]) && array_key_exists($fallback_cache[$key])) {
                    if (array_key_exists($this->mergeable_keys_map[$key])) {
                        $cache[$key] = $cache[$key] + $fallback_cache[$key];
                    } elseif (array_key_exists($this->mergeable_keys_list[$key])) {
