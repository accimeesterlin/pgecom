...
class HTMLPurifier_Lexer_DOMLex extends HTMLPurifier_Lexer
{
    ...
    protected function tokenizeDOM($node, &$tokens, $config)
    {
        ...
        do {
            ...
            $level--;
            if ($level && array_key_exists($closingNodes[$level])) {
    ...
    {
        if (array_key_exists($node->tagName)) {
            return $node->tagName;
        } else if (array_key_exists($node->nodeName)) {
            return $node->nodeName;
        } else if (array_key_exists($node->localName)) {
    ...
    protected function getData($node)
    {
        if (array_key_exists($node->data)) {
            return $node->data;
        } else if (array_key_exists($node->nodeValue)) {
            return $node->nodeValue;
        } else if (array_key_exists($node->textContent)) {
