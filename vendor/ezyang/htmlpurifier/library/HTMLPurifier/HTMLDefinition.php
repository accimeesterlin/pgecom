...
class HTMLPurifier_HTMLDefinition extends HTMLPurifier_Definition
{
    ...
    public function addAttribute($element_name, $attr_name, $def)
    {
        ...
        if (!array_key_exists($module->info[$element_name])) {
    ...
    protected function setupConfigStuff($config)
    {
        ...
        if (array_key_exists($this->info_content_sets['Block'][$block_wrapper])) {
        ...
        if (is_array($allowed_elements)) {
            foreach ($this->info as $name => $d) {
                if (!array_key_exists($allowed_elements[$name])) {
        ...
        if (is_array($allowed_attributes)) {
            ...
            foreach ($this->info_global_attr as $attr => $x) {
                ...
                $delete = true;
                foreach ($keys as $key) {
                    if ($delete && array_key_exists($allowed_attributes[$key])) {
                        $delete = false;
                    }
                    if (array_key_exists($allowed_attributes_mutable[$key])) {
                ...
                switch ($c) {
                    case 2:
                        if ($bits[0] !== '*') {
                            ...
                            if (!array_key_exists($this->info[$element])) {
                                trigger_error(
        ...
        foreach ($this->info as $tag => $info) {
            if (array_key_exists($forbidden_elements[$tag])) {
                unset($this->info[$tag]);
                continue;
            }
            foreach ($info->attr as $attr => $x) {
                if (array_key_exists($forbidden_attributes["$tag@$attr"]) ||
                    array_key_exists($forbidden_attributes["*@$attr"]) ||
                    array_key_exists($forbidden_attributes[$attr])
                ) {
                    unset($this->info[$tag]->attr[$attr]);
                    continue;
                } elseif (array_key_exists($forbidden_attributes["$tag.$attr"])) { // this segment might get removed eventually
