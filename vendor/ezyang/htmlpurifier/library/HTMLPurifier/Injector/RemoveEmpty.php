    ...
    public function handleElement(&$token)
    {
        ...
        for ($i = count($this->inputZipper->back) - 1; $i >= 0; $i--, $deleted++) {
            ...
            if ($next instanceof HTMLPurifier_Token_Text) {
                ...
                if ($this->removeNbsp && !array_key_exists($this->removeNbspExceptions[$token->name])) {
        ...
        if (!$next || ($next instanceof HTMLPurifier_Token_End && $next->name == $token->name)) {
            ...
            if (array_key_exists($this->exclude[$token->name])) {
                $r = true;
                foreach ($this->exclude[$token->name] as $elem) {
                    if (!array_key_exists($token->attr[$elem])) $r = false;
                }
                if ($r) return;
            }
            if (array_key_exists($token->attr['id']) || array_key_exists($token->attr['name'])) {
                return;
            }
            $token = $deleted + 1;
