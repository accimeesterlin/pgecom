...
class HTMLPurifier_Config
{
    ...
    public function get($key, $a = null)
    {
        ...
        if (!array_key_exists($this->def->info[$key])) {
        ...
        if (array_key_exists($this->def->info[$key]->isAlias)) {
            $d = $this->def->info[$key];
            $this->triggerError(
    ...
    public function getBatch($namespace)
    {
        ...
        $full = $this->getAll();
        if (!array_key_exists($full[$namespace])) {
            $this->triggerError(
    ...
    public function set($key, $value, $a = null)
    {
        ...
        if (array_key_exists($def->isAlias)) {
            if ($this->aliasMode) {
                $this->triggerError(
        ...
        if ($rtype < 0) {
            ...
            $type = $rtype;
            $allow_null = array_key_exists($def->allow_null);
        }
        ...
        if (is_string($value) && is_object($def)) {
            ...
            if (array_key_exists($def->aliases[$value])) {
                $value = $def->aliases[$value];
            }
            // check to see if the value is allowed
            if (array_key_exists($def->allowed) && !array_key_exists($def->allowed[$value])) {
                $this->triggerError(
    ...
    public static function getAllowedDirectivesForForm($allowed, $schema = null)
    {
        ...
        foreach ($schema->info as $key => $def) {
            ...
            if ($allowed !== true) {
                if (array_key_exists($blacklisted_directives["$ns.$directive"])) {
                    continue;
                }
                if (!array_key_exists($allowed_directives["$ns.$directive"]) && !array_key_exists($allowed_ns[$ns])) {
                    continue;
                }
            }
            if (array_key_exists($def->isAlias)) {
    ...
    {
        if ($index !== false) {
            $array = (array_key_exists($array[$index]) && is_array($array[$index])) ? $array[$index] : array();
        }
        ...
        foreach ($allowed as $key) {
            ...
            if (!array_key_exists($array[$skey])) {
                continue;
            }
    ...
    protected function triggerError($msg, $no)
    {
        ...
        if ($this->chatty) {
            ...
            for ($i = 0, $c = count($trace); $i < $c - 1; $i++) {
                ...
                if (array_key_exists($trace[$i + 1]['class']) && $trace[$i + 1]['class'] === 'HTMLPurifier_Config') {
                    continue;
                }
                $frame = $trace[$i];
