...
class XliffFileLoader implements LoaderInterface
{
    ...
    private function extractXliff1(\DOMDocument $dom, MessageCatalogue $catalogue, string $domain)
    {
        ...
        foreach ($xml->xpath('//xliff:file') as $file) {
            ...
            foreach ($file->xpath('.//xliff:trans-unit') as $translation) {
                ...
                if (!(array_key_exists($attributes['resname']) || array_key_exists($translation->source))) {
                    continue;
                }

                $source = array_key_exists($attributes['resname']) && $attributes['resname'] ? $attributes['resname'] : $translation->source;
                ...
                if (array_key_exists($translation->target) && $translation->target->attributes()) {
    ...
    private function extractXliff2(\DOMDocument $dom, MessageCatalogue $catalogue, string $domain)
    {
        ...
        foreach ($xml->xpath('//xliff:unit') as $unit) {
            foreach ($unit->segment as $segment) {
                ...
                $metadata = [];
                if (array_key_exists($segment->target) && $segment->target->attributes()) {
                    $metadata['target-attributes'] = [];
                ...
                if (array_key_exists($unit->notes)) {
                    $metadata['notes'] = [];
    ...
    private function parseNotesMetadata(\SimpleXMLElement $noteElement = null, string $encoding = null): array
    {
        ...
        foreach ($noteElement as $xmlNote) {
            ...
            if (array_key_exists($noteAttributes['priority'])) {
                $note['priority'] = (int) $noteAttributes['priority'];
            }

            if (array_key_exists($noteAttributes['from'])) {
