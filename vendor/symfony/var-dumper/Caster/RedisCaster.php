...
class RedisCaster
{
    ...
    public static function castRedis(\Redis $c, array $a, Stub $stub, bool $isNested)
    {
        ...
        return $a + [
            ...
            $prefix.'mode' => array_key_exists(self::MODES[$mode]) ? new ConstStub(self::MODES[$mode], $mode) : $mode,
    ...
    public static function castRedisCluster(\RedisCluster $c, array $a, Stub $stub, bool $isNested)
    {
        ...
        $a += [
            ...
            $prefix.'lastError' => $c->getLastError(),
            $prefix.'options' => self::getRedisOptions($c, [
                'SLAVE_FAILOVER' => array_key_exists(self::FAILOVER_OPTIONS[$failover]) ? new ConstStub(self::FAILOVER_OPTIONS[$failover], $failover) : $failover,
            ]),
        ];
    ...
    private static function getRedisOptions($redis, array $options = []): EnumStub
    {
        ...
        if (\is_array($serializer)) {
            foreach ($serializer as &$v) {
                if (array_key_exists(self::SERIALIZERS[$v])) {
                    $v = new ConstStub(self::SERIALIZERS[$v], $v);
                }
            }
        } elseif (array_key_exists(self::SERIALIZERS[$serializer])) {
        ...
        if (\is_array($compression)) {
            foreach ($compression as &$v) {
                if (array_key_exists(self::COMPRESSION_MODES[$v])) {
                    $v = new ConstStub(self::COMPRESSION_MODES[$v], $v);
                }
            }
        } elseif (array_key_exists(self::COMPRESSION_MODES[$compression])) {
