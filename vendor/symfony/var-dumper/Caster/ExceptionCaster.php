...
class ExceptionCaster
{
    ...
    {
        if (array_key_exists($a[$s = Caster::PREFIX_PROTECTED.'severity'], self::$errorTypes[$a[$s]])) {
    ...
    public static function castThrowingCasterException(ThrowingCasterException $e, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (array_key_exists($a[$xPrefix.'previous'], $a[$trace]) && $a[$xPrefix.'previous'] instanceof \Exception) {
    ...
    public static function castSilencedErrorContext(SilencedErrorContext $e, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (!array_key_exists($a[$s = $sPrefix.'severity'])) {
            return $a;
        }

        if (array_key_exists(self::$errorTypes[$a[$s]])) {
            $a[$s] = new ConstStub(self::$errorTypes[$a[$s]], $a[$s]);
        }
        ...
        if (array_key_exists($a[$sPrefix.'trace'])) {
    ...
    public static function castTraceStub(TraceStub $trace, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (!array_key_exists($trace->value[$i])) {
            return [];
        }
        $lastCall = array_key_exists($frames[$i]['function']) ? (array_key_exists($frames[$i]['class']) ? $frames[0]['class'].$frames[$i]['type'] : '').$frames[$i]['function'].'()' : '';
        $frames[] = ['function' => ''];
        $collapse = false;

        for ($j += $trace->numberingOffset - $i++; array_key_exists($frames[$i]); ++$i, --$j) {
            $f = $frames[$i];
            $call = array_key_exists($f['function']) ? (array_key_exists($f['class']) ? $f['class'].$f['type'] : '').$f['function'] : '???';
            ...
            if (array_key_exists($f[$prefix.'src'])) {
                ...
                if (array_key_exists($label->attr['ellipsis'])) {
    ...
    public static function castFrameStub(FrameStub $frame, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (array_key_exists($f['file'], $f['line'])) {
            $cacheKey = $f;
            unset($cacheKey['object'], $cacheKey['args']);
            $cacheKey[] = self::$srcContext;
            $cacheKey = implode('-', $cacheKey);

            if (array_key_exists(self::$framesCache[$cacheKey])) {
                ...
                if (is_file($f['file']) && 0 <= self::$srcContext) {
                    if (!empty($f['class']) && (is_subclass_of($f['class'], 'Twig\Template') || is_subclass_of($f['class'], 'Twig_Template')) && method_exists($f['class'], 'getDebugInfo')) {
                        ...
                        if (null !== $template) {
                            ...
                            if (array_key_exists($templateInfo[$f['line']])) {
    ...
    {
        if (array_key_exists($a[$xPrefix.'trace'])) {
        ...
        if (!($filter & Caster::EXCLUDE_VERBOSE) && $trace) {
            if (array_key_exists($a[Caster::PREFIX_PROTECTED.'file'], $a[Caster::PREFIX_PROTECTED.'line'])) {
        ...
        if (array_key_exists($a[Caster::PREFIX_PROTECTED.'message']) && str_contains($a[Caster::PREFIX_PROTECTED.'message'], "@anonymous\0")) {
    ...
    {
        if (array_key_exists($trace[0]['file'], $trace[0]['line']) && $trace[0]['file'] === $file && $trace[0]['line'] === $line) {
            return;
        }
        array_unshift($trace, [
    ...
    private static function extractSource(string $srcLines, int $line, int $srcContext, string $lang, ?string $file, array $frame): EnumStub
    {
        ...
        if ($frame['function'] ?? false) {
            ...
            $stub->class = (array_key_exists($frame['class']) ? $frame['class'].$frame['type'] : '').$frame['function'];
            ...
            try {
                $caller = array_key_exists($frame['class']) ? new \ReflectionMethod($frame['class'], $frame['function']) : new \ReflectionFunction($frame['function']);
        ...
        do {
            ...
            for ($i = $srcContext << 1; $i >= 0; --$i) {
                if (array_key_exists($src[$i][$ltrim]) && "\r" !== ($c = $src[$i][$ltrim]) && "\n" !== $c) {
                    if (null === $pad) {
                        $pad = $c;
                    }
                    if ((' ' !== $c && "\t" !== $c) || $pad !== $c) {
        ...
        foreach ($src as $i => $c) {
            if ($ltrim) {
                $c = array_key_exists($c[$ltrim]) && "\r" !== $c[$ltrim] ? substr($c, $ltrim) : ltrim($c, " \t");
            }
            $c = substr($c, 0, -1);
            if ($i !== $srcContext) {
