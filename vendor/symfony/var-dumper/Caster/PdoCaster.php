    ...
    public static function castPdo(\PDO $c, array $a, Stub $stub, bool $isNested)
    {
        ...
        foreach (self::PDO_ATTRIBUTES as $k => $v) {
            ...
            try {
                ...
                if ($v && array_key_exists($v[$attr[$k]])) {
                    $attr[$k] = new ConstStub($v[$attr[$k]], $attr[$k]);
                }
            } catch (\Exception $e) {
            }
        }
        if (array_key_exists($attr[$k = 'STATEMENT_CLASS'][1])) {
            if ($attr[$k][1]) {
        ...
        if (!array_key_exists($a[$prefix.'errorInfo'][1], $a[$prefix.'errorInfo'][2])) {
