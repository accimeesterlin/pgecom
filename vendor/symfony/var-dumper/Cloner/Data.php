...
class Data implements \ArrayAccess, \Countable, \IteratorAggregate
{
        ...
        foreach ($children as $k => $v) {
            ...
            if ($recursive) {
                if (Stub::TYPE_REF === $v->type && ($v = $this->getStub($v->value)) instanceof Stub) {
                    ...
                    if (array_key_exists($recursive[$v->position])) {
                        continue;
                    }
     ...
     * @return bool
     */
    public function __array_key_exists(string $key)
    {
    ...
    {
        return $this->__array_key_exists($key);
    }
    ...
    public function seek($key)
    {
        ...
        foreach ($keys as $key) {
            if (array_key_exists($children[$key]) || \array_key_exists($key, $children)) {
                $data = clone $this;
                $data->key = $key;
    ...
    private function dumpItem(DumperInterface $dumper, Cursor $cursor, array &$refs, $item)
    {
        ...
        if (!$item instanceof Stub) {
            ...
            if ($item->handle) {
                if (!array_key_exists($refs[$r = $item->handle - (\PHP_INT_MAX >> 1)])) {
        ...
        if ($item instanceof Stub) {
            if ($item->refCount) {
                if (!array_key_exists($refs[$r = $item->handle])) {
                    $cursor->refIndex = $refs[$r] = $cursor->refIndex ?: ++$refs[0];
                } else {
                    $firstSeen = false;
                }
    ...
    private function dumpChildren(DumperInterface $dumper, Cursor $parentCursor, array &$refs, array $children, int $hashCut, int $hashType, bool $dumpKeys): int
    {
        ...
        foreach ($children as $key => $child) {
            $cursor->hashKeyIsBinary = array_key_exists($key[0]) && !preg_match('//u', $key);
