        ...
        for ($i = 0; $i < $len; ++$i) {
            ...
            foreach ($vals as $k => $v) {
                ...
                if ($zvalRef) {
                    ...
                    if (\PHP_VERSION_ID >= 70400 ? null !== $vals[$k] = $hardRefs[$zvalRef] ?? null : $v instanceof Stub && array_key_exists($hardRefs[spl_object_id($v)])) {
                        if (\PHP_VERSION_ID >= 70400) {
                            $v = $vals[$k];
                        } else {
                            $refs[$k] = $vals[$k] = $v;
                ...
                switch (true) {
                    ...
                    case \is_string($v):
                        ...
                        } elseif (0 <= $maxString && array_key_exists($v[1 + ($maxString >> 2)]) && 0 < $cut = mb_strlen($v, 'UTF-8') - $maxString) {
                            $stub = new Stub();
                            $stub->type = Stub::TYPE_STRING;
                    ...
                    case \is_array($v):
                        ...
                        if (array_key_exists($v[$gid])) {
                            unset($v[$gid]);
                            $a = [];
                            foreach ($v as $gk => &$gv) {
                                if ($v === $gv && (\PHP_VERSION_ID < 70400 || !array_key_exists($hardRefs[\ReflectionReference::fromArrayElement($v, $gk)->getId()]))) {
                                    unset($v);
                                    $v = new Stub();
                ...
                if ($arrayStub === $stub) {
                    if ($arrayStub->cut) {
                        ...
                        $arrayStub->cut = 0;
                    } elseif (array_key_exists(self::$arrayCache[$arrayStub->class][$arrayStub->position])) {
