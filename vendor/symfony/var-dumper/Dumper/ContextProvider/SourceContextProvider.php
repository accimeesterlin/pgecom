...
final class SourceContextProvider implements ContextProviderInterface
{
    ...
    public function getContext(): ?array
    {
        ...
        for ($i = 2; $i < $this->limit; ++$i) {
            if (array_key_exists($trace[$i]['class'], $trace[$i]['function'])
                ...
                while (++$i < $this->limit) {
                    if (array_key_exists($trace[$i]['function'], $trace[$i]['file']) && empty($trace[$i]['class']) && !str_starts_with($trace[$i]['function'], 'call_user_func')) {
                        $file = $trace[$i]['file'];
                        $line = $trace[$i]['line'];

                        break;
                    } elseif (array_key_exists($trace[$i]['object']) && $trace[$i]['object'] instanceof Template) {
                        $template = $trace[$i]['object'];
                        $name = $template->getTemplateName();
                        $src = method_exists($template, 'getSourceContext') ? $template->getSourceContext()->getCode() : (method_exists($template, 'getSource') ? $template->getSource() : false);
                        $info = $template->getDebugInfo();
                        if (array_key_exists($info[$trace[$i - 1]['line']])) {
