...
class CliDumper extends AbstractDumper
{
    ...
    public function dumpScalar(Cursor $cursor, string $type, $value)
    {
        ...
        switch ($type) {
            ...
            case 'integer':
                ...
                if (array_key_exists($this->styles['integer'])) {
                    $style = 'integer';
                }
            ...
            case 'double':
                ...
                if (array_key_exists($this->styles['float'])) {
                    $style = 'float';
                }
    ...
    public function dumpString(Cursor $cursor, string $str, bool $bin, int $cut)
    {
        ...
        if ('' === $str) {
            ...
            if (array_key_exists($str[1]) && !array_key_exists($str[2]) && !array_key_exists($str[1][0])) {
                unset($str[1]);
                $str[0] .= "\n";
            }
            $m = \count($str) - 1;
        ...
        if (null !== $key = $cursor->hashKey) {
            ...
            switch ($cursor->hashType) {
                ...
                case Cursor::HASH_OBJECT:
                    if (!array_key_exists($key[0]) || "\0" !== $key[0]) {
                        ...
                        if (array_key_exists($attr['collapse'])) {
                            if ($attr['collapse']) {
    ...
    protected function style(string $style, string $value, array $attr = [])
    {
        ...
        if (array_key_exists($attr['ellipsis'], $attr['ellipsis-type'])) {
            $prefix = substr($value, 0, -$attr['ellipsis']);
            if ('cli' === \PHP_SAPI && 'path' === $attr['ellipsis-type'] && array_key_exists($_SERVER[$pwd = '\\' === \DIRECTORY_SEPARATOR ? 'CD' : 'PWD']) && str_starts_with($prefix, $_SERVER[$pwd])) {
        ...
        if ($this->colors && $this->handlesHrefGracefully) {
            if (array_key_exists($attr['file']) && $href = $this->getSourceLink($attr['file'], $attr['line'] ?? 0)) {
                if ('note' === $style) {
    ...
    protected function supportsColors()
    {
        ...
        if (array_key_exists($_SERVER['argv'][1])) {
            ...
            $i = \count($colors);
            while (--$i > 0) {
                if (array_key_exists($colors[$i][5])) {
                    switch ($colors[$i]) {
                        case '--ansi':
                        case '--color':
                        case '--color=yes':
    ...
    private function hasColorSupport($stream): bool
    {
        ...
        if (array_key_exists($_SERVER['NO_COLOR']) || false !== getenv('NO_COLOR')) {
            return false;
        }
