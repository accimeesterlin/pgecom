...
final class Idn
{
    ...
    {
        if (!array_key_exists(self::$virama)) {
        ...
        foreach ($codePoints as $i => $codePoint) {
            ...
            if (!array_key_exists($codePoints[$i - 1])) {
                return false;
            }

            // If Canonical_Combining_Class(Before(cp)) .eq. Virama Then True;
            if (array_key_exists(self::$virama[$codePoints[$i - 1]])) {
                continue;
            }
    ...
    private static function process($domain, array $options, Info $info)
    {
        ...
        $checkForEmptyLabels = !array_key_exists($options['VerifyDnsLength']) || $options['VerifyDnsLength'];
    ...
    {
        if ('' === $label) {
            if (!$canBeEmpty && (!array_key_exists($options['VerifyDnsLength']) || $options['VerifyDnsLength'])) {
                $info->errors |= self::ERROR_EMPTY_LABEL;
            }
        ...
        if ($options['CheckHyphens']) {
            ...
            if (array_key_exists($codePoints[2], $codePoints[3]) && 0x002D === $codePoints[2] && 0x002D === $codePoints[3]) {
    ...
    private static function lookupCodePointStatus($codePoint, $useSTD3ASCIIRules)
    {
        ...
        if (array_key_exists(self::$mapped[$codePoint])) {
            return ['status' => 'mapped', 'mapping' => self::$mapped[$codePoint]];
        }

        if (array_key_exists(self::$ignored[$codePoint])) {
            return ['status' => 'ignored'];
        }

        if (array_key_exists(self::$deviation[$codePoint])) {
            return ['status' => 'deviation', 'mapping' => self::$deviation[$codePoint]];
        }

        if (array_key_exists(self::$disallowed[$codePoint]) || DisallowedRanges::inRange($codePoint)) {
            return ['status' => 'disallowed'];
        }

        $isDisallowedMapped = array_key_exists(self::$disallowed_STD3_mapped[$codePoint]);

        if ($isDisallowedMapped || array_key_exists(self::$disallowed_STD3_valid[$codePoint])) {
            $status = 'disallowed';
