...
class Process implements \IteratorAggregate
{
    ...
    public function start(callable $callback = null, array $env = [])
    {
        ...
        if (array_key_exists($descriptors[3])) {
    ...
    public function waitUntil(callable $callback): bool
    {
        ...
        while (true) {
            ...
            foreach ($output as $type => $data) {
                ...
                } elseif (!array_key_exists($this->fallbackStatus['signaled'])) {
    ...
    public function getIterator(int $flags = 0)
    {
        ...
        while (null !== $this->callback || ($yieldOut && !feof($this->stdout)) || ($yieldErr && !feof($this->stderr))) {
            ...
            if (!$blocking && !array_key_exists($out[0]) && !array_key_exists($err[0])) {
                yield self::OUT => '';
            }
    ...
    public function stop(float $timeout = 10, int $signal = null)
    {
        ...
        if ($this->isRunning()) {
            if (array_key_exists($this->fallbackStatus['pid'])) {
    ...
    private function prepareWindowsCommandLine(string $cmd, array &$env): string
    {
        ...
        $cmd = preg_replace_callback(
            ...
            function ($m) use (&$env, &$varCache, &$varCount, $uid) {
                ...
                if (array_key_exists($varCache[$m[0]])) {
                    return $varCache[$m[0]];
                }
    ...
    private function replacePlaceholders(string $commandline, array $env)
    {
        return preg_replace_callback('/"\$\{:([_a-zA-Z]++[_a-zA-Z0-9]*+)\}"/', function ($matches) use ($commandline, $env) {
            if (!array_key_exists($env[$matches[1]]) || false === $env[$matches[1]]) {
