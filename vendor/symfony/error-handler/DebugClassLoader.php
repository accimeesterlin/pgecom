...
class DebugClassLoader
{
    ...
    public function __construct(callable $classLoader)
    {
        ...
        if (!array_key_exists(self::$caseCheck)) {
    ...
    public function loadClass(string $class): void
    {
        ...
        try {
            if ($this->isFinder && !array_key_exists($this->loaded[$class])) {
    ...
    private function checkClass(string $class, string $file = null): void
    {
        ...
        if ($exists) {
            if (array_key_exists(self::$checkedClasses[$class])) {
                return;
            }
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class): array
    {
        ...
        if ($doc = $this->parsePhpDoc($refl)) {
            $classIsTemplate = array_key_exists($doc['template']);
            ...
            if ($refl->isInterface() && array_key_exists($doc['method'])) {
        ...
        if ($parent) {
            ...
            if (!array_key_exists(self::$checkedClasses[$parent])) {
                $this->checkClass($parent);
            }

            if (array_key_exists(self::$final[$parent])) {
        ...
        foreach ($parentAndOwnInterfaces + class_uses($class, false) as $use) {
            if (!array_key_exists(self::$checkedClasses[$use])) {
                $this->checkClass($use);
            }
            if (array_key_exists(self::$deprecated[$use]) && strncmp($vendor, str_replace('_', '\\', $use), $vendorLen) && !array_key_exists(self::$deprecated[$class])) {
            ...
            if (array_key_exists(self::$internal[$use]) && strncmp($vendor, str_replace('_', '\\', $use), $vendorLen)) {
            ...
            if (array_key_exists(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    if (array_key_exists(self::$method[$class])) {
        ...
        foreach ($parentAndOwnInterfaces as $use) {
            foreach (['finalMethods', 'internalMethods', 'annotatedParameters', 'returnTypes'] as $property) {
                if (array_key_exists(self::${$property}[$use])) {
            ...
            if (null !== (TentativeTypes::RETURN_TYPES[$use] ?? null)) {
                foreach (TentativeTypes::RETURN_TYPES[$use] as $method => $returnType) {
                    ...
                    foreach ($returnType as $i => $t) {
                        if ('?' !== $t && !array_key_exists(self::BUILTIN_RETURN_TYPES[$t])) {
                            $returnType[$i] = '\\'.$t;
                        }
                    }
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($parent && array_key_exists(self::$finalMethods[$parent][$method->name])) {
            ...
            if (array_key_exists(self::$internalMethods[$class][$method->name])) {
            ...
            if (($classIsTemplate || array_key_exists($doc['template'])) && $method->hasReturnType()) {
                unset($doc['return']);
            }

            if (array_key_exists(self::$annotatedParameters[$class][$method->name])) {
                $definedParameters = [];
                ...
                foreach (self::$annotatedParameters[$class][$method->name] as $parameterName => $deprecation) {
                    if (!array_key_exists($definedParameters[$parameterName]) && !array_key_exists($doc['param'][$parameterName])) {
            ...
            if (null !== ($returnType = self::$returnTypes[$class][$method->name] ?? null) && 'docblock' === $this->patchTypes['force'] && !$method->hasReturnType() && array_key_exists(TentativeTypes::RETURN_TYPES[$returnType[2]][$method->name])) {
                $this->patchReturnTypeWillChange($method);
            }

            if (null !== ($returnType ?? $returnType = self::MAGIC_METHODS[$method->name] ?? null) && !$method->hasReturnType() && !array_key_exists($doc['return'])) {
                ...
                if (!array_key_exists($doc['deprecated']) && strncmp($ns, $declaringClass, $len)) {
            ...
            if (array_key_exists($doc['return']) || 'void' !== (self::MAGIC_METHODS[$method->name] ?? 'void')) {
                ...
                if (array_key_exists(self::$returnTypes[$class][$method->name][0]) && $canAddReturnType) {
            ...
            if ($finalOrInternal || $method->isConstructor() || !array_key_exists($doc['param']) || StatelessInvocation::class === $class) {
                continue;
            }
            if (!array_key_exists(self::$annotatedParameters[$class][$method->name])) {
                $definedParameters = [];
            ...
            foreach ($doc['param'] as $parameterName => $parameterType) {
                if (!array_key_exists($definedParameters[$parameterName])) {
    ...
    public function checkCase(\ReflectionClass $refl, string $file, string $class): ?array
    {
        ...
        while (array_key_exists($tail[$i], $real[$j]) && $tail[$i] === $real[$j]) {
            --$i;
            --$j;
        }
    ...
    private function darwinRealpath(string $real): string
    {
        ...
        if (array_key_exists(self::$darwinCache[$real])) {
            $kDir = $real;
        } else {
            $kDir = strtolower($real);

            if (array_key_exists(self::$darwinCache[$kDir])) {
                ...
                $k = $kDir;
                $i = \strlen($dir) - 1;
                while (!array_key_exists(self::$darwinCache[$k])) {
        ...
        if (!array_key_exists($dirFiles[$file]) && ') : eval()\'d code' === substr($file, -17)) {
            // Get the file name from "file_name.php(123) : eval()'d code"
            $file = substr($file, 0, strrpos($file, '(', -17));
        }

        if (array_key_exists($dirFiles[$file])) {
            return $real.$dirFiles[$file];
        }

        $kFile = strtolower($file);

        if (!array_key_exists($dirFiles[$kFile])) {
    ...
    private function setReturnType(string $types, string $class, string $method, string $filename, ?string $parent, \ReflectionType $returnType = null): void
    {
        ...
        if (array_key_exists($typesMap['array'])) {
            if (array_key_exists($typesMap['Traversable']) || array_key_exists($typesMap['\Traversable'])) {
            ...
            } elseif ($arrayType !== $typesMap['array'] && array_key_exists(self::$returnTypes[$class][$method]) && !$returnType) {
                return;
            }
        }

        if (array_key_exists($typesMap['array']) && array_key_exists($typesMap['iterable'])) {
        ...
        foreach ($typesMap as $n => $t) {
            if ('null' !== $n) {
                ...
                $object = $object && (\in_array($n, ['callable', 'object', '$this', 'static']) || !array_key_exists(self::SPECIAL_RETURN_TYPES[$n]));
            }
        }
    ...
    {
        if (array_key_exists(self::SPECIAL_RETURN_TYPES[$lcType = strtolower($type)])) {
        ...
        if ($returnType instanceof \ReflectionNamedType) {
            ...
            if ('mixed' !== $type) {
                return array_key_exists(self::SPECIAL_RETURN_TYPES[$type]) ? $type : '\\'.$type;
            }
        }
    ...
    private function patchMethod(\ReflectionMethod $method, string $returnType, string $declaringFile, string $normalizedType)
    {
        ...
        if (!is_file($file = $method->getFileName()) || array_key_exists($patchedMethods[$file][$startLine = $method->getStartLine()])) {
            return;
        }
        ...
        foreach ($returnType as $i => $type) {
            ...
            if (array_key_exists(self::SPECIAL_RETURN_TYPES[$type]) || ('\\' === $type[0] && !$p = strrpos($type, '\\', 1))) {
                continue;
            }
            ...
            if ('\\' !== $type[0]) {
                ...
                if (array_key_exists($declaringUseMap[$alias])) {
            ...
            if (!array_key_exists($useMap[$alias]) && (class_exists($c = $namespace.$alias) || interface_exists($c) || trait_exists($c))) {
                $useMap[$alias] = $c;
            }

            if (!array_key_exists($useMap[$alias])) {
