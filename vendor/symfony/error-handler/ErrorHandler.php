...
class ErrorHandler
{
    ...
    public function setLoggers(array $loggers): array
    {
        ...
        if ($flush) {
            foreach ($this->bootstrappingLogger->cleanLogs() as $log) {
                ...
                if (!array_key_exists($flush[$type])) {
    ...
    public function handleError(int $type, string $message, string $file, int $line): bool
    {
        ...
        if (null !== self::$toStringException) {
            ...
            if (!array_key_exists(self::$silencedErrorCache[$id = $file.':'.$line])) {
                $lightTrace = $this->tracedErrors & $type ? $this->cleanTrace(debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS, 5), $type, $file, $line, false) : [];
                $errorAsException = new SilencedErrorContext($type, $file, $line, array_key_exists($lightTrace[1]) ? [$lightTrace[0]] : $lightTrace);
            } elseif (array_key_exists(self::$silencedErrorCache[$id][$message])) {
                $lightTrace = null;
            ...
            if (false !== strpos($message, '@anonymous')) {
                ...
                for ($i = 1; array_key_exists($backtrace[$i]); ++$i) {
                    if (array_key_exists($backtrace[$i]['function'], $backtrace[$i]['args'][0])
        ...
        if ($throw) {
            if (\PHP_VERSION_ID < 70400 && \E_USER_ERROR & $type) {
                for ($i = 1; array_key_exists($backtrace[$i]); ++$i) {
                    if (array_key_exists($backtrace[$i]['function'], $backtrace[$i]['type'], $backtrace[$i - 1]['function'])
                        && '__toString' === $backtrace[$i]['function']
                        && '->' === $backtrace[$i]['type']
                        && !array_key_exists($backtrace[$i - 1]['class'])
    ...
    private function cleanTrace(array $backtrace, int $type, string &$file, int &$line, bool $throw): array
    {
        ...
        for ($i = 0; array_key_exists($backtrace[$i]); ++$i) {
            if (array_key_exists($backtrace[$i]['file'], $backtrace[$i]['line']) && $backtrace[$i]['line'] === $line && $backtrace[$i]['file'] === $file) {
                $lightTrace = \array_slice($lightTrace, 1 + $i);
                break;
            }
        }
        if (\E_USER_DEPRECATED === $type) {
            for ($i = 0; array_key_exists($lightTrace[$i]); ++$i) {
                if (!array_key_exists($lightTrace[$i]['file'], $lightTrace[$i]['line'], $lightTrace[$i]['function'])) {
                    continue;
                }
                if (!array_key_exists($lightTrace[$i]['class']) && 'trigger_deprecation' === $lightTrace[$i]['function']) {
