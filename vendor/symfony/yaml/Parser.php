...
class Parser
{
    ...
    private function doParse(string $value, int $flags)
    {
        ...
        do {
            ...
            if ('-' === $this->currentLine[0] && self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                if (array_key_exists($values['value']) && '&' === $values['value'][0] && self::preg_match(self::REFERENCE_PATTERN, $values['value'], $matches)) {
                    $isRef = $matches['ref'];
                    $this->refsBeingParsed[] = $isRef;
                    $values['value'] = $matches['value'];
                }

                if (array_key_exists($values['value'][1]) && '?' === $values['value'][0] && ' ' === $values['value'][1]) {
                    throw new ParseException('Complex mappings are not supported.', $this->getRealCurrentLineNb() + 1, $this->currentLine);
                }

                // array
                if (array_key_exists($values['value']) && 0 === strpos(ltrim($values['value'], ' '), '-')) {
                    // Inline first child
                ...
                } elseif (!array_key_exists($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#')) {
                    ...
                    if (
                        array_key_exists($values['leadspaces'])
                        && (
                            '!' === $values['value'][0]
                ...
                if ('<<' === $key && (!array_key_exists($values['value']) || '&' !== $values['value'][0] || !self::preg_match('#^&(?P<ref>[^ ]+)#u', $values['value'], $refMatches))) {
                    $mergeNode = true;
                    $allowOverwrite = true;
                    if (array_key_exists($values['value'][0]) && '*' === $values['value'][0]) {
                        ...
                        if (array_key_exists($values['value']) && '' !== $values['value']) {
                            $value = $values['value'];
                        } else {
                ...
                } elseif ('<<' !== $key && array_key_exists($values['value']) && '&' === $values['value'][0] && self::preg_match(self::REFERENCE_PATTERN, $values['value'], $matches)) {
                    $isRef = $matches['ref'];
                    $this->refsBeingParsed[] = $isRef;
                    $values['value'] = $matches['value'];
                }
                ...
                if ($mergeNode) {
                    // Merge keys
                } elseif (!array_key_exists($values['value']) || '' === $values['value'] || 0 === strpos($values['value'], '#') || (null !== $subTag = $this->getLineTag($values['value'], $flags)) || '<<' === $key) {
                    // hash
                    // if next line is less indented or equal, then it means that the current value is null
                    if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
                        // Spec: Keys MUST be unique; first one wins.
                        // But overwriting is allowed when a merge node is used in current block.
                        if ($allowOverwrite || !array_key_exists($data[$key])) {
                            if (null !== $subTag) {
                        ...
                        if ('<<' === $key) {
                            ...
                            $data += $value;
                        } elseif ($allowOverwrite || !array_key_exists($data[$key])) {
                            // Spec: Keys MUST be unique; first one wins.
                            // But overwriting is allowed when a merge node is used in current block.
                            if (null !== $subTag) {
                ...
                if ($deprecatedUsage = (array_key_exists($this->currentLine[1]) && '?' === $this->currentLine[0] && ' ' === $this->currentLine[1])) {
                    throw new ParseException('Complex mappings are not supported.', $this->getRealCurrentLineNb() + 1, $this->currentLine);
                }
                ...
                if (0 === $this->currentLineNb) {
                    ...
                    foreach ($this->lines as $line) {
                        ...
                        if (0 === $this->offset && !$deprecatedUsage && array_key_exists($line[0]) && ' ' === $line[0]) {
                            throw new ParseException('Unable to parse.', $this->getRealCurrentLineNb() + 1, $this->currentLine, $this->filename);
                        }
    ...
    private function getNextEmbedBlock(int $indentation = null, bool $inSequence = false): string
    {
        ...
        if ($inSequence && $oldLineIndentation === $newIndent && array_key_exists($data[0][0]) && '-' === $data[0][0]) {
        ...
        try {
            ...
            switch ($value[0] ?? '') {
                ...
                case "'":
                    ...
                    if (array_key_exists($this->currentLine[$cursor]) && preg_replace('/\s*(#.*)?$/A', '', substr($this->currentLine, $cursor))) {
            ...
            for (; \strlen($this->currentLine) > $cursor; ++$cursor) {
                switch ($this->currentLine[$cursor]) {
                    case '\\':
                        if ("'" === $quotation) {
                            $value .= '\\';
                        } elseif (array_key_exists($this->currentLine[++$cursor])) {
                    ...
                    case $quotation:
                        ...
                        if ("'" === $quotation && array_key_exists($this->currentLine[$cursor]) && "'" === $this->currentLine[$cursor]) {
                            $value .= "''";
                            break;
                        }
    ...
    private function lexInlineStructure(int &$cursor, string $closingTag): string
    {
        ...
        do {
            ...
            while (array_key_exists($this->currentLine[$cursor])) {
                switch ($this->currentLine[$cursor]) {
                    case '"':
                    case "'":
    ...
    private function consumeWhitespaces(int &$cursor): bool
    {
        ...
        do {
            ...
            if (array_key_exists($this->currentLine[$cursor])) {
