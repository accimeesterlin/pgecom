...
class Inline
{
    ...
    public static function parse(string $value = null, int $flags = 0, array &$references = [])
    {
        ...
        try {
            ...
            if (array_key_exists($mbEncoding)) {
    ...
    private static function parseMapping(string $mapping, int $flags, int &$i = 0, array &$references = [])
    {
        ...
        while ($i < $len) {
            ...
            if (!$isKeyQuoted && (!array_key_exists($mapping[$i + 1]) || !\in_array($mapping[$i + 1], [' ', ',', '[', ']', '{', '}', "\n"], true))) {
            ...
            while ($i < $len) {
                ...
                switch ($mapping[$i]) {
                    case '[':
                        ...
                        } elseif ($allowOverwrite || !array_key_exists($output[$key])) {
                            if (null !== $tag) {
                                $output[$key] = new TaggedValue($tag, $value);
                            } else {
                                $output[$key] = $value;
                            }
                        } elseif (array_key_exists($output[$key])) {
    ...
    private static function parseTag(string $value, int &$i, int $flags): ?string
    {
        ...
        if ('' === $tag && (!array_key_exists($value[$nextOffset]) || \in_array($value[$nextOffset], [']', '}', ','], true))) {
        ...
        if ('' !== $tag && (!array_key_exists($value[$nextOffset]) || !\in_array($value[$nextOffset], ['[', '{'], true)) && ('!' === $tag[0] || 'str' === $tag || 'php/const' === $tag || 'php/object' === $tag)) {
        ...
        if ('' !== $tag && !array_key_exists($value[$i])) {
