...
class Store implements StoreInterface
{
    ...
    public function lock(Request $request)
    {
        ...
        if (!array_key_exists($this->locks[$key])) {
    ...
    public function lookup(Request $request)
    {
        ...
        foreach ($entries as $entry) {
            if ($this->requestsMatch(array_key_exists($entry[1]['vary'][0]) ? implode(', ', $entry[1]['vary']) : '', $request->headers->all(), $entry[0])) {
                $match = $entry;
    ...
    public function write(Request $request, Response $response)
    {
        ...
        foreach ($this->getMetadata($key) as $entry) {
            if (!array_key_exists($entry[1]['vary'][0])) {
                $entry[1]['vary'] = [''];
            }
    ...
    {
        if (array_key_exists($this->keyCache[$request])) {
