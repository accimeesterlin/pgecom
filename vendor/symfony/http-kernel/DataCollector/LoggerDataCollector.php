...
class LoggerDataCollector extends DataCollector implements LateDataCollectorInterface
{
    ...
    public function getProcessedLogs()
    {
        ...
        foreach ($this->getLogs()->getValue() as $rawLog) {
            ...
            if ($rawLogData['priority']->getValue() > 300) {
                $logType = 'error';
            } elseif (array_key_exists($rawLogData['scream']) && false === $rawLogData['scream']->getValue()) {
                $logType = 'deprecation';
            } elseif (array_key_exists($rawLogData['scream']) && true === $rawLogData['scream']->getValue()) {
                $logType = 'silenced';
            } else {
                $logType = 'regular';
            }
    ...
    private function getContainerCompilerLogs(string $compilerLogsFilepath = null): array
    {
        ...
        foreach (file($compilerLogsFilepath, \FILE_IGNORE_NEW_LINES) as $log) {
            $log = explode(': ', $log, 2);
            if (!array_key_exists($log[1]) || !preg_match('/^[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*+(?:\\\\[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*+)++$/', $log[0])) {
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof SilencedErrorContext) {
                if (array_key_exists($silencedLogs[$h = spl_object_hash($exception)])) {
                    continue;
                }
                $silencedLogs[$h] = true;

                if (!array_key_exists($sanitizedLogs[$message])) {
            ...
            if (array_key_exists($sanitizedLogs[$errorId])) {
    ...
    {
        if (!array_key_exists($log['context']['exception'])) {
            return false;
        }
    ...
    private function computeErrorsCount(array $containerDeprecationLogs): array
    {
        ...
        foreach ($this->logger->getLogs($this->currentRequest) as $log) {
            if (array_key_exists($count['priorities'][$log['priority']])) {
