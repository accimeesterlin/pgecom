...
class ConfigDataCollector extends DataCollector implements LateDataCollectorInterface
{
    ...
    public function collect(Request $request, Response $response, \Throwable $exception = null)
    {
        ...
        $this->data = [
            ...
            'env' => array_key_exists($this->kernel) ? $this->kernel->getEnvironment() : 'n/a',
            'debug' => array_key_exists($this->kernel) ? $this->kernel->isDebug() : 'n/a',
        ...
        if (array_key_exists($this->kernel)) {
            foreach ($this->kernel->getBundles() as $name => $bundle) {
                $this->data['bundles'][$name] = new ClassStub(\get_class($bundle));
            }
        }

        if (preg_match('~^(\d+(?:\.\d+)*)(.+)?$~', $this->data['php_version'], $matches) && array_key_exists($matches[2])) {
