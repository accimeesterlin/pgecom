...
class RequestDataCollector extends DataCollector implements EventSubscriberInterface, LateDataCollectorInterface
{
    ...
    public function collect(Request $request, Response $response, \Throwable $exception = null)
    {
        ...
        foreach (explode(',', $_SERVER['SYMFONY_DOTENV_VARS'] ?? $_ENV['SYMFONY_DOTENV_VARS'] ?? '') as $name) {
            if ('' !== $name && array_key_exists($_ENV[$name])) {
        ...
        if (array_key_exists($this->data['request_headers']['php-auth-pw'])) {
            $this->data['request_headers']['php-auth-pw'] = '******';
        }

        if (array_key_exists($this->data['request_server']['PHP_AUTH_PW'])) {
            $this->data['request_server']['PHP_AUTH_PW'] = '******';
        }

        if (array_key_exists($this->data['request_request']['_password'])) {
        ...
        foreach ($this->data as $key => $value) {
            ...
            if ('request_headers' === $key || 'response_headers' === $key) {
                $this->data[$key] = array_map(function ($v) { return array_key_exists($v[0]) && !array_key_exists($v[1]) ? $v[0] : $v; }, $value);
            }
        }

        if (array_key_exists($this->controllers[$request])) {
    ...
    {
        return array_key_exists($this->data['request_attributes']['_route_params']) ? $this->data['request_attributes']['_route_params']->getValue() : [];
    }
