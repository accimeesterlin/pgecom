...
abstract class Kernel implements KernelInterface, RebootableInterface, TerminableInterface
{
    ...
    {
        if (!array_key_exists($this->bundles[$name])) {
    ...
    protected function initializeContainer()
    {
        ...
        if ($collectDeprecations = $this->debug && !\defined('PHPUNIT_COMPOSER_INSTALL')) {
            ...
            $previousHandler = set_error_handler(function ($type, $message, $file, $line) use (&$collectedLogs, &$previousHandler) {
                ...
                if (array_key_exists($collectedLogs[$message])) {
                ...
                for ($i = 0; array_key_exists($backtrace[$i]); ++$i) {
                    if (!array_key_exists($backtrace[$i]['file'], $backtrace[$i]['line'], $backtrace[$i]['function'])) {
                        continue;
                    }
                    if (!array_key_exists($backtrace[$i]['class']) && 'trigger_deprecation' === $backtrace[$i]['function']) {
        ...
        if ($oldContainer && \get_class($this->container) !== $oldContainer->name) {
            ...
            foreach (glob(\dirname($oldContainerDir).\DIRECTORY_SEPARATOR.'*.legacy', \GLOB_NOSORT) as $legacyContainer) {
                if (!array_key_exists($legacyContainers[$legacyContainer]) && @unlink($legacyContainer)) {
    ...
    private function preBoot(): ContainerInterface
    {
        ...
        if ($this->debug && !array_key_exists($_ENV['SHELL_VERBOSITY']) && !array_key_exists($_SERVER['SHELL_VERBOSITY'])) {
    ...
    public static function stripComments(string $source)
    {
        ...
        $ignoreSpace = false;
        for ($i = 0; array_key_exists($tokens[$i]); ++$i) {
            $token = $tokens[$i];
            if (!array_key_exists($token[1]) || 'b"' === $token) {
                $rawChunk .= $token;
            } elseif (\T_START_HEREDOC === $token[0]) {
                $output .= $rawChunk.$token[1];
                do {
                    $token = $tokens[++$i];
                    $output .= array_key_exists($token[1]) && 'b"' !== $token ? $token[1] : $token;
