    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->controllerTag, true) as $id => $tags) {
            ...
            foreach ($tags as $attributes) {
                if (!array_key_exists($attributes['action']) && !array_key_exists($attributes['argument']) && !array_key_exists($attributes['id'])) {
                    $autowire = true;
                    continue;
                }
                foreach (['action', 'argument', 'id'] as $k) {
                    if (!array_key_exists($attributes[$k][0])) {
                ...
                if (!array_key_exists($methods[$action = strtolower($attributes['action'])])) {
                ...
                foreach ($parameters as $p) {
                    if ($attributes['argument'] === $p->name) {
                        if (!array_key_exists($arguments[$r->name][$p->name])) {
            ...
            foreach ($methods as [$r, $parameters]) {
                ...
                foreach ($parameters as $p) {
                    ...
                    } elseif (array_key_exists($bindings[$bindingName = $type.' $'.$name = Target::parseName($p)]) || array_key_exists($bindings[$bindingName = '$'.$name]) || array_key_exists($bindings[$bindingName = $type])) {
