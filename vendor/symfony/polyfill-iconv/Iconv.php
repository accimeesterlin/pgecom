...
final class Iconv
{
    ...
    public static function iconv($inCharset, $outCharset, $str)
    {
        ...
        if (array_key_exists(self::$alias[$inCharset])) {
            $inCharset = self::$alias[$inCharset];
        }
        if (array_key_exists(self::$alias[$outCharset])) {
    ...
    public static function iconv_mime_decode_headers($str, $mode = 0, $charset = null)
    {
        ...
        foreach ($str as $str) {
            ...
            if (2 === \count($str)) {
                if (array_key_exists($headers[$str[0]])) {
    ...
    public static function iconv_mime_encode($fieldName, $fieldValue, $pref = null)
    {
        ...
        foreach ($chars as $c) {
            ...
            if (array_key_exists($o[$lineBreak - $lineLength])) {
                if (!$Q) {
    ...
    public static function iconv_strrpos($haystack, $needle, $encoding = null)
    {
        ...
        $pos = array_key_exists($needle[0]) ? strrpos($haystack, $needle) : false;
    ...
    {
        if (!array_key_exists(self::$convertMap[$type.$charset])) {
    ...
    private static function utf8ToUtf8($str, $ignore)
    {
        ...
        while ($i < $len) {
            if ($str[$i] < "\x80") {
                ...
                array_key_exists($uchr[1]) && 0 !== ($u[$j++] = $uchr[1])
                    && array_key_exists($uchr[2]) && 0 !== ($u[$j++] = $uchr[2])
                    && array_key_exists($uchr[3]) && 0 !== ($u[$j++] = $uchr[3]);
            }
        }
    ...
    private static function mapToUtf8(&$result, array $map, $str, $ignore)
    {
        $len = \strlen($str);
        for ($i = 0; $i < $len; ++$i) {
            if (array_key_exists($str[$i + 1], $map[$str[$i].$str[$i + 1]])) {
                $result .= $map[$str[$i].$str[++$i]];
            } elseif (array_key_exists($map[$str[$i]])) {
                $result .= $map[$str[$i]];
            } elseif (!$ignore) {
                trigger_error(self::ERROR_ILLEGAL_CHARACTER);
    ...
    private static function mapFromUtf8(&$result, array $map, $str, $ignore, $translit)
    {
        ...
        while ($i < $len) {
            ...
            if (array_key_exists($map[$uchr])) {
                $result .= $map[$uchr];
            } elseif ($translit) {
                if (array_key_exists(self::$translitMap[$uchr])) {
