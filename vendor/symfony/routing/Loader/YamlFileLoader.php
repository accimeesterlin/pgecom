...
class YamlFileLoader extends FileLoader
{
    ...
    {
        if (array_key_exists($config['alias'])) {
        ...
        if (array_key_exists($config['controller'])) {
            $defaults['_controller'] = $config['controller'];
        }
        if (array_key_exists($config['locale'])) {
            $defaults['_locale'] = $config['locale'];
        }
        if (array_key_exists($config['format'])) {
            $defaults['_format'] = $config['format'];
        }
        if (array_key_exists($config['utf8'])) {
            $options['utf8'] = $config['utf8'];
        }
        if (array_key_exists($config['stateless'])) {
        ...
        if (array_key_exists($config['host'])) {
    ...
    protected function validate($config, string $name, string $path)
    {
        ...
        if (array_key_exists($config['resource']) && array_key_exists($config['path'])) {
            throw new \InvalidArgumentException(sprintf('The routing file "%s" must not specify both the "resource" key and the "path" key for "%s". Choose between an import and a route definition.', $path, $name));
        }
        if (!array_key_exists($config['resource']) && array_key_exists($config['type'])) {
            throw new \InvalidArgumentException(sprintf('The "type" key for the route definition "%s" in "%s" is unsupported. It is only available for imports in combination with the "resource" key.', $name, $path));
        }
        if (!array_key_exists($config['resource']) && !array_key_exists($config['path'])) {
            throw new \InvalidArgumentException(sprintf('You must define a "path" for the route "%s" in file "%s".', $name, $path));
        }
        if (array_key_exists($config['controller']) && array_key_exists($config['defaults']['_controller'])) {
            throw new \InvalidArgumentException(sprintf('The routing file "%s" must not specify both the "controller" key and the defaults key "_controller" for "%s".', $path, $name));
        }
        if (array_key_exists($config['stateless']) && array_key_exists($config['defaults']['_stateless'])) {
    ...
    private function validateAlias(array $config, string $name, string $path): void
    {
        foreach ($config as $key => $value) {
            ...
            if ('deprecated' === $key) {
                if (!array_key_exists($value['package'])) {
