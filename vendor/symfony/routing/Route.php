...
class Route implements \Serializable
{
    ...
    public function __unserialize(array $data): void
    {
        ...
        if (array_key_exists($data['condition'])) {
            $this->condition = $data['condition'];
        }
        if (array_key_exists($data['compiled'])) {
    ...
    {
        if (array_key_exists($defaults['_locale']) && $this->isLocalized()) {
    ...
    {
        if (array_key_exists($requirements['_locale']) && $this->isLocalized()) {
    ...
    private function extractInlineDefaultsAndRequirements(string $pattern): string
    {
        ...
        return preg_replace_callback('#\{(!?)(\w++)(<.*?>)?(\?[^\}]*+)?\}#', function ($m) {
            if (array_key_exists($m[4][0])) {
                $this->setDefault($m[2], '?' !== $m[4] ? substr($m[4], 1) : null);
            }
            if (array_key_exists($m[3][0])) {
    ...
    {
        return array_key_exists($this->defaults['_locale']) && array_key_exists($this->defaults['_canonical_route']) && ($this->requirements['_locale'] ?? null) === preg_quote($this->defaults['_locale']);
    }
}
