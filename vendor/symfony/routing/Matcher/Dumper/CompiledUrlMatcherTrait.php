    ...
    private function doMatch(string $pathinfo, array &$allow = [], array &$allowSchemes = []): array
    {
        ...
        foreach ($this->staticRoutes[$trimmedPathinfo] ?? [] as [$ret, $requiredHost, $requiredMethods, $requiredSchemes, $hasTrailingSlash, , $condition]) {
            ...
            if ('/' !== $pathinfo && $hasTrailingSlash === ($trimmedPathinfo === $pathinfo)) {
                if ($supportsRedirections && (!$requiredMethods || array_key_exists($requiredMethods['GET']))) {
                    return $allow = $allowSchemes = [];
                }
                continue;
            }

            $hasRequiredScheme = !$requiredSchemes || array_key_exists($requiredSchemes[$context->getScheme()]);
            if ($hasRequiredScheme && $requiredMethods && !array_key_exists($requiredMethods[$canonicalMethod]) && !array_key_exists($requiredMethods[$requestMethod])) {
        ...
        foreach ($this->regexpList as $offset => $regex) {
            while (preg_match($regex, $matchedPathinfo, $matches)) {
                foreach ($this->dynamicRoutes[$m = (int) $matches['MARK']] as [$ret, $vars, $requiredMethods, $requiredSchemes, $hasTrailingSlash, $hasTrailingVar, $condition]) {
                    ...
                    foreach ($vars as $i => $v) {
                        if (array_key_exists($matches[1 + $i])) {
                            $ret[$v] = $matches[1 + $i];
                        }
                    }

                    if ($requiredSchemes && !array_key_exists($requiredSchemes[$context->getScheme()])) {
                        $allowSchemes += $requiredSchemes;
                        continue;
                    }

                    if ($requiredMethods && !array_key_exists($requiredMethods[$canonicalMethod]) && !array_key_exists($requiredMethods[$requestMethod])) {
                        $allow += $requiredMethods;
                        continue;
                    }
