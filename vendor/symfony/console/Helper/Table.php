...
class Table
{
    ...
    public static function getStyleDefinition(string $name)
    {
        ...
        if (array_key_exists(self::$styles[$name])) {
    ...
    private function buildTableRows(array $rows): TableRows
    {
        ...
        for ($rowKey = 0; $rowKey < \count($rows); ++$rowKey) {
            ...
            foreach ($rows[$rowKey] as $column => $cell) {
                ...
                if (array_key_exists($this->columnMaxWidths[$column]) && Helper::width(Helper::removeDecoration($formatter, $cell)) > $this->columnMaxWidths[$column]) {
        ...
        return new TableRows(function () use ($rows, $unmergedRows): \Traversable {
            foreach ($rows as $rowKey => $row) {
                ...
                if (array_key_exists($unmergedRows[$rowKey])) {
    ...
    private function fillNextRows(array $rows, int $line): array
    {
        ...
        foreach ($unmergedRows as $unmergedRowKey => $unmergedRow) {
            ...
            if (array_key_exists($rows[$unmergedRowKey]) && \is_array($rows[$unmergedRowKey]) && ($this->getNumberOfColumns($rows[$unmergedRowKey]) + $this->getNumberOfColumns($unmergedRows[$unmergedRowKey]) <= $this->numberOfColumns)) {
    ...
    private function getCellWidth(array $row, int $column): int
    {
        ...
        if (array_key_exists($row[$column])) {
            $cell = $row[$column];
        ...
        return array_key_exists($this->columnMaxWidths[$column]) ? min($this->columnMaxWidths[$column], $cellWidth) : $cellWidth;
    }
