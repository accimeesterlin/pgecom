...
class DebugFormatterHelper extends Helper
{
    ...
    public function progress(string $id, string $buffer, bool $error = false, string $prefix = 'OUT', string $errorPrefix = 'ERR')
    {
        ...
        if ($error) {
            if (array_key_exists($this->started[$id]['out'])) {
                $message .= "\n";
                unset($this->started[$id]['out']);
            }
            if (!array_key_exists($this->started[$id]['err'])) {
    ...
    {
        $trailingEOL = array_key_exists($this->started[$id]['out']) || array_key_exists($this->started[$id]['err']) ? "\n" : '';
