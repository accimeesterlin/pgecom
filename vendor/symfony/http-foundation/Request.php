...
class Request
{
    ...
    public static function create(string $uri, string $method = 'GET', array $parameters = [], array $cookies = [], array $files = [], array $server = [], $content = null)
    {
        ...
        if (array_key_exists($components['scheme'])) {
        ...
        if (array_key_exists($components['port'])) {
            $server['SERVER_PORT'] = $components['port'];
            $server['HTTP_HOST'] .= ':'.$components['port'];
        }

        if (array_key_exists($components['user'])) {
            $server['PHP_AUTH_USER'] = $components['user'];
        }

        if (array_key_exists($components['pass'])) {
            $server['PHP_AUTH_PW'] = $components['pass'];
        }

        if (!array_key_exists($components['path'])) {
            $components['path'] = '/';
        }
        ...
        switch (strtoupper($method)) {
            ...
            case 'PUT':
            case 'DELETE':
                if (!array_key_exists($server['CONTENT_TYPE'])) {
        ...
        $queryString = '';
        if (array_key_exists($components['query'])) {
    ...
    public static function setTrustedProxies(array $proxies, int $trustedHeaderSet)
    {
        ...
        self::$trustedProxies = array_reduce($proxies, function ($proxies, $proxy) {
            if ('REMOTE_ADDR' !== $proxy) {
                $proxies[] = $proxy;
            } elseif (array_key_exists($_SERVER['REMOTE_ADDR'])) {
    ...
    public function getRelativeUriForPath(string $path)
    {
        ...
        if (!array_key_exists($path[0]) || '/' !== $path[0]) {
            return $path;
        }
        ...
        $sourceDirs = explode('/', array_key_exists($basePath[0]) && '/' === $basePath[0] ? substr($basePath, 1) : $basePath);
        $targetDirs = explode('/', substr($path, 1));
        array_pop($sourceDirs);
        $targetFile = array_pop($targetDirs);

        foreach ($sourceDirs as $i => $dir) {
            if (array_key_exists($targetDirs[$i]) && $dir === $targetDirs[$i]) {
        ...
        return !array_key_exists($path[0]) || '/' === $path[0]
    ...
    public function getMimeType(string $format)
    {
        ...
        return array_key_exists(static::$formats[$format]) ? static::$formats[$format][0] : null;
    }
    ...
    protected function prepareRequestUri()
    {
        ...
        if ('1' == $this->server->get('IIS_WasUrlRewritten') && '' != $this->server->get('UNENCODED_URL')) {
            ...
            if ('' !== $requestUri && '/' === $requestUri[0]) {
                ...
                if (array_key_exists($uriComponents['path'])) {
                    $requestUri = $uriComponents['path'];
                }

                if (array_key_exists($uriComponents['query'])) {
    ...
    private function getTrustedValues(int $type, string $ip = null): array
    {
        ...
        if ((self::$trustedHeaderSet & self::HEADER_FORWARDED) && (array_key_exists(self::FORWARDED_PARAMS[$type])) && $this->headers->has(self::TRUSTED_HEADERS[self::HEADER_FORWARDED])) {
