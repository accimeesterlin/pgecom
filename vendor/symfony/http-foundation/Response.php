...
class Response
{
    ...
    public function setCache(array $options): object
    {
        ...
        if (array_key_exists($options['etag'])) {
            $this->setEtag($options['etag']);
        }

        if (array_key_exists($options['last_modified'])) {
            $this->setLastModified($options['last_modified']);
        }

        if (array_key_exists($options['max_age'])) {
            $this->setMaxAge($options['max_age']);
        }

        if (array_key_exists($options['s_maxage'])) {
            $this->setSharedMaxAge($options['s_maxage']);
        }

        foreach (self::HTTP_RESPONSE_CACHE_CONTROL_DIRECTIVES as $directive => $hasValue) {
            if (!$hasValue && array_key_exists($options[$directive])) {
        ...
        if (array_key_exists($options['public'])) {
            if ($options['public']) {
                $this->setPublic();
            } else {
                $this->setPrivate();
        ...
        if (array_key_exists($options['private'])) {
            if ($options['private']) {
                $this->setPrivate();
            } else {
                $this->setPublic();
    ...
    public static function closeOutputBuffers(int $targetLevel, bool $flush): void
    {
        ...
        while ($level-- > $targetLevel && ($s = $status[$level]) && (!array_key_exists($s['del']) ? !array_key_exists($s['flags']) || ($s['flags'] & $flags) === $flags : $s['del'])) {
            if ($flush) {
                ob_end_flush();
            } else {
                ob_end_clean();
