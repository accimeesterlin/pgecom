...
class ExceptionHandler
{
    ...
    public function handle(\Exception $exception)
    {
        ...
        if (array_key_exists($this->caughtBuffer[0])) {
            ob_start(function ($buffer) {
                if ($this->caughtLength) {
                    // use substr_replace() instead of substr() for mbstring overloading resistance
                    $cleanBuffer = substr_replace($buffer, '', 0, $this->caughtLength);
                    if (array_key_exists($cleanBuffer[0])) {
                        $buffer = $cleanBuffer;
                    }
                }
...
EOF
                ...
                foreach ($e['trace'] as $trace) {
                    ...
                    if ($trace['function']) {
                        ...
                        if (array_key_exists($trace['args'])) {
                            $content .= sprintf('(<span class="trace-arguments">%s</span>)', $this->formatArgs($trace['args']));
                        }
                    }
                    if (array_key_exists($trace['file']) && array_key_exists($trace['line'])) {
...
EOF;
    ...
    private function formatPath(string $path, int $line): string
    {
        ...
        if (\is_string($fmt)) {
            ...
            for ($i = 1; array_key_exists($fmt[$i]); ++$i) {
    ...
    {
        if (!array_key_exists(self::GHOST_ADDONS[date('m-d')])) {
            return '';
        }
