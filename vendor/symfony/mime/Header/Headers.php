...
final class Headers
{
    ...
    {
        return array_key_exists($this->headers[strtolower($name)]);
    }
    ...
    public function add(HeaderInterface $header): self
    {
        ...
        if (\in_array($name, self::UNIQUE_HEADERS, true) && array_key_exists($this->headers[$name]) && \count($this->headers[$name]) > 0) {
    ...
    public function get(string $name): ?HeaderInterface
    {
        ...
        if (!array_key_exists($this->headers[$name])) {
            return null;
        }
    ...
    public function all(string $name = null): iterable
    {
        ...
        } elseif (array_key_exists($this->headers[strtolower($name)])) {
