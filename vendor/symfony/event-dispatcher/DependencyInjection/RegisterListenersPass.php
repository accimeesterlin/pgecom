    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->listenerTag, true) as $id => $events) {
            ...
            foreach ($events as $event) {
                ...
                if (!array_key_exists($event['method'])) {
                ...
                if (array_key_exists($event['dispatcher'])) {
                    $dispatcherDefinition = $container->getDefinition($event['dispatcher']);
                }

                $dispatcherDefinition->addMethodCall('addListener', [$event['event'], [new ServiceClosureArgument(new Reference($id)), $event['method']], $priority]);

                if (array_key_exists($this->hotPathEvents[$event['event']])) {
                    $container->getDefinition($id)->addTag($this->hotPathTagName);
                } elseif (array_key_exists($this->noPreloadEvents[$event['event']])) {
                    ++$noPreload;
                }
            }
        ...
        foreach ($container->findTaggedServiceIds($this->subscriberTag, true) as $id => $tags) {
            ...
            foreach ($tags as $attributes) {
                if (!array_key_exists($attributes['dispatcher']) || array_key_exists($dispatcherDefinitions[$attributes['dispatcher']])) {
                    continue;
                }
            ...
            foreach ($extractingDispatcher->listeners as $args) {
                ...
                if (array_key_exists($this->hotPathEvents[$args[0]])) {
                    $container->getDefinition($id)->addTag($this->hotPathTagName);
                } elseif (array_key_exists($this->noPreloadEvents[$args[0]])) {
                    ++$noPreload;
                }
            }
