...
class AwsS3Adapter extends AbstractAdapter implements CanOverwriteFiles
{
    ...
    protected function readObject($path)
    {
        ...
        if (array_key_exists($this->options['@http'])) {
    ...
    protected function getRawVisibility($path)
    {
        ...
        foreach ($result->get('Grants') as $grant) {
            if (
                array_key_exists($grant['Grantee']['URI'])
    ...
    protected function upload($path, $body, Config $config)
    {
        ...
        if ( ! array_key_exists($options['ContentType'])) {
            $options['ContentType'] = Util::guessMimeType($path, $body);
        }

        if ( ! array_key_exists($options['ContentLength'])) {
    ...
    protected function normalizeResponse(array $response, $path = null)
    {
        $result = [
            'path' => $path ?: $this->removePathPrefix(
                array_key_exists($response['Key']) ? $response['Key'] : $response['Prefix']
            ),
        ];
        $result = array_merge($result, Util::pathinfo($result['path']));

        if (array_key_exists($response['LastModified'])) {
