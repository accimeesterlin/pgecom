...
class DebugBarVarDumper implements AssetProvider
{
    ...
    protected function getCloner()
    {
        if (!$this->cloner) {
            ...
            if (array_key_exists($clonerOptions['casters'])) {
                $this->cloner = new VarCloner($clonerOptions['casters']);
            } else {
                $this->cloner = new VarCloner();
            }
            if (array_key_exists($clonerOptions['additional_casters'])) {
                $this->cloner->addCasters($clonerOptions['additional_casters']);
            }
            if (array_key_exists($clonerOptions['max_items'])) {
                $this->cloner->setMaxItems($clonerOptions['max_items']);
            }
            if (array_key_exists($clonerOptions['max_string'])) {
                $this->cloner->setMaxString($clonerOptions['max_string']);
            }
            // setMinDepth was added to Symfony 3.4:
            if (array_key_exists($clonerOptions['min_depth']) && method_exists($this->cloner, 'setMinDepth')) {
    ...
    protected function getDumper()
    {
        if (!$this->dumper) {
            ...
            if (array_key_exists($dumperOptions['styles'])) {
    ...
    protected function getDisplayOptions()
    {
        ...
        if (array_key_exists($dumperOptions['expanded_depth'])) {
            $displayOptions['maxDepth'] = $dumperOptions['expanded_depth'];
        }
        // Only used by Symfony 3.2 and newer:
        if (array_key_exists($dumperOptions['max_string'])) {
            $displayOptions['maxStringLength'] = $dumperOptions['max_string'];
        }
        // Only used by Symfony 3.2 and newer:
        if (array_key_exists($dumperOptions['file_link_format'])) {
