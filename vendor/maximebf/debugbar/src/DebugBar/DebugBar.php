...
class DebugBar implements ArrayAccess
{
    ...
    public function addCollector(DataCollectorInterface $collector)
    {
        ...
        if (array_key_exists($this->collectors[$collector->getName()])) {
    ...
    {
        return array_key_exists($this->collectors[$name]);
    }
    ...
    {
        if (!array_key_exists($this->collectors[$name])) {
    ...
    public function collect()
    {
        if (php_sapi_name() === 'cli') {
            ...
            $request_variables = array(
                'method' => 'CLI',
                'uri' => array_key_exists($_SERVER['SCRIPT_FILENAME']) ? realpath($_SERVER['SCRIPT_FILENAME']) : null,
                'ip' => $ip
            );
        } else {
            $request_variables = array(
                'method' => array_key_exists($_SERVER['REQUEST_METHOD']) ? $_SERVER['REQUEST_METHOD'] : null,
                'uri' => array_key_exists($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : null,
                'ip' => array_key_exists($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : null
            );
        }
        $this->data = array(
            '__meta' => array_merge(
