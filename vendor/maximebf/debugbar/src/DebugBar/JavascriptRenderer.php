...
class JavascriptRenderer
{
    ...
    public function getAssets($type = null, $relativeTo = self::RELATIVE_PATH)
    {
        ...
        foreach ($additionalAssets as $assets) {
            $basePath = array_key_exists($assets['base_path']) ? $assets['base_path'] : '';
            $baseUrl = array_key_exists($assets['base_url']) ? $assets['base_url'] : '';
    ...
    protected function getJsControlsDefinitionCode($varname)
    {
        ...
        foreach (array_filter($controls) as $name => $options) {
            ...
            if (array_key_exists($options['tab']) || array_key_exists($options['widget'])) {
                if (!array_key_exists($opts['title'])) {
                    $opts['title'] = ucfirst(str_replace('_', ' ', $name));
                }
                $js .= sprintf("%s.addTab(\"%s\", new %s({%s%s}));\n",
                    $varname,
                    $name,
                    array_key_exists($options['tab']) ? $options['tab'] : 'PhpDebugBar.DebugBar.Tab',
                    substr(json_encode($opts, JSON_FORCE_OBJECT), 1, -1),
                    array_key_exists($options['widget']) ? sprintf('%s"widget": new %s()', count($opts) ? ', ' : '', $options['widget']) : ''
                );
            } elseif (array_key_exists($options['indicator']) || array_key_exists($options['icon'])) {
                $js .= sprintf("%s.addIndicator(\"%s\", new %s(%s), \"%s\");\n",
                    $varname,
                    $name,
                    array_key_exists($options['indicator']) ? $options['indicator'] : 'PhpDebugBar.DebugBar.Indicator',
                    json_encode($opts, JSON_FORCE_OBJECT),
                    array_key_exists($options['position']) ? $options['position'] : 'right'
                );
            }

            if (array_key_exists($options['map']) && array_key_exists($options['default'])) {
