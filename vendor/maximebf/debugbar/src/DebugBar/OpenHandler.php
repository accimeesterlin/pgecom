...
class OpenHandler
{
    ...
    public function handle($request = null, $echo = true, $sendHeader = true)
    {
        ...
        $op = 'find';
        if (array_key_exists($request['op'])) {
            $op = $request['op'];
    ...
    {
        $max = 20;
        if (array_key_exists($request['max'])) {
            $max = $request['max'];
        }

        $offset = 0;
        if (array_key_exists($request['offset'])) {
            $offset = $request['offset'];
        }

        $filters = array();
        foreach (array('utime', 'datetime', 'ip', 'uri', 'method') as $key) {
            if (array_key_exists($request[$key])) {
    ...
    {
        if (!array_key_exists($request['id'])) {
