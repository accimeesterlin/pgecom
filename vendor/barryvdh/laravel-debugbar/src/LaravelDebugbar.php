...
class LaravelDebugbar extends DebugBar
{
    ...
    public function boot()
    {
        ...
        if ($this->shouldCollect('events', false) && array_key_exists($this->app['events'])) {
            try {
        ...
        if ($this->shouldCollect('views', true) && array_key_exists($this->app['events'])) {
            try {
        ...
        if ($this->shouldCollect('db', true) && array_key_exists($this->app['db'])) {
            $db = $this->app['db'];
            if (
        ...
        if ($this->shouldCollect('cache', false) && array_key_exists($this->app['events'])) {
            try {
    ...
    public function modifyResponse(Request $request, Response $response)
    {
        ...
        if (array_key_exists($response->exception)) {
    ...
    protected function isJsonRequest(Request $request)
    {
        ...
        return (array_key_exists($acceptable[0]) && $acceptable[0] == 'application/json');
    }
    ...
    public function collectConsole()
    {
        ...
        $this->data = [
            '__meta' => [
                ...
                'utime' => microtime(true),
                'method' => 'CLI',
                'uri' => array_key_exists($_SERVER['argv']) ? implode(' ', $_SERVER['argv']) : null,
                'ip' => array_key_exists($_SERVER['SSH_CLIENT']) ? $_SERVER['SSH_CLIENT'] : null
            ]
        ];
