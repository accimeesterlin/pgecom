...
//        }
    ...
    protected function parseTrace($index, array $trace)
    {
        $frame = (object) [
            ...
            'namespace' => null,
            'name' => null,
            'line' => array_key_exists($trace['line']) ? $trace['line'] : '?',
        ];

        if (array_key_exists($trace['function']) && $trace['function'] == 'substituteBindings') {
        ...
        if (
            array_key_exists($trace['class']) &&
            array_key_exists($trace['file']) &&
            !$this->fileIsInExcludedPath($trace['file'])
        ) {
            $file = $trace['file'];

            if (array_key_exists($trace['object']) && is_a($trace['object'], 'Twig_Template')) {
    ...
    protected function findViewFromHash($hash)
    {
        ...
        if (array_key_exists($this->reflection['viewfinderViews'])) {
    ...
    protected function getTwigInfo($trace)
    {
        ...
        if (array_key_exists($trace['line'])) {
        ...
        if ($this->durationBackground) {
            if ($totalTime > 0) {
                ...
                foreach ($statements as $i => $statement) {
                    if (!array_key_exists($statement['duration'])) {
                        continue;
                    }
