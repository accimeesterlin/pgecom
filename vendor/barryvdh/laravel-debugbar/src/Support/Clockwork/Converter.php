...
class Converter
{
    ...
    public function convert($data)
    {
        ...
        if (array_key_exists($data['clockwork'])) {
            $output = array_merge($output, $data['clockwork']);
        }

        if (array_key_exists($data['time'])) {
            $time = $data['time'];
        ...
        if (array_key_exists($data['route'])) {
            ...
            $controller = null;
            if (array_key_exists($route['controller'])) {
                $controller = $route['controller'];
            } elseif (array_key_exists($route['uses'])) {
            ...
            $output['routes'][] = [
                'action' => $controller,
                'after' => array_key_exists($route['after']) ? $route['after'] : null,
                'before' => array_key_exists($route['before']) ? $route['before'] : null,
                'method' => $method,
                'name' => array_key_exists($route['as']) ? $route['as'] : null,
                'uri' => $uri,
            ];
        }

        if (array_key_exists($data['messages'])) {
        ...
        if (array_key_exists($data['queries'])) {
        ...
        if (array_key_exists($data['views'])) {
        ...
        if (array_key_exists($data['swiftmailer_mails'])) {
            foreach ($data['swiftmailer_mails']['mails'] as $mail) {
                $output['emailsData'][] = [
                    'data' => [
                        'to' => $mail['to'],
                        'subject' => $mail['subject'],
                        'headers' => array_key_exists($mail['headers']) ? explode("\n", $mail['headers']) : null,
                    ],
                ];
            }
        }
