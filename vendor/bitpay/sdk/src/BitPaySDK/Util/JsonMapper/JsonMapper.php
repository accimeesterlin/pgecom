...
class JsonMapper
{
    ...
    public function map($json, $object)
    {
        ...
        foreach ($json as $key => $jvalue) {
            ...
            if (!array_key_exists($this->arInspectedClasses[$strClassName][$key])) {
    ...
    protected function checkMissingData($providedProperties, ReflectionClass $rc)
    {
        foreach ($rc->getProperties() as $property) {
            ...
            if (array_key_exists($annotations['required'])
                && !array_key_exists($providedProperties[$property->name])
            ) {
                throw new JsonMapperException(
    ...
    protected function removeUndefinedAttributes($object, $providedProperties)
    {
        foreach (get_object_vars($object) as $propertyName => $dummy) {
            if (!array_key_exists($providedProperties[$propertyName])) {
    ...
    protected function inspectProperty(ReflectionClass $rc, $name)
    {
        ...
        if ($rc->hasMethod($setter)) {
            ...
            if ($rmeth->isPublic() || $this->bIgnoreVisibility) {
                ...
                if (!array_key_exists($annotations['param'][0])) {
        ...
        if ($rprop !== null) {
            if ($rprop->isPublic() || $this->bIgnoreVisibility) {
                ...
                if (!array_key_exists($annotations['var'][0])) {
    ...
    {
        if (array_key_exists($this->classMap[$type])) {
            $target = $this->classMap[$type];
        } else {
            if (is_string($type) && $type !== '' && $type[0] == '\\'
                && array_key_exists($this->classMap[substr($type, 1)])
            ) {
