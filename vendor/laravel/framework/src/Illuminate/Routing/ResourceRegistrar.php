...
class ResourceRegistrar
{
    ...
    {
        if (array_key_exists($options['parameters']) && ! array_key_exists($this->parameters)) {
        ...
        foreach ($this->getResourceMethods($defaults, $options) as $m) {
            ...
            if (array_key_exists($options['bindingFields'])) {
    ...
    protected function getResourceMethods($defaults, $options)
    {
        ...
        if (array_key_exists($options['only'])) {
            $methods = array_intersect($methods, (array) $options['only']);
        }

        if (array_key_exists($options['except'])) {
    ...
    {
        return array_key_exists($options['shallow']) && $options['shallow']
                    ? last(explode('.', $name))
                    : $name;
    }
    ...
    {
        if (array_key_exists($this->parameters[$value])) {
            $value = $this->parameters[$value];
        } elseif (array_key_exists(static::$parameterMap[$value])) {
    ...
    protected function getResourceAction($resource, $controller, $method, $options)
    {
        ...
        if (array_key_exists($options['middleware'])) {
            $action['middleware'] = $options['middleware'];
        }

        if (array_key_exists($options['excluded_middleware'])) {
            $action['excluded_middleware'] = $options['excluded_middleware'];
        }

        if (array_key_exists($options['wheres'])) {
    ...
    protected function getResourceRouteName($resource, $method, $options)
    {
        ...
        if (array_key_exists($options['names'])) {
            if (is_string($options['names'])) {
                $name = $options['names'];
            } elseif (array_key_exists($options['names'][$method])) {
        ...
        $prefix = array_key_exists($options['as']) ? $options['as'].'.' : '';
