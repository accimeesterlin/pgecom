...
class CompiledRouteCollection extends AbstractRouteCollection
{
    ...
    protected function requestWithoutTrailingSlash(Request $request)
    {
        ...
        $trimmedRequest->server->set(
            'REQUEST_URI', rtrim($parts[0], '/').(array_key_exists($parts[1]) ? '?'.$parts[1] : '')
        );
    ...
    {
        return array_key_exists($this->attributes[$name]) || $this->routes->hasNamedRoute($name);
    }
    ...
    public function getByAction($action)
    {
        $attributes = collect($this->attributes)->first(function (array $attributes) use ($action) {
            if (array_key_exists($attributes['action']['controller'])) {
