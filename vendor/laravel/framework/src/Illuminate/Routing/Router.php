...
class Router implements BindingRegistrar, RegistrarContract
{
    ...
    protected function actionReferencesController($action)
    {
        if (! $action instanceof Closure) {
            return is_string($action) || (array_key_exists($action['uses']) && is_string($action['uses']));
        }
    ...
    protected function prependGroupNamespace($class)
    {
        ...
        return array_key_exists($group['namespace']) && strpos($class, '\\') !== 0
    ...
    public function substituteBindings($route)
    {
        foreach ($route->parameters() as $key => $value) {
            if (array_key_exists($this->binders[$key])) {
    ...
    {
        if (array_key_exists($this->middlewareGroups[$group]) && ! in_array($middleware, $this->middlewareGroups[$group])) {
    ...
    {
        if (array_key_exists($this->binders[$key = str_replace('-', '_', $key)])) {
