...
trait ValidatesAttributes
{
    ...
    {
        return (array_key_exists($parameters['width']) && $parameters['width'] != $width) ||
               (array_key_exists($parameters['min_width']) && $parameters['min_width'] > $width) ||
               (array_key_exists($parameters['max_width']) && $parameters['max_width'] < $width) ||
               (array_key_exists($parameters['height']) && $parameters['height'] != $height) ||
               (array_key_exists($parameters['min_height']) && $parameters['min_height'] > $height) ||
               (array_key_exists($parameters['max_height']) && $parameters['max_height'] < $height);
    }
    ...
    {
        if (! array_key_exists($parameters['ratio'])) {
            return false;
        }
    ...
    public function validateUnique($attribute, $value, $parameters)
    {
        ...
        if (array_key_exists($parameters[2])) {
    ...
    {
        if (array_key_exists($parameters[4])) {
    ...
    {
        return array_key_exists($parameters[1]) && $parameters[1] !== 'NULL'
