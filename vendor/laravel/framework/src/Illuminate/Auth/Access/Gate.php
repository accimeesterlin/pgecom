...
class Gate implements GateContract
{
    ...
    public function has($ability)
    {
        ...
        foreach ($abilities as $ability) {
            if (! array_key_exists($this->abilities[$ability])) {
                return false;
            }
        }
    ...
    {
        if (is_array($callback) && array_key_exists($callback[0]) && is_string($callback[0])) {
    ...
    protected function buildAbilityCallback($ability, $callback)
    {
        return function () use ($ability, $callback) {
            ...
            return array_key_exists($method)
    ...
    protected function methodAllowsGuests($class, $method)
    {
        ...
        if ($method) {
            ...
            return array_key_exists($parameters[0]) && $this->parameterAllowsGuests($parameters[0]);
        }
    ...
    {
        if (array_key_exists($arguments[0]) &&
            ! is_null($policy = $this->getPolicyFor($arguments[0])) &&
            $callback = $this->resolvePolicyCallback($user, $ability, $arguments, $policy)) {
            return $callback;
        }

        if (array_key_exists($this->stringCallbacks[$ability])) {
    ...
    public function getPolicyFor($class)
    {
        ...
        if (array_key_exists($this->policies[$class])) {
    ...
    protected function callPolicyMethod($policy, $method, $user, array $arguments)
    {
        ...
        if (array_key_exists($arguments[0]) && is_string($arguments[0])) {
