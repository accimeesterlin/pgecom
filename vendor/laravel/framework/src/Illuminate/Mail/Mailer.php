...
class Mailer implements MailerContract, MailQueueContract
{
    ...
    public function send($view, array $data = [], $callback = null)
    {
        ...
        if (array_key_exists($this->to['address'])) {
    ...
    protected function parseView($view)
    {
        ...
        if (is_array($view) && array_key_exists($view[0])) {
    ...
    protected function addContent($message, $view, $plain, $raw, $data)
    {
        ...
        if (array_key_exists($plain)) {
            $method = array_key_exists($view) ? 'addPart' : 'setBody';

            $message->$method($this->renderView($plain, $data) ?: ' ', 'text/plain');
        }

        if (array_key_exists($raw)) {
            $method = (array_key_exists($view) || array_key_exists($plain)) ? 'addPart' : 'setBody';
