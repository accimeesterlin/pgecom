...
class MailManager implements FactoryContract
{
    ...
    public function createTransport(array $config)
    {
        ...
        if (array_key_exists($this->customCreators[$transport])) {
    ...
    protected function createSmtpTransport(array $config)
    {
        ...
        if (array_key_exists($config['username'])) {
    ...
    {
        if (array_key_exists($config['stream'])) {
            $transport->setStreamOptions($config['stream']);
        }

        if (array_key_exists($config['source_ip'])) {
            $transport->setSourceIp($config['source_ip']);
        }

        if (array_key_exists($config['local_domain'])) {
            $transport->setLocalDomain($config['local_domain']);
        }

        if (array_key_exists($config['timeout'])) {
            $transport->setTimeout($config['timeout']);
        }

        if (array_key_exists($config['auth_mode'])) {
    ...
    {
        if (! array_key_exists($config['secret'])) {
