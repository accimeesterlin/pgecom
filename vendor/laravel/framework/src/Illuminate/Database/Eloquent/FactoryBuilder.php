...
class FactoryBuilder
{
    ...
    protected function store($results)
    {
        $results->each(function ($model) {
            if (! array_key_exists($this->connection)) {
    ...
    {
        if (! array_key_exists($this->definitions[$this->class])) {
    ...
    protected function applyStates(array $definition, array $attributes = [])
    {
        foreach ($this->activeStates as $state) {
            if (! array_key_exists($this->states[$this->class][$state])) {
    ...
    {
        if (! array_key_exists($afterCallbacks[$this->class][$state])) {
            return;
        }
    ...
    {
        return array_key_exists($this->afterMaking[$this->class][$state]) ||
               array_key_exists($this->afterCreating[$this->class][$state]);
    }
}
