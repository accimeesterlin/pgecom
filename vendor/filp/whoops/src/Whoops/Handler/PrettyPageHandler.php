...
class PrettyPageHandler extends Handler
{
        ...
        if (class_exists('Symfony\Component\VarDumper\Cloner\VarCloner')) {
            ...
            $cloner->addCasters(['*' => function ($obj, $a, $stub, $isNested, $filter = 0) {
                ...
                foreach ($classes as $class) {
                    if (array_key_exists(AbstractCloner::$defaultCasters[$class])) {
                        return $a;
                    }
                }
    ...
    public function handle()
    {
        if (!$this->handleUnconditionally()) {
            ...
            if (PHP_SAPI === 'cli') {
                ...
                if (array_key_exists($_ENV['whoops-test'])) {
                    throw new \Exception(
        ...
        $vars = [
            ...
            "tables"      => [
                ...
                "Files"                 => array_key_exists($_FILES) ? $this->masked($_FILES, '_FILES') : [],
                "Cookies"               => $this->masked($_COOKIE, '_COOKIE'),
                "Session"               => array_key_exists($_SESSION) ? $this->masked($_SESSION, '_SESSION') :  [],
                "Server/Request Data"   => $this->masked($_SERVER, '_SERVER'),
                "Environment Variables" => $this->masked($_ENV, '_ENV'),
            ],
        ];

        if (array_key_exists($customCssFile)) {
            $vars["stylesheet"] .= file_get_contents($customCssFile);
        }

        if (array_key_exists($customJsFile)) {
    ...
    {
        if ($label !== null) {
            return array_key_exists($this->extraTables[$label]) ?
    ...
    {
        if (!is_callable($editor) && !array_key_exists($this->editors[$editor])) {
    ...
    public function getEditorHref($filePath, $line)
    {
        ...
        if (!array_key_exists($editor['url']) || !is_string($editor['url'])) {
    ...
    public function getEditorAjax($filePath, $line)
    {
        ...
        if (!array_key_exists($editor['ajax']) || !is_bool($editor['ajax'])) {
            throw new UnexpectedValueException(
    ...
    protected function getEditor($filePath, $line)
    {
        ...
        if (is_string($this->editor) && array_key_exists($this->editors[$this->editor]) && !is_callable($this->editors[$this->editor])) {
            return [
                'ajax' => false,
        ...
        if (is_callable($this->editor) || (array_key_exists($this->editors[$this->editor]) && is_callable($this->editors[$this->editor]))) {
            ...
            return [
                'ajax' => array_key_exists($callback['ajax']) ? $callback['ajax'] : false,
                'url' => array_key_exists($callback['url']) ? $callback['url'] : $callback,
            ];
        }
    ...
    protected function getResource($resource)
    {
        ...
        if (array_key_exists($this->resourceCache[$resource])) {
    ...
    private function masked($superGlobal, $superGlobalName)
    {
        ...
        foreach ($blacklisted as $key) {
            if (array_key_exists($superGlobal[$key])) {
