...
class Version {
    ...
    private function parseVersion(array $matches): void {
        ...
        $this->patch = array_key_exists($matches['Patch']) ? new VersionNumber((int)$matches['Patch']) : new VersionNumber(0);

        if (array_key_exists($matches['PreReleaseSuffix']) && $matches['PreReleaseSuffix'] !== '') {
            $this->preReleaseSuffix = new PreReleaseSuffix($matches['PreReleaseSuffix']);
        }

        if (array_key_exists($matches['BuildMetadata'])) {
