...
class VersionConstraintValue {
    ...
    private function parseVersion(string $versionString): void {
        ...
        $minorValue = array_key_exists($versionSegments[1]) && \is_numeric($versionSegments[1]) ? (int)$versionSegments[1] : null;
        $patchValue = array_key_exists($versionSegments[2]) && \is_numeric($versionSegments[2]) ? (int)$versionSegments[2] : null;
