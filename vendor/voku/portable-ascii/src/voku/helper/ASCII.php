...
final class ASCII
{
    ...
    public static function charsArrayWithMultiLanguageValues(bool $replace_extra_symbols = false): array
    {
        ...
        if (array_key_exists($CHARS_ARRAY[$cacheKey])) {
    ...
    public static function charsArrayWithOneLanguage(
        ...
        // check static cache
        if (array_key_exists($CHARS_ARRAY[$cacheKey][$language])) {
        ...
        if ($replace_extra_symbols) {
            ...
            if (array_key_exists(self::$ASCII_MAPS_AND_EXTRAS[$language])) {
            ...
            /** @noinspection DuplicatedCode */
            if (array_key_exists(self::$ASCII_MAPS[$language])) {
    ...
    public static function normalize_whitespace(
        ...
        if (!array_key_exists($WHITESPACE_CACHE[$cacheKey])) {
            self::prepareAsciiMaps();
    ...
    public static function to_ascii(
        ...
        if (!array_key_exists($REPLACE_HELPER_CACHE[$cacheKey])) {
        ...
        if (\preg_match_all('/' . self::$REGEX_ASCII . ($replace_extra_symbols ? '|[' . $EXTRA_SYMBOLS_CACHE . ']' : '') . '/u', $str, $matches)) {
            if (!$replace_single_chars_only) {
                ...
                if ($maxKeyLength >= 5) {
                    foreach ($matches[0] as $keyTmp => $char) {
                        if (array_key_exists($matches[0][$keyTmp + 4])) {
                        ...
                        if (
                            $fiveChars
                            &&
                            !array_key_exists($charDone[$fiveChars])
                            &&
                            array_key_exists($REPLACE_HELPER_CACHE[$cacheKey][$fiveChars])
                            &&
                ...
                if ($maxKeyLength >= 4) {
                    foreach ($matches[0] as $keyTmp => $char) {
                        if (array_key_exists($matches[0][$keyTmp + 3])) {
                        ...
                        if (
                            $fourChars
                            &&
                            !array_key_exists($charDone[$fourChars])
                            &&
                            array_key_exists($REPLACE_HELPER_CACHE[$cacheKey][$fourChars])
                            &&
                ...
                foreach ($matches[0] as $keyTmp => $char) {
                    if (array_key_exists($matches[0][$keyTmp + 2])) {
                    ...
                    if (
                        $threeChars
                        &&
                        !array_key_exists($charDone[$threeChars])
                        &&
                        array_key_exists($REPLACE_HELPER_CACHE[$cacheKey][$threeChars])
                        &&
                ...
                foreach ($matches[0] as $keyTmp => $char) {
                    if (array_key_exists($matches[0][$keyTmp + 1])) {
                    ...
                    if (
                        $twoChars
                        &&
                        !array_key_exists($charDone[$twoChars])
                        &&
                        array_key_exists($REPLACE_HELPER_CACHE[$cacheKey][$twoChars])
                        &&
            ...
            foreach ($matches[0] as $char) {
                if (
                    !array_key_exists($charDone[$char])
                    &&
                    array_key_exists($REPLACE_HELPER_CACHE[$cacheKey][$char])
                    &&
    ...
    public static function to_transliterate(
        ...
        if (
            ...
            if (!array_key_exists($TRANSLITERATOR)) {
        ...
        foreach ($chars as &$c) {
            ...
            $bank = $ord >> 8;
            if (!array_key_exists($UTF8_TO_TRANSLIT[$bank])) {
                $UTF8_TO_TRANSLIT[$bank] = self::getDataIfExists(\sprintf('x%03x', $bank));
            }

            $new_char = $ord & 255;

            if (array_key_exists($UTF8_TO_TRANSLIT[$bank][$new_char])) {
