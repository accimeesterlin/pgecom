...
abstract class PHP extends Engine
{
    ...
    {
        if (!array_key_exists(static::$isValidEngine[static::class])) {
    ...
    public function toString()
    {
        ...
        while (count($temp->value)) {
            ...
            $result = str_pad(
                array_key_exists($mod->value[0]) ? $mod->value[0] : '',
                static::MAX10LEN,
                '0',
                STR_PAD_LEFT
            ) . $result;
    ...
    protected function divideHelper(PHP $y)
    {
        ...
        for ($i = $x_max; $i >= $y_max + 1; --$i) {
            $x_value = &$x->value;
            $x_window = [
                array_key_exists($x_value[$i]) ? $x_value[$i] : 0,
                array_key_exists($x_value[$i - 1]) ? $x_value[$i - 1] : 0,
                array_key_exists($x_value[$i - 2]) ? $x_value[$i - 2] : 0
            ];
            $y_window = [
                $y_value[$y_max],
    ...
    public function testBit($x)
    {
        ...
        if (!array_key_exists($this->value[$digit])) {
            return false;
        }
    ...
    public function bitwise_split($split)
    {
        ...
        while ($i < $len) {
            ...
            if (!$overflow) {
                ...
                if ($overflow) {
                    $mask = (1 << $overflow) - 1;
                    $temp = array_key_exists($val[$i]) ? $val[$i] : 0;
                    $digit[] = $temp & $mask;
                }
            } else {
