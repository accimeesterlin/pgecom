...
abstract class EC extends AsymmetricKey
{
    ...
    protected static function onLoad($components)
    {
        ...
        if (!array_key_exists($components['dA']) && !array_key_exists($components['QA'])) {
            $new = new Parameters();
            $new->curve = $components['curve'];
            return $new;
        }

        $new = array_key_exists($components['dA']) ?
            new PrivateKey() :
            new PublicKey();
        $new->curve = $components['curve'];
        $new->QA = $components['QA'];

        if (array_key_exists($components['dA'])) {
    ...
    public function getCurve()
    {
        ...
        if (array_key_exists($decoded['namedCurve'])) {
    ...
    public function getEngine()
    {
        ...
        if ($this->curve instanceof TwistedEdwardsCurve) {
            return $this->curve instanceof Ed25519 && self::$engines['libsodium'] && !array_key_exists($this->context) ?
                'libsodium' : 'PHP';
        }
