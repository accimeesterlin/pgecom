...
abstract class OpenSSH
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (!array_key_exists($parts[1])) {
            ...
            $comment = array_key_exists($parts[1]) ? $parts[1] : false;
        } else {
            $asciiType = $parts[0];
            self::checkType($parts[0]);
            $key = base64_decode($parts[1]);
            $comment = array_key_exists($parts[2]) ? $parts[2] : false;
        }
        if ($key === false) {
            throw new \UnexpectedValueException('Key should be a string - not a ' . gettype($key));
        }
        ...
        self::checkType($type);
        if (array_key_exists($asciiType) && $asciiType != $type) {
    ...
    protected static function wrapPrivateKey($publicKey, $privateKey, $password, $options)
    {
        ...
        $comment = array_key_exists($options['comment']) ? $options['comment'] : self::$comment;
