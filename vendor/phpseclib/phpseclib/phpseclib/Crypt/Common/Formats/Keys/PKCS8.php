...
abstract class PKCS8 extends PKCS
{
    ...
    {
        if (!array_key_exists(static::$childOIDsLoaded)) {
    ...
    protected static function load($key, $password = '')
    {
        ...
        if (is_array($private)) {
            if (array_key_exists($private['privateKeyAlgorithm']['parameters']) && !$private['privateKeyAlgorithm']['parameters'] instanceof ASN1\Element && array_key_exists($decoded[0]['content'][1]['content'][1])) {
            ...
            if (array_key_exists($private['publicKey'])) {
        ...
        if (is_array($public)) {
            ...
            if (array_key_exists($public['publicKeyAlgorithm']['parameters']) && !$public['publicKeyAlgorithm']['parameters'] instanceof ASN1\Element && array_key_exists($decoded[0]['content'][0]['content'][1])) {
    ...
    protected static function wrapPrivateKey($key, $attr, $params, $password, $oid = null, $publicKey = '', array $options = [])
    {
        ...
        if (!empty($password) && is_string($password)) {
            ...
            $iterationCount = array_key_exists($options['iterationCount']) ? $options['iterationCount'] : self::$defaultIterationCount;
            $encryptionAlgorithm = array_key_exists($options['encryptionAlgorithm']) ? $options['encryptionAlgorithm'] : self::$defaultEncryptionAlgorithm;
            $encryptionScheme = array_key_exists($options['encryptionScheme']) ? $options['encryptionScheme'] : self::$defaultEncryptionScheme;
            $prf = array_key_exists($options['PRF']) ? $options['PRF'] : self::$defaultPRF;
