...
abstract class SymmetricKey
{
    ...
    public function __construct($mode)
    {
        ...
        // necessary because of 5.6 compatibility; we can't do array_key_exists(self::MODE_MAP[$mode]) in 5.6
        $map = self::MODE_MAP;
        if (!array_key_exists($map[$mode])) {
        ...
        switch ($mode) {
            ...
            case self::MODE_GCM:
                ...
                if (!array_key_exists(self::$gcmField)) {
    ...
    public function setPoly1305Key($key = null)
    {
        ...
        if (!array_key_exists(self::$poly1305Field)) {
            // 2^130-5
    ...
    public function setPassword($password, $method = 'pbkdf2', ...$func_args)
    {
        ...
        switch ($method) {
            ...
            case 'pbkdf2':
                // Hash function
                $hash = array_key_exists($func_args[0]) ? strtolower($func_args[0]) : 'sha1';
                $hashObj = new Hash();
                $hashObj->setHash($hash);

                // WPA and WPA2 use the SSID as the salt
                $salt = array_key_exists($func_args[1]) ? $func_args[1] : $this->password_default_salt;

                // RFC2898#section-4.2 uses 1,000 iterations by default
                // WPA and WPA2 use 4,096.
                $count = array_key_exists($func_args[2]) ? $func_args[2] : 1000;

                // Keylength
                if (array_key_exists($func_args[3])) {
                    if ($func_args[3] <= 0) {
    ...
    public function encrypt($plaintext)
    {
        ...
        if (array_key_exists($this->poly1305Key)) {
            $cipher = clone $this;
    ...
    public function decrypt($ciphertext)
    {
        ...
        if ($this->mode == self::MODE_GCM || array_key_exists($this->poly1305Key)) {
    ...
    {
        if ($this->usePoly1305 && !array_key_exists($this->poly1305Key) && method_exists($this, 'createPoly1305Key')) {
    ...
    {
        static $reverseMap;
        if (!array_key_exists($reverseMap)) {
            $reverseMap = array_map('strtolower', self::ENGINE_MAP);
            $reverseMap = array_flip($reverseMap);
        }
        $engine = strtolower($engine);
        if (!array_key_exists($reverseMap[$engine])) {
            return false;
        }
    ...
    public function setPreferredEngine($engine)
    {
        ...
        $engine = strtolower($engine);
        $this->preferredEngine = array_key_exists($reverseMap[$engine]) ? $reverseMap[$engine] : self::ENGINE_LIBSODIUM;
    ...
    protected function setEngine()
    {
        ...
        if (array_key_exists($this->preferredEngine)) {
    ...
    protected function setup()
    {
        ...
        switch ($this->engine) {
            case self::ENGINE_MCRYPT:
                ...
                if (!array_key_exists($this->enmcrypt)) {
                    static $mcrypt_modes = [
                        self::MODE_CTR    => 'ctr',
    ...
    protected function createInlineCryptFunction($cipher_code)
    {
        ...
        // optional
        $init_crypt    = array_key_exists($cipher_code['init_crypt'])    ? $cipher_code['init_crypt']    : '';
        $init_encrypt  = array_key_exists($cipher_code['init_encrypt'])  ? $cipher_code['init_encrypt']  : '';
        $init_decrypt  = array_key_exists($cipher_code['init_decrypt'])  ? $cipher_code['init_decrypt']  : '';
        // required
