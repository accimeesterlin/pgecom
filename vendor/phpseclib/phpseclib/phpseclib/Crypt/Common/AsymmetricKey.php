...
abstract class AsymmetricKey
{
    ...
    public static function load($key, $password = false)
    {
        ...
        foreach (self::$plugins[static::ALGORITHM]['Keys'] as $format) {
            if (array_key_exists(self::$invisiblePlugins[static::ALGORITHM]) && in_array($format, self::$invisiblePlugins[static::ALGORITHM])) {
                continue;
            }
            try {
        ...
        $comment = array_key_exists($components['comment']) ? $components['comment'] : null;
    ...
    public static function loadFormat($type, $key, $password = false)
    {
        ...
        if (array_key_exists(self::$plugins[static::ALGORITHM]['Keys'][$format])) {
    ...
    protected static function validatePlugin($format, $type, $method = null)
    {
        ...
        if (!array_key_exists(self::$plugins[static::ALGORITHM][$format][$type])) {
            throw new UnsupportedFormatException("$type is not a supported format");
        }
        $type = self::$plugins[static::ALGORITHM][$format][$type];
        if (array_key_exists($method) && !method_exists($type, $method)) {
    ...
    {
        if (!array_key_exists(self::$plugins[static::ALGORITHM][$format])) {
