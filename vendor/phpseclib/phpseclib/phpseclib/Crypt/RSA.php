...
abstract class RSA extends AsymmetricKey
{
    ...
    public static function createKey($bits = 2048)
    {
        ...
        if ($num_primes == 2 && $bits >= 384 && self::$defaultExponent == 65537) {
            if (!array_key_exists(self::$engines['PHP'])) {
                self::useBestEngine();
            }
    ...
    protected static function onLoad($components)
    {
        ...
        if ($components['isPublicKey'] || !array_key_exists($components['privateExponent'])) {
        ...
        if ($components['format'] == PSS::class) {
            ...
            if (array_key_exists($components['hash'])) {
                $key = $key->withHash($components['hash']);
            }
            if (array_key_exists($components['MGFHash'])) {
                $key = $key->withMGFHash($components['MGFHash']);
            }
            if (array_key_exists($components['saltLength'])) {
    ...
    {
        if (!array_key_exists(self::$configFile)) {
    ...
    {
        return !array_key_exists($this->modulus) ? 0 : $this->modulus->getLength();
    }
