...
abstract class Random
{
    ...
    public static function string($length)
    {
        ...
        if ($crypto === false) {
            ...
            $_OLD_SESSION = array_key_exists($_SESSION) ? $_SESSION : false;
            if ($old_session_id != '') {
                session_write_close();
            }
            ...
            $v = (array_key_exists($_SERVER) ? self::safe_serialize($_SERVER) : '') .
                 (array_key_exists($_POST) ? self::safe_serialize($_POST) : '') .
                 (array_key_exists($_GET) ? self::safe_serialize($_GET) : '') .
                 (array_key_exists($_COOKIE) ? self::safe_serialize($_COOKIE) : '') .
                 self::safe_serialize($GLOBALS) .
                 self::safe_serialize($_SESSION) .
                 self::safe_serialize($_OLD_SESSION);
            $v = $seed = $_SESSION['seed'] = sha1($v, true);
            if (!array_key_exists($_SESSION['count'])) {
                $_SESSION['count'] = 0;
            }
            $_SESSION['count']++;
    ...
    private static function safe_serialize(&$arr)
    {
        ...
        if (array_key_exists($arr['__phpseclib_marker'])) {
            return '';
        }
        $safearr = [];
