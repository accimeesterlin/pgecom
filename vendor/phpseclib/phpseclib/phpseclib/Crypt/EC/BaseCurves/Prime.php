...
class Prime extends Base
{
    ...
    {
        if (!array_key_exists($this->factory)) {
    ...
    public function addPoint(array $p, array $q)
    {
        ...
        if (array_key_exists($p[2]) && array_key_exists($q[2])) {
            if (array_key_exists($p['fresh']) && array_key_exists($q['fresh'])) {
                return $this->jacobianAddPointMixedXY($p, $q);
            }
            if (array_key_exists($p['fresh'])) {
                return $this->jacobianAddPointMixedX($q, $p);
            }
            if (array_key_exists($q['fresh'])) {
    ...
    public function multiplyAddPoints(array $points, array $scalars)
    {
        ...
        $wndWidth = [array_key_exists($points[0]['nafwidth']) ? $points[0]['nafwidth'] : 7];
        for ($i = 1; $i < $length; $i++) {
            $wnd[] = $this->getNAFPoints($points[$i], 1);
            $wndWidth[] = array_key_exists($points[$i]['nafwidth']) ? $points[$i]['nafwidth'] : 1;
        }
        ...
        for ($i = $length - 1; $i >= 1; $i -= 2) {
            ...
            for ($j = 0; $j < $max; $j++) {
                $ja = array_key_exists($jsf[0][$j]) ? $jsf[0][$j] : 0;
                $jb = array_key_exists($jsf[1][$j]) ? $jsf[1][$j] : 0;
        ...
        for ($i = $max; $i >= 0; $i--) {
            ...
            while ($i >= 0) {
                $zero = true;
                for ($j = 0; $j < $length; $j++) {
                    $temp[$j] = array_key_exists($naf[$j][$i]) ? $naf[$j][$i] : 0;
                    if ($temp[$j] != 0) {
                        $zero = false;
                    }
                }
