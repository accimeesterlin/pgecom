...
class PublicKey extends EC implements Common\PublicKey
{
    ...
    public function verify($message, $signature)
    {
        ...
        if ($this->curve instanceof TwistedEdwardsCurve) {
            ...
            if ($this->curve instanceof Ed25519 && self::$engines['libsodium'] && !array_key_exists($this->context)) {
            ...
            if ($curve instanceof Ed25519) {
                $dom2 = !array_key_exists($this->context) ? '' :
                    'SigEd25519 no Ed25519 collisions' . "\0" . chr(strlen($this->context)) . $this->context;
            } else {
                $context = array_key_exists($this->context) ? $this->context : '';
