...
trait Common
{
    ...
    protected static function loadCurveByParam(array $params)
    {
        ...
        if (array_key_exists($params['namedCurve'])) {
        ...
        if (array_key_exists($params['implicitCurve'])) {
            if (!array_key_exists(self::$implicitCurve)) {
                throw new \RuntimeException('Implicit curves can be provided by calling setImplicitCurve');
            }
            return self::$implicitCurve;
        }
        if (array_key_exists($params['specifiedCurve'])) {
    ...
    {
        $useNamedCurves = array_key_exists($options['namedCurve']) ? $options['namedCurve'] : self::$useNamedCurves;

        $reflect = new \ReflectionClass($curve);
        $name = $reflect->getShortName();
        if ($useNamedCurves) {
            if (array_key_exists(self::$curveOIDs[$name])) {
                if ($reflect->isFinal()) {
