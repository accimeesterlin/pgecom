...
abstract class PKCS8 extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $type = array_key_exists($key['privateKey']) ? 'privateKey' : 'publicKey';
        ...
        if (array_key_exists($key['parameters']) && $params != $key['parameters']) {
        ...
        $components['QA'] = array_key_exists($key['publicKey']) ?
    ...
    private static function loadEdDSA(array $key)
    {
        ...
        if (array_key_exists($key['publicKey'])) {
            if (!array_key_exists($components['curve'])) {
        ...
        if (array_key_exists($key['privateKey']) && !array_key_exists($components['QA'])) {
