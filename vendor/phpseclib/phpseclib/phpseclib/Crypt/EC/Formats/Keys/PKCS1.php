...
abstract class PKCS1 extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (strpos($key, 'BEGIN EC PARAMETERS') && strpos($key, 'BEGIN EC PRIVATE KEY')) {
            ...
            if (array_key_exists($ecPrivate['parameters'])) {
            ...
            if (array_key_exists($components['curve']) && self::encodeParameters($ecParams, false, []) != self::encodeParameters($components['curve'], false, [])) {
            ...
            $components['QA'] = array_key_exists($ecPrivate['publicKey']) ?
