...
abstract class DH extends AsymmetricKey
{
    ...
    {
        if (!array_key_exists($components['privateKey']) && !array_key_exists($components['publicKey'])) {
            $new = new Parameters();
        } else {
            $new = array_key_exists($components['privateKey']) ?
                new PrivateKey() :
                new PublicKey();
        }
        ...
        if (array_key_exists($components['privateKey'])) {
            $new->privateKey = $components['privateKey'];
        }
        if (array_key_exists($components['publicKey'])) {
