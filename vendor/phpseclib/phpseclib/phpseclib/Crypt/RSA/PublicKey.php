...
class PublicKey extends RSA implements Common\PublicKey
{
    ...
    private function rsassa_pkcs1_v1_5_relaxed_verify($m, $s)
    {
        ...
        if (!array_key_exists($decoded) || $decoded === false) {
            return false;
        }

        if (!array_key_exists($oids[$decoded['digestAlgorithm']['algorithm']])) {
            return false;
        }

        if (array_key_exists($decoded['digestAlgorithm']['parameters']) && $decoded['digestAlgorithm']['parameters'] !== ['null' => '']) {
            return false;
        }
