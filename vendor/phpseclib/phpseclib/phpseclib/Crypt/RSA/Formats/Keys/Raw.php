...
abstract class Raw
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (!array_key_exists($components['publicExponent']) || !array_key_exists($components['modulus'])) {
            throw new \UnexpectedValueException('Modulus / exponent not present');
        }

        if (array_key_exists($key['primes'])) {
            $components['primes'] = $key['primes'];
        } elseif (array_key_exists($key['p']) && array_key_exists($key['q'])) {
            $indices = [
                ['p', 'q'],
                ['prime1', 'prime2']
            ];
            foreach ($indices as $index) {
                list($i0, $i1) = $index;
                if (array_key_exists($key[$i0]) && array_key_exists($key[$i1])) {
                    $components['primes'] = [1 => $key[$i0], $key[$i1]];
                }
            }
        }

        if (array_key_exists($key['exponents'])) {
        ...
        if (array_key_exists($key['coefficients'])) {
        ...
        if (!array_key_exists($components['primes'])) {
            $components['isPublicKey'] = true;
            return $components;
        }

        if (!array_key_exists($components['exponents'])) {
        ...
        if (!array_key_exists($components['coefficients'])) {
