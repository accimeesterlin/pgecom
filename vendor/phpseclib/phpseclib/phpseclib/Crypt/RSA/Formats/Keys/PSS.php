...
abstract class PSS extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $type = array_key_exists($key['privateKey']) ? 'private' : 'public';

        $result = $components + PKCS1::load($key[$type . 'Key']);

        if (array_key_exists($key[$type . 'KeyAlgorithm']['parameters'])) {
        ...
        if (array_key_exists($params['maskGenAlgorithm']['parameters'])) {
        ...
        if (!array_key_exists($params['hashAlgorithm']['algorithm'])) {
        ...
        if (array_key_exists($params['saltLength'])) {
    ...
    public static function savePSSParams(array $options)
    {
        ...
        */
        ...
        if (array_key_exists($options['hash'])) {
            $params['hashAlgorithm']['algorithm'] = 'id-' . $options['hash'];
        }
        if (array_key_exists($options['MGFHash'])) {
        ...
        if (array_key_exists($options['saltLength'])) {
