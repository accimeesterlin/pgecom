...
abstract class ASN1
{
    ...
    private static function decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        if (!array_key_exists($encoded[$encoded_pos])) {
            return false;
        }
        ...
        switch ($tag) {
            ...
            case self::TYPE_SET:
                ...
                while ($content_pos < $content_len) {
                    ...
                    if (!array_key_exists($current['headerlength']) && substr($content, $content_pos, 2) == "\0\0") {
    ...
    public static function asn1map($decoded, $mapping, $special = [])
    {
        ...
        if (array_key_exists($mapping['explicit']) && is_array($decoded['content'])) {
        ...
        switch (true) {
            case $mapping['type'] == self::TYPE_ANY:
                $intype = $decoded['type'];
                // !array_key_exists(self::ANY_MAP[$intype]) produces a fatal error on PHP 5.6
                if (array_key_exists($decoded['constant']) || !array_key_exists($intype, self::ANY_MAP) || (ord(self::$encoded[$decoded['start']]) & 0x20)) {
            ...
            case $mapping['type'] == self::TYPE_CHOICE:
                foreach ($mapping['children'] as $key => $option) {
                    switch (true) {
                        case array_key_exists($option['constant']) && $option['constant'] == $decoded['constant']:
                        case !array_key_exists($option['constant']) && $option['type'] == $decoded['type']:
                            $value = self::asn1map($decoded, $option, $special);
                            break;
                        case !array_key_exists($option['constant']) && $option['type'] == self::TYPE_CHOICE:
                    ...
                    if (array_key_exists($value)) {
                        if (array_key_exists($special[$key])) {
                ...
                return null;
            case array_key_exists($mapping['implicit']):
            case array_key_exists($mapping['explicit']):
        ...
        if (array_key_exists($mapping['implicit'])) {
        ...
        switch ($decoded['type']) {
            case self::TYPE_SEQUENCE:
                ...
                if (array_key_exists($mapping['min']) && array_key_exists($mapping['max'])) {
                ...
                foreach ($mapping['children'] as $key => $child) {
                    ...
                    if ($maymatch) {
                        ...
                        if ($child['type'] != self::TYPE_CHOICE) {
                            ...
                            $constant = null;
                            if (array_key_exists($temp['constant'])) {
                                $tempClass = $temp['type'];
                            }
                            if (array_key_exists($child['class'])) {
                                $childClass = $child['class'];
                                $constant = $child['cast'];
                            } elseif (array_key_exists($child['constant'])) {
                                $childClass = self::CLASS_CONTEXT_SPECIFIC;
                                $constant = $child['constant'];
                            }

                            if (array_key_exists($constant) && array_key_exists($temp['constant'])) {
                                // Can only match if constants and class match.
                                $maymatch = $constant == $temp['constant'] && $childClass == $tempClass;
                            } else {
                                // Can only match if no constant expected and type matches or is generic.
                                $maymatch = !array_key_exists($child['constant']) && array_search($child['type'], [$temp['type'], self::TYPE_ANY, self::TYPE_CHOICE]) !== false;
                            }
                        }
                    }
                    ...
                    if ($maymatch) {
                        ...
                        $map[$key] = $candidate;
                        $i++;
                    } elseif (array_key_exists($child['default'])) {
                        $map[$key] = $child['default'];
                    } elseif (!array_key_exists($child['optional'])) {
            ...
            case self::TYPE_SET:
                ...
                return $map;
            case self::TYPE_OBJECT_IDENTIFIER:
                return array_key_exists(self::$oids[$decoded['content']]) ? self::$oids[$decoded['content']] : $decoded['content'];
            case self::TYPE_UTC_TIME:
            case self::TYPE_GENERALIZED_TIME:
                ...
                // in theory, doing array_key_exists($mapping['implicit']) would work but malformed certs do exist
            ...
            case self::TYPE_ENUMERATED:
                ...
                if (array_key_exists($mapping['mapping'])) {
                    ...
                    return array_key_exists($mapping['mapping'][$temp]) ?
    ...
    private static function encode_der($source, $mapping, $idx = null, $special = [])
    {
        ...
        if (array_key_exists($mapping['default']) && $source === $mapping['default']) {
            return '';
        }

        if (array_key_exists($idx)) {
            if (array_key_exists($special[$idx])) {
        ...
        switch ($tag) {
            ...
            case self::TYPE_SEQUENCE:
                ...
                foreach ($mapping['children'] as $key => $child) {
                    if (!array_key_exists($key, $source)) {
                        if (!array_key_exists($child['optional'])) {
                            return false;
                        }
                        continue;
                    }
                    ...
                    // if array_key_exists($child['constant']) is true then array_key_exists($child['optional']) should be true as well
                    if (array_key_exists($child['constant'])) {
                        /*
                         ...
                         */
                        if (array_key_exists($child['explicit']) || $child['type'] == self::TYPE_CHOICE) {
            ...
            case self::TYPE_CHOICE:
                ...
                if ($temp && array_key_exists($mapping['cast'])) {
            ...
            case self::TYPE_BIT_STRING:
                if (array_key_exists($mapping['mapping'])) {
                    ...
                    if (array_key_exists($mapping['min']) && $mapping['min'] >= 1 && $size < $mapping['min']) {
                        $size = $mapping['min'] - 1;
                    }
            ...
            case self::TYPE_ANY:
                ...
                foreach ($loc as $part) {
                    if (!array_key_exists($filters[$part])) {
                        $filters = false;
                        break;
                    }
        ...
        if (array_key_exists($mapping['cast'])) {
            if (array_key_exists($mapping['explicit']) || $mapping['type'] == self::TYPE_CHOICE) {
    ...
    public static function encodeOID($source)
    {
        ...
        if (!preg_match('#(?:\d+\.)+#', $source)) {
            $oid = array_key_exists(self::$reverseOIDs[$source]) ? self::$reverseOIDs[$source] : false;
        } else {
            $oid = $source;
        }
        if ($oid === false) {
    ...
    {
        // array_key_exists(self::STRING_TYPE_SIZE[$from] returns a fatal error on PHP 5.6
    ...
    {
        return array_key_exists(self::$reverseOIDs[$name]) ? self::$reverseOIDs[$name] : $name;
    }
}
