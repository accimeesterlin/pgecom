...
class X509
{
    ...
    {
        if (is_array($cert) && array_key_exists($cert['tbsCertificate'])) {
            unset($this->currentCert);
            unset($this->currentKeyIdentifier);
            $this->dn = $cert['tbsCertificate']['subject'];
            if (!array_key_exists($this->dn)) {
                return false;
            }
        ...
        if (!array_key_exists($x509) || $x509 === false) {
            $this->currentCert = false;
            return false;
        }
                ...
                if (!is_bool($map)) {
                    ...
                    if ($id == 'id-ce-certificatePolicies') {
                        for ($j = 0; $j < count($value); $j++) {
                            if (!array_key_exists($value[$j]['policyQualifiers'])) {
                                continue;
                            }
            ...
            for ($i = 0; $i < $size; $i++) {
                ...
                switch ($id) {
                    ...
                    case 'id-ce-authorityKeyIdentifier': // use 00 as the serial number instead of an empty string
                        if (array_key_exists($value['authorityCertSerialNumber'])) {
    ...
    private function getMapping($extnId)
    {
        ...
        if (array_key_exists(self::$extensions[$extnId])) {
    ...
    {
        if (!is_array($this->currentCert) || !array_key_exists($this->currentCert['tbsCertificate'])) {
            return false;
        }

        $components = parse_url($url);
        if (!array_key_exists($components['host'])) {
            return false;
        }
    ...
    public function validateDate($date = null)
    {
        ...
        $notBefore = array_key_exists($notBefore['generalTime']) ? $notBefore['generalTime'] : $notBefore['utcTime'];

        $notAfter = $this->currentCert['tbsCertificate']['validity']['notAfter'];
        $notAfter = array_key_exists($notAfter['generalTime']) ? $notAfter['generalTime'] : $notAfter['utcTime'];
    ...
    private static function fetchURL($url)
    {
        ...
        switch ($parts['scheme']) {
            case 'http':
                $fsock = @fsockopen($parts['host'], array_key_exists($parts['port']) ? $parts['port'] : 80);
                if (!$fsock) {
                    return false;
                }
    ...
    private function testForIntermediate($caonly, $count)
    {
        ...
        foreach ($opts as $opt) {
            if ($opt['accessMethod'] == 'id-ad-caIssuers') {
                ...
                // discusses
                if (array_key_exists($opt['accessLocation']['uniformResourceIdentifier'])) {
    ...
    {
        if (!is_array($this->currentCert) || !array_key_exists($this->signatureSubject)) {
            return null;
        }
        ...
        switch (true) {
            case array_key_exists($this->currentCert['tbsCertificate']):
                // self-signed cert
                switch (true) {
                    ...
                    case defined('FILE_X509_IGNORE_TYPE') && $this->getIssuerDN(self::DN_STRING) === $this->getDN(self::DN_STRING):
                        ...
                        switch (true) {
                            ...
                            case !$subjectKeyID:
                            case array_key_exists($authorityKey['keyIdentifier']) && $authorityKey['keyIdentifier'] === $subjectKeyID:
                        ...
                        switch (true) {
                            ...
                            case defined('FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertificate']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                ...
                                switch (true) {
                                    ...
                                    case !$subjectKeyID:
                                    case array_key_exists($authorityKey['keyIdentifier']) && $authorityKey['keyIdentifier'] === $subjectKeyID:
                                        if (is_array($authorityKey) && array_key_exists($authorityKey['authorityCertSerialNumber']) && !$authorityKey['authorityCertSerialNumber']->equals($ca['tbsCertificate']['serialNumber'])) {
                ...
                } elseif (!array_key_exists($signingCert) || $caonly) {
                    return $this->testForIntermediate($caonly, $count) && $this->validateSignature($caonly);
                }
            ...
            case array_key_exists($this->currentCert['certificationRequestInfo']):
                return $this->validateSignatureHelper(
            ...
            case array_key_exists($this->currentCert['publicKeyAndChallenge']):
                return $this->validateSignatureHelper(
            ...
            case array_key_exists($this->currentCert['tbsCertList']):
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < count($this->CAs); $i++) {
                        $ca = $this->CAs[$i];
                        switch (true) {
                ...
                if (!array_key_exists($signingCert)) {
                    return false;
                }
                return $this->validateSignatureHelper(
                    $signingCert['tbsCertificate']['subjectPublicKeyInfo']['algorithm']['algorithm'],
    ...
    public function setDNProp($propName, $propValue, $type = 'utf8String')
    {
        ...
        foreach ((array) $propValue as $v) {
            if (!is_array($v) && array_key_exists($type)) {
                $v = [$type => $v];
            }
            $this->dn['rdnSequence'][] = [
                [
    ...
    public function setDN($dn, $merge = false, $type = 'utf8String')
    {
        ...
        if (is_array($dn)) {
            if (array_key_exists($dn['rdnSequence'])) {
    ...
    {
        if (!array_key_exists($dn)) {
            $dn = array_key_exists($this->currentCert['tbsCertList']) ? $this->currentCert['tbsCertList']['issuer'] : $this->dn;
        }
        ...
        foreach ($dn['rdnSequence'] as $field) {
            ...
            $output .= $desc . '=' . $value;
            $result[$desc] = array_key_exists($result[$desc]) ?
    ...
    {
        switch (true) {
            case !array_key_exists($this->currentCert) || !is_array($this->currentCert):
                break;
            case array_key_exists($this->currentCert['tbsCertificate']):
        ...
        while (true) {
            ...
            for ($i = 0; $i < count($this->CAs); $i++) {
                ...
                if ($currentCert['tbsCertificate']['issuer'] === $ca['tbsCertificate']['subject']) {
                    ...
                    switch (true) {
                        case !is_array($authorityKey):
                        case is_array($authorityKey) && array_key_exists($authorityKey['keyIdentifier']) && $authorityKey['keyIdentifier'] === $subjectKeyID:
                            if ($currentCert === $ca) {
                                break 3;
                            }
                            $chain[] = $ca;
    ...
    {
        if (array_key_exists($this->publicKey)) {
            return $this->publicKey;
        }

        if (array_key_exists($this->currentCert) && is_array($this->currentCert)) {
            $paths = [
    ...
    {
        if (is_array($csr) && array_key_exists($csr['certificationRequestInfo'])) {
            unset($this->currentCert);
            unset($this->currentKeyIdentifier);
            unset($this->signatureSubject);
        ...
        if (!array_key_exists($csr) || $csr === false) {
            $this->currentCert = false;
            return false;
        }
    ...
    {
        if (is_array($spkac) && array_key_exists($spkac['publicKeyAndChallenge'])) {
            unset($this->currentCert);
            unset($this->currentKeyIdentifier);
            unset($this->signatureSubject);
        ...
        if (!array_key_exists($spkac) || !is_array($spkac)) {
            $this->currentCert = false;
            return false;
        }
    ...
    {
        if (is_array($crl) && array_key_exists($crl['tbsCertList'])) {
        ...
        if (!array_key_exists($crl) || $crl === false) {
            $this->currentCert = false;
            return false;
        }
    ...
    public function sign($issuer, $subject)
    {
        ...
        if (array_key_exists($subject->publicKey) && !($subjectPublicKey = $subject->formatSubjectPublicKey())) {
            return false;
        }

        $currentCert = array_key_exists($this->currentCert) ? $this->currentCert : null;
        $signatureSubject = array_key_exists($this->signatureSubject) ? $this->signatureSubject : null;
        ...
        if (array_key_exists($subject->currentCert) && is_array($subject->currentCert) && array_key_exists($subject->currentCert['tbsCertificate'])) {
            ...
            if (array_key_exists($subject->domains)) {
                $this->removeExtension('id-ce-subjectAltName');
            }
        } elseif (array_key_exists($subject->currentCert) && is_array($subject->currentCert) && array_key_exists($subject->currentCert['tbsCertList'])) {
            return false;
        } else {
            if (!array_key_exists($subject->publicKey)) {
                return false;
            }
        ...
        if (array_key_exists($issuer->currentKeyIdentifier)) {
            ...
            //if (array_key_exists($issuer->serialNumber)) {
            //    $extensions[count($extensions) - 1]['authorityCertSerialNumber'] = $issuer->serialNumber;
            //}
            //unset($extensions);
        }

        if (array_key_exists($subject->currentKeyIdentifier)) {
            $this->setExtension('id-ce-subjectKeyIdentifier', $subject->currentKeyIdentifier);
        }

        $altName = [];

        if (array_key_exists($subject->domains) && count($subject->domains)) {
            $altName = array_map(['\phpseclib3\File\X509', 'dnsName'], $subject->domains);
        }

        if (array_key_exists($subject->ipAddresses) && count($subject->ipAddresses)) {
    ...
    public function signCSR()
    {
        ...
        if (array_key_exists($this->currentCert) && is_array($this->currentCert) && array_key_exists($this->currentCert['certificationRequestInfo'])) {
    ...
    public function signSPKAC()
    {
        ...
        if (array_key_exists($this->currentCert) && is_array($this->currentCert) && array_key_exists($this->currentCert['publicKeyAndChallenge'])) {
            $this->currentCert['signatureAlgorithm']['algorithm'] = $signatureAlgorithm;
    ...
    public function signCRL($issuer, $crl)
    {
        ...
        if (array_key_exists($crl->currentCert) && is_array($crl->currentCert) && array_key_exists($crl->currentCert['tbsCertList'])) {
        ...
        $version = array_key_exists($tbsCertList['version']) ? $tbsCertList['version'] : 0;
        if (!$version) {
    ...
    private function &extensions(&$root, $path = null, $create = false)
    {
        ...
        switch (true) {
            ...
            case !is_array($root):
                break;
            case array_key_exists($root['tbsCertificate']):
                $path = 'tbsCertificate/extensions';
                break;
            case array_key_exists($root['tbsCertList']):
                $path = 'tbsCertList/crlExtensions';
                break;
            case array_key_exists($root['certificationRequestInfo']):
    ...
    private function removeExtensionHelper($id, $path = null)
    {
        ...
        // fix for https://bugs.php.net/75433 affecting PHP 7.2
        if (!array_key_exists($extensions[0])) {
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (true) {
            case is_string($key):
                break;
            case is_array($key) && array_key_exists($key['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey']):
                return $this->computeKeyIdentifier($key['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey'], $method);
            case is_array($key) && array_key_exists($key['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey']):
            ...
            case $key instanceof X509:
                if (array_key_exists($key->publicKey)) {
                    return $this->computeKeyIdentifier($key->publicKey, $method);
                }
                if (array_key_exists($key->privateKey)) {
                    return $this->computeKeyIdentifier($key->privateKey, $method);
                }
                if (array_key_exists($key->currentCert['tbsCertificate']) || array_key_exists($key->currentCert['certificationRequestInfo'])) {
    ...
    public function setIPAddress(...$ipAddresses)
    {
        ...
        /*
        if (!array_key_exists($this->domains)) {
            $this->removeDNProp('id-at-commonName');
    ...
    {
        if (array_key_exists($this->currentCert['tbsCertList'])) {
    ...
    {
        if (!array_key_exists($crl)) {
            $crl = $this->currentCert;
        }

        if (!array_key_exists($crl['tbsCertList'])) {
            return false;
        }
    ...
    {
        if (array_key_exists(self::$extensions[$id]) && self::$extensions[$id] !== $mapping) {
    ...
    {
        return array_key_exists(self::$extensions[$id]) ? self::$extensions[$id] : null;
    }
