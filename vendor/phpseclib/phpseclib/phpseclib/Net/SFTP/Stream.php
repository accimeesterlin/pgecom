...
class Stream
{
    ...
    protected function parse_path($path)
    {
        ...
        if (array_key_exists($this->context)) {
            ...
            if (array_key_exists($context['notification'])) {
        ...
        if (preg_match('/^{[a-z0-9]+}$/i', $host)) {
            ...
            if (array_key_exists($context[$scheme]['session'])) {
                $sftp = $context[$scheme]['session'];
            }
            if (array_key_exists($context[$scheme]['sftp'])) {
                $sftp = $context[$scheme]['sftp'];
            }
            if (array_key_exists($sftp) && $sftp instanceof SFTP) {
                $this->sftp = $sftp;
                return $path;
            }
            if (array_key_exists($context[$scheme]['username'])) {
                $user = $context[$scheme]['username'];
            }
            if (array_key_exists($context[$scheme]['password'])) {
                $pass = $context[$scheme]['password'];
            }
            if (array_key_exists($context[$scheme]['privkey']) && $context[$scheme]['privkey'] instanceof PrivateKey) {
                $pass = $context[$scheme]['privkey'];
            }

            if (!array_key_exists($user) || !array_key_exists($pass)) {
                return false;
            }

            // casting $pass to a string is necessary in the event that it's a \phpseclib3\Crypt\RSA object
            if (array_key_exists(self::$instances[$host][$port][$user][(string) $pass])) {
                $this->sftp = self::$instances[$host][$port][$user][(string) $pass];
            } else {
                $this->sftp = new SFTP($host, $port);
                $this->sftp->disableStatCache();
                if (array_key_exists($this->notification) && is_callable($this->notification)) {
    ...
    private function _stream_metadata($path, $option, $var)
    {
        ...
        switch ($option) {
            case 1: // PHP_STREAM_META_TOUCH
                $time = array_key_exists($var[0]) ? $var[0] : null;
                $atime = array_key_exists($var[1]) ? $var[1] : null;
    ...
    {
        if (array_key_exists($this->entries[$this->pos])) {
