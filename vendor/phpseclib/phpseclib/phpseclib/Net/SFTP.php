...
class SFTP extends SSH2
{
    ...
    private function init_sftp_connection()
    {
        ...
        */
        ...
        if (array_key_exists($this->extensions['versions']) && (!$this->preferredVersion || $this->preferredVersion != $this->version)) {
        ...
        */
        /*
        if (array_key_exists($this->extensions['newline@vandyke.com'])) {
            ...
            switch ($this->packet_type) {
                case NET_SFTP_NAME:
                    ...
                    for ($i = 0; $i < $count; $i++) {
                        ...
                        if (!array_key_exists($attributes['type']) && $this->version < 4) {
                        ...
                        if (array_key_exists($attributes['type']) && $attributes['type'] == NET_SFTP_TYPE_DIRECTORY && ($shortname != '.' && $shortname != '..')) {
    ...
    private function comparator($a, $b)
    {
        switch (true) {
            ...
            case $a['filename'] === '..' || $b['filename'] === '..':
                ...
                return $a['filename'] === '..' ? -1 : 1;
            case array_key_exists($a['type']) && $a['type'] === NET_SFTP_TYPE_DIRECTORY:
                if (!array_key_exists($b['type'])) {
                    return 1;
                }
                if ($b['type'] !== $a['type']) {
                    return -1;
                ...
                break;
            case array_key_exists($b['type']) && $b['type'] === NET_SFTP_TYPE_DIRECTORY:
                return 1;
        }
        foreach ($this->sortOptions as $sort => $order) {
            if (!array_key_exists($a[$sort]) || !array_key_exists($b[$sort])) {
                if (array_key_exists($a[$sort])) {
                    return -1;
                }
                if (array_key_exists($b[$sort])) {
                    return 1;
                }
                return 0;
            }
    ...
    private function update_stat_cache($path, $value)
    {
        ...
        foreach ($dirs as $i => $dir) {
            ...
            if (!array_key_exists($temp[$dir])) {
                $temp[$dir] = [];
            }
            if ($i === $max) {
                if (is_object($temp[$dir]) && is_object($value)) {
                    if (!array_key_exists($value->stat) && array_key_exists($temp[$dir]->stat)) {
                        $value->stat = $temp[$dir]->stat;
                    }
                    if (!array_key_exists($value->lstat) && array_key_exists($temp[$dir]->lstat)) {
    ...
    public function stat($filename)
    {
        ...
        if ($this->use_stat_cache) {
            ...
            if (is_array($result) && array_key_exists($result['.']) && array_key_exists($result['.']->stat)) {
                return $result['.']->stat;
            }
            if (is_object($result) && array_key_exists($result->stat)) {
                return $result->stat;
            }
        }
    ...
    public function lstat($filename)
    {
        ...
        if ($this->use_stat_cache) {
            $result = $this->query_stat_cache($filename);
            if (is_array($result) && array_key_exists($result['.']) && array_key_exists($result['.']->lstat)) {
                return $result['.']->lstat;
            }
            if (is_object($result) && array_key_exists($result->lstat)) {
                return $result->lstat;
            }
        }
        ...
        if (array_key_exists($lstat['type'])) {
    ...
    public function touch($filename, $time = null, $atime = null)
    {
        ...
        if (!array_key_exists($time)) {
            $time = time();
        }
        if (!array_key_exists($atime)) {
            $atime = $time;
        }
    ...
    private function setstat_recursive($path, $attr, &$i)
    {
        ...
        foreach ($entries as $filename => $props) {
            if (!array_key_exists($props['type'])) {
                return false;
            }
    ...
    public function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null)
    {
        ...
        while ($dataCallback || ($size === 0 || $sent < $size)) {
            if ($dataCallback) {
                ...
                $temp = array_key_exists($fp) ? fread($fp, $sftp_packet_size) : substr($data, $sent, $sftp_packet_size);
                if ($temp === false || $temp === '') {
                    break;
                }
            }
        ...
        if ($mode & SFTP::SOURCE_LOCAL_FILE) {
            if (array_key_exists($fp) && is_resource($fp)) {
                fclose($fp);
            }
    ...
    public function get($remote_file, $local_file = false, $offset = 0, $length = -1, $progressCallback = null)
    {
        ...
        return array_key_exists($content) ? $content : true;
    }
    ...
    private function get_xstat_cache_prop($path, $prop, $type)
    {
        ...
        if ($this->use_stat_cache) {
            ...
            if (is_object($result) && array_key_exists($result->$type)) {
        ...
        if ($result === false || !array_key_exists($result[$prop])) {
            return false;
        }
    ...
    private function get_sftp_packet($request_id = null)
    {
        ...
        if (array_key_exists($request_id) && array_key_exists($this->requestBuffer[$request_id])) {
        ...
        if (array_key_exists($request_id) && $this->use_request_id && $packet_id != $request_id) {
    ...
    public function getSupportedVersions()
    {
        ...
        if (array_key_exists($this->extensions['versions'])) {
