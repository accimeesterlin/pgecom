...
class Account extends ApiResource
{
    ...
    public function serializeParameters($force = false)
    {
        ...
        if (array_key_exists($this->_values['legal_entity'])) {
            $entity = $this['legal_entity'];
            if (array_key_exists($entity->_values['additional_owners'])) {
                $owners = $entity['additional_owners'];
                $entityUpdate = array_key_exists($update['legal_entity']) ? $update['legal_entity'] : [];
                $entityUpdate['additional_owners'] = $this->serializeAdditionalOwners($entity, $owners);
                $update['legal_entity'] = $entityUpdate;
            }
        }
        if (array_key_exists($this->_values['individual'])) {
            $individual = $this['individual'];
            if (($individual instanceof Person) && !array_key_exists($update['individual'])) {
    ...
    {
        if (array_key_exists($legalEntity->_originalValues['additional_owners'])) {
