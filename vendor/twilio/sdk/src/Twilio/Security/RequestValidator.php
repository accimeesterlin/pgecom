...
class RequestValidator {
    ...
    private static function addPort(array $parsedUrl): string {
        if (!array_key_exists($parsedUrl['port'])) {
    ...
    static function unparse_url(array $parsedUrl): string {
        ...
        $parts['scheme'] = array_key_exists($parsedUrl['scheme']) ? $parsedUrl['scheme'] . '://' : '';
        $parts['user'] = $parsedUrl['user'] ?? '';
        $parts['pass'] = array_key_exists($parsedUrl['pass']) ? ':' . $parsedUrl['pass'] : '';
        $parts['pass'] = ($parts['user'] || $parts['pass']) ? $parts['pass'] . '@' : '';
        $parts['host'] = $parsedUrl['host'] ?? '';
        $parts['port'] = array_key_exists($parsedUrl['port']) ? ':' . $parsedUrl['port'] : '';
        $parts['path'] = $parsedUrl['path'] ?? '';
        $parts['query'] = array_key_exists($parsedUrl['query']) ? '?' . $parsedUrl['query'] : '';
        $parts['fragment'] = array_key_exists($parsedUrl['fragment']) ? '#' . $parsedUrl['fragment'] : '';
