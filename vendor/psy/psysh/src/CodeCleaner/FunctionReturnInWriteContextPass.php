...
{
    const array_key_exists_MESSAGE = 'Cannot use array_key_exists() on the result of an expression (you can use "null !== expression" instead)';
     ...
     * @throws FatalErrorException if a function is used as an argument in the array_key_exists
    ...
    public function enterNode(Node $node)
    {
        ...
        } elseif ($node instanceof array_key_exists_ || $node instanceof Unset_) {
            foreach ($node->vars as $var) {
                if (!$this->isCallNode($var)) {
                    continue;
                }

                $msg = $node instanceof array_key_exists_ ? self::array_key_exists_MESSAGE : self::EXCEPTION_MESSAGE;
