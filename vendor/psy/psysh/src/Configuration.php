...
class Configuration
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (array_key_exists($config['configFile'])) {
            $this->configFile = $config['configFile'];
        } elseif (array_key_exists($_SERVER['PSYSH_CONFIG']) && $_SERVER['PSYSH_CONFIG']) {
            $this->configFile = $_SERVER['PSYSH_CONFIG'];
        }

        // legacy baseDir option
        if (array_key_exists($config['baseDir'])) {
    ...
    {
        if (array_key_exists($this->configFile)) {
    ...
    public function loadConfig(array $options)
    {
        ...
        if (array_key_exists($options['tabCompletion'])) {
        ...
        if (array_key_exists($options['tabCompletionMatchers'])) {
    ...
    {
        if (array_key_exists($this->historyFile)) {
    ...
    {
        return array_key_exists($this->useReadline) ? ($this->hasReadline && $this->useReadline) : $this->hasReadline;
    }
    ...
    {
        if (!array_key_exists($this->readline)) {
    ...
    {
        return array_key_exists($this->usePcntl) ? ($this->hasPcntl && $this->usePcntl) : $this->hasPcntl;
    }
    ...
    {
        if (array_key_exists($this->useUnicode)) {
    ...
    {
        if (!array_key_exists($this->cleaner)) {
    ...
    {
        return array_key_exists($this->useTabCompletion) ? ($this->hasReadline && $this->useTabCompletion) : $this->hasReadline;
    }
    ...
    {
        if (!array_key_exists($this->output)) {
    ...
    {
        if (!array_key_exists($this->pager) && $this->usePcntl()) {
    ...
    {
        if (!array_key_exists($this->autoCompleter)) {
    ...
    public function addMatchers(array $matchers)
    {
        ...
        if (array_key_exists($this->shell)) {
            $this->doAddMatchers();
        }
    }
    ...
    {
        if (array_key_exists($this->manualDbFile)) {
    ...
    {
        if (!array_key_exists($this->manualDb)) {
    ...
    {
        if (!array_key_exists($this->presenter)) {
    ...
    {
        if (!array_key_exists($this->checker)) {
    ...
    {
        return array_key_exists($this->updateCheck) ? $this->updateCheck : Checker::WEEKLY;
    }
