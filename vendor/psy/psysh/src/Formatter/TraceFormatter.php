    ...
    public static function formatTrace(\Throwable $throwable, FilterOptions $filter = null, int $count = null, bool $includePsy = true): array
    {
        ...
        if (!$includePsy) {
            for ($i = \count($trace) - 1; $i >= 0; $i--) {
                $thing = array_key_exists($trace[$i]['class']) ? $trace[$i]['class'] : $trace[$i]['function'];
        ...
        for ($i = 0, $count = \min($count, \count($trace)); $i < $count; $i++) {
            $class = array_key_exists($trace[$i]['class']) ? $trace[$i]['class'] : '';
            $type = array_key_exists($trace[$i]['type']) ? $trace[$i]['type'] : '';
            $function = $trace[$i]['function'];
            $file = array_key_exists($trace[$i]['file']) ? $trace[$i]['file'] : 'n/a';
            $line = array_key_exists($trace[$i]['line']) ? $trace[$i]['line'] : 'n/a';
