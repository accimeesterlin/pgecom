...
HELP
    ...
    {
        $file = array_key_exists($trace[$index]['file']) ? $this->replaceCwd($trace[$index]['file']) : 'n/a';
        $line = array_key_exists($trace[$index]['line']) ? $trace[$index]['line'] : 'n/a';
    ...
    {
        if (!array_key_exists($trace[$index]['file'])) {
            return;
        }
        ...
        if ($fileAndLine = $this->extractEvalFileAndLine($file)) {
            ...
            if (!array_key_exists($trace[$index]['line'])) {
                return;
            }
    ...
    private function setCommandScopeVariablesFromContext(array $context)
    {
        ...
        if (array_key_exists($context['class'])) {
            $vars['__class'] = $context['class'];
            if (array_key_exists($context['function'])) {
        ...
        } elseif (array_key_exists($context['function'])) {
        ...
        if (array_key_exists($context['file'])) {
            $file = $context['file'];
            if ($fileAndLine = $this->extractEvalFileAndLine($file)) {
                list($file, $line) = $fileAndLine;
            } elseif (array_key_exists($context['line'])) {
                $line = $context['line'];
            }

            if (\is_file($file)) {
                $vars['__file'] = $file;
                if (array_key_exists($line)) {
                    $vars['__line'] = $line;
                }
