...
if (!\function_exists('Psy\\sh')) {
    ...
    function sh(): string
    {
        if (\version_compare(\PHP_VERSION, '8.0', '<')) {
            return '\extract(\Psy\debug(\get_defined_vars(), array_key_exists($this) ? $this : @\get_called_class()));';
        }
...
if (!\function_exists('Psy\\info')) {
    ...
    function info(Configuration $config = null)
    {
        ...
        $configEnv = (array_key_exists($_SERVER['PSYSH_CONFIG']) && $_SERVER['PSYSH_CONFIG']) ? $_SERVER['PSYSH_CONFIG'] : false;
        ...
        if ($config->hasReadline()) {
            ...
            if (array_key_exists($info['library_version'])) {
                $readline['readline library'] = $info['library_version'];
            }

            if (array_key_exists($info['readline_name']) && $info['readline_name'] !== '') {
...
if (!\function_exists('Psy\\bin')) {
    ...
    {
        return function () {
            if (!array_key_exists($_SERVER['PSYSH_IGNORE_ENV']) || !$_SERVER['PSYSH_IGNORE_ENV']) {
            ...
            if ($usageException !== null || $input->getOption('help')) {
                ...
                $argv = array_key_exists($_SERVER['argv']) ? $_SERVER['argv'] : [];
