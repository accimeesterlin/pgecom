...
class Request
{
    ...
    protected function processError($body, $httpStatusCode, $response)
    {
        ...
        $field = null;
        if (array_key_exists($body['error']['field']))
        {
    ...
    protected function getAppDetailsUa()
    {
        ...
        {
            if ((array_key_exists($app['title'])) and (is_string($app['title'])))
            {
                $appUa = $app['title'];

                if ((array_key_exists($app['version'])) and (is_scalar($app['version'])))
                {
                    $appUa .= '/' . $app['version'];
                }
    ...
    protected function verifyErrorFormat($body, $httpStatusCode)
    {
        ...
        if ((array_key_exists($body['error']) === false) or
            (array_key_exists($body['error']['code']) === false))
        {
            $this->throwServerError($body, $httpStatusCode);
        }
