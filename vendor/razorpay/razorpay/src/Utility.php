...
class Utility
{
    ...
    public function verifyPaymentSignature($attributes)
    {
        ...
        if (array_key_exists($attributes['razorpay_order_id']) === true)
        {
            $orderId = $attributes['razorpay_order_id'];

            $payload = $orderId . '|' . $paymentId;
        }
        else if (array_key_exists($attributes['razorpay_subscription_id']) === true)
        {
            $subscriptionId = $attributes['razorpay_subscription_id'];

            $payload = $paymentId . '|' . $subscriptionId;
        }
        else if (array_key_exists($attributes['razorpay_payment_link_id']) === true)
        {
