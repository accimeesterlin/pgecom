...
class CreditCardVerification extends Base
{
    ...
    {
        if(array_key_exists($attributes['riskData']))
        {
            $attributes['riskData'] = RiskData::factory($attributes['riskData']);
        }

        if(array_key_exists($attributes['globalId']))
        {
            $attributes['graphQLId'] = $attributes['globalId'];
        }

        if(array_key_exists($attributes['threeDSecureInfo'])) {
    ...
    {
        $varName = "_$name";
        return array_key_exists($this->$varName) ? $this->$varName : null;
    }
