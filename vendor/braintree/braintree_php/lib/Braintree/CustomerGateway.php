...
class CustomerGateway
{
    ...
    {
        if (array_key_exists($response['customer'])) {
            // return a populated instance of Customer
            return new Result\Successful(
                    Customer::factory($response['customer'])
            );
        } else if (array_key_exists($response['apiErrorResponse'])) {
    ...
    {
        if (array_key_exists($attributes['deviceSessionId'])) {
            trigger_error('$deviceSessionId is deprecated, use $deviceData instead', E_USER_DEPRECATED);
        }
        if (array_key_exists($attributes['fraudMerchantId'])) {
