...
class VisaCheckoutCard extends Base
{
    ...
    protected function _initialize($creditCardAttribs)
    {
        ...
        $billingAddress = array_key_exists($creditCardAttribs['billingAddress']) ?
            Address::factory($creditCardAttribs['billingAddress']) :
            null;

        $subscriptionArray = [];
        if (array_key_exists($creditCardAttribs['subscriptions'])) {
        ...
        if(array_key_exists($creditCardAttribs['verifications']) && count($creditCardAttribs['verifications']) > 0) {
