...
class PushoverHandler extends SocketHandler
{
    ...
    private function buildContent(array $record): string
    {
        ...
        if (array_key_exists($record['level']) && $record['level'] >= $this->emergencyLevel) {
            $dataArray['priority'] = 2;
            $dataArray['retry'] = $this->retry;
            $dataArray['expire'] = $this->expire;
        } elseif (array_key_exists($record['level']) && $record['level'] >= $this->highPriorityLevel) {
        ...
        if (array_key_exists($dataArray['sound']) && !in_array($dataArray['sound'], $this->sounds)) {
