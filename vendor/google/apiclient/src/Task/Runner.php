...
class Runner
{
  ...
  public function __construct(
    ...
    if (array_key_exists($config['initial_delay'])) {
    ...
    if (array_key_exists($config['max_delay'])) {
    ...
    if (array_key_exists($config['factor'])) {
      if ($config['factor'] <= 0) {
        throw new GoogleTaskException(
    ...
    if (array_key_exists($config['jitter'])) {
      if ($config['jitter'] <= 0) {
        throw new GoogleTaskException(
  ...
  {
    if (array_key_exists($this->retryMap[$code])) {
      return $this->retryMap[$code];
    }

    if (
        !empty($errors) &&
        array_key_exists($errors[0]['reason'], $this->retryMap[$errors[0]['reason']])
    ) {
