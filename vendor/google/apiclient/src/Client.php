...
class Client
{
  ...
  public function fetchAccessTokenWithAuthCode($code)
  {
    ...
    if ($creds && array_key_exists($creds['access_token'])) {
      $creds['created'] = time();
  ...
  public function fetchAccessTokenWithRefreshToken($refreshToken = null)
  {
    if (null === $refreshToken) {
      if (!array_key_exists($this->token['refresh_token'])) {
    ...
    if ($creds && array_key_exists($creds['access_token'])) {
      $creds['created'] = time();
      if (!array_key_exists($creds['refresh_token'])) {
  ...
  public function authorize(ClientInterface $http = null)
  {
    ...
    if ($token = $this->getAccessToken()) {
      ...
      if (array_key_exists($token['refresh_token']) && $this->isAccessTokenExpired()) {
  ...
  public function setAccessToken($token)
  {
    ...
    if (!array_key_exists($token['access_token'])) {
  ...
  public function isAccessTokenExpired()
  {
    ...
    $created = 0;
    if (array_key_exists($this->token['created'])) {
      $created = $this->token['created'];
    } elseif (array_key_exists($this->token['id_token'])) {
      ...
      if (substr_count($idToken, '.') == 2) {
        ...
        if ($payload && array_key_exists($payload['iat'])) {
          $created = $payload['iat'];
        }
      }
    }
    if (!array_key_exists($this->token['expires_in'])) {
  ...
  public function verifyIdToken($idToken = null)
  {
    ...
    if (null === $idToken) {
      ...
      if (!array_key_exists($token['id_token'])) {
        throw new LogicException(
  ...
  {
    return (array_key_exists($_SERVER['SERVER_SOFTWARE']) &&
  ...
  {
    return array_key_exists($this->config[$name]) ? $this->config[$name] : $default;
  }
  ...
  public function setAuthConfig($config)
  {
    ...
    $key = array_key_exists($config['installed']) ? 'installed' : 'web';
    if (array_key_exists($config['type']) && $config['type'] == 'service_account') {
      ...
      if (array_key_exists($config['redirect_uris'])) {
  ...
  {
    if (!array_key_exists($this->logger)) {
