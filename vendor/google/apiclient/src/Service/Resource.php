...
class Resource
{
  ...
  public function __construct($service, $serviceName, $resourceName, $resource)
  {
    ...
    $this->methods = is_array($resource) && array_key_exists($resource['methods']) ?
        $resource['methods'] :
  ...
  {
    if (! array_key_exists($this->methods[$name])) {
    ...
    $postBody = null;
    if (array_key_exists($parameters['postBody'])) {
    ...
    if (array_key_exists($parameters['optParams'])) {
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! array_key_exists($method['parameters'][$key])) {
        $this->client->getLogger()->error(
    ...
    foreach ($method['parameters'] as $paramName => $paramSpec) {
      if (array_key_exists($paramSpec['required']) &&
          $paramSpec['required'] &&
          ! array_key_exists($parameters[$paramName])
      ) {
        $this->client->getLogger()->error(
      ...
      if (array_key_exists($parameters[$paramName])) {
    ...
    if (array_key_exists($parameters['data'])) {
      $mimeType = array_key_exists($parameters['mimeType'])
    ...
    if (array_key_exists($parameters['alt']) && $parameters['alt']['value'] == 'media') {
      $expectedClass = null;
    }
