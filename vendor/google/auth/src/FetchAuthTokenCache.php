    ...
    UpdateMetadataInterface
{
    ...
    public function signBlob($stringToSign, $forceOpenSsl = false)
    {
        ...
        if ($this->fetcher instanceof Credentials\GCECredentials) {
            ...
            $accessToken = array_key_exists($cached['access_token']) ? $cached['access_token'] : null;
    ...
    public function updateMetadata(
        ...
        if ($cached) {
            ...
            // fetch another token.
            if (array_key_exists($cached['access_token'])) {
    ...
    {
        if (array_key_exists($authToken['access_token']) ||
            array_key_exists($authToken['id_token'])) {
            // if $authUri is set, use it as the cache key
            $cacheKey = $authUri
                ? $this->getFullCacheKey($authUri)
                : $this->fetcher->getCacheKey();
