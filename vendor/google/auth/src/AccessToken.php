...
class AccessToken
{
    ...
    {
        $audience = array_key_exists($options['audience'])
            ? $options['audience']
            : null;
        $issuer = array_key_exists($options['issuer'])
            ? $options['issuer']
            : null;
        $certsLocation = array_key_exists($options['certsLocation'])
            ? $options['certsLocation']
            : self::FEDERATED_SIGNON_CERT_URL;
        $cacheKey = array_key_exists($options['cacheKey'])
            ? $options['cacheKey']
            : $this->getCacheKeyFromCertLocation($certsLocation);
        $throwException = array_key_exists($options['throwException'])
    ...
    private function verifyEs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        if ($audience) {
            if (!array_key_exists($payload['aud']) || $payload['aud'] != $audience) {
        ...
        if (!array_key_exists($payload['iss']) || $payload['iss'] !== $issuer) {
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        if (!array_key_exists($payload->iss) || !in_array($payload->iss, $issuers)) {
            throw new UnexpectedValueException('Issuer does not match');
        }
    ...
    {
        if (is_array($token)) {
            if (array_key_exists($token['refresh_token'])) {
    ...
    private function getCerts($location, $cacheKey, array $options = [])
    {
        ...
        if (!array_key_exists($certs['keys'])) {
