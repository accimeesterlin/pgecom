    ...
    UpdateMetadataInterface
{
    ...
    public function updateMetadata(
        ...
        if (array_key_exists($metadata[self::AUTH_METADATA_KEY])) {
            // Auth metadata has already been set
            return $metadata;
        }
        $result = $this->fetchAuthToken($httpHandler);
        if (!array_key_exists($result['access_token'])) {
            return $metadata;
        }
    ...
    private static function loadDefaultClientCertSourceFile()
    {
        ...
        if (!array_key_exists($clientCertSourceJson['cert_provider_command'])) {
