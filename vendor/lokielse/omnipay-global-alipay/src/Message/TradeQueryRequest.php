...
class TradeQueryRequest extends AbstractRequest
{
    ...
    public function sendData($data)
    {
         ...
         * is paid?
         */
        if (array_key_exists($payload['is_success']) && $payload['is_success'] == 'T' && array_get($payload, 'response.trade.trade_status') == 'TRADE_FINISHED') {
            $paid = true;
        } else {
            $paid = false;
        }
    ...
    protected function getRequestParam($key)
    {
        ...
        return array_key_exists($params[$key]) ? $params[$key] : null;
    }
