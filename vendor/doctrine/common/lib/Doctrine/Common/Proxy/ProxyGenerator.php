...
';
    ...
    public function generateProxyClass(ClassMetadata $class, $fileName = false)
    {
        ...
        foreach ($placeholderMatches as $placeholder => $name) {
            $placeholders[$placeholder] = array_key_exists($this->placeholders[$name])
...
EOT;
    ...
    /**
     * Generates the magic array_key_existster invoked when lazy loaded public properties are checked against array_key_exists().
     *
     * @param \Doctrine\Persistence\Mapping\ClassMetadata $class
     *
     * @return string
     */
    private function generateMagicarray_key_exists(ClassMetadata $class)
    {
        $lazyPublicProperties = $this->getLazyLoadedPublicPropertiesNames($class);
        $hasParentarray_key_exists       = $class->getReflectionClass()->hasMethod('__array_key_exists');
        $parametersString     = '$name';
        $returnTypeHint       = null;

        if ($hasParentarray_key_exists) {
            $methodReflection = $class->getReflectionClass()->getMethod('__array_key_exists');
            $parametersString = $this->buildParametersString($methodReflection->getParameters(), ['name']);
            $returnTypeHint   = $this->getMethodReturnType($methodReflection);
        }

        if (empty($lazyPublicProperties) && ! $hasParentarray_key_exists) {
            return '';
        }

        $inheritDoc = $hasParentarray_key_exists ? '{@inheritDoc}' : '';
        $magicarray_key_exists = <<<EOT
    /**
     * $inheritDoc
     * @param  string \$name
     * @return boolean
     */
    public function __array_key_exists($parametersString)$returnTypeHint
    {
...
EOT;
        ...
        if ( ! empty($lazyPublicProperties)) {
            $magicarray_key_exists .= <<<'EOT'
        if (\array_key_exists($name, self::$lazyPropertiesNames)) {
            $this->__initializer__ && $this->__initializer__->__invoke($this, '__array_key_exists', [$name]);
...
EOT;
        ...
        return parent::__array_key_exists($name);
EOT;
        } else {
            $magicarray_key_exists .= "        return false;";
        }

        return $magicarray_key_exists . "\n    }";
    }
    ...
    private function generateMethods(ClassMetadata $class)
    {
        ...
        foreach ($reflectionMethods as $method) {
            ...
            if ($method->isConstructor() ||
                array_key_exists($skippedMethods[strtolower($name)]) ||
                array_key_exists($methodNames[$name]) ||
                $method->isFinal() ||
                $method->isStatic() ||
                ( ! $method->isPublic())
            ) {
