...
final class DocParser
{
    ...
    {
        if (array_key_exists($this->classExists[$fqcn])) {
    ...
    private function collectAttributeTypeMetadata(array &$metadata, Attribute $attribute): void
    {
        ...
        if ($pos !== false) {
            ...
            if (array_key_exists(self::$typeMap[$arrayType])) {
    ...
    private function Annotation()
    {
        ...
        if ($name[0] !== '\\') {
            ...
            if ($this->namespaces) {
            ...
            } elseif (array_key_exists($this->imports[$loweredAlias])) {
                ...
                ! array_key_exists($this->ignoredAnnotationNames[$name])
                && array_key_exists($this->imports['__NAMESPACE__'])
                && $this->classExists($this->imports['__NAMESPACE__'] . '\\' . $name)
            ) {
                $name  = $this->imports['__NAMESPACE__'] . '\\' . $name;
                $found = true;
            } elseif (! array_key_exists($this->ignoredAnnotationNames[$name]) && $this->classExists($name)) {
                $found = true;
            }
...
EXCEPTION
        ...
        if (! array_key_exists(self::$annotationMetadata[$name])) {
...
EXCEPTION
        ...
        if (array_key_exists(self::$annotationMetadata[$name]['enum'])) {
            // checks all declared attributes
            foreach (self::$annotationMetadata[$name]['enum'] as $property => $enum) {
                // checks if the attribute is a valid enumerator
                if (array_key_exists($values[$property]) && ! in_array($values[$property], $enum['value'])) {
        ...
        foreach (self::$annotationMetadata[$name]['attribute_types'] as $property => $type) {
            if (
                ...
                && ! array_key_exists($values[$property]) && array_key_exists($values['value'])
            ) {
                $property = 'value';
            }
        ...
        if (self::$annotationMetadata[$name]['has_named_argument_constructor']) {
            ...
            foreach ($values as $property => $value) {
                if (! array_key_exists(self::$annotationMetadata[$name]['constructor_args'][$property])) {
...
EXCEPTION
        ...
        foreach ($values as $property => $value) {
            if (! array_key_exists(self::$annotationMetadata[$name]['properties'][$property])) {
                if ($property !== 'value') {
                    throw AnnotationException::creationError(sprintf(
                        <<<'EXCEPTION'
The annotation @%s declared on %s does not have a property named "%s".
...
EXCEPTION
    ...
    private function Constant()
    {
        ...
        if (! defined($identifier) && strpos($identifier, '::') !== false && $identifier[0] !== '\\') {
            ...
            switch (true) {
                ...
                case array_key_exists($this->imports[$loweredAlias]):
                    $found     = true;
                    $className = ($pos !== false)
                ...
                default:
                    if (array_key_exists($this->imports['__NAMESPACE__'])) {
    ...
    {
        if ($this->ignoreNotImportedAnnotations || array_key_exists($this->ignoredAnnotationNames[$name])) {
            return true;
        }
    ...
    private function resolvePositionalValues(array $arguments, string $name): array
    {
        ...
        if (
            ...
            foreach (self::$annotationMetadata[$name]['constructor_args'] as $property => $parameter) {
                ...
                if (array_key_exists($values[$property]) || ! array_key_exists($positionalArguments[$position])) {
                    continue;
                }
            ...
            if (count($positionalArguments) > 0 && ! array_key_exists($values['value'])) {
